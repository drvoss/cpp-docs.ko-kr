...
형식 문자열 뒤에 나오는 인수는 해당 *type* 문자 및 선택적인 [size](#size) 접두사에 따라 해석됩니다. 문자 형식 및에 대 한 변환은 **`char`** **`wchar_t`** **c** 또는 **c**를 사용 하 여 지정 되 고, 싱글바이트 및 멀티 바이트 또는 와이드 문자열은 사용 중인 형식 지정 함수에 따라 **s** 또는 **s**를 사용 하 여 지정 됩니다. **C** 및 **s** 를 사용 하 여 지정 된 문자 및 문자열 인수는 패밀리 함수에서 및로 해석 되 고 **`char`** **`char*`** `printf` **`wchar_t`** `wchar_t*` `wprintf` 패밀리 함수는 및로 해석 됩니다. **C** 및 **S** 를 사용 하 여 지정 된 문자 및 문자열 인수는 패밀리 함수에서 및로 해석 되 고 **`wchar_t`** `wchar_t*` `printf` **`char`** **`char*`** `wprintf` 패밀리 함수는 및로 해석 됩니다. 이 동작은 Microsoft 전용입니다.

,,,, 등의 정수 형식은 **`short`** **`int`** **`long`** **`long long`** **`unsigned`** **d**, **i**, **o**, **u**, **x**및 **x**를 사용 하 여 지정 합니다. ,, 등의 부동 소수점 **`float`** 형식은 **`double`** **`long double`** **a**, **a**, **e**, **e**, **f**, **f**, **g**및 **g**를 사용 하 여 지정 합니다. 기본적으로 *크기* 접두사에 의해 수정 되지 않는 한 정수 인수는 형식으로 강제 변환 되 **`int`** 고 부동 소수점 인수는로 강제 변환 됩니다 **`double`** . 64 비트 시스템에서는 **`int`** 32 비트 값 이므로 **Ll** 또는 **I64** 의 *크기* 접두사를 사용 하지 않는 한 출력 형식이 지정 되 면 64 비트 정수가 잘립니다. **p**로 지정되는 포인터 형식은 플랫폼에 대해 기본 포인터 크기를 사용합니다.
...
|**우표**|부동 소수점|**e**가 아니라 **E**가 지수를 도입한다는 점을 제외하고는 **e** 형식과 동일합니다.|
...
|**Express-g**|부동 소수점|**e**가 아니라 **E**가 지수를 도입(해당하는 경우)한다는 점을 제외하고는 **g** 형식과 동일합니다.|
...
너비 지정이 별표 () 인 경우 `*` **`int`** 인수 목록의 인수가 값을 제공 합니다. 아래 예제와 같이 *width* 인수는 반드시 인수 목록에서 형식이 지정되는 값 앞에 와야 합니다.
...
전체 자릿수 사양이 별표 () 인 경우 \* **`int`** 인수 목록의 인수가 값을 제공 합니다. 인수 목록에서 *precision* 인수는 아래 예제에 나와 있는 것처럼 형식이 지정되는 값 앞에 와야 합니다.
...
`ptrdiff_t`및 `size_t` 형식은 **`__int32`** **`unsigned __int32`** 32 비트 플랫폼 및 **`__int64`** 또는 **`unsigned __int64`** 64 비트 플랫폼에서 또는입니다. **I**(대문자 i), **j**, **t** 및 **z** 크기 접두사는 플랫폼에 대해 올바른 인수를 사용합니다.

Visual C++에서 **`long double`** 는 고유한 형식 이지만와 동일한 내부 표현을 갖습니다 **`double`** .

**hc** 또는 **hC** 형식 지정자는 `printf` 함수의 **c** 및 `wprintf` 함수의 **C**와 같습니다. **Lc**, **lc**, **wc.exe**또는 **wc.exe** 형식 지정자는 함수의 **c** `printf` 및 함수의 **c** 와 동의어입니다 `wprintf` . **hs** 또는 **hS** 형식 지정자는 `printf` 함수의 **s** 및 `wprintf` 함수의 **S**와 같습니다. **ls**, **lS**, **ws** 또는 **wS** 형식 지정자는 `printf` 함수의 **S** 및 `wprintf` 함수의 **s**와 같습니다.

> [!NOTE]
> **Microsoft 전용:**\
> **I**(대문자 i), **I32**, **I64** 및 **w** 인수 크기 한정자 접두사는 Microsoft 확장이며 ISO C와 호환되지 않습니다. 형식의 데이터와 함께 사용 되는 **h** 접두사 **`char`** 및 형식의 데이터와 함께 사용 되는 **l** (소문자 l) 접두사는 **`double`** Microsoft 확장입니다.
