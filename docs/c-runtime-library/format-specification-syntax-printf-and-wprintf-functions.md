...
**%**[[*플래그*](#flags)] 【[*너비*](#width)】 [. [*정밀도*](#precision)] 【[*크기*](#size)】 [*유형*](#type)
...
> [!NOTE]
> Visual Studio 2015 `printf` `scanf` 함수 및 패밀리를 **인라인으로** 선언하고 `<stdio.h>` `<conio.h>` 헤더로 이동했습니다. 이전 코드를 마이그레이션하는 경우 이러한 함수와 관련하여 *LNK2019가* 표시될 수 있습니다. 자세한 내용은 [Visual C++ 변경 기록 2003 - 2015를](../porting/visual-cpp-change-history-2003-2015.md#stdio_and_conio)참조하십시오.
...
형식 문자열 뒤에 나오는 인수는 해당 *type* 문자 및 선택적인 [size](#size) 접두사에 따라 해석됩니다. 문자 형식 `char` 및 `wchar_t`에 대한 변환은 **c** 또는 **C**를 사용하여 지정되고, 싱글바이트/멀티바이트 또는 와이드 문자의 문자열은 사용 중인 형식 지정 함수에 따라 **s** 또는 **S**를 사용하여 지정됩니다. **c와** **s를** 사용하여 지정된 문자 및 문자열 `char` 인수는 `printf` 패밀리 함수 또는 `wchar_t` `char*` `wchar_t*` `wprintf` 패밀리 함수로 해석됩니다. **C** 및 **S**를 사용하여 지정된 문자 및 문자열 인수는 `printf` 패밀리 함수에서는 `wchar_t` 및 `wchar_t*`로 해석되고 `wprintf` 패밀리 함수에서는 `char` 및 `char*`로 해석됩니다. 이 동작은 Microsoft에 따라 다릅니다.

" `short` `long` `long long` `unsigned` **u** **d** **i** **o** **x** **X**및 해당 변형과 같은 정수 유형은 d . i , o , u , x 및 X를 사용하여 지정됩니다. `int` **E**와 **f** **g** **F** **G** **e** **A** **a** `float` `double`같은 부동 점 유형은 을 사용하여 지정됩니다 . `long double` 기본적으로 *크기* 접두사로 수정되지 않는 한 정수 인수는 입력하도록 `int` 강제 변환되고 부동 점 인수는 `double`로 강제 변환됩니다. 64비트 시스템에서, `int`는 32비트 값이므로 **ll** 또는 **I64**의 *size* 접두사가 사용되지 않는 경우 64비트 정수는 출력을 위해 형식이 지정될 때 잘립니다. **p**로 지정되는 포인터 형식은 플랫폼에 대해 기본 포인터 크기를 사용합니다.

> [!NOTE]
> **마이크로소프트 관련:** **Z** 형식 문자및 **c,** **C**, **s**및 **S** 형 문자의 동작은 `printf` 및 함수와 `wprintf` 함께 사용될 때 Microsoft 확장입니다. ISO C 표준에서는 모든 형식 지정 함수에서 반각 문자 및 문자열에 대해 일관되게 **c** 및 **s**를 사용하고 와이드 문자 및 문자열에 대해 **C** 및 **S**를 사용합니다.
...
|**Ⅰ**|정수|부호 있는 10진수 정수입니다.|
|**O**|정수|부호 없는 8진수 정수입니다.|
|**u**|정수|부호 없는 10진수 정수.|
|**x**|정수|부호 없는 16진수 정수이며 "abcdef"를 사용합니다.|
|**X**|정수|부호 없는 16진수 정수이며 "ABCDEF"를 사용합니다.|
|**전자**|부동 소수점|부호 있는 값으로서 형식은 [–]*d.dddd*__e±__*dd*\[*d*]입니다. 여기서 *d*는 하나의 10진수이고 *dddd*는 지정된 전체 자릿수에 따라 하나 이상의 10진수이거나 기본적으로 6 자릿수입니다. *dd*\[*d*]는 [출력 형식](../c-runtime-library/set-output-format.md) 및 지수 크기에 따라 두 개 또는 세 개의 10진수입니다.|
|**전자**|부동 소수점|**e**가 아니라 **E**가 지수를 도입한다는 점을 제외하고는 **e** 형식과 동일합니다.|
...
||**e**, **E**, **f**, **F**, **a**또는 **A** **#** 형식과 함께 사용되는 경우 플래그는 출력 값에 소수점을 포함하도록 강제로 합니다.|소수점 뒤에 숫자가 있는 경우에만 소수점이 표시됩니다.|
...
변환 사양에서 선택적인 너비 사양 필드는 *flags* 문자 뒤에 표시됩니다. *width* 인수는 출력 문자의 최소 개수를 제어하는 음수가 아닌 10진수 정수입니다. 출력 값의 문자 수가 지정된 너비보다 작으면 최소 너비에 도달할 때까지 왼쪽 맞춤 플래그()가**-** 지정되었는지 여부에 따라 값의 왼쪽 또는 오른쪽에 공백이 추가됩니다. *width*가 0으로 시작하는 경우 변환이 무한대 또는 NAN인 경우를 제외하고 최소 너비에 도달할 때까지 정수 또는 부동 소수점 변환에 선행 0이 추가됩니다.
...
너비 사양과 달리 전체 자릿수 사양을 사용하는 경우에는 출력 값이 잘리거나 부동 소수점 값이 반올림될 수 있습니다. *정밀도가* 0으로 지정되고 변환할 값이 0이면 이 예제와 같이 결과가 문자 출력되지 않습니다.
...
|Type|의미|기본|
|----------|-------------|-------------|
|**a**, **A**|정밀도는 소수점 뒤의 자릿수를 지정합니다.|기본 전체 자릿수는 13입니다. 정밀도가 0이면 **#** 플래그를 사용하지 않는 한 소수점이 인쇄되지 않습니다.|
...
**hc** 또는 **hC** 형식 지정자는 `printf` 함수의 **c** 및 `wprintf` 함수의 **C**와 같습니다. **lc,** **lC,** **wc,** 또는 **wC** 형식 지정자는 함수의 **C와** `printf` 함수에서 `wprintf` **c와** 동의어입니다. **hs** 또는 **hS** 형식 지정자는 `printf` 함수의 **s** 및 `wprintf` 함수의 **S**와 같습니다. **ls**, **lS**, **ws** 또는 **wS** 형식 지정자는 `printf` 함수의 **S** 및 `wprintf` 함수의 **s**와 같습니다.

> [!NOTE]
> **마이크로소프트 관련:** **I** (대문자 i), **I32**, **I64**및 **w** 인수 크기 수정자 접두사는 Microsoft 확장이며 ISO C 호환되지 않습니다. `char` 형식의 데이터와 함께 사용되는 **h** 접두사 및 `double` 형식의 데이터와 함께 사용되는 **l**(소문자 L) 접두사는 Microsoft 확장입니다.

## <a name="see-also"></a>참조

[printf, _printf_l, wprintf, _wprintf_l](../c-runtime-library/reference/printf-printf-l-wprintf-wprintf-l.md)<br/>
[printf_s, _printf_s_l, wprintf_s, _wprintf_s_l](../c-runtime-library/reference/printf-s-printf-s-l-wprintf-s-wprintf-s-l.md)<br/>
[printf_p 위치 매개변수](../c-runtime-library/printf-p-positional-parameters.md)
