...
ms.locfileid: "79443439"
---
# <a name="countryregion-strings"></a>국가/지역 문자열

국가 및 지역 문자열을 언어 문자열과 결합하여 `setlocale`, `_wsetlocale`, `_create_locale`및 `_wcreate_locale` 함수에 대한 로캘 사양을 만들 수 있습니다. 다양한 Windows 운영 체제 버전에서 지원하는 국가 및 지역 이름 목록은 [MS-LCID]: Windows LCID(언어 코드 식별자) 참조에서 **Appendix A: Product Behavior**(부록 A: 제품 동작)의 표에 있는 **언어**, **위치** 및 [언어 태그](https://msdn.microsoft.com/library/cc233982.aspx) 열을 참조하세요. 사용 가능한 로캘 이름 및 관련 값을 열거하는 코드 예제는 [NLS: 이름 기반 API 샘플](/windows/win32/intl/nls--name-based-apis-sample)을 참조하세요.
