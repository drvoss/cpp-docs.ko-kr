...
C + +에서 **isnan** 템플릿 함수는 **`true`** *x* 인수가 NaN 이면를 반환 하 고 그렇지 않으면를 반환 **`false`** 합니다.
...
C + +로 컴파일될 때 **isnan** 매크로가 정의 되어 있지 않고 **isnan** 템플릿 함수가 정의 됩니다. 매크로와 동일한 방식으로 동작 하지만 정수 대신 형식의 값을 반환 합니다 **`bool`** .
