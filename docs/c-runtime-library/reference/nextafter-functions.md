...
C + +에서는 오버 로드를 허용 하므로를 포함 하 \<cmath> 는 경우이 반환 및 형식에 대해 **nextafter** 및 **nextafter** 의 오버 로드를 호출할 수 있습니다 **`float`** **`long double`** . C 프로그램에서 후에 **nextafter** **nextafter** 항상를 반환 **`double`** 합니다.
