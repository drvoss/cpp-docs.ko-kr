...
> [!IMPORTANT]
> **_mbsset_s** 및 **_mbsset_s_l** Windows 런타임에서 실행되는 응용 프로그램에서사용할 수 없습니다. 자세한 내용은 [유니버설 Windows 플랫폼 앱에서 지원되지 않는 CRT 함수](../../cppcx/crt-functions-not-supported-in-universal-windows-platform-apps.md)를 참조하세요.
...
이러한 함수는 해당 인수의 유효성을 검사합니다. *str이* null 포인터이거나 *numberOfElements* 인수가 0보다 적거나 같거나 전달된 블록이 null-terminated가 아닌 경우 매개 변수 유효성 [검사에](../../c-runtime-library/parameter-validation.md)설명된 대로 잘못된 매개 변수 처리기가 호출됩니다. 실행을 계속할 수 있는 경우 이러한 함수는 **EINVAL을** 반환하고 **errno를** **EINVAL로**설정합니다.

## <a name="remarks"></a>설명

**_strset_s** 함수는 null 문자 종료를 제외한 *str의* 모든 문자를 **c(char로**변환됨)로 설정합니다. *c* **_wcsset_s** 및 **_mbsset_s** **_strset_s**와이드 문자 및 멀티 바이트 문자 버전입니다. 인수 및 반환 값의 데이터 형식은 버전에 따라 달라집니다. 그 외의 경우에는 이들 함수가 동일하게 작동합니다.
