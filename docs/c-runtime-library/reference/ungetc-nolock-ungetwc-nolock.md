...
성공 하는 경우 이러한 각 함수는 문자 인수 *c*를 반환 합니다. *C* 를 다시 푸시할 수 없거나 문자를 읽지 않은 경우 입력 스트림은 변경 되지 않으며 **_ungetc_nolock** **EOF**를 반환 합니다. **_Ungetwc_nolock** **는를**반환 합니다. *Stream* 이 **NULL**이면 **EOF** 또는 **Weof** 가 반환 되 고 **errno** 가 **EINVAL**로 설정 됩니다.
