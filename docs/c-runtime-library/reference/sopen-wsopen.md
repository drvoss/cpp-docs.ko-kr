...
ms.locfileid: "81356395"
---
# <a name="_sopen-_wsopen"></a>_sopen, _wsopen

공유할 파일을 엽니다. 이러한 함수의 보다 안전한 버전을 사용할 수 있습니다 [_wsopen_s _sopen_s.](sopen-s-wsopen-s.md)
...
*파일 이름* 또는 *oflag가* **NULL** 포인터이거나 *oflag* 또는 *shflag가* 유효한 값 범위 내에 있지 않은 경우 [매개 변수 유효성 검사에](../../c-runtime-library/parameter-validation.md)설명된 대로 잘못된 매개 변수 처리기가 호출됩니다. 실행을 계속할 수 있는 경우 이러한 함수는 -1을 반환하고 **errno를** 다음 값 중 하나로 설정합니다.

|errno 값|조건|
|-|-|
| **EACCES** | 지정된 경로가 디렉터리거나 파일이 읽기 전용인데 쓰기 위한 열기 작업을 시도했습니다. |
| **EEXIST** | **_O_CREAT** **_O_EXCL** 플래그가 지정되었지만 *파일 이름이* 이미 있습니다. |
| **아인발 ()에인발 (것)** | *oflag* 또는 *shflag* 인수가 잘못되었습니다. |
...
**_sopen** 함수는 *파일 이름으로* 지정된 파일을 열고 *oflag* 및 *shflag에*의해 정의된 대로 파일을 공유 읽기 또는 쓰기를 위해 준비합니다. **_wsopen** **_sopen**와이드 문자 버전입니다. **_wsopen** *파일 이름* 인수는 와이드 문자 문자열입니다. **_wsopen** **_sopen** 달리 동일하게 행동합니다.
...
정수 *발현은* \<fcntl.h> 정의된 다음 매니페스트 상수 중 하나 이상을 결합하여 형성됩니다. 두 개 이상의 상수가 의 인수를 형성할 *때lag,* 그들은 비트 OR 연산자 **(&#124;)와** 결합된다.
...
**_sopen** **_O_WRONLY** | **_O_APPEND** (부속 모드)와 **_O_WTEXT**, **_O_U16TEXT**또는 **_O_U8TEXT**호출하는 경우 먼저 읽기 및 쓰기를 위해 파일을 열고 BOM을 읽은 다음 쓰기 위해 다시 엽니 다. 읽고 쓰기 위해 파일을 여는데 실패하면 쓰기 위해서만 파일을 열고 유니코드 모드 설정에 기본값을 사용합니다.

인수 *shflag는* \<share.h> 정의된 다음 매니페스트 상수 중 하나로 구성된 상수 표현식입니다.
...
*pmode* 인수는 **_O_CREAT** 지정된 경우에만 필요합니다. 파일이 없는 경우 *pmode는* 새 파일이 처음 닫힐 때 설정된 파일의 권한 설정을 지정합니다. 그렇지 않으면 *pmode가* 무시됩니다. *pmode는* \<sys\stat.h> 정의된 매니페스트 **상수 _S_IWRITE** _S_IREAD 중 하나 또는 둘 모두를 포함하는 정수 표현식입니다. **_S_IREAD** 두 상수가 지정된 경우 비트 OR 연산자를 사용하여 결합합니다. *pmode의* 의미는 다음과 같습니다.
...
**_sopen** 사용 권한이 설정되기 전에 현재 파일 권한 마스크를 *pmode에* 적용합니다. [_umask](umask.md)를 참조하세요.
