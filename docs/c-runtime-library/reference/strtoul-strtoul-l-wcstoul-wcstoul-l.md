...
**strtoul은** 오버플로에서 변환된 값(있는 경우)을 반환하거나 **ULONG_MAX.** 변환을 수행할 수 없는 경우 **strtoul은 0을** 반환합니다. **wcstoul은** **strtoul과**유사하게 값을 반환합니다. 두 함수 모두 오버플로 또는 언더플로우가 발생하는 경우 **errno가** **ERANGE로** 설정됩니다.

이러한 반환 코드 및 기타 반환 코드에 대한 자세한 내용은 [_doserrno, errno, _sys_errlist 및 _sys_nerr](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md) 을 참조하세요.

## <a name="remarks"></a>설명

이러한 각 함수는 입력 문자열 *strSource를* **서명되지 않은** **긴 으로**변환합니다.

**strtoul은** 숫자의 일부로 인식할 수 없는 첫 번째 문자에서 문자열 *strSource읽기를* 중지합니다. null 문자를 종료하거나 *기본*문자보다 크거나 같을 수 있는 첫 번째 숫자 문자일 수 있습니다. 로캘의 **LC_NUMERIC** 범주 설정은 *strSource에서*radix 문자의 인식을 결정합니다. 자세한 내용은 [setlocale](setlocale-wsetlocale.md)을 참조하십시오. **스트툴** 및 **wcstoul은** 현재 로케일을 사용; **_strtoul_l** **_wcstoul_l** 대신 전달된 로캘을 사용한다는 점을 제외하면 동일합니다. 자세한 내용은 [Locale](../../c-runtime-library/locale.md)을 참조하세요.
...
**wcstoul은** **스트툴의**넓은 문자 버전입니다; 해당 *strSource* 인수는 와이드 문자 문자열입니다. 그 외의 경우에는 이들 함수가 동일하게 동작합니다.
