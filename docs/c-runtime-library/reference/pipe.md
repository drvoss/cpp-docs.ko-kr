...
*pfds*<br/>
**`int`** 읽기 및 쓰기 파일 설명자를 저장할 두 배열에 대 한 포인터입니다.
...
**_pipe** 는 **_open** 와 유사 하지만 읽기 및 쓰기를 위해 파이프를 열고 하나 대신 두 개의 파일 설명자를 반환 합니다. 프로그램은 파이프 양쪽을 모두 사용할 수도 있고 필요하지 않은 쪽을 닫을 수도 있습니다. 예를 들어 Windows의 명령 프로세서는 **PROGRAM1**program2.c와 같은 명령을 실행 하면 파이프를 만듭니다  |  **PROGRAM2**.
