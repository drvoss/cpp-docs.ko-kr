...
**Gets_s** 함수는 표준 입력 스트림 **stdin** 에서 줄을 읽고 *버퍼*에 저장 합니다. 줄은 첫 번째 줄 바꿈 문자('\n')까지 모든 문자로 구성됩니다. 그런 다음 **gets_s** 는 줄을 반환 하기 전에 줄 바꿈 문자를 null 문자 (' \ 0 ')로 바꿉니다. 반면 **fgets_s** 함수는 줄 바꿈 문자를 유지 합니다.

첫 번째 문자 읽기가 파일 끝 문자인 경우 null 문자는 *버퍼* 의 시작 부분에 저장 되 고 **null** 이 반환 됩니다.

**_getws_s** 은 **gets_s**의 와이드 문자 버전입니다. 해당 인수 및 반환 값은 와이드 문자열입니다.

*Buffer* 가 **NULL** 이거나 *sizeincharacters* 가 0 보다 작거나 같은 경우 또는 버퍼가 너무 작아서 입력 줄과 NULL 종결자를 포함할 수 없는 경우 이러한 함수는 [매개 변수 유효성 검사](../../c-runtime-library/parameter-validation.md)에 설명 된 대로 잘못 된 매개 변수 처리기를 호출 합니다. 계속 해 서 실행 하도록 허용한 경우 이러한 함수는 **NULL** 을 반환 하 고 Errno를 **ERANGE**로 설정 합니다.
