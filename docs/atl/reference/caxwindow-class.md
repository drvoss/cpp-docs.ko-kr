...
|[CreateControl](#createcontrol)|ActiveX 컨트롤을 만들어 초기화 하 고 `CAxWindow` 창에 호스팅합니다.|
...
|[QueryControl](#querycontrol)|호스팅된 ActiveX 컨트롤의 `IUnknown`를 검색 합니다.|
|[QueryHost](#queryhost)|`CAxWindow` 개체의 `IUnknown` 포인터를 검색 합니다.|
|[SetExternalDispatch](#setexternaldispatch)|`CAxWindow` 개체에서 사용 하는 외부 디스패치 인터페이스를 설정 합니다.|
|[SetExternalUIHandler](#setexternaluihandler)|`CAxWindow` 개체에서 사용 하는 외부 `IDocHostUIHandler` 인터페이스를 설정 합니다.|
...
이 클래스는 ActiveX 컨트롤을 호스트 하는 창을 조작 하기 위한 메서드를 제공 합니다. 호스트는 `CAxWindow`로 래핑된 " **AtlAxWin80"** 에서 제공 됩니다.

클래스 `CAxWindow`은 `CAxWindowT` 클래스의 특수화로 구현 됩니다. 이 특수화는 다음과 같이 선언 됩니다.

`typedef CAxWindowT<CWindow> CAxWindow;`

기본 클래스를 변경 해야 하는 경우 `CAxWindowT`를 사용 하 고 새 기본 클래스를 템플릿 인수로 지정할 수 있습니다.
...
`AttachControl`를 호출 하기 전에 연결 중인 컨트롤 개체를 올바르게 초기화 해야 합니다.

##  <a name="caxwindow"></a>CAxWindow:: CAxWindow

기존 창 개체 핸들을 사용 하 여 `CAxWindow` 개체를 생성 합니다.
...
`CreateControl`를 사용 하는 샘플은 [ATL AXHost를 사용 하 여 ActiveX 컨트롤 호스팅](../../atl/hosting-activex-controls-using-atl-axhost.md) 을 참조 하세요.
...
*ppUnkControl*<br/>
제한이 컨트롤의 `IUnknown` 받는 포인터의 주소입니다. NULL일 수 있습니다.

*iidSink*<br/>
진행 포함 된 개체의 송신 인터페이스에 대 한 인터페이스 식별자입니다. IID_NULL 수 있습니다.

*punkSink*<br/>
진행 *Iidsink*에서 지정한 포함 된 개체의 연결 지점에 연결할 싱크 개체의 `IUnknown` 인터페이스에 대 한 포인터입니다.
...
`CreateControlEx`를 사용 하는 샘플은 [ATL AXHost를 사용 하 여 ActiveX 컨트롤 호스팅](../../atl/hosting-activex-controls-using-atl-axhost.md) 을 참조 하세요.
...
호스트의 인터페이스를 사용 하면 `AxWin`에 의해 구현 되는 창 호스팅 코드의 기본 기능에 액세스할 수 있습니다.
...
외부 `IDocHostUIHandlerDispatch` 인터페이스는 호스트의 사이트에서 `IDocHostUIHandlerDispatch` 인터페이스를 쿼리 하는 컨트롤에 사용 됩니다. WebBrowser 컨트롤은이를 수행 하는 한 가지 컨트롤입니다.
