...
|[CWindow:: Attach](#attach)|`CWindow` 개체에 창을 연결 합니다.|
...
|[CWindow::D estroyWindow](#destroywindow)|`CWindow` 개체와 연결 된 창을 소멸 시킵니다.|
|[CWindow::D etach](#detach)|`CWindow` 개체에서 창을 분리 합니다.|
...
|[CWindow:: SendNotifyMessage](#sendnotifymessage)|창에 메시지를 보냅니다. 호출 스레드에 의해 창이 만들어진 경우에는 창 프로시저에서 메시지를 처리할 때까지 `SendNotifyMessage` 반환 되지 않습니다. 그렇지 않으면 즉시 반환 됩니다.|
...
|속성|Description|
|----------|-----------------|
|[CWindow:: operator HWND](#operator_hwnd)|`CWindow` 개체를 HWND로 변환 합니다.|
...
`CWindow`는 ATL에서 창을 조작 하는 기본 기능을 제공 합니다. 대부분의 `CWindow` 메서드는 Win32 API 함수 중 하나를 래핑합니다. 예를 들어 `CWindow::ShowWindow` 및 `ShowWindow`에 대 한 프로토타입을 비교 합니다.

|CWindow 메서드|Win32 함수|
|--------------------|--------------------|
|**BOOL ShowWindow (int** `nCmdShow` **);**|**BOOL ShowWindow (HWND** `hWnd` **, int** `nCmdShow` **);**|

`CWindow::ShowWindow`은 `CWindow::m_hWnd`을 첫 번째 매개 변수로 전달 하 여 `ShowWindow` Win32 함수를 호출 합니다. Win32 함수를 직접 래핑하는 모든 `CWindow` 메서드는 `m_hWnd` 멤버를 전달 합니다. 따라서 `CWindow` 설명서의 대부분은 Windows SDK을 참조 합니다.

> [!NOTE]
>  모든 `CWindow` 메서드가 Win32 함수를 래핑하는 것은 아니므로 모든 창 관련 Win32 함수를 `CWindow`로 래핑할 수는 없습니다.

`CWindow::m_hWnd`는 창을 식별 하는 HWND를 저장 합니다. HWND는 다음과 같은 경우 개체에 연결 됩니다.

- `CWindow`의 생성자에서 HWND를 지정 합니다.

- `CWindow::Attach`을 호출합니다.

- `CWindow`의 **operator =** 를 사용 합니다.

- `CWindow`에서 파생 된 다음 클래스 중 하나를 사용 하 여 창을 만들거나 하위 클래스 합니다.
...
##  <a name="centerwindow"></a>CWindow:: 가운데 창
...
*rect*<br/>
진행 창의 위치를 지정 하 [_U_RECT](../../atl/reference/u-rect-class.md) 형식의 변수입니다. 기본값은 NULL입니다. 이 매개 변수가 NULL 이면 `CWindow::rcDefault` 값이 사용 됩니다.
...
> [!NOTE]
> `CWindow::CWindow`은 창을 만들지 않습니다. [CWindowImpl](../../atl/reference/cwindowimpl-class.md), [CContainedWindow](../../atl/reference/ccontainedwindowt-class.md)및 [cdialogimpl](../../atl/reference/cdialogimpl-class.md) 모두 `CWindow`에서 파생 되는 클래스는 창이 나 대화 상자를 만들 수 있는 메서드를 제공 합니다 .이 메서드는 `CWindow::m_hWnd`에 할당 됩니다. 또한 [CreateWindow](/windows/win32/api/winuser/nf-winuser-createwindoww) Win32 함수를 사용할 수 있습니다.
...
`CWindow` 개체와 연결 된 창을 소멸 하 고 [m_hWnd](#m_hwnd) 을 NULL로 설정 합니다.
...
`GetDescendantWindow`는 직계 자식인 창이 아니라 자식 창의 전체 트리를 검색 합니다.
...
Windows SDK `GetWindow`를 참조 하세요.
...
포인터나 핸들을 검색 하는 경우이 함수는 `CWindow::GetWindowLong` 메서드를 대체 합니다.

> [!NOTE]
>  포인터와 핸들은 32 비트 windows에서 32 비트이 고 64 비트 Windows에서는 64 비트입니다.

32 비트 및 64 비트 버전의 Windows와 호환 되는 코드를 작성 하려면 `CWindow::GetWindowLongPtr`을 사용 합니다.
...
이 메서드의 두 번째 버전에서는 BSTR에 텍스트를 저장할 수 있습니다. 세 번째 버전에서는 `CSimpleString`이 `CString`의 기본 클래스 이므로 [CString](../../atl-mfc-shared/reference/cstringt-class.md)에 결과를 저장할 수 있습니다.
...
##  <a name="invalidate"></a>CWindow:: 무효화
...
`RECT` 매개 변수에 대 한 NULL을 `InvalidateRect` Win32 함수로 전달 합니다.
...
`CWindow::SetTimer`에서 만든 타이머 이벤트를 소멸 시킵니다.
...
*Nflags* 가 0이 아닌 경우 `ModifyStyle`는 Win32 함수 `SetWindowPos`를 호출 하 고 *nflags* 를 다음 4 개의 플래그와 결합 하 여 창을 다시 그립니다.
...
*Nflags* 가 0이 아닌 경우 `ModifyStyleEx`는 Win32 함수 `SetWindowPos`를 호출 하 고 *nflags* 를 다음 4 개의 플래그와 결합 하 여 창을 다시 그립니다.
...
[M_hWnd](#m_hwnd) 멤버를 `hWnd`로 설정 하 여 HWND를 `CWindow` 개체에 할당 합니다.
...
호출 스레드에 의해 창이 만들어진 경우에는 창 프로시저에서 메시지를 처리할 때까지 `SendNotifyMessage` 반환 되지 않습니다. 그렇지 않으면 즉시 반환 됩니다.
...
##  <a name="setredraw"></a>CWindow:: SetRedraw 그리기
...
`SetRedraw`를 호출 하 여 변경 내용을 다시 그릴 수 있게 하거나 변경 내용을 다시 그릴 수 없도록 합니다.
...
이 함수는 `CWindow::SetWindowLong` 메서드를 대체 합니다. 32 비트 및 64 비트 버전의 Windows와 호환 되는 코드를 작성 하려면 `CWindow::SetWindowLongPtr`을 사용 합니다.
