...
|[CWindow::D eferWindowPos](#deferwindowpos)|지정 된 창에 대 한 지정 된 다중 창 위치 구조를 업데이트 합니다.|
|[CWindow::D estroyWindow](#destroywindow)|개체와 연결 된 창을 소멸 시킵니다 `CWindow` .|
|[CWindow::D etach](#detach)|개체에서 창을 분리 `CWindow` 합니다.|
|[CWindow::D lgDirList](#dlgdirlist)|지정 된 경로 또는 파일 이름과 일치 하는 모든 파일의 이름으로 목록 상자를 채웁니다.|
|[CWindow::D lgDirListComboBox](#dlgdirlistcombobox)|지정 된 경로 또는 파일 이름과 일치 하는 모든 파일의 이름으로 콤보 상자를 채웁니다.|
|[CWindow::D lgDirSelect](#dlgdirselect)|목록 상자에서 현재 선택 영역을 검색합니다.|
|[CWindow::D lgDirSelectComboBox](#dlgdirselectcombobox)|콤보 상자에서 현재 선택 영역을 검색 합니다.|
|[CWindow::D ragAcceptFiles](#dragacceptfiles)|창에서 끌어서 놓은 파일을 허용 하는지 여부를 등록 합니다.|
|[CWindow::D rawMenuBar](#drawmenubar)|창의 메뉴 모음을 다시 그립니다.|
...
|[CWindow::P ostMessage](#postmessage)|창을 만든 스레드와 연결 된 메시지 큐에 메시지를 배치 합니다. 스레드가 메시지를 처리할 때까지 기다리지 않고을 반환 합니다.|
|[CWindow::P revDlgCtrl](#prevdlgctrl)|대화 상자의 이전 컨트롤로 키보드 포커스를 설정 합니다.|
|[CWindow::P rint](#print)|지정 된 장치 컨텍스트에서 창을 그리도록 요청 합니다.|
|[CWindow::P rintClient](#printclient)|지정 된 장치 컨텍스트에서 창의 클라이언트 영역을 그리도록 요청 합니다.|
...
|[CWindow:: SendNotifyMessage](#sendnotifymessage)|창에 메시지를 보냅니다. 호출 스레드에서 창을 만든 경우 `SendNotifyMessage` 창 프로시저에서 메시지를 처리할 때까지이 반환 되지 않습니다. 그렇지 않으면 즉시 반환 됩니다.|
...
|[CWindow:: SetRedraw 그리기](#setredraw)|다시 그리기 플래그를 설정 하거나 지웁니다.|
...
|[CWindow:: operator =](#operator_eq)|개체에 HWND를 할당 `CWindow` 합니다.|

### <a name="public-data-members"></a>공용 데이터 멤버

|Name|설명|
|----------|-----------------|
|[CWindow:: m_hWnd](#m_hwnd)|개체와 연결 된 창에 대 한 핸들 `CWindow` 입니다.|
|[CWindow:: rcDefault](#rcdefault)|기본 창 크기를 포함 합니다.|

## <a name="remarks"></a>설명

`CWindow`ATL에서 창을 조작 하는 기본 기능을 제공 합니다. 대부분의 `CWindow` 메서드는 Win32 API 함수 중 하나를 래핑합니다. 예를 들어 및에 대 한 프로토타입을 비교 합니다 `CWindow::ShowWindow` `ShowWindow` .

|CWindow 메서드|Win32 함수|
|--------------------|--------------------|
|**BOOL ShowWindow (int** `nCmdShow` **);**|**BOOL ShowWindow (HWND** `hWnd` **, int** `nCmdShow` **);**|

`CWindow::ShowWindow``ShowWindow` `CWindow::m_hWnd` 첫 번째 매개 변수로를 전달 하 여 Win32 함수를 호출 합니다. `CWindow`Win32 함수를 직접 래핑하는 모든 메서드는 멤버를 전달 합니다. `m_hWnd` 따라서 설명서의 대부분은 `CWindow` Windows SDK를 참조 합니다.

> [!NOTE]
> 모든 창 관련 Win32 함수는로 래핑되는 것이 아니라 `CWindow` 모든 `CWindow` 메서드가 win32 함수를 래핑하는 것은 아닙니다.

`CWindow::m_hWnd`창을 식별 하는 HWND를 저장 합니다. HWND는 다음과 같은 경우 개체에 연결 됩니다.

- 의 생성자에서 HWND를 지정 `CWindow` 합니다.

- `CWindow::Attach`를 호출합니다.

- `CWindow` **연산자 =** 를 사용 합니다.

- 에서 파생 된 다음 클래스 중 하나를 사용 하 여 창을 만들거나 하위 클래스 합니다 `CWindow` .
...
*HWndNew* 로 식별 되는 창을 개체에 연결 합니다 `CWindow` .
...
## <a name="cwindowcenterwindow"></a><a name="centerwindow"></a>CWindow:: 가운데 창
...
*rect*<br/>
진행 창의 위치를 지정 하 [_U_RECT](../../atl/reference/u-rect-class.md) 형식의 변수입니다. 기본값은 NULL입니다. 이 매개 변수가 NULL 이면의 값이 `CWindow::rcDefault` 사용 됩니다.
...
*MenuOrID*<br/>
진행 메뉴 또는 창 식별자에 대 한 핸들을 지정 하 [_U_MENUorID](../../atl/reference/u-menuorid-class.md) 형식의 변수입니다. 기본값은 0U입니다.
...
성공 하면 [m_hWnd](#m_hwnd)에서 지정한 새로 만든 창에 대 한 핸들입니다. 그렇지 않으면 NULL입니다.
...
> [!NOTE]
> `CWindow::CWindow`창을 만들지 않습니다. [CWindowImpl](../../atl/reference/cwindowimpl-class.md), [CContainedWindow](../../atl/reference/ccontainedwindowt-class.md)및 [cdialogimpl](../../atl/reference/cdialogimpl-class.md) 모두에서 파생 되는 클래스는 `CWindow` 창 또는 대화 상자를 만들 수 있는 메서드를 제공 하며,이를에 할당 `CWindow::m_hWnd` 합니다. 또한 [CreateWindow](/windows/win32/api/winuser/nf-winuser-createwindoww) Win32 함수를 사용할 수 있습니다.
...
개체와 연결 된 창을 소멸 `CWindow` 하 고 [M_HWND](#m_hwnd) 를 NULL로 설정 합니다.
...
개체에서 [m_hWnd](#m_hwnd) 를 분리 하 `CWindow` 고 `m_hWnd` 를 NULL로 설정 합니다.
...
*Iid* 로 지정 된 창이 컨트롤 컨테이너이 고 요청 된 인터페이스를 검색할 수 있는 경우 S_OK을 반환 합니다. 창이 컨트롤 컨테이너가 아니거나 요청한 인터페이스를 검색할 수 없는 경우 E_FAIL를 반환 합니다. 지정 된 ID의 창을 찾을 수 없는 경우 반환 값은 HRESULT_FROM_WIN32 (ERROR_CONTROL_ID_NOT_FOUND)와 같습니다.
...
창에 [WM_GETFONT](/windows/win32/winmsg/wm-getfont) 메시지를 보내 창의 현재 글꼴을 검색 합니다.
...
WM_GETHOTKEY 메시지를 전송 하 여 창과 연결 된 바로 가기 키를 확인 합니다.
...
창과 연결 된 바로 가기 키의 가상 키 코드 및 한정자입니다. 가능한 한정자 목록은 Windows SDK에서 [WM_GETHOTKEY](/windows/win32/inputdev/wm-gethotkey) 을 참조 하세요. 표준 가상 키 코드의 목록은 Winuser.h를 참조 하세요.
...
`GetWindow`Windows SDK에서을 참조 하세요.
...
포인터나 핸들을 검색 하는 경우이 함수는 메서드를 대체 `CWindow::GetWindowLong` 합니다.
...
이 메서드의 두 번째 버전에서는 BSTR에 텍스트를 저장할 수 있습니다. 세 번째 버전에서는의 기본 클래스 이므로 [CString](../../atl-mfc-shared/reference/cstringt-class.md)에 결과를 저장할 수 있습니다 `CSimpleString` `CString` .
...
Windows SDK [WM_NEXTDLGCTL](/windows/win32/dlgbox/wm-nextdlgctl) 를 참조 하세요.
...
매개 변수에 대 한 NULL을 `RECT` `InvalidateRect` Win32 함수로 전달 합니다.
...
에서 만든 타이머 이벤트를 소멸 시킵니다 `CWindow::SetTimer` .
...
*BLock* 이 TRUE 이면이 메서드는 [m_hWnd](#m_hwnd) 를 Win32 함수로 전달 합니다. 그렇지 않으면 NULL을 전달 합니다.

## <a name="cwindowm_hwnd"></a><a name="m_hwnd"></a>CWindow:: m_hWnd

개체와 연결 된 창에 대 한 핸들을 포함 `CWindow` 합니다.
...
*Nflags* 가 0이 아닌 경우는 `ModifyStyle` Win32 함수를 호출 하 `SetWindowPos` 고 다음 4 개의 플래그와 *n 플래그* 를 결합 하 여 창을 다시 그립니다.
...
*Nflags* 가 0이 아닌 경우는 `ModifyStyleEx` Win32 함수를 호출 하 `SetWindowPos` 고 다음 4 개의 플래그와 *n 플래그* 를 결합 하 여 창을 다시 그립니다.
...
이 메서드의 두 번째 버전은 [RECT](/windows/win32/api/windef/ns-windef-rect) 구조체를 사용 하 여 창의 새 위치, 너비 및 높이를 확인 합니다.
...
`CWindow` [M_hWnd](#m_hwnd) 멤버를로 설정 하 여 개체에 HWND를 할당 `hWnd` 합니다.
...
지정 된 장치 컨텍스트에서 자신을 그리도록 요청 하는 [WM_PRINT](/windows/win32/gdi/wm-print) 메시지를 창으로 보냅니다.
...
지정 된 메시지를 개체의 모든 직계 자식으로 보냅니다 `CWindow` .
...
호출 스레드에서 창을 만든 경우 `SendNotifyMessage` 창 프로시저에서 메시지를 처리할 때까지이 반환 되지 않습니다. 그렇지 않으면 즉시 반환 됩니다.
...
*bRedraw*<br/>
진행 TRUE (기본값) 이면 창이 다시 그려집니다. 그렇지 않으면 False입니다.
...
*wModifiers*<br/>
진행 핫 키의 한정자입니다. 가능한 값 목록은 Windows SDK에서 WM_SETHOTKEY을 참조 하세요.

### <a name="return-value"></a>Return Value

가능한 반환 값 목록은 Windows SDK에서 [WM_SETHOTKEY](/windows/win32/inputdev/wm-sethotkey) 을 참조 하세요.
...
## <a name="cwindowsetredraw"></a><a name="setredraw"></a>CWindow:: SetRedraw 그리기
...
`SetRedraw`을 호출 하 여 변경 내용을 다시 그릴 수 있게 하거나 변경 내용을 다시 그릴 수 없도록 합니다.
...
이 함수는 메서드를 대체 `CWindow::SetWindowLong` 합니다. 32 비트 및 64 비트 버전의 Windows와 호환 되는 코드를 작성 하려면를 사용 `CWindow::SetWindowLongPtr` 합니다.
...
이 메서드의 두 번째 버전은 [RECT](/windows/win32/api/windef/ns-windef-rect) 구조체를 사용 하 여 창의 새 위치, 너비 및 높이를 설정 합니다.
