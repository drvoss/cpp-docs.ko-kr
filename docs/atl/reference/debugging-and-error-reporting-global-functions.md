...
> [!NOTE]
>  `HRESULT_FROM_WIN32(GetLastError())`를 사용 하는 대신 [AtlHresultFromLastError](debugging-and-error-reporting-global-functions.md#atlhresultfromlasterror)함수를 사용 합니다.
...
개체의 클라이언트에 오류 정보를 제공 하도록 `IErrorInfo` 인터페이스를 설정 합니다.
...
> [!CAUTION]
>  Catch 처리기에 `AtlReportError`을 C++ 사용 하지 마십시오. 이러한 함수의 일부 재정의에서는 내부적으로 `_alloca` 함수를 사용 하는 ATL 문자열 변환 매크로를 사용 합니다. C++ Catch 처리기에서 `AtlReportError`를 사용 하면 catch 처리기에서 C++ 예외가 발생할 수 있습니다.
...
ATL 프로젝트의 경우 오류가 발생 하는 경우 ATL에서 사용할이 함수의 고유한 구현을 제공할 수 있습니다. 이렇게 하려면 `AtlThrow`와 동일한 서명으로 고유한 함수를 정의 하 고 `AtlThrow` #define 함수 이름으로 사용 합니다. 이는 .h를 제외 하 고 (예를 들어, ATL 헤더를 포함 하기 전에이 작업을 수행 해야 함을 의미 합니다. 의사 SAL 경고를 방지 하기 위해 함수 `__declspec(noreturn)` 특성입니다.
...
_ATL_NO_EXCEPTIONS MFC 프로젝트에 정의 되어 있지 않은 경우이 함수는 `GetLastError`에서 반환 된 값에 따라 [Cmemoryexception](../../mfc/reference/cmemoryexception-class.md) 또는 [coleexception](../../mfc/reference/coleexception-class.md) 을 throw 합니다.
