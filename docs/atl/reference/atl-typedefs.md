...
|[CContainedWindow](#ccontainedwindow)|이 클래스는의 `CContainedWindowT`특수화입니다.|
|[CPath](#cpath)|을 사용 하 `CString`는 [cpatht](../../atl/reference/cpatht-class.md) 의 특수화입니다.|
|[CPathA](#cpatha)|을 사용 하 `CStringA`는 [cpatht](../../atl/reference/cpatht-class.md) 의 특수화입니다.|
|[CPathW](#cpathw)|을 사용 하 `CStringW`는 [cpatht](../../atl/reference/cpatht-class.md) 의 특수화입니다.|
...
응용 프로그램에서 사용 하는 스레딩 모델에 따라 **typedef** 이름이 `CComGlobalsThreadModel` [CComSingleThreadModel](../../atl/reference/ccomsinglethreadmodel-class.md) 또는 [CComMultiThreadModel](../../atl/reference/ccommultithreadmodel-class.md)를 참조 합니다. 이러한 클래스는 임계 `typedef` 영역 클래스를 참조 하는 추가 이름을 제공 합니다.

> [!NOTE]
> `CComGlobalsThreadModel`[CComMultiThreadModelNoCS](../../atl/reference/ccommultithreadmodelnocs-class.md)클래스를 참조 하지 않습니다.

를 `CComGlobalsThreadModel` 사용 하면 특정 스레딩 모델 클래스를 지정할 수 있습니다. 사용 되는 스레딩 모델에 관계 없이 적절 한 메서드가 호출 됩니다.

뿐만 아니라 `CComGlobalsThreadModel`ATL은 [CComObjectThreadModel](#ccomobjectthreadmodel) **typedef** 이름을 제공 합니다. 각 `typedef` 에서 참조 하는 클래스는 다음 표에 나와 있는 것 처럼 사용 된 스레딩 모델에 따라 달라 집니다.
...
단일 `CComObjectThreadModel` 개체 클래스 내에서를 사용 합니다. 프로그램 `CComGlobalsThreadModel` 에서 전역적으로 사용할 수 있는 개체에 사용 하거나 여러 스레드에서 모듈 리소스를 보호 하려는 경우를 사용 합니다.
...
응용 프로그램에서 사용 하는 스레딩 모델에 `typedef` 따라 이름이 `CComObjectThreadModel` [CComSingleThreadModel](../../atl/reference/ccomsinglethreadmodel-class.md) 또는 [CComMultiThreadModel](../../atl/reference/ccommultithreadmodel-class.md)를 참조 합니다. 이러한 클래스는 임계 `typedef` 영역 클래스를 참조 하는 추가 이름을 제공 합니다.

> [!NOTE]
> `CComObjectThreadModel`[CComMultiThreadModelNoCS](../../atl/reference/ccommultithreadmodelnocs-class.md)클래스를 참조 하지 않습니다.

를 `CComObjectThreadModel` 사용 하면 특정 스레딩 모델 클래스를 지정할 수 있습니다. 사용 되는 스레딩 모델에 관계 없이 적절 한 메서드가 호출 됩니다.

뿐만 아니라 `CComObjectThreadModel`ATL은 [CComGlobalsThreadModel](#ccomglobalsthreadmodel) **typedef** 이름을 제공 합니다. 각 **typedef** 에서 참조 하는 클래스는 다음 표에 나와 있는 것 처럼 사용 된 스레딩 모델에 따라 달라 집니다.
...
단일 `CComObjectThreadModel` 개체 클래스 내에서를 사용 합니다. 프로그램 `CComGlobalsThreadModel` 에서 전역적으로 사용할 수 있는 개체 또는 여러 스레드에서 모듈 리소스를 보호 하려는 경우에 사용 합니다.
...
이 클래스는의 `CContainedWindowT`특수화입니다.
...
을 사용 하 `CString`는 [cpatht](../../atl/reference/cpatht-class.md) 의 특수화입니다.
...
을 사용 하 `CStringA`는 [cpatht](../../atl/reference/cpatht-class.md) 의 특수화입니다.
...
을 사용 하 `CStringW`는 [cpatht](../../atl/reference/cpatht-class.md) 의 특수화입니다.
