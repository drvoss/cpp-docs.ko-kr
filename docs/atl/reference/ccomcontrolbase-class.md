...
|[CComControlBase::DoesVerbActivate](#doesverbactivate)|`IOleObjectImpl::DoVerb`에서 사용 하는 *iverb* 매개 변수가 컨트롤의 사용자 인터페이스 (*iverb* equals OLEIVERB_UIACTIVATE)를 활성화 하는지 확인 하 고, 사용자가 컨트롤을 두 번 클릭할 때 수행 되는 동작을 정의*합니다 (iverb equals OLEIVERB_PRIMARY* ). 컨트롤을 표시 하거나 (*iverb가* OLEIVERB_SHOW) 컨트롤을 활성화 합니다 (*iverb* equals OLEIVERB_INPLACEACTIVATE).|
|[CComControlBase::DoesVerbUIActivate](#doesverbuiactivate)|`IOleObjectImpl::DoVerb`에서 사용 하는 *Iverb* 매개 변수가 컨트롤의 사용자 인터페이스를 활성화 하 고 TRUE를 반환 하는지 확인 합니다.|
...
|[CComControlBase::GetAmbientAppearance](#getambientappearance)|컨트롤에 대 한 현재 모양 설정인 DISPID_AMBIENT_APPEARANCE를 검색 합니다. 즉, 평면에 대해 0을, 3D의 경우 1을 검색 합니다.|
|[CComControlBase::GetAmbientAutoClip](#getambientautoclip)|컨테이너가 컨트롤 표시 영역의 자동 클리핑을 지원 하는지 여부를 나타내는 플래그 인 DISPID_AMBIENT_AUTOCLIP를 검색 합니다.|
...
|[CComControlBase::GetAmbientDisplayAsDefault](#getambientdisplayasdefault)|컨테이너에서이 사이트의 컨트롤을 기본 단추로 표시 한 경우 TRUE 인 플래그 인 DISPID_AMBIENT_DISPLAYASDEFAULT를 검색 하므로 단추 컨트롤이 더 두꺼운 프레임을 사용 하 여 자체적으로 그려야 합니다.|
...
|[CComControlBase::GetAmbientMessageReflect](#getambientmessagereflect)|컨테이너가 이벤트로 창 메시지 (예: WM_DRAWITEM)를 받을지 여부를 나타내는 플래그 DISPID_AMBIENT_MESSAGEREFLECT를 검색 합니다.|
...
|[CComControlBase::GetAmbientRightToLeft](#getambientrighttoleft)|컨테이너에서 콘텐츠를 표시 하는 방향으로 DISPID_AMBIENT_RIGHTTOLEFT를 검색 합니다.|
|[CComControlBase::GetAmbientScaleUnits](#getambientscaleunits)|레이블 표시를 위해 컨테이너의 주변 단위 (예: 인치 또는 센티미터)를 DISPID_AMBIENT_SCALEUNITS 검색 합니다.|
...
|[CComControlBase::GetAmbientTextAlign](#getambienttextalign)|컨테이너에서 선호 하는 텍스트 맞춤 인 DISPID_AMBIENT_TEXTALIGN를 검색 합니다. 즉, 일반 맞춤의 경우 0, 왼쪽에는 텍스트, 왼쪽 맞춤의 경우 1, 가운데 맞춤의 경우 2, 오른쪽 맞춤의 경우 3입니다.|
|[CComControlBase::GetAmbientTopToBottom](#getambienttoptobottom)|컨테이너에서 콘텐츠를 표시 하는 방향으로 DISPID_AMBIENT_TOPTOBOTTOM를 검색 합니다.|
|[CComControlBase::GetAmbientUIDead](#getambientuidead)|컨테이너에서 컨트롤을 사용자 인터페이스 작업에 응답할지 여부를 나타내는 플래그 인 DISPID_AMBIENT_UIDEAD를 검색 합니다.|
|[CComControlBase::GetAmbientUserMode](#getambientusermode)|컨테이너가 실행 모드 (TRUE) 인지, 아니면 디자인 모드 (FALSE) 인지를 나타내는 플래그를 DISPID_AMBIENT_USERMODE를 검색 합니다.|
...
|[CComControlBase::OnDrawAdvanced](#ondrawadvanced)|기본 `OnDrawAdvanced`는 그리기를 위해 정규화 된 장치 컨텍스트를 준비한 다음 컨트롤 클래스의 `OnDraw` 메서드를 호출 합니다.|
...
|[CComControlBase::OnPaint](#onpaint)|그리기를 위해 컨테이너를 준비 하 고, 컨트롤의 클라이언트 영역을 가져온 다음, 컨트롤 클래스의 `OnDraw` 메서드를 호출 합니다.|
...
|[CComControlBase::m_bDrawFromNatural](#m_bdrawfromnatural)|`IDataObjectImpl::GetData` 및 `CComControlBase::GetZoomInfo`가 `m_sizeExtent`아닌 `m_sizeNatural`에서 컨트롤 크기를 설정 해야 함을 나타내는 플래그입니다.|
|[CComControlBase::m_bDrawGetDataInHimetric](#m_bdrawgetdatainhimetric)|그릴 때 `IDataObjectImpl::GetData`에서 픽셀을 사용 하지 않고 HIMETRIC 단위를 사용 해야 함을 나타내는 플래그입니다.|
...
|[CComControlBase::m_spAmbientDispatch](#m_spambientdispatch)|`IDispatch` 포인터를 통해 컨테이너의 속성을 검색 하 고 설정할 수 있는 `CComDispatchDriver` 개체입니다.|
...
이 클래스는 ATL 컨트롤을 만들고 관리 하기 위한 메서드를 제공 합니다. [CComControl 클래스](../../atl/reference/ccomcontrol-class.md) 는 `CComControlBase`에서 파생 됩니다. ATL 컨트롤 마법사를 사용 하 여 표준 컨트롤 또는 DHTML 컨트롤을 만들면 마법사가 자동으로 `CComControlBase`에서 클래스를 파생 시킵니다.
...
ATL 컨트롤 마법사는 short 형식의 `m_nAppearance` 스톡 속성을 추가 합니다. 다른 데이터 형식을 사용 하는 경우 `AppearanceType`를 재정의 합니다.
...
컨트롤이 창에 있는 경우 [DestroyWindow](/windows/win32/api/winuser/nf-winuser-destroywindow)를 호출 하 여이를 소멸 `~CComControlBase`.
...
`IOleObjectImpl::DoVerb`에서 사용 하는 *iverb* 매개 변수가 컨트롤의 사용자 인터페이스 (*iverb* equals OLEIVERB_UIACTIVATE)를 활성화 하는지 확인 하 고, 사용자가 컨트롤을 두 번 클릭할 때 수행 되는 동작을 정의*합니다 (iverb equals OLEIVERB_PRIMARY* ). 컨트롤을 표시 하거나 (*iverb가* OLEIVERB_SHOW) 컨트롤을 활성화 합니다 (*iverb* equals OLEIVERB_INPLACEACTIVATE).
...
`IOleObjectImpl::DoVerb`에서 사용 하는 *Iverb* 매개 변수가 컨트롤의 사용자 인터페이스를 활성화 하 고 TRUE를 반환 하는지 확인 합니다.
...
컨테이너가 컨트롤 표시 영역의 자동 클리핑을 지원 하는지 여부를 나타내는 플래그 인 DISPID_AMBIENT_AUTOCLIP를 검색 합니다.
...
컨테이너가 이벤트로 창 메시지 (예: `WM_DRAWITEM`)를 받을지 여부를 나타내는 플래그 DISPID_AMBIENT_MESSAGEREFLECT를 검색 합니다.
...
ATL은 특정 속성을 검색 하는 도우미 함수 집합을 제공 합니다 (예 [: CComControlBase:: GetAmbientBackColor](#getambientbackcolor)). 사용할 수 있는 적절 한 방법이 없는 경우 `GetAmbientProperty`를 사용 합니다.
...
또한이 메서드는 컨트롤에 대 한 `IOleInPlaceSite`, `IOleInPlaceSiteEx`또는 `IOleInPlaceSiteWindowless` 인터페이스 포인터를 검색 하 여 컨트롤 클래스의 데이터 멤버 [CComControlBase:: m_spInPlaceSite](#m_spinplacesite)에 저장 합니다. 컨트롤 클래스 데이터 멤버 [CComControlBase:: m_bInPlaceSiteEx](#m_binplacesiteex), [CComControlBase:: m_bWndLess](#m_bwndless), [CComControlBase:: m_bWasOnceWindowless](#m_bwasoncewindowless)및 [CComControlBase:: m_bNegotiatedWnd](#m_bnegotiatedwnd) 은 적절 하 게 true로 설정 됩니다.
...
이 플래그는 `IOleObjectImpl::SetExtent`에 의해 확인 되며 TRUE 이면 함수가 E_FAIL을 반환 합니다.
...
ATL 컨트롤 마법사의 **스톡 속성** 탭에서 [자동 크기](../../atl/reference/stock-properties-atl-control-wizard.md) 옵션을 추가 하는 경우 마법사는이 데이터 멤버를 컨트롤 클래스에 자동으로 만들고, 속성에 대 한 put 및 get 메서드를 만들며, [IPropertyNotifySink](/windows/win32/api/ocidl/nn-ocidl-ipropertynotifysink)을 지원 합니다. 속성이 변경 될 때 컨테이너에 자동으로 알리려면입니다.

##  <a name="m_bdrawfromnatural"></a>  CComControlBase::m_bDrawFromNatural

`IDataObjectImpl::GetData` 및 `CComControlBase::GetZoomInfo`가 `m_sizeExtent`아닌 `m_sizeNatural`에서 컨트롤 크기를 설정 해야 함을 나타내는 플래그입니다.
...
그릴 때 `IDataObjectImpl::GetData`에서 픽셀을 사용 하지 않고 HIMETRIC 단위를 사용 해야 함을 나타내는 플래그입니다.
...
데이터 멤버 `m_spInPlaceSite`는 [ 및 ](/windows/win32/api/oleidl/nn-oleidl-ioleinplacesite) 플래그의 값에 따라 [IOleInPlaceSite](/windows/win32/api/ocidl/nn-ocidl-ioleinplacesiteex), [IOleInPlaceSiteEx](/windows/win32/api/ocidl/nn-ocidl-ioleinplacesitewindowless) 또는 `m_bWndLess`IOleInPlaceSiteWindowless`m_bInPlaceSiteEx` 인터페이스를 가리킵니다. `m_spInPlaceSite` 포인터가 유효 하려면 데이터 멤버 `m_bNegotiatedWnd` TRUE 여야 합니다.

`m_bWndLess` FALSE이 고 `m_bInPlaceSiteEx` TRUE 이면 `m_spInPlaceSite` `IOleInPlaceSiteEx` 인터페이스 포인터입니다. 이러한 세 데이터 멤버 간의 관계를 보여 주는 테이블은 [m_spInPlaceSite](#m_spinplacesite) 를 참조 하세요.
...
`m_spInPlaceSite` 포인터가 유효 하려면 `m_bNegotiatedWnd` 플래그가 TRUE 여야 합니다.
...
이 플래그는 [IOleObjectImpl:: SetExtent](../../atl/reference/ioleobjectimpl-class.md#setextent) 확인 하 고 TRUE 인 경우 `SetExtent`는 뷰 변경 내용으로 컨테이너에 알립니다. 이 플래그가 설정 되 면 [Olemisc](/windows/win32/api/oleidl/ne-oleidl-olemisc) 열거의 OLEMISC_RECOMPOSEONRESIZE 비트가 설정 되어야 합니다.
...
`m_bRequiresSave`의 값은 [CComControlBase:: SetDirty](#setdirty) 를 사용 하 여 설정 하 고 [CComControlBase:: getdirty](#getdirty)로 검색할 수 있습니다.
...
이 플래그는 `IOleObjectImpl::SetExtent`에 의해 확인 되며 TRUE 이면 `SetExtent`에 전달 된 크기가 `m_sizeNatural`에 할당 됩니다.

`SetExtent`에 전달 되는 크기는 `m_bResizeNatural`값에 관계 없이 항상 `m_sizeExtent`에 할당 됩니다.
...
`m_bWndLess` 및 [CComControlBase:: m_bInPlaceSiteEx](#m_binplacesiteex) 플래그의 값에 따라 `m_spInPlaceSite` 데이터 멤버가 [IOleInPlaceSite](/windows/win32/api/oleidl/nn-oleidl-ioleinplacesite), [IOleInPlaceSiteEx](/windows/win32/api/ocidl/nn-ocidl-ioleinplacesiteex)또는 [IOleInPlaceSiteWindowless](/windows/win32/api/ocidl/nn-ocidl-ioleinplacesitewindowless) 인터페이스를 가리킵니다. [CComControlBase:: m_spInPlaceSite](#m_spinplacesite) 포인터가 유효 하려면 [CComControlBase:: m_bNegotiatedWnd](#m_bnegotiatedwnd) 데이터 멤버를 TRUE로 설정 해야 합니다.

`m_bWndLess` TRUE 이면 `m_spInPlaceSite` `IOleInPlaceSiteWindowless` 인터페이스 포인터입니다. 이러한 데이터 멤버 간의 전체 관계를 보여 주는 테이블은 [CComControlBase:: m_spInPlaceSite](#m_spinplacesite) 를 참조 하세요.
...
이 크기는 고정 되어 있지만 `m_sizeExtent` 크기는 표시에 의해 조정 됩니다.
...
`IDispatch` 포인터를 통해 개체의 속성을 검색 하 고 설정할 수 있는 `CComDispatchDriver` 개체입니다.
...
기본 `OnDraw`는 [CComControlBase:: OnDrawAdvanced](#ondrawadvanced)에 설정 된 플래그에 따라 장치 컨텍스트를 삭제 또는 복원 하거나 아무 작업도 수행 하지 않습니다.

`OnDraw` 메서드는 ATL 컨트롤 마법사를 사용 하 여 컨트롤을 만들 때 컨트롤 클래스에 자동으로 추가 됩니다. 마법사의 기본 `OnDraw` 레이블이 "ATL 8.0" 인 사각형을 그립니다.
...
기본 `OnDrawAdvanced`는 그리기를 위해 정규화 된 장치 컨텍스트를 준비한 다음 컨트롤 클래스의 `OnDraw` 메서드를 호출 합니다.
...
그리기를 위해 컨테이너를 준비 하 고, 컨트롤의 클라이언트 영역을 가져온 다음, 컨트롤 클래스의 `OnDrawAdvanced` 메서드를 호출 합니다.
...
*WParam* 이 NULL이 아닌 경우 `OnPaint`는 유효한 HDC를 포함 하 고 있는 것으로 가정 하 고 [CComControlBase:: m_hWndCD](#m_hwndcd)대신 사용 합니다.
...
컨트롤이 마지막으로 저장 된 후에 변경 되었음을 플래그를 지정 하기 위해 `SetDirty(TRUE)`를 호출 해야 합니다. `m_bRequiresSave`의 값은 [CComControlBase:: GetDirty](#getdirty)를 사용 하 여 검색 됩니다.
