...
|[AtlSetChildSite](#atlsetchildsite)|자식 사이트의 `IUnknown`를 초기화 합니다.|
...
ActiveX 컨트롤을 포함 하는 대화 상자 템플릿을 사용 하 여 `AtlAxDialogBox`를 사용 하려면 동일한 섹션의 *클래스 이름* 필드와 함께 "AtlAxWin80"와 함께 대화 리소스의 **control** 섹션 *텍스트* 필드로 올바른 CLSID, APPID 또는 URL 문자열을 지정 합니다. 다음은 유효한 **컨트롤** 섹션이 표시 되는 모습을 보여 줍니다.
...
- `"MSCAL.Calendar.7"`와 같은 ProgID

- `"{8E27C92B-1264-101C-8A2F-040224009C02}"`와 같은 CLSID

- URL (예: `"<https://www.microsoft.com>"`

- `"file://\\\Documents\MyDoc.doc"` 같은 활성 문서에 대 한 참조

- `"MSHTML:\<HTML>\<BODY>This is a line of text\</BODY>\</HTML>"`와 같은 HTML 조각
...
*ppUnkContainer*<br/>
제한이 컨테이너의 `IUnknown` 받는 포인터의 주소입니다. NULL일 수 있습니다.
...
*ppUnkControl*<br/>
제한이 만든 컨트롤의 `IUnknown`을 받는 포인터의 주소입니다. NULL일 수 있습니다.

*iidSink*<br/>
포함 된 개체의 송신 인터페이스에 대 한 인터페이스 식별자입니다.

*punkSink*<br/>
포함 된 개체가 성공적으로 생성 된 후 포함 된 개체에서 *Iidsink* 로 지정 된 연결 지점에 연결할 싱크 개체의 `IUnknown` 인터페이스에 대 한 포인터입니다.
...
*ppUnkContainer*<br/>
컨테이너의 `IUnknown` 받는 포인터의 주소입니다. NULL일 수 있습니다.
...
`AtlAxCreateControlLic`사용 방법에 대 한 샘플은 [ATL AXHost를 사용 하 여 ActiveX 컨트롤 호스팅](../../atl/hosting-activex-controls-using-atl-axhost.md) 을 참조 하세요.
...
`AtlAxCreateControlLicEx`는 [AtlAxCreateControlLic](#atlaxcreatecontrollic) 와 비슷하지만 새로 만든 컨트롤에 대 한 인터페이스 포인터를 받고 컨트롤에서 발생 한 이벤트를 수신 하도록 이벤트 싱크를 설정할 수도 있습니다.

### <a name="example"></a>예제

`AtlAxCreateControlLicEx`사용 방법에 대 한 샘플은 [ATL AXHost를 사용 하 여 ActiveX 컨트롤 호스팅](../../atl/hosting-activex-controls-using-atl-axhost.md) 을 참조 하세요.
...
*pControl*<br/>
진행 컨트롤의 `IUnknown`에 대 한 포인터입니다.

*hWnd*<br/>
진행 컨트롤을 호스팅하는 창에 대 한 핸들입니다.

*ppUnkContainer*<br/>
제한이 컨테이너 개체의 `IUnknown`에 대 한 포인터에 대 한 포인터입니다.
...
> [!NOTE]
>  `AtlAxAttachControl`를 호출 하기 전에 연결 중인 컨트롤 개체를 올바르게 초기화 해야 합니다.
...
자식 개체의 사이트를 부모 개체의 `IUnknown` 설정 하려면이 함수를 호출 합니다.
...
> [!NOTE]
>  이 함수에서 요청 된 정보를 성공적으로 검색 하려면 *punkObj* 로 표시 되는 개체는 `IDispatch` (및 `IDispatch::GetTypeInfo`를 통해 반환 형식 정보)를 구현 하 고 `IProvideClassInfo2` 또는 `IPersist`도 구현 해야 합니다. 원본 인터페이스에 대 한 형식 정보는 `IDispatch`에 대 한 형식 정보와 동일한 형식 라이브러리에 있어야 합니다.

### <a name="example"></a>예제

아래 예제에서는 `CEasySink`이벤트 싱크 클래스를 정의 하는 방법을 보여 줍니다 .이를 통해 `IDispEventImpl`에 전달할 수 있는 템플릿 인수의 수가 완전 하지 않을 수 있습니다. [DispEventAdvise](idispeventsimpleimpl-class.md#dispeventadvise) 또는 [DispEventUnadvise](idispeventsimpleimpl-class.md#dispeventunadvise)를 호출 하기 전에 `EasyAdvise` 및 `EasyUnadvise` `AtlGetObjectSourceInterface`를 사용 하 여 [IDispEventImpl](../../atl/reference/idispeventimpl-class.md) 멤버를 초기화 합니다.
