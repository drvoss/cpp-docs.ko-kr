...
|[Copy](#copy)|한 배열의 요소를 다른 배열의 요소에 복사 하려면이 메서드를 호출 합니다.|
...
`CAtlArray`는 사용자 정의 형식의 요소 배열을 만들고 관리 하는 메서드를 제공 합니다. 표준 C 배열과 유사 하지만 `CAtlArray` 개체는 필요에 따라 동적으로 축소 하 고 확장할 수 있습니다. 배열 인덱스는 항상 0 위치에서 시작 하 고, 상한을 고정 하거나, 새 요소가 추가 될 때 확장할 수 있습니다.
...
디버그 빌드에서는 `CAtlArray` 인수가 올바른 배열이 아니거나 *Asrc* 가 같은 개체를 참조 하는 경우에는이 예외가 발생 합니다. 릴리스 빌드에서 잘못 된 인수로 인해 예기치 않은 동작이 발생할 수 있습니다.
...
디버그 빌드에서는 기존 `CAtlArray` 개체가 유효 하지 않거나 *Asrc* 가 같은 개체를 참조 하는 경우에는 해당 어설션이 발생 합니다. 릴리스 빌드에서 잘못 된 인수로 인해 예기치 않은 동작이 발생할 수 있습니다.
...
배열의 첫 번째 요소가 위치 0에 있기 때문에 `GetCount`에서 반환 된 값은 항상 가장 큰 인덱스 보다 1 큽니다.
...
디버그 빌드에서는 `CAtlArray` 개체가 유효 하지 않거나 *paNew* 포인터가 NULL 또는 잘못 된 경우에는 해당 되지 않는 어설션이 발생 합니다.
...
디버그 빌드에서는 `CAtlArray` 개체가 잘못 되었거나, 추가할 요소의 수가 0 이거나, 결합 된 요소의 수가 너무 커서 배열에 포함 될 수 없는 경우에는이 예외가 발생 합니다. 정품 빌드에서 잘못 된 매개 변수를 전달 하면 예기치 않은 결과가 발생할 수 있습니다.
...
디버그 빌드에서는 `CAtlArray` 개체가 유효 하지 않은 경우 또는 결합 된 총 합계와 *n 수가* 배열의 총 요소 수를 *초과 하는* 경우 관리 되지 않는 어설션이 발생 합니다. 정품 빌드에서 잘못 된 매개 변수는 예기치 않은 결과를 일으킬 수 있습니다.
...
이 메서드를 사용 하 여 배열의 크기를 설정 합니다. `SetCount`를 사용 하지 않으면 요소를 추가 하는 프로세스와 후속 메모리 할당이 수행 되 면 성능 및 조각 메모리가 줄어듭니다.
...
인덱스에서 가리키는 요소의 값을 바꿉니다. *IElement* 가 배열의 현재 크기 보다 크면 [CAtlArray:: setcount](#setcount)호출을 사용 하 여 배열이 자동으로 증가 합니다. 디버그 빌드에서는 `CAtlArray` 개체가 유효 하지 않은 경우에는 해당 되지 않는 어설션이 발생 합니다. 정품 빌드에서 잘못 된 매개 변수는 예기치 않은 결과를 일으킬 수 있습니다.
