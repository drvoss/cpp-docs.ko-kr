...
|[CAxDialogImpl::D estroyWindow](#destroywindow)|모덜리스 대화 상자를 삭제 하려면이 메서드를 호출 합니다.|
|[CAxDialogImpl::D oModal](#domodal)|이 메서드를 호출 하 여 모달 대화 상자를 만듭니다.|
...
`CAxDialogImpl`를 사용 하 여 모달 또는 모덜리스 대화 상자를 만들 수 있습니다. `CAxDialogImpl`는 기본 메시지 맵을 사용 하 여 메시지를 적절 한 처리기에 전달 하는 대화 상자 프로시저를 제공 합니다.

`CAxDialogImpl`는 `CDialogImplBaseT`에서 파생 되며,이는 기본적으로 `CWindow`) 및 *`CMessageMap`에서 파생* 됩니다.
...
`CAxDialogImpl`를 사용 하 여 만든 모달 대화 상자의 ActiveX 컨트롤은 액셀러레이터 키를 지원 하지 않습니다. `CAxDialogImpl`를 사용 하 여 만든 대화 상자에서 액셀러레이터 키를 지원 하려면 모덜리스 대화 상자를 만들고 고유한 메시지 루프를 사용 하 여 큐에서 메시지를 가져와 액셀러레이터 키를 처리 한 후 [Caxdialogimpl:: IsDialogMessage](#isdialogmessage) 를 사용 합니다.

`CAxDialogImpl`에 대 한 자세한 내용은 [ATL 컨트롤 포함 FAQ](../../atl/atl-control-containment-faq.md)를 참조 하십시오.
...
`DestroyWindow`를 호출 하 여 모달 대화 상자를 삭제 하지 마십시오. 대신 [EndDialog](#enddialog) 를 호출 합니다.
...
대화 상자 프로시저를 통해 `EndDialog`를 호출 해야 합니다. 대화 상자를 삭제 한 후에는 *nRetCode* 의 값을 대화 상자를 만든 `DoModal`의 반환 값으로 사용 합니다.

> [!NOTE]
>  `EndDialog`를 호출 하 여 모덜리스 대화 상자를 삭제 하지 마십시오. 대신 [DestroyWindow](#destroywindow) 를 호출 합니다.
