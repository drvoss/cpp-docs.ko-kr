...
- **데이터 원본**
  ...
  - **연결** 탭
      ...
      이 탭의 내용은 선택한 공급자에 따라 달라집니다. 여러 가지 유형의 공급자가 있지만 이 섹션에서는 SQL 및 ODBC 데이터라는 두 가지 가장 일반적인 연결에 대해 다룹니다. 다른 공급자도 여기서 설명하는 필드와 비슷합니다.

      SQL 데이터:

      1. **서버 이름을 선택하거나 입력합니다.** 드롭다운 목록 메뉴를 클릭하여 네트워크에 등록된 모든 데이터 서버를 표시하고 하나를 선택합니다.

      1. **서버에 로그온할 정보를 입력합니다.** 데이터 서버에 로그온할 사용자 이름과 암호를 입력합니다.
      ...
      1. **서버에서 데이터베이스를 선택합니다.** 드롭다운 목록 메뉴를 클릭하여 데이터 서버에 등록된 모든 데이터베이스를 표시하고 하나를 선택합니다.

         \- 또는-

         **데이터베이스 파일을 데이터베이스 이름으로 연결합니다.** 데이터베이스로 사용할 파일을 지정합니다. 을 입력합니다.

      ODBC 데이터:

      1. **데이터 원본 지정:** 데이터 원본 이름 또는 연결 문자열을 사용할 수 있습니다.

         **데이터 원본 이름 사용:** 이 드롭다운 목록에는 컴퓨터에 등록된 데이터 원본이 표시됩니다. ODBC 데이터 소스 관리자를 사용하여 데이터 소스를 미리 설정할 수 있습니다.

         \- 또는-

         **연결 문자열 사용:** 이미 얻은 연결 문자열을 입력하거나 **빌드** 단추를 클릭합니다. **데이터 원본 선택** 대화 상자가 나타납니다. 파일 또는 머신 데이터 소스를 선택하고 **확인**을 클릭합니다.
  ...
  - **고급** 탭
      ...
      **네트워크 설정:** 가장 **수준(클라이언트를** 가장할 때 서버가 사용할 수 있는 가장 수준, RPC 가장 수준에 직접 해당) 및 **보호 수준(클라이언트와** 서버 간에 전송되는 데이터 보호 수준, RPC 보호 수준에 직접 해당)을 지정합니다.

      **기타:** **연결 시간 지정에서**시간 지정이 발생하기 전에 허용되는 유휴 시간 의 초 수를 지정합니다. **액세스 권한**에서 데이터 연결에 대한 액세스 권한을 지정합니다.
  ...
  - **모두** 탭
      ...
      **확인**을 클릭하여 작업을 마칩니다. **데이터베이스 개체 선택** 대화 상자가 나타납니다. 이 대화 상자에서 소비자가 사용할 테이블, 뷰 또는 저장 프로시저를 선택합니다.
...
- **Type**
  ...
  - **명령**
      ...
      `CCommand` 또는 `db_command`를 사용하여 명령 및 명령 접근자 클래스 선언을 만들려는 경우 이 옵션을 선택합니다. 이 옵션이 기본 옵션입니다.
