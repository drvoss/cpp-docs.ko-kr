...
이 코드 **`typedef`** `CComEnum` 는 인터페이스를 통해 변형 벡터를 노출 하는에 대 한를 만듭니다 `IEnumVariant` . `CVariantArrayCollection`클래스는 `CreateEnumerator` 이 형식의 열거자 개체를 사용 하 여 작업 하 고 필요한 인수를 전달 하는 작업을 간단 하 게 합니다.
