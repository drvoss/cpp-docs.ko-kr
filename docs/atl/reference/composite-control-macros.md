...
*fn*<br/>
진행 이벤트 처리기 함수의 이름입니다. 이 함수는 `_stdcall` 호출 규칙을 사용 해야 하 고 적절 한 기능 스타일의 서명이 있어야 합니다.
...
*info*<br/>
진행 이벤트 처리기 함수에 대 한 정보를 입력 합니다. 이 형식 정보는 `_ATL_FUNC_INFO` 구조체에 대 한 포인터 형식으로 제공 됩니다. CC_CDECL는 `_ATL_FUNC_INFO` 구조의 CALLCONV 필드에 대해 Windows CE에서 유일 하 게 지원 되는 옵션입니다. 다른 모든 값은 지원 되지 않으므로 동작이 정의 되지 않습니다.
