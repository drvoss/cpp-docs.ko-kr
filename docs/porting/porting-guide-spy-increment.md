...
전체에서 표준 **bool** 형식을 사용하는 것이 유용한지 궁금할 수도 있습니다. BOOL 형식과 같은 이전 코드 패턴은 대부분 표준 C++에서 해결된 문제를 해결하기 위한 것이므로 BOOL에서 **bool** 기본 제공 형식으로 변경하는 것은 새 버전에서 코드를 처음 실행한 후에 고려할 변경 내용 중 한 가지 예일 뿐입니다.
