...
들어오는 메시지를 필터링 하지 않는 메시지 블록의 기본 사용법을 보여 주는 다음 함수 `count_primes`를 고려 하십시오. 메시지 블록은 [std:: vector](../../standard-library/vector-class.md) 개체에 소수를 추가 합니다. `count_primes` 함수는 메시지 블록에 여러 개의 숫자를 보내고, 메시지 블록에서 출력 값을 받고, 콘솔에 소수를 인쇄 하는 숫자를 출력 합니다.

[!code-cpp[concrt-primes-filter#1](../../parallel/concrt/codesnippet/cpp/how-to-use-a-message-block-filter_1.cpp)]

`transformer` 개체는 모든 입력 값을 처리 합니다. 그러나 소수 값만 필요 합니다. 메시지 발신자가 프라임 번호만 전송 하도록 응용 프로그램을 작성할 수 있지만 메시지 수신자의 요구 사항은 항상 알 수 없습니다.

## <a name="example"></a>예제

다음 함수 `count_primes_filter`은 `count_primes` 함수와 동일한 작업을 수행 합니다. 그러나이 버전의 `transformer` 개체는 필터 함수를 사용 하 여 소수 인 값만 수락 합니다. 작업을 수행 하는 함수는 프라임 번호만 받습니다. 따라서 `is_prime` 함수를 호출할 필요가 없습니다.

`transformer` 개체는 소수 숫자만 수신 하기 때문에 `transformer` 개체 자체는 소수를 보유할 수 있습니다. 즉,이 예제에서 `transformer` 개체는 `vector` 개체에 소수를 추가 하는 데 필요 하지 않습니다.

[!code-cpp[concrt-primes-filter#2](../../parallel/concrt/codesnippet/cpp/how-to-use-a-message-block-filter_2.cpp)]

`transformer` 개체는 이제 소수 인 값만 처리 합니다. 이전 예제에서 `transformer` 개체는 모든 메시지를 처리 합니다. 따라서 이전 예제는 보내는 메시지와 동일한 수의 메시지를 받아야 합니다. 이 예제에서는 [concurrency:: send](reference/concurrency-namespace-functions.md#send) 함수의 결과를 사용 하 여 `transformer` 개체에서 수신할 메시지 수를 확인 합니다. `send` 함수는 메시지 버퍼가 메시지를 수락 하는 경우 **true** 를 반환 하 고 메시지 버퍼가 메시지를 거부 하면 **false** 를 반환 합니다. 따라서 메시지 버퍼가 메시지를 수락 하는 횟수는 소수 수와 일치 합니다.

## <a name="example"></a>예제

다음 코드에서는 전체 예제를 보여 줍니다. 이 예제에서는 `count_primes` 함수와 `count_primes_filter` 함수를 모두 호출 합니다.

[!code-cpp[concrt-primes-filter#3](../../parallel/concrt/codesnippet/cpp/how-to-use-a-message-block-filter_3.cpp)]

## <a name="compiling-the-code"></a>코드 컴파일

예제 코드를 복사 하 여 Visual Studio 프로젝트에 붙여넣거나, `primes-filter.cpp` 이름이 지정 된 파일에 붙여 넣은 후 Visual Studio 명령 프롬프트 창에서 다음 명령을 실행 합니다.
