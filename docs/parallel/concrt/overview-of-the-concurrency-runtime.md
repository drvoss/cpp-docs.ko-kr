...
Visual Studio 2010부터 2013까지 동시성 런타임 msvcr120.dll를 통해 경우 msvcr100.dll 내에 통합 되었습니다.  Visual Studio 2015에서 작업 중에 발생 한 DLL은 다음 세 부분으로 리팩터링 되었습니다.

- Windows 10에서 제공 되 고 Windows 업데이트를 통해 서비스 하위 수준으로 제공 되는 C API

- vcruntime140 – Visual Studio를 통해 제공 되는 컴파일러 지원 함수 및 EH 런타임

- concrt140 – 동시성 런타임 Visual Studio를 통해 제공 됩니다. `concurrency::parallel_for`와 같은 병렬 컨테이너 및 알고리즘에 필요 합니다. 또한 Windows XP에는 조건 변수가 없으므로 STL을 사용 하려면 Windows XP에서이 DLL을 전원 동기화 기본 형식으로 설정 해야 합니다.
...
## <a name="runtime"></a>동시성 런타임이 중요 한 이유
...
다음 예제에서는 [동시성::p arallel_for_each](reference/concurrency-namespace-functions.md#parallel_for_each) 알고리즘에 대 한 여러 호출에서 람다 함수, 함수 개체 및 함수 포인터의 구문을 비교 합니다. `parallel_for_each`에 대 한 각 호출에서는 다른 기법을 사용 하 여 [std:: array](../../standard-library/array-class-stl.md) 개체의 각 요소에 대 한 사각형을 계산 합니다.
...
의 C++람다 함수에 대 한 자세한 내용은 [람다 식](../../cpp/lambda-expressions-in-cpp.md)을 참조 하세요.
