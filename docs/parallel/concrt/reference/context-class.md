...
`Block` 메서드를 호출 하거나 호출한 후에는 해당 메서드를 다시 실행 하기 위해 다른 실행 컨텍스트에서 [차단 해제](#unblock) 메서드 호출과 연결 해야 합니다. 코드에서 `Unblock` 메서드를 호출할 수 있는 다른 스레드에 대 한 컨텍스트를 게시 하는 지점과 실제 메서드 호출이 `Block` 되는 지점 사이에는 중요 한 기간이 있습니다. 이 기간 동안에는 잠금 가져오기 등과 같은 개별적인 이유로 차단하거나 차단 해제할 수 있는 메서드를 호출해서는 안 됩니다. `Block` 및 `Unblock` 메서드를 호출 하면 차단 및 차단 해제 이유가 추적 되지 않습니다. 하나의 개체만 `Block`- `Unblock` 쌍의 소유권을 가져야 합니다.
...
현재 컨텍스트가 가상 프로세서에서 실행 되 고 있는 경우 컨텍스트가 현재 실행 중인 가상 프로세서의 식별자입니다. 그렇지 않으면 값이 `-1`됩니다.
...
현재 컨텍스트가 스케줄러에 연결 된 경우 현재 컨텍스트가 속한 스케줄러 내에서 고유한 현재 컨텍스트의 식별자입니다. 그렇지 않으면 값이 `-1`됩니다.
...
스케줄러를 호출 컨텍스트에 연결 하 고 작업 그룹에서 해당 컨텍스트에서 작업을 인라인으로 실행 하는 경우 해당 작업 그룹이 활성 취소를 기반으로 하 고 있는지 여부를 나타냅니다. 그렇지 않으면 값이 `false`됩니다.
...
런타임에 의해 `Context` 개체가 내부적으로 소멸 됩니다. 개체를 명시적으로 삭제할 수 없습니다.
...
**True**이면 초과 구독 기간 동안 추가 가상 프로세서를 추가 해야 함을 나타냅니다. **False**이면 초과 구독을 종료 하 고 이전에 추가한 가상 프로세서를 제거 해야 함을 나타냅니다.
...
현재 컨텍스트가 스케줄러에 연결 되어 있고 일정 그룹에서 작업 하는 경우 현재 컨텍스트가 작동 중인 스케줄러 그룹의 식별자입니다. 그렇지 않으면 값이 `-1`됩니다.
...
`Unblock` 메서드를 호출 하는 것은 해당 하는 [Block](#block) 메서드에 대 한 해당 호출 앞에와 야 합니다. `Block` 및 `Unblock` 메서드에 대 한 호출이 적절 하 게 쌍을 이루는 경우 런타임에서는 두 순서 중 어느 것의 자연 레이스를 적절히 처리 합니다. `Block` 호출 이전에 발생 하는 `Unblock` 호출은 단순히 `Block` 호출의 효과를 부정 합니다.

이 메서드에서 throw 할 수 있는 몇 가지 예외가 있습니다. 컨텍스트에서 `Unblock` 메서드를 자체적으로 호출 하려고 하면 [context_self_unblock](context-self-unblock-class.md) 예외가 throw 됩니다. `Block`에 대 한 호출이 제대로 쌍을 이루지 않는 경우 (예: 현재 실행 되 `Unblock` 고 있는 컨텍스트에 대해 두 번의 `Unblock` 호출이 수행 되는 경우) [context_unblock_unbalanced](context-unblock-unbalanced-class.md) 예외가 throw 됩니다.

코드에서 `Unblock` 메서드를 호출할 수 있는 다른 스레드에 대 한 컨텍스트를 게시 하는 지점과 실제 메서드 호출이 `Block` 되는 지점 사이에는 중요 한 기간이 있습니다. 이 기간 동안에는 잠금 가져오기 등과 같은 개별적인 이유로 차단하거나 차단 해제할 수 있는 메서드를 호출해서는 안 됩니다. `Block` 및 `Unblock` 메서드를 호출 하면 차단 및 차단 해제 이유가 추적 되지 않습니다. 하나의 개체만 `Block`와 `Unblock` 쌍의 소유권을 가져야 합니다.
...
현재 컨텍스트가 스케줄러에 연결 된 경우 현재 컨텍스트가 실행 되 고 있는 가상 프로세서의 식별자입니다. 그렇지 않으면 값이 `-1`됩니다.
