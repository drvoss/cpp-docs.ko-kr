...
`IXMLHTTPRequest2`를 사용 하지만 작업을 사용 하지 않는 예제는 빠른 시작 [: XML HTTP 요청을 사용 하 여 연결 (IXMLHTTPRequest2)](/previous-versions/windows/apps/hh770550\(v=win.10\))을 참조 하세요.
...
1. MainPage.xaml.cpp에서 `GetButton_Click` 클래스의 `PostButton_Click`, `CancelButton_Click` 및 `MainPage` 메서드를 구현합니다.
   ...
   > [!TIP]
   > 앱에서 취소를 지원할 필요가 없는 경우 `HttpRequest::GetAsync` 및 `HttpRequest::PostAsync` 메서드에 [concurrency:: cancellation_token:: none](reference/cancellation-token-class.md#none) 을 전달 합니다.

1. MainPage.xaml.cpp에서 `MainPage::ProcessHttpRequest` 메서드를 구현합니다.

   [!code-cpp[concrt-using-ixhr2#A7](../../parallel/concrt/codesnippet/cpp/walkthrough-connecting-using-tasks-and-xml-http-requests_10.cpp)]

1. 프로젝트 속성의 **링커**, **입력**에서 `shcore.lib` 및 `msxml6.lib`를 지정 합니다.
...
[빠른 시작: IXMLHTTPRequest2 (XML HTTP 요청)을 사용 하 여 연결](/previous-versions/windows/apps/hh770550\(v=win.10\))
