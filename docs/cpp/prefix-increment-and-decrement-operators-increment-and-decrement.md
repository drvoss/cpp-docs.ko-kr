...
전위 증가 연산자 ( **++** )는 해당 피연산자에 하나를 추가 합니다. 이 증가 된 값은 식의 결과입니다. 피연산자는 **const**형식이 아닌 l-value 여야 합니다. 결과는 피연산자와 동일한 형식의 l-value입니다.

전위 감소 연산자 ( **--** )는 전위 증가 연산자와 유사 합니다. 단, 피연산자를 1 씩 감소 시키고 그 결과는 감소 된 값입니다.

**Visual Studio 2017 버전 15.3 이상** ( [/std: c + + 17](../build/reference/std-specify-language-standard-version.md)과 함께 사용 가능): 증가 또는 감소 연산자의 피연산자는 **bool**형식일 수 없습니다.
