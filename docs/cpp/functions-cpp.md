...
함수는 일부 작업을 수행하는 코드 블록입니다. 함수는 호출자가 함수에 인수를 전달할 수 있도록 하는 입력 매개 변수를 필요에 따라 정의할 수 있습니다. 함수는 필요에 따라 출력으로 값을 반환할 수 있습니다. 함수는 이상적으로 함수의 기능을 명확하게 설명하는 이름을 사용하여 재사용 가능한 단일 블록에서 일반 작업을 캡슐화하는 데 유용합니다. 다음 함수는 호출자의 두 정수를 허용 하 고 합계를 반환 합니다. *a* 와 *b* 는 **int**형식의 *매개 변수* 입니다.
...
1. 반환 형식-함수가 반환 하는 값의 형식을 지정 하 고, 반환 되는 값이 없는 경우에는 **void** 를 반환 합니다. C + + 11에서 **auto** 는 return 문에서 형식을 유추 하도록 컴파일러에 지시 하는 유효한 반환 형식입니다. C++14에서는 decltype(auto)도 허용됩니다. 자세한 내용은 아래의 반환 형식에서 형식 추론을 참조하세요.
...
1. 해당 링크 사양의 **extern** 또는 **static**입니다.
...
1. **인라인**-함수에 대 한 모든 호출을 함수 코드 자체로 바꾸도록 컴파일러에 지시 합니다. 인라인 처리는 성능이 중요한 코드 섹션에서 함수가 신속하게 실행되고 반복적으로 호출되는 시나리오의 성능 향상에 도움이 됩니다.
...
1. 함수에서 예외를 throw 할 수 있는지 여부를 지정 하는 `noexcept` 식입니다. 다음 예제에서는 `is_pod` 식이 **true**로 평가 되는 경우 함수에서 예외를 throw 하지 않습니다.
   ...
   자세한 내용은 [noexcept](../cpp/noexcept-cpp.md)를 참조 하세요.
...
여기서 설명 하는 경우를 제외 하 고 void **인수를** 지정 하는 것은 유효 하지 않지만 void 형식에서 파생 된 **형식 (예** **: void의 배열 및** **void**의 배열)은 인수 선언 목록의 어디에 나 나타날 수 있습니다.
...
"일반" 반환 형식은 함수 서명의 왼쪽에 있습니다. *후행 반환 형식은* 시그니처의 오른쪽에 있으며-> 연산자 앞에 있습니다. 후행 반환 형식은 반환 값의 형식이 템플릿 매개 변수에 따라 달라지는 경우 함수 템플릿에서 특히 유용합니다.
...
**Auto** 가 후행 반환 형식과 함께 사용 되는 경우 decltype 식이 생성 하는 모든 항목에 대 한 자리 표시자 역할을 하 고 자체는 형식 추론을 수행 하지 않습니다.
...
이 예에서 **auto** 는 lhs 및 rhs 합계의 비 const 값 복사로 추론 됩니다.
...
1. **Visual Studio 2017 버전 15.3 이상** ( [/std: c + + 17](../build/reference/std-specify-language-standard-version.md)과 함께 사용 가능): 구조적 바인딩을 사용 합니다. 구조적 바인딩의 장점은 반환 값을 저장 하는 변수가 선언 될 때 초기화 되므로 경우에 따라 훨씬 더 효율적일 수 있습니다. 이 문에서--`auto[x, y, z] = f();`--대괄호는 전체 함수 블록의 범위에 있는 이름을 도입 하 고 초기화 합니다.
