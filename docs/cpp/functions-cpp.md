...
매개 변수 선언 목록에서 단일 키워드를 사용 하 여 선언 된 함수는 **`void`** 키워드가 **`void`** 인수 선언 목록의 첫 번째 및 유일한 멤버인 경우 인수를 사용 하지 않습니다. **`void`** 목록의 다른 곳에 있는 형식의 인수는 오류를 생성 합니다. 예를 들면 다음과 같습니다.
