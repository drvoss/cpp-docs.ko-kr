...
멤버 선택 연산자 (**.** or **->** )는 연산자의 왼쪽에 지정 된 개체의 클래스 형식 멤버 또는 해당 기본 클래스의 멤버 여야 합니다. **->** 의 왼쪽 **->** 이 클래스 개체이 고 클래스에서 **->** 다른 클래스 형식에 대 한 포인터로 계산 되는 오버 로드 된 멤버 선택 연산자 ()를 정의 하는 경우 멤버 선택 연산자 ()의 오른쪽에 지정 된 이름은 다른 클래스 형식의 개체 일 수도 있습니다. 이 프로 비전은 [클래스 멤버 액세스](../cpp/member-access.md)에 자세히 설명 되어 있습니다.
...
1. , 및 키워드가 앞에 오는 이름은 **`class`** **`struct`** **`union`** 컴파일러가 **`class`** , 또는 이름만 검색 하도록 합니다 **`struct`** **`union`** .

1. 범위 결정 연산자 ()의 왼쪽에 있는 이름에는 `::` **`class`** ,, **`struct`** **`namespace`** 또는 이름만 사용할 수 있습니다 **`union`** .

이름이 비정적 멤버를 참조하지만 정적 멤버 함수에 사용된 경우 오류 메시지가 생성됩니다. 마찬가지로, 이름이 바깥쪽 클래스의 비정적 멤버를 참조 하는 경우에는 포함 된 클래스에는 바깥쪽 클래스 포인터가 없기 때문에 오류 메시지가 생성 됩니다 **`this`** .
