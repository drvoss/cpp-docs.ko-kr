...
ms.locfileid: "80187936"
---
# <a name="user-defined-type-conversions-c"></a>사용자 정의 형식 변환(C++)

*변환은* 다른 형식의 값에서 형식의 새 값을 생성 합니다. *표준 변환은* 언어에서 C++ 기본 제공 되는 형식을 지원 하 고, 사용자 정의 *변환을* 만들어 사용자 정의 형식 간의 변환을 수행할 수 있습니다.
...
문제를 일으킬 수 있는 암시적 변환의 한 가지 잘 알려진 예는 **bool**로의 변환입니다. 부울 컨텍스트에서 사용할 수 있는 클래스 형식을 만들 수 있습니다. 예를 들어 **if** 문 또는 루프를 제어 하는 데 사용할 수 있지만 컴파일러에서 기본 제공 형식으로의 사용자 정의 변환을 수행 하는 경우 컴파일러는 나중에 표준 변환을 추가로 적용할 수 있습니다. 이러한 추가 표준 변환은 **short** 에서 **int**로의 승격과 같은 작업을 허용 하는 것입니다. 그러나 **bool** 에서 **int**로의 경우 처럼 명확 하지 않은 변환에 대 한 도어가 열리므로 의도 하지 않은 정수 컨텍스트에서 클래스 형식을 사용할 수 있습니다. 이 특정 문제를 *안전한 Bool 문제*라고 합니다. 이러한 종류의 문제는 **explicit** 키워드에서 도움이 될 수 있습니다.

**Explicit** 키워드는 지정 된 변환을 사용 하 여 암시적 변환을 수행할 수 없음을 컴파일러에 알립니다. **명시적** 키워드를 도입 하기 전에 암시적 변환의 구문을 편리 하 게 사용 하려는 경우 암시적 변환으로 인해 발생 하는 의도 하지 않은 결과를 그대로 사용 하거나, 보다 편리 하 고 명명 된 변환 함수를 해결 방법으로 사용 해야 했습니다. 이제 **explicit** 키워드를 사용 하 여 명시적 캐스트 또는 직접 초기화를 수행 하는 데만 사용할 수 있는 편리한 변환을 만들 수 있으며,이로 인해 Safe Bool 문제로 인해 발생 하는 문제 종류를 예시 수 없습니다.

**Explicit** 키워드는 c + + 98 이후의 변환 생성자와 c + + 11 이후의 변환 함수에 적용 될 수 있습니다. 다음 섹션에는 **explicit** 키워드를 사용 하는 방법에 대 한 자세한 내용이 포함 되어 있습니다.

## <a name="conversion-constructors"></a><a name="ConvCTOR"></a>변환 생성자

변환 생성자는 사용자 정의 또는 기본 제공 형식에서 사용자 정의 형식으로의 변환을 정의합니다. 다음 예제에서는 기본 제공 형식 **double** 을 `Money`사용자 정의 형식으로 변환 하는 변환 생성자를 보여 줍니다.
...
`display_balance` 형식의 인수를 사용하는 `Money` 함수에 대한 첫 번째 호출의 경우 인수가 올바른 형식이므로 변환이 필요하지 않습니다. 그러나 `display_balance`에 대 한 두 번째 호출에서 인수의 형식, 값이 `49.95`인 **double** 이 함수에 필요한 항목이 아니기 때문에 변환이 필요 합니다. 함수는이 값을 직접 사용할 수 없지만 인수 형식에서 일치 하는 매개 변수의 형식으로 변환 하는**경우 (`Money`** ) `Money` 형식의 임시 값은 인수에서 생성 되 고 함수 호출을 완료 하는 데 사용 됩니다. `display_balance`에 대 한 세 번째 호출에서 인수는 **double**이 아니라 값이 `9.99`인 **float** 입니다. 그러나 컴파일러가 표준 변환 (이 경우에는 **float** 에서 **double**로)을 수행한 다음 **double** 에서 `Money`으로 사용자 정의 변환을 수행 하 여 필요한 변환을 완료할 수 있으므로 함수 호출을 계속 완료할 수 있습니다.
...
변환 생성자를 **명시적**으로 선언 하 여 개체의 직접 초기화를 수행 하거나 명시적 캐스트를 수행 하는 데만 사용할 수 있습니다. 이를 통해, 클래스 형식 인수를 사용하는 함수가 변환 생성자의 소스 형식 인수를 암시적으로 사용할 수 없도록 하고 클래스 형식이 소스 형식 값을 통해 복사 초기화되지 않도록 할 수 있습니다. 다음 예제에서는 명시적 변환 생성자를 정의하는 방법과 올바른 형식의 코드에서의 그 효과를 보여줍니다.
...
이 예제에서는 명시적 변환 생성자를 사용하여 `payable`에 대한 직접 초기화를 수행할 수도 있습니다. 대신, `Money payable = 79.99;`을 복사 초기화하려는 경우에는 오류가 됩니다. `display_balance`에 대한 첫 번째 호출의 경우 인수의 형식이 올바르므로 오류가 아닙니다. `display_balance`에 대한 두 번째 호출의 경우에는 암시적 변환을 수행하는 데 변환 생성자를 사용할 수 없으므로 오류입니다. `display_balance`에 대 한 세 번째 호출은 `Money`명시적 캐스트로 인해 유효 하지만, 컴파일러는 **float** 에서 **double**로의 암시적 캐스트를 삽입 하 여 캐스트를 완료 하는 데 도움이 됩니다.
...
변환 함수는 사용자 정의 형식에서 다른 형식으로의 변환을 정의합니다. 변환 생성자와 함께 이러한 함수는 값이 다른 형식으로 캐스트될 때 호출되기 때문에 종종 "캐스트 연산자"라고 합니다. 다음 예제에서는 사용자 정의 형식 `Money`를 기본 제공 형식인 **double**형식으로 변환 하는 변환 함수를 보여 줍니다.
...
`amount` 멤버 변수는 private으로 설정 되 고 **double** 형식에 대 한 공용 변환 함수는 `amount`값을 반환 하기 위해 도입 되었습니다. `display_balance` 함수에서는 스트림 삽입 연산자 `balance`를 통해 `<<`의 값이 표준 출력에 스트리밍될 때 암시적 변환이 발생합니다. `Money`사용자 정의 형식에 대해 스트림 삽입 연산자가 정의 되어 있지 않지만 기본 제공 형식에 대 한 스트림 삽입 연산자가 하나씩 있으므로 컴파일러는 변환 함수를 사용 하 여 `Money`에서 **double** 로 변환 **함수를 사용**하 여 스트림 삽입 연산자를 만족할 수 있습니다.

변환 함수는 파생 클래스에 의해 상속됩니다. 파생 클래스의 변환 함수는 완전히 같은 형식으로 변환될 경우에만 상속된 변환 함수를 재정의합니다. 예를 들어, 파생 된 클래스 **연산자 int** 의 사용자 정의 변환 함수는 표준 변환에서 **int** 와 **short**사이의 변환 관계를 정의 하는 경우에도 기본 클래스 **operator short**의 사용자 정의 변환 함수를 재정의 하거나 영향을 주지 않습니다.
...
여기서는 변환 함수 **연산자 double** 이 명시적으로 설정 되 고 변환을 수행 하는 `display_balance` 함수에서 **double** 형식으로의 명시적 캐스트가 도입 되었습니다. 이 캐스트가 생략되었다면 컴파일러는 `<<` 형식에 대한 적절한 스트림 삽입 연산자 `Money`를 찾지 못하고 오류가 발생됩니다.
