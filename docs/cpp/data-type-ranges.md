...
|`long double`|와 동일 합니다.`double`|없음|`double`과 동일|
|`wchar_t`|2|`__wchar_t`|0 ~ 65,535|

`__wchar_t` 변수는 사용 방법에 따라 와이드 문자 형식 또는 멀티바이트 문자 형식을 지정합니다. `L`문자 또는 문자열 상수 앞에 접두사를 사용 하 여 와이드 문자 형식 상수를 선택 합니다.
...
`int`및 `unsigned int` 형식의 크기는 4 바이트입니다. 그러나 `int` 언어 표준에서 구현 전용으로 허용 되므로 이식 가능한 코드는의 크기에 의존해 서는 안 됩니다.

Visual Studio의 C/C++에서도 크기가 지정된 정수 형식을 지원합니다. 자세한 내용은 [__int8, \_ _int16, \_ _int32, \_ _int64](../cpp/int8-int16-int32-int64.md)및 [정수 제한](../cpp/integer-limits.md)을 참조 하세요.
