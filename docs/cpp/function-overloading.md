...
예를 들어 `std::string` 인수를 사용 하는 `print` 함수는 **double**형식의 인수를 사용 하는 것과 매우 다른 작업을 수행할 수 있습니다. 오버 로드를 사용 하면 `print_string` 또는 `print_double`와 같은 이름을 사용할 필요가 없습니다. 컴파일 시간에 컴파일러는 호출자가 전달 하는 인수 형식을 기반으로 사용할 오버 로드를 선택 합니다.  `print(42.0)`를 호출 하면 `void print(double d)` 함수가 호출 됩니다. `print("hello world")`를 호출 하면 `void print(std::string)` 오버 로드가 호출 됩니다.
...
|집합 1: 분수 형식의 첫 번째 인수가 있는 후보 함수|설정 2: 두 번째 인수를 **Int** 형식으로 변환할 수 있는 후보 함수|
|--------------------------------------------------------------------------|-----------------------------------------------------------------------------------|
|변형 1|변형 1 (표준 변환을 사용 하 여**int** 를 **long** 으로 변환할 수 있음)|
|변형 3||

집합 2의 함수는 실제 매개 변수 형식에서 정식 매개 변수 형식으로의 암시적 변환이 있는 함수 이며, 이러한 함수 중에 실제 매개 변수 형식을 정식 매개 변수 형식으로 변환 하는 "cost"가 있는 함수는 다음과 같습니다. 적은.
...
|설정 1: **Int** 형식의 첫 번째 인수가 있는 후보 함수|설정 2: **Int** 형식의 두 번째 인수가 있는 후보 함수|
|---------------------------------------------------------------------|----------------------------------------------------------------------|
|변형 2 (표준 변환을 사용 하 여**int** 를 **long** 으로 변환할 수 있음)|변형 1 (표준 변환을 사용 하 여**int** 를 **long** 으로 변환할 수 있음)|
...
이와 같은 이유로 **const** 또는 **volatile** 로 수정 된 형식의 함수 인수는 오버 로드를 위해 기본 형식과 다르게 처리 되지 않습니다.

그러나 함수 오버 로드 메커니즘은 **const** 와 **volatile** 로 정규화 된 참조와 기본 형식에 대 한 참조를 구분할 수 있습니다. 다음과 같은 코드를 사용할 수 있습니다.
...
또한 **const** 및 **volatile** 개체에 대 한 포인터는 오버 로드를 위해 기본 형식에 대 한 포인터와는 다른 것으로 간주 됩니다.
...
변환 결과 시퀀스가 있을 경우 이를 가장 일치하는 시퀀스라고 합니다. 표준 변환 ( [표준 변환](../cpp/standard-conversions.md)참조)을 사용 하 여 **int** 형식의 개체를 **부호 없는 long** 형식으로 변환 하는 방법에는 여러 가지가 있습니다.

- **Int** 에서 **long** 으로 변환한 다음 **long** 에서 **unsigned long**으로 변환 합니다.

- **Int** 에서 **unsigned long**으로 변환 합니다.
...
1. 정확한 일치. 함수가 호출되는 형식과 함수 프로토타입에서 선언된 형식 간 정확한 일치는 항상 가장 좋은 일치입니다. trivial 변환 시퀀스는 정확히 일치하는 항목으로 분류됩니다. 그러나 이러한 변환 작업을 수행 하지 않는 시퀀스는를 변환 하는 시퀀스 보다 더 나은 것으로 간주 됩니다.
   ...
   - 포인터에서 **const** 에 대 한 포인터로 (`type` <strong>\*</strong> **const** `type` <strong>\*</strong>).

   - 포인터에서 **volatile** 로 (`type` <strong>\*</strong> **휘발성** `type` <strong>\*</strong>) 포인터로

   - 참조에서 **const** (`type` **&** 을 **const** `type` **&** )에 참조 합니다.

   - 참조에서 **volatile** (`type` **&** 를 **휘발성** `type` **&** )에 참조 합니다.

1. 승격을 통한 일치. 정수 계열 확장만 포함 하는 정확히 일치 하는 것으로 분류 되지 않은 모든 시퀀스, **float** 에서 **double**로의 변환 및 trivial 변환은 승격을 사용 하 여 일치 항목으로 분류 됩니다. 승격을 통한 일치는 정확한 일치만큼 양호하지는 않지만 표준 변환을 통한 일치에 비해 좋습니다.

1. 표준 변환을 통한 일치. 표준 변환과 trivial 변환만 포함 포함하는, 정확한 일치 또는 승격을 통한 일치로 분류되지 않은 시퀀스는 표준 변환을 통한 일치로 분류됩니다. 이 범주에는 다음 규칙이 적용됩니다.

   - 파생 클래스에 대 한 포인터에서 직접 또는 간접 기본 클래스에 대 한 포인터로 변환 하는 것은 `void *` 또는 `const void *`으로 변환 하는 것이 좋습니다.
...
`C*` 형식에서 `B*` 형식으로 변환하는 것이 `C*` 형식에서 `A*` 형식으로 변환하는 것보다 좋습니다. 이유는 이들이 동일한 경로에 있고 `B*`가 더 가깝기 때문입니다. 그러나 형식 `C*` 형식 `D*` 형식으로 변환 하는 것은 `A*`형식으로 변환 하는 것 보다 좋습니다. 변환이 다른 경로를 따르기 때문에 기본 설정이 없습니다.
...
`UDC` 클래스에 사용할 수 있는 사용자 정의 변환은 **int** 형식에서 **long**형식으로 변환 됩니다. 따라서 컴파일러가 일치하는 개체 형식을 위한 변환을 고려합니다(`UDC`). **Int** 로의 변환이 있으며 선택 됩니다.
...
앞의 예제에서 사용자 정의 변환 **연산자 long**은 `udc` **long**형식으로 변환 하기 위해 호출 됩니다. **Long** 형식으로의 사용자 정의 변환이 정의 되지 않은 경우 변환은 다음과 같이 진행 됩니다. 형식 `UDC`는 사용자 정의 변환을 사용 하 여 **int** 형식으로 변환 됩니다. 그런 다음 **int** 형식에서 **long** 형식으로의 표준 변환이 선언에 있는 인수와 일치 하도록 적용 되었습니다.

인수와 일치 하는 사용자 정의 변환이 필요한 경우 가장 일치 하는 항목을 평가할 때 표준 변환이 사용 되지 않습니다. 둘 이상의 후보 함수에 사용자 정의 변환이 필요한 경우에도 함수는 동일한 것으로 간주 됩니다. 다음은 그 예입니다.
...
두 버전의 `Func` 모두 **int** 형식을 클래스 형식 인수로 변환 하려면 사용자 정의 변환이 필요 합니다. 가능한 변환은 다음과 같습니다.

- **Int** 형식에서 형식 `UDC1` (사용자 정의 변환)로 변환 합니다.

- **Int** 형식에서 **long**형식으로 변환 합니다. 그런 다음 `UDC2` 형식으로 변환 합니다 (2 단계 변환).
...
클래스 멤버 함수는 **static**으로 선언 되었는지 여부에 따라 다르게 처리 됩니다. 비정적 함수에는 **this** 포인터를 제공 하는 암시적 인수가 있으므로 비정적 함수는 정적 함수 보다 하나 이상의 인수를 포함 하는 것으로 간주 됩니다. 그렇지 않으면 동일 하 게 선언 됩니다.

이러한 비정적 멤버 함수를 사용할 때는 암시적 **this** 포인터가 함수가 호출 되는 개체 유형과 일치 해야 합니다. 그렇지 않으면 오버 로드 된 연산자의 경우 첫 번째 인수가 연산자가 적용 되는 개체와 일치 해야 합니다. 오버 로드 된 연산자에 대 한 자세한 내용은 [오버 로드 된 연산자](../cpp/operator-overloading.md)를 참조 하세요.

오버 로드 된 함수의 다른 인수와 달리 임시 개체는 도입 되지 않으며 **이** 포인터 인수와 일치 하려고 할 때 변환이 시도 되지 않습니다.

`->` 멤버 선택 연산자를 사용 하 여 `class_name`클래스의 멤버 함수에 액세스 하는 경우 **이** 포인터 인수의 형식은 `class_name * const`입니다. 멤버가 **const** 또는 **volatile**로 선언 된 경우 형식은 각각 `const class_name * const` `volatile class_name * const`됩니다.
...
Ref 한정자를 사용 하면 **this** 가 가리키는 개체가 rvalue 또는 lvalue 인지 여부를 기준으로 멤버 함수를 오버 로드할 수 있습니다.  이 기능을 사용 하면 데이터에 대 한 포인터 액세스를 제공 하지 않도록 선택 하는 시나리오에서 불필요 한 복사 작업을 방지할 수 있습니다. 예를 들어 `C` 클래스가 생성자에서 일부 데이터를 초기화 하 고 `get_data()`멤버 함수에서 해당 데이터의 복사본을 반환 한다고 가정 합니다. `C` 형식의 개체가 소멸 될 rvalue 인 경우 컴파일러는 데이터를 복사 하는 대신 이동 하는 `get_data() &&` 오버 로드를 선택 합니다.
...
- **typedef** 선언은 새 형식을 정의 하지 않습니다. 기존 형식에 대 한 동의어를 소개 합니다. 오버 로드 메커니즘에는 영향을 주지 않습니다. 다음 코드를 살펴보세요.
   ...
   위의 두 함수에는 동일한 인수 목록이 있습니다. `PSTR`은 형식 `char *`의 동의어입니다. 멤버 범위에서 이 코드를 사용하면 오류가 발생합니다.
...
블록 범위는 엄격 하 게 관찰 됩니다. 따라서 파일 범위에서 선언 된 함수는 로컬로 선언 된 함수와 같은 범위에 있지 않습니다. 로컬로 선언된 함수의 이름이 파일 범위에서 선언된 함수의 이름과 같을 경우 로컬로 선언된 함수는 오버로드를 유발하는 대신 파일 범위의 함수를 숨깁니다. 다음은 그 예입니다.
...
위의 코드에서는 `func` 함수의 두 정의를 보여 줍니다. `char *` 형식의 인수를 사용 하는 정의는 **extern** 문으로 인해 `main`에 대해 로컬입니다. 따라서 **int** 형식의 인수를 사용 하는 정의가 숨겨지고 `func`에 대 한 첫 번째 호출에서 오류가 발생 합니다.
...
`Account::Deposit`에서 `Deposit`에 대 한 호출은 전용 멤버 함수를 호출 합니다. `Account::Deposit`는 멤버 함수이 고 클래스의 private 멤버에 액세스할 수 있기 때문에이 호출은 올바릅니다.
