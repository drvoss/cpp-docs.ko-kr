...
ms.locfileid: "80160751"
---
# <a name="temporary-objects"></a>임시 개체

경우에 따라 컴파일러가 임시 개체를 만들어야 합니다. 다음과 같은 이유로 이 임시 개체를 만들 수 있습니다.

- 초기화 되는 참조의 기본 형식과 다른 형식의 이니셜라이저를 사용 하 여 **const** 참조를 초기화 하려면입니다.

- 사용자 정의 형식을 반환하는 함수의 반환 값을 저장하려는 경우 이러한 임시 개체는 프로그램이 반환 값을 개체에 복사하지 않는 경우에만 만들어집니다. 다음은 그 예입니다.
   ...
   `ComplexResult = Complex1 + Complex2 + Complex3` 식을 참조하십시오. `Complex1 + Complex2` 식이 계산되고 결과가 임시 개체에 저장됩니다. 그런 다음, 식 *임시* `+ Complex3`을 계산 하 고 결과를 `ComplexResult`에 복사 합니다 (대입 연산자가 오버 로드 되지 않는다고 가정).
