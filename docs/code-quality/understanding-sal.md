...
ms.locfileid: "82825732"
---
# <a name="understanding-sal"></a>SAL 이해

Microsoft SAL (소스 코드 주석 언어)은 함수에서 매개 변수를 사용 하는 방법, 해당 매개 변수를 사용 하는 가정 및 완료 될 때의 보장을 설명 하는 데 사용할 수 있는 주석 집합을 제공 합니다. 주석은 헤더 파일 `<sal.h>`에 정의 됩니다. C + + 용 Visual Studio code 분석은 SAL 주석을 사용 하 여 함수 분석을 수정 합니다. Windows 드라이버 개발용 SAL 2.0에 대 한 자세한 내용은 [Windows 드라이버에 대 한 sal 2.0 주석](/windows-hardware/drivers/devtest/sal-2-annotations-for-windows-drivers)을 참조 하세요.
...
> "Src의 바이트 수를 dest로 복사 합니다. 원본과 대상이 겹치면 memcpy의 동작이 정의 되지 않습니다. Memmove를 사용 하 여 겹치는 영역을 처리 하세요.
...
1. 메뉴 모음에서 **빌드**, **솔루션에서 코드 분석 실행**을 선택 합니다.
     ...
     \_이 섹션의\_ 예제를 참조 하세요. 코드 분석을 실행 하는 경우 다음과 같은 경고가 표시 됩니다.

    > **C6387 매개 변수 값이 잘못 되었습니다** . ' 고정 '는 ' 0 ' 일 수 있습니다 .이는 ' InCallee ' 함수에 대 한 사양을 따르지 않습니다.

### <a name="example-the-_in_-annotation"></a>예: \_In\_ 주석
...
### <a name="example-the-_in_opt_-annotation"></a>예: \_in\_opt 주석\_
...
### <a name="example-the-_out_opt_-annotation"></a>예: \_Out\_opt 주석\_
...
Visual Studio Code 분석에서는가 역참조 되기 전에 `pInt` 이 함수가 null을 확인 하는지 확인 하 `pInt` 고,가 null이 아닌 경우 버퍼는 함수에 의해 초기화 되어 반환 됩니다.
...
> [!NOTE]
> 와 `_Out_`마찬가지로 `_Inout_` 은 수정 가능한 값에 적용 해야 합니다.
...
### <a name="example-the-_inout_opt_-annotation"></a>예: \_Inout\_opt 주석\_
...
### <a name="example-the-_outptr_opt_-annotation"></a>예: \_outptr\_opt 주석\_
...
`_Out_` 주석은 Visual Studio Code 분석을 통해 호출자가 NULL이 아닌 포인터를에 대 한 `pInt`버퍼에 전달 하는지, 버퍼가 반환 되기 전에 함수에 의해 초기화 되는지 유효성을 검사 합니다.
