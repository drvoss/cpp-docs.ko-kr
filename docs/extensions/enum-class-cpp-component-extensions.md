...
(선택 사항. Windows 런타임에만 해당) 열거형의 기본 형식으로, **bool**, **char**, `char16`, `int16`, `uint16`, **int**, `uint32`, `int64` 또는 `uint64`일 수 있습니다.
...
- *underlying-type*이 **bool**이고 세 개 이상의 열거자가 암시적으로 정의된 경우, 처음 두 열거자 다음의 열거자는 표현할 수 없습니다.
...
*type*<br/>
(선택 사항) ‘식별자’의 기본 형식입니다. 부호 있는 버전 또는 부호 없는 버전의 **int**, **short**, **long** 등의 스칼라 형식일 수 있습니다.  **bool** 또는 **char**도 사용할 수 있습니다.
...
표준 C++ 열거형이 **class** 또는 **struct** 없이 정의된 경우 `/clr`로 컴파일하면 열거형이 관리형 열거형으로 컴파일됩니다.  열거형이 여전히 관리되지 않는 열거형의 의미 체계를 사용합니다.  컴파일러는 `Microsoft::VisualC::NativeEnumAttribute` 특성을 삽입하여 열거형을 네이티브 열거형으로 만들려는 프로그래머의 의도를 식별합니다.  다른 컴파일러에는 표준 열거형이 관리되는 열거형으로 표시됩니다.
