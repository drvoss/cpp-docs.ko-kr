...
|이름|설명|
|---------|-----------|
|[auto_gcroot::attach](#attach)|`auto_gcroot`를 개체에 연결 합니다.|
|[auto_gcroot::get](#get)|포함 된 개체를 가져옵니다.|
|[auto_gcroot::release](#release)|`auto_gcroot` management에서 개체를 해제 합니다.|
|[auto_gcroot::reset](#reset)|현재 소유 하 고 있는 개체를 삭제 하 고 필요에 따라 새 개체를 소유 합니다.|
|[auto_gcroot::swap](#swap)|개체를 다른 `auto_gcroot`바꿉니다.|
...
|[auto_gcroot:: operator&nbsp;auto_gcroot](#operator-auto-gcroot)|`auto_gcroot` 형식과 호환 되는 형식 사이의 형식 캐스팅 연산자입니다.|
|[auto_gcroot:: operator&nbsp;bool](#operator-bool)|조건식에 `auto_gcroot`을 사용 하기 위한 연산자입니다.|  
|[auto_gcroot::operator!](#operator-logical-not)|조건식에 `auto_gcroot`을 사용 하기 위한 연산자입니다.|
...
기존 `auto_gcroot`에서 `auto_gcroot`를 생성 하는 경우 기존 `auto_gcroot` 개체의 소유권을 새 `auto_gcroot`로 전송 하기 전에 개체를 해제 합니다.
...
*_right*<br/>
연결할 개체 이거나 연결할 개체를 포함 하는 `auto_gcroot`입니다.
...
`_right` `auto_gcroot`이면 개체가 현재 `auto_gcroot`에 연결 되기 전에 해당 개체의 소유권을 해제 합니다.
...
`auto_gcroot` management에서 개체를 해제 합니다.
...
*_right*<br/>
현재 `auto_gcroot`에 할당할 개체 또는 `auto_gcroot`입니다.

### <a name="return-value"></a>반환 값

현재 `auto_gcroot`이제 `_right`소유 합니다.
...
`auto_gcroot` 형식과 호환 되는 형식 사이의 형식 캐스팅 연산자입니다.
...
현재 `auto_gcroot` `auto_gcroot<_other_type>`으로 캐스팅 됩니다.
...
조건식에 `auto_gcroot`을 사용 하기 위한 연산자입니다.
...
래핑된 개체가 유효한 지 여부를 `true` 합니다. 그렇지 않으면 `false` 합니다.
...
래핑된 개체가 잘못 된 경우 `true` 합니다. 그렇지 않으면 `false` 합니다.
