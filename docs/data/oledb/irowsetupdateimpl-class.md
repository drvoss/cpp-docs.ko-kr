...
*T*<br/>
`IRowsetUpdateImpl`에서 파생 된 클래스입니다.
...
`IRowsetUpdateImpl`는 OLE DB `IRowsetUpdate` 인터페이스를 구현 합니다 .이 인터페이스를 통해 소비자는 `IRowsetChange` 데이터 원본에 대 한 변경 내용 전송을 지연 하 고 전송 전에 변경 내용을 취소할 수 있습니다.
...
업데이트를 허용 해야 하는 것으로 확인 되 면 S_OK을 반환 합니다. 그렇지 않으면 E_FAIL을 반환 합니다. 업데이트를 허용 하는 경우 [IRowsetUpdateImpl:: update](../../data/oledb/irowsetupdateimpl-update.md) 의 `DBROWSTATUS`를 적절 한 [행 상태로](/previous-versions/windows/desktop/ms722752(v=vs.85))설정 해야 합니다.
