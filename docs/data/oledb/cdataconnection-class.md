...
|||
|-|-|
|[CDataConnection](#cdataconnection)|생성자입니다. `CDataConnection` 개체를 인스턴스화하고 초기화 합니다.|
|[Copy](#copy)|기존 데이터 연결의 복사본을 만듭니다.|
...
|[operator bool](#op_bool_ole)|현재 세션이 열려 있는지 여부를 확인 합니다.|
|[operator CDataSource &](#op_cdata_amp)|포함 된 `CDataSource` 개체에 대 한 참조를 반환 합니다.|
|[연산자 CDataSource *](#op_cdata_star)|포함된 `CDataSource` 개체에 대한 포인터를 반환합니다.|
|[연산자 CSession &](#op_csession_amp)|포함 된 `CSession` 개체에 대 한 참조를 반환 합니다.|
...
`CDataConnection`없는 경우 `CDataSource` 개체를 만들고 [OpenFromInitializationString](../../data/oledb/cdatasource-openfrominitializationstring.md) 메서드를 호출한 다음, [Open](../../data/oledb/csession-open.md) 메서드를 호출한 다음 [CCommand](../../data/oledb/ccommand-class.md) [개체를](../../data/oledb/csession-class.md) 만들고 해당 `Open`* 메서드를 호출 해야 합니다.

`CDataConnection`를 사용 하면 연결 개체만 만들고 초기화 문자열을 전달한 다음 해당 연결을 사용 하 여 명령을 열 수 있습니다. 데이터베이스에 대 한 연결을 반복 해 서 사용 하려는 경우에는 연결을 열어 두는 것이 좋습니다 .이 작업을 수행 하는 편리한 방법을 제공 하는 `CDataConnection`.
...
`CDataConnection` 개체를 인스턴스화하고 초기화 합니다.
...
첫 번째 재정의는 기본 설정을 사용 하 여 새 `CDataConnection` 개체를 만듭니다.

두 번째 재정의는 지정한 데이터 연결 개체에 해당 하는 설정을 사용 하 여 새 `CDataConnection` 개체를 만듭니다.
...
**Bool** (C++ 데이터 형식) 값을 반환 합니다. **true** 는 현재 세션이 열려 있음을 의미 합니다. **false** 는 현재 세션이 닫 혔 음을 의미 합니다.

## <a name="cdataconnectionoperator-cdatasourceamp"></a><a name="op_cdata_amp"></a>CDataConnection:: operator CDataSource&amp;

포함 된 `CDataSource` 개체에 대 한 참조를 반환 합니다.
...
이 연산자는 포함 된 `CDataSource` 개체에 대 한 참조를 반환 하므로 `CDataSource` 참조가 필요한 `CDataConnection` 개체를 전달할 수 있습니다.

### <a name="example"></a>예제

`CDataSource` 참조를 사용 하는 함수 (예: 아래 `func`)가 있는 경우 `CDataSource&`를 사용 하 여 `CDataConnection` 개체를 전달할 수 있습니다.
...
포함 된 `CSession` 개체에 대 한 참조를 반환 합니다.
...
이 연산자는 포함 된 `CSession` 개체에 대 한 참조를 반환 하므로 `CSession` 참조가 필요한 `CDataConnection` 개체를 전달할 수 있습니다.

### <a name="example"></a>예제

`CSession` 참조를 사용 하는 함수 (예: 아래 `func`)가 있는 경우 `CSession&`를 사용 하 여 `CDataConnection` 개체를 전달할 수 있습니다.
