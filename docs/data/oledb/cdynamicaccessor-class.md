...
|[CDynamicAccessor](#cdynamicaccessor)|`CDynamicAccessor` 개체를 인스턴스화하고 초기화 합니다.|
...
`CDynamicAccessor` 메서드를 사용 하 여 열 이름, 열 수, 데이터 형식 등의 열 정보를 가져올 수 있습니다. 그런 다음이 열 정보를 사용 하 여 런타임에 동적으로 접근자를 만듭니다.
...
*info*<br/>
진행 열 정보를 포함 하는 `DBCOLUMNINFO` 구조입니다. *OLE DB 프로그래머 참조*에서 [IColumnsInfo:: GETCOLUMNINFO](/previous-versions/windows/desktop/ms722704\(v=vs.85\)) 의 "DBCOLUMNINFO 구조체"를 참조 하세요.
...
`CDynamicAccessor`를 사용 하 여 만든 기본 접근자를 재정의 하는 경우이 메서드를 사용 합니다 ( [데이터를 인출 하려면 어떻게 해야 하나요?](../../data/oledb/fetching-data.md)참조).

## <a name="cdynamicaccessorcdynamicaccessor"></a><a name="cdynamicaccessor"></a>CDynamicAccessor:: CDynamicAccessor

`CDynamicAccessor` 개체를 인스턴스화하고 초기화 합니다.
...
생성자를 사용 하 여 `CDynamicAccessor` 개체를 초기화 하는 경우 Blob을 바인딩하는 방법을 지정할 수 있습니다. Blob은 그래픽, 소리 또는 컴파일된 코드와 같은 이진 데이터를 포함할 수 있습니다. 기본 동작은 8000 바이트를 초과 하는 열을 Blob으로 처리 하 고 `ISequentialStream` 개체에 바인딩하는 것입니다. 그러나 다른 값을 BLOB 크기로 지정할 수 있습니다.

또한 BLOB 데이터로 한정 되는 열 데이터를 `CDynamicAccessor` 처리 하는 방법을 지정할 수 있습니다. 즉, BLOB 데이터를 기본 방식으로 처리할 수 있습니다. BLOB 데이터를 건너뛸 수 있습니다 (바인딩 안 함). 또는 공급자가 할당 한 메모리에 BLOB 데이터를 바인딩할 수 있습니다.
...
책갈피를 검색 하려면 `DBPROP_IRowsetLocate`를 VARIANT_TRUE으로 설정 해야 합니다.
...
열 특징이 성공적으로 검색 되 면 **true** 를 반환 합니다. 그렇지 않으면 **false**를 반환합니다.
...
*ppColumnInfo*<br/>
제한이 `DBCOLUMNINFO` 구조체의 배열을 반환할 메모리에 대 한 포인터입니다. *OLE DB 프로그래머 참조*에서 [IColumnsInfo:: GETCOLUMNINFO](/previous-versions/windows/desktop/ms722704\(v=vs.85\)) 의 "DBCOLUMNINFO 구조체"를 참조 하세요.
...
`DBORDINAL`, `DBCOLUMNINFO`및 `OLECHAR`데이터 형식에 대 한 자세한 내용은 *OLE DB 프로그래머 참조* 에서 [IColumnsInfo:: GetColumnInfo](/previous-versions/windows/desktop/ms722704\(v=vs.85\)) 를 참조 하세요.
...
성공 하면 **true** 를 반환 하 고 실패 하면 **false** 를 반환 합니다.
...
지정 된 열을 찾은 경우 **true** 를 반환 합니다. 그렇지 않으면이 함수는 **false**를 반환 합니다.
...
지정 된 이름의 열을 찾은 경우 **true** 를 반환 합니다. 그렇지 않으면이 함수는 **false**를 반환 합니다.
...
*ctype*<br/>
진행 특수 처리를 필요로 하는 문자열 형식 (`CHAR*`, `WCHAR*`)을 제외한 모든 데이터 형식을 처리 하는 템플릿 매개 변수입니다. `GetValue`는 여기에서 지정한 내용에 따라 적절 한 데이터 형식을 사용 합니다.
...
문자열 데이터를 전달 하려는 경우 템플릿이 아닌 버전의 `GetValue`을 사용 합니다. 이 메서드의 템플릿이 아닌 버전은 지정 된 열 데이터를 포함 하는 버퍼의 일부를 가리키는 `void*`반환 합니다. 열을 찾을 수 없는 경우 NULL을 반환 합니다.

다른 모든 데이터 형식의 경우 `GetValue`의 템플릿 버전을 사용 하는 것이 더 간단 합니다. 템플릿 버전은 성공 시 **true** 를 반환 하 고 실패 하면 **false** 를 반환 합니다.
...
*Eblob 처리*<br/>
BLOB 데이터를 처리 하는 방법을 지정 합니다. 이 속성에 가능한 값은 다음과 같습니다.

- DBBLOBHANDLING_DEFAULT: BLOB 데이터로 *nBlobSize* (`SetBlobSizeLimit`에 의해 설정 됨) 보다 큰 열 데이터를 처리 하 고 `ISequentialStream` 또는 `IStream` 개체를 통해 검색 합니다. 이 옵션은 *nBlobSize* 보다 큰 데이터를 포함 하는 모든 열을 바인딩하거나 BLOB 데이터로 DBTYPE_IUNKNOWN로 나열 하려고 합니다.

- DBBLOBHANDLING_NOSTREAMS: *nBlobSize* (`SetBlobSizeLimit`에 의해 설정 됨) 보다 큰 열 데이터를 BLOB 데이터로 처리 하 고 공급자가 할당 한 소비자 소유의 메모리에서 참조를 통해 검색 합니다. 이 옵션은 둘 이상의 BLOB 열이 있는 테이블에 유용 하며, 공급자는 접근자 당 하나의 `ISequentialStream` 개체만 지원 합니다.
...
지정 된 열 길이가 성공적으로 설정 되 면 **true** 를 반환 합니다. 그렇지 않으면이 함수는 **false**를 반환 합니다.
...
지정 된 열 상태가 성공적으로 설정 된 경우 **true** 를 반환 합니다. 그렇지 않으면이 함수는 **false**를 반환 합니다.
...
문자열 데이터를 설정 하려면 비 템플릿 버전의 `GetValue`을 사용 합니다. 이 메서드의 템플릿이 아닌 버전은 지정 된 열 데이터를 포함 하는 버퍼의 일부를 가리키는 `void*`반환 합니다. 열을 찾을 수 없는 경우 NULL을 반환 합니다.
