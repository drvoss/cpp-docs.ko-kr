...
> [!NOTE]
>  이 토픽은 대량 행 페치가 구현되지 않은 `CRecordset`에서 파생된 개체에 적용됩니다. 여기서 설명하는 방법은 대량 행 페치를 사용 중인 경우 일반적으로 권장되지 않습니다. 대량 행 페치에 대 한 자세한 내용은 [레코드 집합: 대량 레코드 페치 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)를 참조 하세요.
...
Columns-to-Bind-Dynamically와 병렬로 만듭니다.
   ...
   예를 들어 다음 그림에서는 하나의 요소가 있는 동적 열 값 (목록 4)을 보여 줍니다. 현재 레코드에 대 한 실제 전화 번호를 포함 하는 `CString` 개체는 "555-1212"입니다.
...
일반적인 경우 Columns-to-Bind-Dynamically가 `RFX_Text`이라는 `CString`이고 Dynamic-Column-Values가 `CStringList`라는 `m_listName`인 다음 코드 줄과 같이 `CStringList` 함수 호출에서 `m_listValue` 개체를 목록에서 추출합니다.
...
RFX 함수에 대한 자세한 내용은 [클래스 라이브러리 참조](../../mfc/reference/mfc-macros-and-globals.md)의 *매크로 및 전역 요소*를 참조하세요.

> [!TIP]
> 새 열의 데이터 형식이 다른 경우 루프에 switch 문을 사용하여 각 형식에 적절한 RFX 함수를 호출합니다.

프레임워크가 `DoFieldExchange` 프로세스 중에 `Open`를 호출하면 정적 열에 대한 RFX 호출이 해당 열을 바인딩합니다. 그러면 루프에서 동적 열에 대해 RFX 함수를 반복적으로 호출합니다.
