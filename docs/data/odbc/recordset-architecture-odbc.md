...
> [!NOTE]
> 이 항목은 대량 행 페치가 구현되지 않은 `CRecordset`에서 파생된 개체에 적용됩니다. 대량 행 페치를 구현하는 경우 아키텍처는 유사합니다. 차이점을 이해하려면 [레코드 집합: 대량 레코드 가져오기(ODBC)를](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)참조하십시오.
...
> [!NOTE]
> 매개 변수의 순서가 중요합니다. 이에 대한 자세한 정보 및 매개 변수에 대한 자세한 내용은 [레코드 집합: 레코드 집합(ODBC) 매개 변수화를](../../data/odbc/recordset-parameterizing-a-recordset-odbc.md)참조하십시오.
...
자세한 정보 및 예제는 [레코드 필드 교환: RFX 사용](../../data/odbc/record-field-exchange-using-rfx.md)을 참조하십시오.
