...
또한 마법사가 클래스 `DoFieldExchange`의 `CRecordset` 멤버 함수를 재정의합니다.
...
- `CFieldExchange::SetFieldType` 포인터를 통해 `pFX`에 대해 호출합니다. 이 호출은 `DoFieldExchange` 끝까지의 모든 RFX 함수 호출 또는 `SetFieldType`의 다음 호출이 출력 열임을 지정합니다. 자세한 내용은 [CFieldExchange::SetFieldType](../../mfc/reference/cfieldexchange-class.md#setfieldtype)을 참조하세요.
...
- `pFX`를 호출할 때 프레임워크가 전달하는 [CFieldExchange](../../mfc/reference/cfieldexchange-class.md) 개체에 대한 `DoFieldExchange` 포인터. `CFieldExchange` 개체는 `DoFieldExchange`가 수행할 작업, 전송 방향 및 기타 컨텍스트 정보를 지정합니다.
