...
![C&#43; &#43; 문서 메뉴로 이동 메뉴 옵션](../ide/media/go-to-document.png "문서로 이동")
...
![C&#43; &#43; 로 이동](../ide/media/go-to-cpp.png "이동")

**이동**에는 다음과 같은 검색 필터가 있습니다.

- **줄 이동** (**Ctrl + G**): 현재 문서에서 다른 줄로 빠르게 이동 합니다.
- **전체로 이동** (**ctrl +,** ) 또는 (**Ctrl + T**): 검색 결과에는 다음 항목이 포함 됩니다.
- **파일로 이동** (**Ctrl 1, F**): 솔루션에서 파일을 검색 합니다.
- **형식으로 이동** (**Ctrl 1, T**): 검색 결과는 다음과 같습니다.
  - 클래스, 구조체 및 열거형
  - 인터페이스 및 대리자(관리형 코드에만 해당)
- **멤버로 이동** (**Ctrl 1, M**): 검색 결과는 다음과 같습니다.
  - 글로벌 변수 및 글로벌 함수
  - 클래스 멤버 변수 및 멤버 함수
  - 상수
  - 열거형 항목
  - 속성 및 이벤트
- **기호로 이동** (**Ctrl 1, S**): 검색 결과는 다음과 같습니다.
...
텍스트 필터를 적용 하려면 필터의 해당 문자 다음에 공백을 사용 하 여 검색 쿼리를 시작 합니다. **줄 이동** 은 필요에 따라 공간을 생략할 수 있습니다. 다음 텍스트 필터를 사용할 수 있습니다.
...
- **확인 된 결과**: 검색 되는 기호에 대 한 실제 코드 참조입니다. 예를 들어 `Size`라는 멤버 함수를 검색하면 `Size`를 정의하는 클래스의 범위와 일치하는 모든 `Size` 참조가 반환됩니다.

- **확인**되지 않은 결과:이 필터는 이름이 일치 하지만 검색 하는 기호에 대 한 실제 참조가 아닌 기호를 표시 하기 때문에 기본적으로 해제 되어 있습니다. 예를 들어 각각 `Size` 멤버 함수를 정의하는 두 클래스가 있고 `Size`의 개체에서 참조의 `Class1`를 선택하면 `Size`에서 `Class2`로의 모든 참조는 확인 취소된 것으로 표시됩니다.

- **처리**되지 않은 결과: **모든 참조 찾기** 작업은 큰 코드 베이스에서 완료 하는 데 시간이 걸릴 수 있으므로 결과 목록에는 "처리 되지 않은" 결과가 여기에 표시 됩니다. 처리되지 않은 결과는 검색 중인 기호의 이름과 일치하지만, 아직 실제 코드 참조로 확인되지 않았습니다. 이 필터를 켜면 더 빠르게 결과를 얻을 수 있습니다. 단, 일부 결과는 실제 참조가 아닐 수 있다는 점에 주의해야 합니다.
