...
description: 비주얼 C++에 대한 비주얼 스튜디오 지원 설치
ms.custom: mvc
ms.date: 04/02/2019
ms.topic: tutorial
ms.devlang: cpp
...
ms.locfileid: "81335359"
---
# <a name="install-c-support-in-visual-studio"></a>Visual Studio에 C++ 지원 설치

Visual Studio 및 Visual C++ 도구를 아직 다운로드하여 설치하지 않은 경우 시작하는 방법은 다음과 같습니다.
...
> [!NOTE]
> 이 항목은 Windows에서 Visual Studio를 설치하는 데 적용됩니다. [Visual Studio 코드는](https://code.visualstudio.com/) Windows, Mac 및 Linux 시스템에서 실행되는 경량의 플랫폼 간 개발 환경입니다. 비주얼 [스튜디오 코드 확장에 대한 Microsoft C/C++는](https://marketplace.visualstudio.com/items?itemName=ms-vscode.cpptools) IntelliSense, 디버깅, 코드 서식 지정, 자동 완성을 지원합니다. Mac용 Visual Studio는 Microsoft C++를 지원하지 않지만 .NET 언어 및 플랫폼 간 개발을 지원합니다. 설치 지침은 [Mac용 시각적 스튜디오 설치를](/visualstudio/mac/installation/)참조하십시오.

이 버전의 다른 새로운 기능에 대해 자세히 알고 싶으세요? 비주얼 스튜디오 [릴리스 노트를](/visualstudio/releases/2019/release-notes/)참조하십시오.
...
1. Microsoft [라이선스 조건](https://visualstudio.microsoft.com/license-terms/) 및 Microsoft 개인 정보 [취급 방침을](https://privacy.microsoft.com/privacystatement)인정하도록 요청합니다. **계속**을 선택합니다.

### <a name="step-4---choose-workloads"></a>4단계 - 워크로드 선택

설치 프로그램을 설치한 후 원하는 워크로드 또는 기능 집합을 선택하여 *설치를*사용자 지정하는 데 사용할 수 있습니다. 방법은 다음과 같습니다.

1. **Visual Studio 설치** 화면에서 원하는 작업을 찾습니다.

   ![비주얼 스튜디오 2019: 워크로드 설치](../get-started/media/vs-installer-workloads.png)

   핵심 C++ 지원의 경우 "C++가 있는 데스크톱 개발" 워크로드를 선택합니다. 20개가 넘는 언어에 대한 기본 코드 편집 기능, 프로젝트 없이도 폴더에서 코드를 열어 편집할 수 있는 기능 및 통합 소스 코드 제어 기능이 포함된 기본 핵심 편집기가 제공됩니다.

   추가 워크로드는 다른 종류의 C++ 개발을 지원합니다. 예를 들어 "유니버설 Windows 플랫폼 개발" 워크로드를 선택하여 Microsoft 스토어에 Windows 런타임을 사용하는 앱을 만듭니다. 다이렉트X, 언리얼, 코코스2d를 사용하는 게임을 만들려면 "C++가 있는 게임 개발"을 선택합니다. IoT 개발을 포함한 Linux 플랫폼을 대상으로 하려면 "C++를 사용하여 Linux 개발"을 선택합니다.

   **설치 세부 정보** 창에는 각 워크로드에 설치된 포함된 구성 요소와 선택적 구성 요소가 나열됩니다. 이 목록에서 선택적 구성요소를 선택하거나 선택 취소할 수 있습니다. 예를 들어 Visual Studio 2017 또는 2015 컴파일러 도구 집합을 사용하여 개발을 지원하려면 MSVC v141 또는 MSVC v140 선택적 구성 요소를 선택합니다. MFC, 실험 모듈 언어 확장, IncrediBuild 등에 대한 지원을 추가할 수 있습니다.

1. 워크로드 및 선택적 구성 요소를 선택한 후 **설치를**선택합니다.

   다음으로 Visual Studio 설치 진행률을 보여 주는 상태 화면이 나타납니다.

> [!TIP]
> 설치 후에 언제든지 초기에 설치하지 않은 워크로드 또는 구성 요소를 설치할 수 있습니다. Visual Studio가 열려 있는 **Tools** > 경우 도구**도구 받기 도구 및 기능으로** 이동하여 Visual Studio 설치 관리자를 엽니다. 또는 [시작] 메뉴에서 **Visual Studio 설치 관리자**를 엽니다. 여기에서 설치할 워크로드 또는 구성 요소를 선택합니다. 그런 다음, **수정**을 선택합니다.

### <a name="step-5---choose-individual-components-optional"></a>5단계 - 개별 구성 요소 선택(선택 사항)

워크로드 기능을 사용하여 Visual Studio 설치를 사용자 지정하지 않으려거나 워크로드 설치보다 더 많은 구성 요소를 추가하려는 경우 **개별 구성 요소** 탭에서 개별 구성 요소를 설치하거나 추가하여 원하는 구성 요소를 선택한 다음 프롬프트를 따르십시오.
...
시스템 드라이브에서 Visual Studio의 설치 공간을 줄일 수 있습니다. 다운로드 캐시, 공유 구성 요소, SDK 및 도구를 다른 드라이브로 이동하고 Visual Studio를 가장 빠르게 실행되는 드라이브에 유지하도록 선택할 수 있습니다.
  ...
  ![비주얼 스튜디오 2019 - 설치 위치 변경](../get-started/media/vs-installer-installation-locations.png "설치 위치 변경")
...
Visual Studio 2017에서는 필요한 기능만 쉽게 선택하고 설치할 수 있습니다. 또한 최소 설치 공간이 줄어들기 때문에 시스템에 미치는 영향을 최소화하면서 빠르게 설치됩니다.

### <a name="prerequisites"></a>사전 요구 사항

- 광대역 인터넷 연결. Visual Studio 설치 프로그램은 몇 기가바이트의 데이터를 다운로드할 수 있습니다.

- Microsoft Windows 7 이상 버전을 실행하는 컴퓨터. 최상의 개발 환경을 위해서는 Windows 10이 권장됩니다. Visual Studio를 설치하기 전에 최신 업데이트가 시스템에 적용되었는지 확인합니다.

- 충분한 여유 디스크 공간. Visual Studio에는 최소 7GB의 디스크 공간이 필요하며 많은 공통 옵션이 설치되어 있는 경우 50GB 이상이 소요될 수 있습니다. C: 드라이브에 설치하는 것이 좋습니다.

디스크 공간 및 운영 체제 요구 사항에 대한 자세한 내용은 [Visual Studio 제품군 시스템 요구 사항을](/visualstudio/productinfo/vs2017-system-requirements-vs)참조하십시오. 설치 관리자는 선택한 옵션에 필요한 디스크 공간의 양을 보고합니다.

### <a name="download-and-install"></a>다운로드 및 설치

1. Windows용 최신 Visual Studio 2017 설치 관리자를 다운로드하십시오.
...
1. 다운로드한 설치 관리자 파일을 찾아 실행합니다. 브라우저에 표시되거나 다운로드 폴더에서 찾을 수 있습니다. 설치 관리자는 실행하려면 관리자 권한이 필요합니다. 설치 관리자가 시스템을 변경할 수 있도록 권한을 부여하라는 **사용자 계정 제어** 대화 상자가 표시될 수 있습니다. **예를 선택합니다.** 문제가 있는 경우 파일 탐색기에서 다운로드한 파일을 찾고, 설치 관리자 아이콘을 마우스 오른쪽 단추로 클릭하고, 컨텍스트 메뉴에서 **관리자로 실행을** 선택합니다.

   ![비주얼 스튜디오 설치 관리자 다운로드 및 설치](media/vscpp-concierge-run-installer.gif "비주얼 스튜디오 설치 관리자 다운로드 및 설치")

1. 설치 관리자는 특정 개발 영역에 대한 관련 옵션의 그룹인 워크로드 목록을 제공합니다. C++에 대한 지원은 이제 기본적으로 설치되지 않은 선택적 워크로드의 일부입니다.

   ![C++ 워크로드를 갖춘 데스크톱 개발](media/desktop-development-with-cpp.png "C++를 사용한 데스크톱 개발")

   C++의 경우 **C++** 워크로드가 있는 데스크톱 개발을 선택한 다음 **설치를**선택합니다.

   ![C++ 워크로드로 데스크톱 개발 설치](media/vscpp-concierge-choose-workload.gif "C++ 워크로드로 데스크톱 개발 설치")

1. 설치가 완료되면 **시작** 버튼을 선택하여 Visual Studio를 시작합니다.

   Visual Studio를 처음 실행할 때 Microsoft 계정으로 로그인하라는 메시지가 표시됩니다. 조직이 없는 경우 무료로 만들 수 있습니다. 또한 테마를 선택해야 합니다. 걱정하지 마세요, 당신이 원하는 경우에 당신은 나중에 변경할 수 있습니다.

   Visual Studio를 처음 실행할 때 사용할 준비를 하는 데 몇 분 정도 걸릴 수 있습니다. 빠른 시간 경과의 모습은 다음과 같습니다.

   ![비주얼 스튜디오 2017 로그인](media/vscpp-quickstart-first-run.gif "비주얼 스튜디오 2017 로그인")

   Visual Studio는 다시 실행할 때 훨씬 더 빠르게 시작됩니다.

1. Visual Studio가 열리면 제목 표시줄의 플래그 아이콘이 강조 표시되어 있는지 확인합니다.

   ![비주얼 스튜디오 2017 알림 플래그](media/vscpp-first-start-page-flag.png "비주얼 스튜디오 2017 알림 플래그")

   강조 표시된 경우 **알림** 창을 열도록 선택합니다. Visual Studio에 사용할 수 있는 업데이트가 있는 경우 지금 설치하는 것이 좋습니다. 설치가 완료되면 Visual Studio를 다시 시작합니다.
...
Visual Studio 2015를 설치하려면 [이전 버전의 Visual Studio 다운로드](https://www.visualstudio.com/vs/older-downloads/)로 이동합니다. 설치 프로그램을 실행하고 **사용자 지정 설치**를 선택한 다음, C++ 구성 요소를 선택합니다. 기존 Visual Studio 2015 설치에 C++ 지원을 추가하려면 Windows 시작 단추를 클릭하고 **프로그램 제거 추가를**입력합니다. 결과 목록에서 프로그램을 연 다음 설치된 프로그램 목록에서 Visual Studio 2015 설치를 찾습니다. 두 번 클릭한 다음 **수정을** 선택하고 설치할 Visual C++ 구성 요소를 선택합니다.
...
Visual Studio를 실행 중이면 다음 단계로 계속 진행할 준비가 된 것입니다.
