...
|[/errorReport](errorreport-report-internal-compiler-errors.md)| 사용되지 않음. 오류 보고는 [WER (Windows 오류 보고)](/windows/win32/wer/windows-error-reporting) 설정에 의해 제어 됩니다. |
...
|[/FS](fs-force-synchronous-pdb-writes.md)|MSPDBSRV.EXE를 통해 PDB (프로그램 데이터베이스) 파일에 대 한 모든 쓰기를 강제로 직렬화 합니다. CONVERT.EXE.|
...
|[/GZ](gz-enable-stack-frame-run-time-error-checking.md)|사용되지 않음. [/RTC1](rtc-run-time-error-checks.md)과 동일합니다.|
...
|[/QIfist](qifist-suppress-ftol.md)|사용되지 않음. 부동 소수점 형식에서 정수 계열 형식으로 변환해야 할 때 `_ftol` 이 사용되지 않도록 합니다. x86 전용입니다.|
