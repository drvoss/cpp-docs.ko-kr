...
description: Visual Studio Microsoft C/C++ Project 속성 페이지 속성에 대 한 참조 가이드입니다.
ms.date: 02/09/2020
ms.topic: article
...
ms.locfileid: "77257756"
---
# <a name="cc-property-pages"></a>C/C++ 속성 페이지

다음 속성 페이지는 **프로젝트** > **속성** > **구성 속성** > **C/C++** : 아래에 있습니다.
...
컴파일러에서 생성되는 디버깅 정보 형식을 지정합니다.  이 속성에는 호환 되는 링커 설정이 필요 합니다. [/Z7,/zi,/zi (디버그 정보 형식)](z7-zi-zi-debug-information-format.md)를 설정 합니다.

#### <a name="choices"></a>선택 항목
...
컴파일러 경고를 오류로 처리 합니다. 새 프로젝트의 경우 모든 컴파일에서 [/wx](wx-treat-linker-warnings-as-errors.md) 를 사용 하는 것이 가장 좋을 수 있습니다. 모든 경고를 해결 하면 찾기 어려운 코드 오류를 최소화할 수 있습니다.

### <a name="warning-version"></a>경고 버전

특정 버전의 컴파일러 이후에 도입 된 경고를 숨깁니다. [/Wv: xx\[\[.zzzzz\]\]](wx-treat-linker-warnings-as-errors.md)를 설정 합니다.
...
- **__Inline만** **인라인**, `__inline`, `__forceinline`또는 `__inline`로 표시 된 함수만 확장 합니다. 또는 C++ 멤버 함수에서 클래스 선언 내에 정의 된 또는입니다.
- **인라인** 또는 `__inline`으로 표시 된 **적절** 한 확장 함수와 컴파일러에서 선택 하는 다른 함수 확장은 컴파일러의 판단에 따라 발생 하며 (종종 *자동 인라이닝*이라고 함)
...
C 및 C++ 소스 파일을 전처리 하 고 전처리 된 출력을 파일에 씁니다. 이 옵션은 컴파일을 억제 하 고 *`.obj`* 파일을 생성 하지 않습니다.
...
컴파일러는 프로그램 이미지에 동일한 문자열의 읽기 전용 복사본을 하나만 만듭니다. 이로 인해 *문자열 풀링*이라는 최적화 인 작은 프로그램을 생성 합니다. [/O1,/O1](o-options-optimize-code.md)및 [/zi](z7-zi-zi-debug-information-format.md) 는 [/gf](gf-eliminate-duplicate-strings.md) 옵션을 자동으로 설정 합니다.

### <a name="enable-minimal-rebuild"></a>최소 다시 빌드 사용

변경 C++ 된 클래스 정의가 포함 된 소스 파일을 C++ 다시 컴파일할지 여부를 결정 하는 최소 다시 빌드를 사용 하도록 설정 하 여 헤더 *`.h`* 파일에 저장 합니다.
...
디버그 이외의 다른 최적화 형식과 호환 되지 않는 기본 런타임 오류 검사를 사용 하도록 설정 합니다. [/RTCs,/RTCu,/RTC1](rtc-run-time-error-checks.md)를 설정 합니다.
...
- **다중 스레드 디버그** -_DEBUG 및 _MT을 정의 합니다. 또한이 옵션을 사용 하면 컴파일러가 *libcmtd.lib* 를 사용 하 여 외부 기호를 확인할 수 있도록 라이브러리 이름 *libcmtd.lib* 를 *`.obj`* 파일에 저장 합니다.
- **다중 스레드 dll** -응용 프로그램이 런타임 라이브러리의 다중 스레드 및 DLL 별 버전을 사용 하도록 합니다. _MT 및 _DLL을 정의 하 고 컴파일러가 라이브러리 이름 *msvcrt.lib* 를 *`.obj`* 파일에 저장 하도록 합니다.
- **다중 스레드 디버그 DLL** -_DEBUG, _MT 및 _DLL를 정의 하 고 응용 프로그램에서 런타임 라이브러리의 디버그 다중 스레드 및 DLL 별 버전을 사용 하도록 합니다. 또한 컴파일러가 라이브러리 이름 *msvcrtd.lib* 를 *`.obj`* 파일에 저장 합니다.
...
- **Strict** -가장 엄격한 부동 소수점 모델입니다. **`/fp:strict`** 하면 **`fp_contract`** 해제 되 고 **`fenv_access`** 됩니다. **`/fp:except`** 은 묵시적 이며 **`/fp:except-`** 를 명시적으로 지정 하 여 사용 하지 않도록 설정할 수 있습니다. **`/fp:except-`** 와 함께 사용 하는 경우 예외 이벤트를 고려 하지 않고 엄격한 부동 소수점 의미 체계를 적용 **`/fp:strict`** 합니다.
...
이를 지정 하면 **wchar_t** 형식이 `__int16`에 매핑되 **는 것과** 같은 방식으로 `__wchar_t`에 매핑되는 네이티브 형식이 됩니다. [/Zc: wchar_t](zc-wchar-t-wchar-t-is-native-type.md) 는 기본적으로 설정 되어 있습니다.
...
OpenMP 2.0 언어 확장을 사용 합니다. [/Openmp](openmp-enable-openmp-2-0-support.md)를 설정 합니다.

### <a name="c-language-standard"></a>C++ 언어 표준

컴파일러가 사용 C++ 하도록 설정 하는 언어 표준을 결정 합니다. 가능 하면 최신 버전을 사용 합니다. Set [/std: c + + 14,/std: c + + 17,/std: c + + 최신](std-specify-language-standard-version.md).
...
빌드하는 동안 미리 컴파일된 헤더를 만들거나 사용하도록 설정합니다. [/Yc](yc-create-precompiled-header-file.md), [/yu](yu-use-precompiled-header-file.md)를 설정 합니다.
...
생성 된 미리 컴파일된 헤더 파일의 경로 또는 이름을 지정 합니다. [/Fp](fp-name-dot-pch-file.md)를 설정 합니다.
...
생성 된 XML 문서 파일의 이름을 지정 합니다. 파일 또는 디렉터리 이름일 수 있습니다. [/Doc:\<name >](doc-process-documentation-comments-c-cpp.md)를 설정 합니다.

## <a name="cc-browse-information-properties"></a>C/C++ 찾아보기 정보 속성

### <a name="enable-browse-information"></a>찾아보기 정보 사용

*`.bsc`* 파일의 찾아보기 정보 수준을 지정 합니다. [/Fr](fr-fr-create-dot-sbr-file.md)를 설정 합니다.

### <a name="browse-information-file"></a>찾아보기 정보 파일

브라우저 정보 파일의 선택적 이름을 지정 합니다. [/Fr\<name >](fr-fr-create-dot-sbr-file.md)를 설정 합니다.
...
- **__cdecl** -멤버 함수와 __stdcall 또는 __fastcall로 표시 된 함수 C++ 를 제외한 모든 함수에 대 한 __cdecl 호출 규칙을 지정 합니다.
- **__fastcall** -멤버 함수와 __cdecl 또는 __stdcall로 표시 된 함수 C++ 를 제외한 모든 함수에 대 한 __fastcall 호출 규칙을 지정 합니다. 모든 __fastcall 함수에는 프로토타입이 있어야 합니다.
- **__stdcall** -멤버 함수와 __cdecl 또는 __fastcall로 표시 된 함수 C++ 를 제외한 모든 함수에 대 한 __stdcall 호출 규칙을 지정 합니다. 모든 __stdcall 함수에는 프로토타입이 있어야 합니다.
- **__vectorcall** -멤버 함수와 __cdecl, __fastcall 또는 __stdcall로 표시 C++ 된 함수를 제외한 모든 함수에 대 한 __vectorcall 호출 규칙을 지정 합니다. 모든 __vectorcall 함수에는 프로토타입이 있어야 합니다.

### <a name="compile-as"></a>다음으로 컴파일

*`.c`* 및 *`.cpp`* 파일에 대 한 컴파일 언어 옵션을 선택 합니다. [/Tc,/tp를](tc-tp-tc-tp-specify-source-file-type.md)설정 합니다.
...
지정 된 경고 번호를 사용 하지 않도록 설정 합니다. 경고 번호를 세미콜론으로 구분 된 목록으로 입력 합니다. [/Wd\<num >](compiler-option-warning-level.md)를 설정 합니다.

### <a name="forced-include-file"></a>강제 포함 파일

하나 이상의 강제 포함 파일입니다. [/Fi\<name >](fi-name-forced-include-file.md)를 설정 합니다.

### <a name="forced-using-file"></a>강제 #using 파일

강제 #using 파일을 하나 이상 지정 합니다. [/Fu\<name >](fu-name-forced-hash-using-file.md)를 설정 합니다.
...
*`.obj`* 파일에 기본 라이브러리 이름을 포함 하지 않습니다. [/Zl](zl-omit-default-library-name.md)을 설정 합니다.
