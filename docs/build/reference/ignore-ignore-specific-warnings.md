...
기본적으로 LINK는 모든 경고를 보고합니다. **/IGNORE:** `warning` 지정 하 여 특정 경고 번호를 표시 하지 않도록 링커에 지시 합니다. 여러 경고를 무시하려면 경고 번호를 쉼표로 구분합니다.
...
|LNK4062|'`option`'이 (가) '`architecture`' 대상 컴퓨터와 호환 되지 않습니다. 옵션이 무시 됨|
...
|LNK4228|DLL에 대해 '`option`'이 (가) 잘못 되었습니다. 무시|
