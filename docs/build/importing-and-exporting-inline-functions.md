...
가져온 함수는 인라인으로 정의할 수 있습니다. 이 효과는 표준 함수 인라인을 정의하는 것과 거의 동일합니다. 함수에 대한 호출은 매크로와 마찬가지로 인라인 코드로 확장됩니다. 이는 주로 효율성을 위해 일부 멤버 함수를 인라인으로 사용할 수 있는 DLL에서 C++ 클래스를 지원하는 방법으로 유용합니다.

가져온 인라인 함수의 한 가지 기능은 C++에서 주소를 가져올 수 있다는 것입니다. 컴파일러는 DLL에 있는 인라인 함수의 복사본 주소를 반환합니다. 가져온 인라인 함수의 또 다른 기능은 가져온 함수의 정적 로컬 데이터를 전역 가져온 데이터와 달리 초기화할 수 있다는 것입니다.

> [!CAUTION]
> 가져온 인라인 함수를 제공할 때는 버전 충돌이 발생할 수 있으므로 주의해야 합니다. 인라인 함수는 응용 프로그램 코드로 확장됩니다. 따라서 나중에 함수를 다시 작성하면 응용 프로그램 자체가 다시 컴파일되지 않는 한 업데이트되지 않습니다. 일반적으로 DLL 함수를 사용하는 응용 프로그램을 다시 빌드하지 않고 업데이트할 수 있습니다.

## <a name="what-do-you-want-to-do"></a>수행 작업

- [DLL에서 내보내기](exporting-from-a-dll.md)

- [을 사용하여 DLL에서 내보내기 DEF 파일](exporting-from-a-dll-using-def-files.md)
