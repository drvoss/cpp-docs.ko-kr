...
Visual Studio를 사용하여 표준 C++ 프로그램을 만들 수 있습니다. 이 연습의 단계를 수행하여 프로젝트를 만들고, 프로젝트에 새 파일을 추가하고, 코드를 추가 C++ 하도록 파일을 수정하고, Visual Studio를 사용하여 프로그램을 컴파일하고 실행할 수 있습니다.
...
1. 다음과 같이 프로젝트에 새 소스 파일을 추가 합니다.
   ...
   1. **솔루션 탐색기**에서 **소스 파일** 폴더를 마우스 오른쪽 단추로 클릭하고 **추가**를 가리킨 다음 **새 항목**을 클릭 합니다.

   1. **코드** 노드에서  **C++ 파일 (.cpp)** 을 클릭하고 파일 이름을 입력 한 다음 **추가**를 클릭 합니다.

   .Cpp 파일은 **솔루션 탐색기**의 **소스 파일** 폴더에 표시되고 파일은 Visual Studio 편집기에서 열립니다.

1. 편집기의 파일에서 C++ C++ 표준 라이브러리를 사용 하는 유효한 프로그램을 입력 하거나 샘플 프로그램 중 하나를 복하하여 파일에 붙여넣습니다.
...
1. **디버그** 메뉴에서 **디버깅하지 않고 시작**을 클릭합니다.
   ...
   샘플 프로그램을 사용 하는 경우 명령 창이 표시되고 특정 정수가 집합에 있는지 여부를 표시 합니다.
