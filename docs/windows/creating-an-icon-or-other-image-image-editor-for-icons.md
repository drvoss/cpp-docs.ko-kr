...
|**Height**|사용자 지정 이미지의 높이를 픽셀 단위로 입력할 수 있는 공간을 제공 합니다 (1-512, 제한인 2048).|
|**색**|사용자 지정 이미지에 대 한 색 수를 선택할 수 있는 공간 (2, 16 또는 256)을 제공 합니다.|

**&lt;장치&gt; 이미지 열기** 대화 상자를 사용 하 여 프로젝트에서 C++ 장치 이미지를 엽니다. 현재 리소스 (현재 리소스의 일부인 이미지)의 기존 장치 이미지를 나열 합니다. 포함 되는 속성은 다음과 같습니다.
...
1. [리소스 삽입 대화 상자](../windows/add-resource-dialog-box.md)에서 **아이콘** 또는 **커서** 를 선택 하 고 **새로 만들기**를 선택 합니다. 아이콘의 경우이 작업은 32 × 32, 16 색 아이콘을 사용 하 여 아이콘 리소스를 만듭니다. 커서의 경우 32 × 32, 단색 (2 색) 이미지가 만들어집니다.
   ...
   **리소스 삽입** 대화 상자에서 이미지 리소스 유형 옆에 더하기 기호 ( **+** )가 표시 되 면 도구 모음 템플릿을 사용할 수 있음을 의미 합니다. 더하기 기호를 선택 하 여 템플릿 목록을 확장 하 고, 템플릿을 선택 하 고, **새로 만들기**를 선택 합니다.
...
1. 메뉴 **이미지로** 이동 > **장치 이미지를 열고** 현재 이미지 목록에서 이미지를 선택 합니다. 예를 들어 32 × 32, 16 색 버전의 아이콘을 선택 합니다.
...
커서의 핫 스폿은 Windows에서 커서 위치를 추적 하는 지점입니다. 기본적으로 핫 스폿은 좌표가 `0,0`있는 커서의 왼쪽 위 모퉁이로 설정 됩니다. **속성 창**의 [Hotspot](/visualstudio/ide/reference/properties-window) 속성은 핫 스폿 좌표를 표시합니다.
...
1. 비트맵이 **이미지 편집기**에 열려 있는 상태에서 메뉴 **파일** > 로 이동 하 여  ***filename*.bmp를로 저장**합니다.
...
1. 메뉴 **파일** > 로 이동 하 여  ***파일 이름* 을로 저장**합니다.
