...
ms.locfileid: "80167565"
---
# <a name="accelerator-editor-c"></a>액셀러레이터 키 편집기C++()

액셀러레이터 키 테이블은 바로 C++ 가기 키 및 해당 키와 연결 된 명령 식별자의 목록이 포함 된 Windows 리소스입니다. 프로그램에는 액셀러레이터 키 테이블이 두 개 이상 있을 수 있습니다.
...
[클래스 뷰](/visualstudio/ide/viewing-the-structure-of-code) 를 사용하여 액셀러레이터 키 명령을 코드에 연결할 수 있습니다. 미리 정의 된 액셀러레이터 키 목록은 [액셀러레이터 키](../windows/predefined-accelerator-keys.md)를 참조 하세요.
...
- **Type** 속성이 **ASCII**인 경우 **Modifier** 속성은 `None` 또는 `Alt`만 가능 하거나 `^`키 앞에 지정 된 대로 **Ctrl** 키를 사용 하는 액셀러레이터 키를 가질 수 있습니다.
...
1. 액셀러레이터 키로 사용할 *키* 를 입력 하거나 마우스 오른쪽 단추를 클릭 하 고 **다음** 키를 선택 하 여 키 조합을 설정 하거나 메뉴 > **편집** 으로 이동을 클릭 하 여 **다음 키를 입력**합니다.

1. **한정자** 와 **형식**(필요한 경우)을 변경 하 고 **enter**키를 누릅니다.

> [!NOTE]
> 정의한 모든 액셀러레이터 키가 고유한지 확인합니다. 동일한 ID에 여러 키 조합을 할당할 수 있습니다. 예를 들어 **Ctrl**+**P** 와 **F8** 모두 ID_PRINT에 할당 될 수 있습니다. 그러나 두 개 이상의 ID에 할당 된 키 조합을 사용 하는 것은 제대로 작동 하지 않습니다. 예를 들어 **Ctrl**+**Z** 는 ID_SPELL_CHECK와 ID_THESAURUS에 모두 할당 됩니다.
...
1. 마우스 오른쪽 단추를 클릭 하 고 **삭제**를 선택 하거나 메뉴에서 **편집** > **삭제**로 이동 합니다.
