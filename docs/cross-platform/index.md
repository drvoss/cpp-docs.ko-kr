---
layout: LandingPage
title: C++를 사용한 모바일 개발
description: Visual Studio에서 iOS, Android 및 Windows 디바이스용 네이티브 C++ 앱을 만듭니다.
ms.date: 10/31/2018
author: jasongroce
ms.author: jasgro
ms.topic: landing-page
ms.openlocfilehash: d250a1586b4cf01dfae2a39b7d595b60c53a5123
ms.sourcegitcommit: a673f6a54cc97e3d4cd032b10aa8dce7f0539d39
ms.translationtype: HT
ms.contentlocale: ko-KR
ms.lasthandoff: 02/28/2020
ms.locfileid: "78171456"
---
# <a name="mobile-development-with-c-documentation"></a>C++을 사용한 모바일 개발 설명서

Visual Studio에서 iOS, Android 및 Windows 디바이스용 네이티브 C++ 앱을 만듭니다.

<!-- markdownlint-disable MD033 -->

<ul class="panelContent cardsF">
    <li>
        <a href="/cpp/cross-platform/install-visual-cpp-for-cross-platform-mobile-development">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="/media/common/i_setup.svg" alt="Set up Visual Studio for cross-platform mobile app development with C++">
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>C++를 사용하여 플랫폼 간 모바일 개발 설치</h3>
                        <p>플랫폼 간 모바일 앱을 빌드하는 데 필요한 도구를 설정합니다.</p>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
    <li>
        <a href="/cpp/cross-platform/visual-cpp-for-cross-platform-mobile-development/">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="/media/common/i_learn-about.svg" alt="Learn about Visual Studio tools for C++ cross-platform mobile development">
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>C++ 플랫폼 간 모바일 개발용 Visual Studio 도구에 대한 자세한 정보</h3>
                        <p>C++를 사용한 플랫폼 간 모바일 개발에 대해 대략적으로 설명합니다.</p>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
    <li>
        <a href="/cpp/cross-platform/cross-platform-mobile-development-examples">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="/media/common/i_xplat-code.svg" alt="Cross-platform mobile development examples">
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>플랫폼 간 모바일 개발 예제</h3>
                        <p>플랫폼 간 모바일 개발을 시작하는 데 도움이 되는 샘플을 참조하세요.</p>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
</ul>

---
<h2>참고</h2>

<ul class="panelContent cardsW">
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardText">
                        <h3><a href="/cpp/visual-cpp-in-visual-studio">Visual Studio의 C++</a></h3>
                        <p><a href="/cpp/ide/read-and-understand-code-cpp">코드 읽기 및 쓰기</a><p>
                    </div>
                </div>
            </div>
        </div>
    </li>
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardText">
                        <h3>플랫폼</h3>
                        <p><a href="/cpp/cppcx/universal-windows-apps-cpp">UWP(유니버설 Windows 플랫폼) 설명서</a></p>
                        <p><a href="https://developer.apple.com/reference">iOS API 설명서</a></p>
                        <p><a href="https://developer.android.com/guide/index.html">Android API 설명서</a></p>
                    </div>
                </div>
            </div>
        </div>
    </li>
</ul>
