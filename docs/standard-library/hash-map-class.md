...
`iterator`Hash_map 정의 된는 형식이 인 [value_type](#value_type)개체인 요소를 가리킵니다 .이는 `pair<const Key, Type>` 첫 번째 멤버가 요소에 대 한 키이 고 두 번째 멤버가 요소에 의해 보유 되는 매핑된 데이텀의 경우입니다.

`Iter`Multimap의 요소를 가리키는 이라는 반복기를 역참조 하려면 연산자를 사용 `->` 합니다.
