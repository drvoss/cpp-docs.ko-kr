...
ms.locfileid: "81366761"
---
# <a name="boyer_moore_searcher-class"></a>boyer_moore_searcher 클래스

클래스는 `boyer_moore_searcher` Boyer-Moore 알고리즘을 사용하여 개체의 생성자에 지정된 시퀀스를 검색하는 함수 개체 유형입니다. 검색은 개체의 함수 호출 연산자에게 제공된 다른 시퀀스 내에서 수행됩니다. 이 클래스는 [std::search의](algorithm-functions.md#search)오버로드 중 하나에 매개 변수로 전달됩니다.
...
시퀀스, `boyer_moore_searcher` 해시 함수 개체 및 같음 조건자 검색을 사용 하 여 함수 개체를 생성 합니다.
