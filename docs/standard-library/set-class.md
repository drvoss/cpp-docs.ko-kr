...
ms.locfileid: "79425180"
---
# <a name="set-class"></a>set 클래스

C++ 표준 라이브러리 컨테이너 집합은 포함된 요소값이 고유하고 데이터가 자동 정렬되는 기준인 키 값으로 사용된 컬렉션의 데이터를 스토리지 및 검색하는 데 사용됩니다. set 요소의 값은 직접 변경할 수 없습니다. 대신, 이전 값을 삭제하고 새 값의 요소를 삽입해야 합니다.
...
*특성*\
두 요소 값을 정렬 키로 비교하여 set에서 상대적인 순서를 결정할 수 있는 함수 개체를 제공하는 형식입니다. 이 인수는 선택 사항이 며, 이진 조건자 *\<키 >* 기본값 보다 낮습니다.
...
집합은 제공 된 기능이 일반적이 고 요소로 포함 된 데이터의 특정 형식과 독립적 이기 때문에 클래스 템플릿으로도 설명 됩니다. 사용될 데이터 형식은 대신 비교 함수 및 할당자와 함께 클래스 템플릿에서 매개 변수로 지정됩니다.
...
애플리케이션에서 값과 해당 키를 연결하는 조건을 만족할 경우 적절한 연관 컨테이너는 set입니다. set의 요소는 고유하며 자체 정렬 키로 사용됩니다. 이 형식의 구조에 대한 모델은 정렬된 목록입니다. 예를 들어, 단어 내의 단어는 한 번만 나타날 수 있습니다. 단어의 여러 번 중복이 허용된 경우는 multiset이 적절한 컨테이너 구조입니다. 고유 키 단어 목록에 값이 연결된 경우 이 데이터를 포함하기 위한 적절한 구조는 map입니다. 대신 Key가 고유하지 않은 경우 multimap이 적절한 컨테이너입니다.
...
`allocator_type`은 템플릿 매개 변수 [할당자](../standard-library/set-class.md)의 동의어입니다.
...
`begin`의 반환 값이 `const_iterator`에 할당 된 경우에는 set 개체의 요소를 수정할 수 없습니다. `begin`의 반환 값이 `iterator`에 할당 된 경우에는 set 개체의 요소를 수정할 수 있습니다.
...
멤버 함수는 작음 비교 가능 보다 작음 관계를 기반으로 순서를 지정 하는 이진 조건자의 인수 *키* 와 동일한 키를 가진 집합의 요소를 참조 하는 반복기를 반환 합니다.

`find`의 반환 값이 `const_iterator`에 할당 된 경우에는 set 개체를 수정할 수 없습니다. `find`의 반환 값이 `iterator`에 할당 된 경우에는 set 개체를 수정할 수 있습니다.
...
*Valty*\
집합에서 [value_type](../standard-library/map-class.md#value_type)요소를 생성 하는 데 사용할 수 있는 인수 형식을 지정 하 고 *Val* 을 인수로 사용 하는 템플릿 매개 변수입니다.
...
*Inputiterator*\
[value_type](../standard-library/input-iterator-tag-struct.md) 개체를 생성하는 데 사용할 수 있는 형식의 요소를 가리키는 [입력 반복기](../standard-library/map-class.md#value_type)의 요구 사항을 충족하는 템플릿 함수 인수입니다.
...
단일 요소 멤버 함수 (1) 및 (2)는 **부울** 구성 요소가 삽입 된 경우 true이 고 집합에 정렬에 동일한 값의 요소가 이미 포함 되어 있는 경우 false 인 [쌍](../standard-library/pair-structure.md) 을 반환 합니다. **부울** 구성 요소가 true 인 경우 반환 값 쌍의 반복기 구성 요소는 새로 삽입 된 요소를 가리키거나 **bool** 구성 요소가 false 인 경우에는 기존 요소를 가리킵니다.
...
컨테이너의 [value_type](../standard-library/map-class.md#value_type)은 컨테이너에 속한 형식 정의이고 set의 경우 `set<V>::value_type`은 `const V` 형식입니다.
...
을 정의합니다. 이 함수는 **이 앞에 오며 정렬 순서가** 과 같지 않으면 `_xVal`true`_yVal`를 반환합니다.
...
`key_compare` 및 [value_compare](#value_compare) 은 템플릿 매개 변수 `Traits`의 동의어입니다. 두 형식 모두 set 및 multiset 클래스용으로 제공되며 이러한 클래스에 사용되는 경우에는 동일하지만, map 및 multimap 클래스와의 호환성을 위해 제공되는 경우에는 서로 다릅니다.
...
`key_type` 및 [value_type](#value_type) 은 템플릿 매개 변수 `Key`의 동의어입니다. 두 형식 모두 set 및 multiset 클래스용으로 제공되며 이러한 클래스에 사용되는 경우에는 동일하지만, map 및 multimap 클래스와의 호환성을 위해 제공되는 경우에는 서로 다릅니다.
...
*오른쪽*\
이 `set`에 할당할 새 요소를 제공하는 `set`입니다.

### <a name="remarks"></a>설명

첫 번째 버전의 `operator=`는 *오른쪽*에 대 한 [lvalue 참조](../cpp/lvalue-reference-declarator-amp.md) 를 사용 하 여 *오른쪽* 에서이 `set`로 요소를 복사 합니다.

두 번째 버전은 right에 대해 [rvalue 참조](../cpp/rvalue-reference-declarator-amp-amp.md)를 사용하여 *오른쪽* 에서이 `set`로 요소를 이동 합니다.
...
`rend`은 set에서 [end](#end)가 사용되는 것처럼 역방향 set에 사용됩니다.
...
*Al*\
이 집합 개체에 사용할 저장소 할당자 클래스로, 기본값은 `Allocator`입니다.

*Comp*\
set의 요소 순서를 지정하는 데 사용되는 `const Traits` 형식의 비교 함수로, 기본값은 `Compare`입니다.
...
모든 생성자는 set의 메모리 스토리지를 관리하며 나중에 [get_allocator](#get_allocator)를 호출하여 반환할 수 있는 할당자 개체 형식을 저장합니다. 할당자 매개 변수는 대체 할당자를 대체하는 데 사용되는 전처리 매크로 및 클래스 선언에서 생략되는 경우가 많습니다.

모든 생성자는 해당 set를 초기화합니다.

모든 생성자는 `Traits` 형식의 함수 개체를 저장 합니다 .이 개체는 집합의 키 사이에 순서를 설정 하는 데 사용 되며 나중에 [key_comp](#key_comp)를 호출 하 여 반환할 수 있습니다.

처음 세 생성자는 빈 초기 집합을 지정 하 고, 두 번째 생성자는 요소의 순서를 설정 하는 데 사용할 비교 함수 (`comp`)의 형식을 지정 하며, 세 번째 생성자는 사용할 할당자 형식 (`al`)을 명시적으로 지정 합니다. **explicit** 키워드를 사용하는 경우 특정 종류의 자동 형식 변환이 수행되지 않습니다.

네 번째 생성자는 `right` set의 복사본을 지정합니다.

그 다음 3개 생성자는 initializer_list를 사용하여 요소를 지정합니다.

다음 세 생성자는 클래스 `Traits` 및 **할당자**의 비교 함수 유형을 지정 하는 내 높아집니다가 늘어나는 집합의 범위 [`first`, `last`)를 복사 합니다.
...
인수 키 보다 큰 키를 포함 하는 집합 내 요소의 위치 주소를 가져오거나, 키와 일치 하는 항목이 없는 경우 set에서 마지막 요소 다음 위치의 주소를 설정 하는 `iterator` 또는 `const_iterator`입니다.
...
[Key_compare](#key_compare) 및 `value_compare`은 템플릿 매개 변수 `Traits`의 동의어입니다. 두 형식 모두 set 및 multiset 클래스용으로 제공되며 이러한 클래스에 사용되는 경우에는 동일하지만, map 및 multimap 클래스와의 호환성을 위해 제공되는 경우에는 서로 다릅니다.

### <a name="example"></a>예제

[를 선언하고 사용하는 방법의 예제는 ](#value_comp)value_comp`value_compare`의 예제를 참조하세요.
...
[Key_type](#key_type) 및 `value_type`은 템플릿 매개 변수 `Key`의 동의어입니다. 두 형식 모두 set 및 multiset 클래스용으로 제공되며 이러한 클래스에 사용되는 경우에는 동일하지만, map 및 multimap 클래스와의 호환성을 위해 제공되는 경우에는 서로 다릅니다.
