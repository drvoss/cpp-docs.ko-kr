...
*Rep2* 가로 암시적으로 변환할 수 없고 true 또는 false를 포함 하는 경우를 제외 하 `rep` 고 `treat_as_floating_point<rep>` *holds true* `treat_as_floating_point<Rep2>` *holds false*두 번째 생성자는 오버 로드 확인에 참여 하지 않습니다. 자세한 내용은 [<type_traits>](../standard-library/type-traits.md)를 참조하세요.
