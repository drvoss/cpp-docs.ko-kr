...
ms.locfileid: "81368521"
---
# <a name="regular-expressions-c"></a>정규식(C++)

C++ 표준 라이브러리는 여러 정규식 문법을 지원합니다. 이 항목에서는 정규식을 사용할 때 사용할 수 있는 문법 변형에 대해 설명합니다.

## <a name="regular-expression-grammar"></a><a name="regexgrammar"></a> 정규식 문법

사용할 정규식 문법은 `std::regex_constants::syntax_option_type` 열거형 값 중 하나를 사용하여 지정합니다. 이러한 정규식 문법은 std:regex_constants 정의됩니다.

- `ECMAScript`: 자바스크립트와 .NET 언어에서 사용하는 문법에 가장 가깝습니다.
- `basic`: POSIX 기본 정규식 또는 BRE.
- `extended`: POSIX 확장 정규식 또는 ERE.
- `awk`: 이것은 `extended`,하지만 인쇄되지 않은 문자에 대한 추가 이스케이프가 있습니다.
- `grep`: 이것은 `basic`, 그러나 그것은 또한 줄 바선 ('\n') 문자를 별도의 교대로 할 수 있습니다.
- `egrep`: 이것은 `extended`, 그러나 그것은 또한 줄 바선 문자가 교대로 분리 할 수 있습니다.

기본적으로 문법이 지정되지 않은 `ECMAScript` 경우 가정됩니다. 하나의 문법만 지정할 수 있습니다.

문법 외에도 여러 플래그를 적용할 수 있습니다.

- `icase`: 일치할 때 대/소문자를 무시합니다.
- `nosubs`: 표시된 일치 항목 무시(즉, 괄호 안에 있는 식)를 무시합니다. 대체품이 저장되지 않습니다.
- `optimize`: 더 큰 건설 시간의 가능한 비용으로, 더 빨리 일치합니다.
- `collate`: 로캘에 민감한 데이터 정렬 시퀀스(예: "[a-z]"의 범위)를 사용합니다.

0 개 이상의 플래그는 정규식 엔진 동작을 지정하는 문법과 결합 될 수있다. 플래그만 지정하면 `ECMAScript` 문법으로 가정됩니다.
...
- `expr` 식으로 정의된 집합에도 있는 대상 시퀀스의 문자 또는 데이터 정렬 요소와 일치하는 "[`expr`]" 형식 또는 `expr` 식으로 정의된 집합에 없는 대상 시퀀스의 문자 또는 데이터 정렬 요소와 일치하는 "[^`expr`]" 형식의 *대괄호 식*.
  ...
  - 형식의 *문자 범위* `ch1`-`ch2`" ". 닫힌 범위 [`ch1`, `ch2`]의 값으로 표현된 문자를 `expr`로 정의된 집합에 추가합니다.
...
구분 기호 간 패턴이 일치하는 대상 시퀀스에서 문자 시퀀스와 일치하는 "(*하위 식*)" 형식 또는 `basic` 및 의 "\\(*하위 식`grep`* \\)"의 *캡처 그룹*.

- 대상 시퀀스의 문자와\\`k` `k` 일치하는 "형식의 *ID 이스케이프입니다.*
...
`basic`에서 `ECMAScript`, `grep`및, 요소는 또한 Nth 캡처 그룹에\\`dd`의해 `dd` 일치하는 문자의 시퀀스와 동일한 대상 시퀀스의 문자 시퀀스와 일치하는 소수점 값 N을 나타내는 "" 양식의 *백 참조일* 수도 *있다.* 예를 들어, "(a)\1"는 대상 시퀀스 "aa"와 일치합니다. 그 이유는 첫 번째이자 유일한 캡처 그룹이 초기 시퀀스인 "a"와 일치하고 \1이 마지막 시퀀스인 "a"와 일치하기 때문입니다.

`ECMAScript`에서 요소는 다음 중 하나가 될 수도 있습니다.

- 형식의 *비 캡처 그룹* "(?: 하위 *표현식)"입니다.* 대상 시퀀스에서 구분 기호 간 패턴과 일치하는 문자 시퀀스와 일치합니다.
...
- ""\\\\" ", "\a", "\b", "\f", "\n", "\r", "\t" 또는 "\v"의 *파일 형식 이스케이프.* 대상 시퀀스에서 각각 백슬래시, 경고, 백스페이스, 폼 피드, 줄 바꿈, 캐리지 리턴, 가로 탭, 수직 탭과 일치합니다.

- 양식 "의 *팔각이 탈출 시퀀스* "\\`ooo`". 대상 시퀀스에서 표현이 한 자리, 두 자리 또는 세 자리 8진수 `ooo`로 표현된 값인 문자와 일치합니다.

### <a name="repetition"></a>반복

*긍정 어설션*, *부정 어설션* 또는 *앵커* 이외의 모든 요소 다음에 반복 횟수가 올 수 있습니다. 가장 일반적인 유형의 반복 횟수는 `basic` 및 `grep`의 "\\{`min`,`max`\\}" 또는 "{`min`,`max`}" 형태입니다. 이러한 형태의 반복 횟수 앞에 오는 요소는 해당 요소와 일치하는 시퀀스가 연속적으로 `min`개 이상, `max`개 이하일 경우 일치합니다. 예를 들어,{2,3}""는 표적 서열 "aaa"와 표적 서열 "aaa"와 일치하지만, 표적 서열 "a" 또는 표적 서열 "aaaa"는 아니다.
...
- "는{2}"표적 서열 "aa"와 일치하지만 표적 서열 "aaa"또는 표적 서열 "aaa"는 일치하지 않는다.

- "는{2,}"표적 서열 "aa", 타겟 서열 "aaa"와 일치하지만, 타겟 서열 "a"와 일치하지 는 않는다.

- "는\*"대상 서열", 타겟 서열 "a", 타겟 서열 "aa", 등등일치한다.
...
- "아?" 대상 시퀀스 "a"와 대상 시퀀스 "a"와 일치하지만 대상 시퀀스 "aa"는 일치하지 않습니다.

- "a+"는 대상 시퀀스 "a", 대상 시퀀스 "aa" 등과 일치하지만 대상 시퀀스 ""와는 일치하지 않습니다.

에서, `ECMAScript`반복 카운트의 모든 형태는 *비 욕심 반복을*지정문자 '?'다음에 할 수 있습니다.

### <a name="concatenation"></a>연결

정규식 요소는 *반복 횟수* 유무와 상관없이 연결하여 긴 정규식을 형성할 수 있습니다. 결과 식은 개별 요소와 일치하는 시퀀스 연결인 대상 시퀀스와 일치합니다. 예를 들어,{2,3}"b"는 표적 서열 "aab" 및 표적 서열 "aaab"과 일치하지만 표적 서열 "ab" 또는 표적 서열 "aaaab"과 일치하지 않는다.
...
## <a name="grammar-summary"></a><a name="grammarsummary"></a> 문법 요약

다음 표에 다양한 정규식 문법에서 사용할 수 있는 기능이 요약되어 있습니다.

|요소|basic|확장|ECMAScript|grep|egrep|awk|
|-------------|---------|---------|----------|----------|-----------|---------|
|'&#124;'를 사용하는 교체||+|+||+|+|
|'\n'을 사용하는 교체||||+|+||
|앵커|+|+|+|+|+|+|
...
## <a name="semantic-details"></a><a name="semanticdetails"></a> 의미 정보
...
ID 이스케이프는 백슬래시와 단일 문자의 순서로 구성되어 있습니다. 해당 문자와 일치합니다. 문자에 특별한 의미가 있을 경우 필요합니다. ID 이스케이프를 사용할 경우 특별한 의미는 제거됩니다. 다음은 그 예입니다.

- "는\*"aaa"라는 대상 서열과 일치하지만 대상 서열 "aaa"와\*일치하지 않습니다.

- "a"는\\\*대상 서열 "aaa"와 일치하지 않지만 대상\*시퀀스 "aaa"와 일치합니다.
...
|`extended`, `egrep`|{ '(),',')', '{', '.',\\'[',', '', '' ',\*',', '^', '$', '+', '?', '&#124;' }|
|`awk`|`extended` 더하기 { '"', '/' }|
|`ECMAScript`|식별자에 포함될 수 있는 문자를 제외한 모든 문자. 일반적으로 여기에는 문자, 숫자, '$', ''\_및 유니코드 이스케이프 시퀀스가 포함됩니다. 자세한 내용은 ECMAScript 언어 사양을 참조하십시오.|
...
비캡처 그룹은 정규식 문법에서 내용을 단일 단위로 표시하지만 대상 텍스트에 레이블을 지정하지 않습니다. 예를 들어 "(a)(?:b)\*(c)"는 대상 텍스트 "abbc"와 일치하고 하위 시퀀스 "a"와 그룹 2를 하위 시퀀스 "c"와 연결합니다.

### <a name="non-greedy-repetition"></a>non-greedy 반복

non-greedy 반복은 패턴과 일치하는 대상 시퀀스의 가장 짧은 하위 시퀀스를 사용합니다. greedy 반복은 가장 긴 항목을 사용합니다. 예를 들어" "(a+)(b)"는\*대상 시퀀스 "aaab"과 일치합니다. non-greedy 반복을 사용할 경우 캡처 그룹 1을 대상 시퀀스의 시작 부분에 있는 시퀀스 "a"와 연결하고 캡처 그룹 2를 대상 시퀀스의 끝에 있는 시퀀스 "aab"와 연결합니다. greedy 일치를 사용할 경우 캡처 그룹 1을 하위 시퀀스 "aaa"와 연결하고 캡처 그룹 2를 하위 시퀀스 "b"와 연결합니다.
...
- '\*정규식의 첫 번째 문자 또는 정규식에서 초기 '^'를 따르는 첫 번째 문자 또는 캡처 그룹의 첫 번째 문자 또는 캡처 그룹의 초기 '^'를 따르는 첫 번째 문자를 제외한 모든 경우에 특별한 의미가 있습니다.
...
- "(=aa)(a)"는\*대상 시퀀스 "aaaa"와 일치하고 캡처 그룹 1을 하위 시퀀스 "aaaa"와 연결합니다.

- "(aa)(a)"는\*타겟 시퀀스 "aaaa"와 일치시키고 타겟 시퀀스의 끝에 있는 서브시퀀스 "aa"와 서브시퀀스 "aa"를 포획하는 그룹 1과 타겟 시퀀스의 시작 부분에서 "aa"를 연관시다.
...
## <a name="matching-and-searching"></a><a name="matchingandsearching"></a> 일치 및 검색
...
## <a name="format-flags"></a><a name="formatflags"></a> 형식 플래그
...
|"$$"||"$"|
||"&"\\|"&"|
|"$\`" (달러 기호 뒤에 다시 따옴표)| 정규식()`[match.prefix().first, match.prefix().second)`과 일치하는 하위 시퀀스 앞에 오는 문자 시퀀스|
|"$'"(달러 기호와 작은따옴표)||시퀀스 다음의 정규식과 일치하는 문자 시퀀스(`[match.suffix().first, match.suffix().second)`)|
|"$n"|"\n"|위치에서 `n`캡처 그룹과 일치하는 문자 시퀀스, 0에서 `n` 9`[match[n].first, match[n].second)`사이의 숫자입니다( )|
||"\\\n"|"\n"|
|"$nn"||위치에서 `nn`캡처 그룹과 일치하는 문자 시퀀스, 10에서 `nn` 99`[match[nn].first, match[nn].second)`사이의 숫자입니다.|
