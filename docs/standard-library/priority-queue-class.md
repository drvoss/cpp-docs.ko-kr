...
ms.locfileid: "81322997"
---
# <a name="priority_queue-class"></a>priority_queue 클래스
...
큐 개체의 `Type` 첫 번째 템플릿 매개 변수에 규정된 클래스의 요소는 [value_type](#value_type) 동의어이며 두 번째 템플릿 `Container` 매개 변수에서 규정한 기본 컨테이너 클래스의 요소 형식과 일치해야 합니다. 해당 `Type` 형식의 개체를 복사하고 해당 형식의 변수에 값을 할당할 수 있도록 할수 있어야 합니다.

priority_queue 클래스의 `Traits`저장된 함수 개체를 호출하여 제어하는 시퀀스를 정렬합니다. 일반적으로, 이 순서를 정하려면 요소의 크기를 비교할 수 있어야 합니다. 즉, 제공된 어떤 두 요소에서 두 요소가 동일하거나(어떤 것도 다른 것보다 작지 않음) 하나가 다른 것보다 작음을 정할 수 있어야 합니다. 그러면 동일하지 않은 요소 사이에 정렬이 수행됩니다. 기술적으로 설명하면, 비교 함수는 표준 함수의 의미에서 엄밀히 약한 정렬을 수행하는 이진 조건자입니다.
...
- [스택 클래스는](../standard-library/stack-class.md) LIFO(선불) 데이터 구조를 지원합니다. 쌓여 있는 접시 더미의 예로 이해할 수 있습니다. 요소(접시)는 기본 컨테이너의 끝에 있는 마지막 요소인 스택의 맨 위에서만 삽입하거나 검사하거나 제거할 수 있습니다. 맨 위 요소로만 액세스를 제한하는 것이 stack 클래스를 사용하는 이유입니다.

- [큐 클래스는](../standard-library/queue-class.md) FIFO(선차) 데이터 구조를 지원합니다. 은행 창구 직원을 만나려고 줄 서 있는 사람들의 예로 이해할 수 있습니다. 요소(사람)는 줄의 뒤에 추가될 수 있고 줄의 앞에서 제거됩니다. 줄의 앞과 뒤는 모두 검사할 수 있습니다. queue 클래스를 사용하는 이유는 이 방식으로 앞과 뒤의 요소만 액세스할 수 있기 때문입니다.
...
|멤버 함수|Description|
|-|-|
|[빈](#empty)|`priority_queue`이 비어 있는지를 테스트합니다.|
|[팝](#pop)|`priority_queue`의 가장 큰 요소를 최상위 위치에서 제거합니다.|
|[push](#push)|operator<의 요소 우선 순위에 따라 priority_queue에 요소를 추가합니다.|
|[크기](#size)|`priority_queue`에 있는 요소 수를 반환합니다.|
|[맨 위로](#top)|`priority_queue`의 최상위 위치에 있는 가장 큰 요소에 대한 const 참조를 반환합니다.|
...
## <a name="priority_queueempty"></a><a name="empty"></a>priority_queue::비어 있음
...
## <a name="priority_queuepop"></a><a name="pop"></a>priority_queue::p
...
## <a name="priority_queuepriority_queue"></a><a name="priority_queue"></a>priority_queue::priority_큐
...
처음 세 생성자는 각각 빈 초기 priority_queue 지정하고, 두 번째 생성자는 요소의 순서를 설정하는 데 사용할 비교 함수() 유형()을`comp`지정하고 세 번째 생성자는 사용할 `container_type` ()`_Cont`)를 명시적으로 지정합니다. **explicit** 키워드를 사용하는 경우 특정 종류의 자동 형식 변환이 수행되지 않습니다.

네 번째 생성자는 priority_queue *오른쪽의*복사본을 지정합니다.

마지막 세 생성자는 일부 \[컨테이너의 범위를 *먼저*, *마지막)* 복사하고 값을 사용하여 클래스 `Traits` 및 의 비교 함수 유형을 `container_type`지정할 때 명시성이 증가하는 priority_queue 초기화합니다.
...
## <a name="priority_queuepush"></a><a name="push"></a>priority_queue::p
...
## <a name="priority_queuesize"></a><a name="size"></a>priority_queue::크기
...
## <a name="priority_queuetop"></a><a name="top"></a>priority_queue:상단
...
함수에 의해 결정된 가장 큰 `Traits` 요소에 대한 참조로, priority_queue 개체입니다.
