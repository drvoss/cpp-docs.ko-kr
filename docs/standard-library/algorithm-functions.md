...
**`true`** 지정 된 범위의 각 요소에 조건이 있는 경우를 반환 합니다.
...
*pred*\
테스트할 조건입니다. 확인 중인 요소가 충족할 조건을 정의하는 사용자 정의 조건자 함수 개체입니다. 단항 조건자는 단일 인수를 사용 하 고 **`true`** 또는를 반환 **`false`** 합니다.

### <a name="return-value"></a>반환 값

**`true`** 지정 된 범위의 각 요소에서 조건이 검색 되거나 범위가 비어 있으면를 반환 하 고 그렇지 않으면를 반환 **`false`** 합니다.

### <a name="remarks"></a>설명

**`true`** `N` 범위에 있는 각에 대해 `[0, last - first)` 조건자 `pred(*(first + N))` 가 인 경우에만 템플릿 함수가를 반환 합니다 **`true`** .
...
**`true`** 지정 된 요소 범위에서 조건이 한 번 이상 있는 경우를 반환 합니다.
...
*pred*\
테스트할 조건입니다. 사용자 정의 조건자 함수 개체에 의해 제공됩니다. 조건자는 테스트 중인 요소가 충족해야 할 조건을 정의합니다. 단항 조건자는 단일 인수를 사용 하 고 **`true`** 또는를 반환 **`false`** 합니다.
...
*pred*\
한 요소가 다른 요소보다 작다는 의미를 정의하는 사용자 정의 조건자 함수 개체입니다. 이진 조건자는 두 개의 인수를 사용 하 고 만족 **`true`** 하지 않을 경우를 반환 **`false`** 합니다.

### <a name="return-value"></a>반환 값

**`true`** 지정 된 값과 같거나 동일한 범위에 요소가 있으면이 고, 그렇지 않으면입니다. 그렇지 않으면 **`false`** 입니다.
...
*pred*\
*값* 을 *lower* 또는 *upper*로 비교 하는 데 사용 되는 조건자입니다. 비교 조건자는 두 개의 인수를 사용 하 고 **`true`** 첫 번째 값이 두 번째 보다 작은 경우에는를 반환 하 고 그렇지 않으면를 반환 **`false`** 합니다.
...
요소의 범위에서 **`true`** 지정 된 조건에 대 한 요소를 복사 합니다.
...
*pred*\
범위의 모든 요소가 테스트되는 조건입니다. 이 조건은 사용자 정의 조건자 함수 개체에 의해 제공됩니다. 단항 조건자는 하나의 인수를 사용 하 고 또는를 반환 **`true`** **`false`** 합니다.
...
*pred*\
요소 개수를 세기 위한 일치 조건을 정의하는 사용자 정의 조건자 함수 개체. 단항 조건자는 단일 인수를 사용 하 고 **`true`** 또는를 반환 **`false`** 합니다.
...
*pred*\
두 요소가 같은 것으로 간주되려면 충족해야 하는 조건을 정의하는 사용자 정의 조건자 함수 개체입니다. 이진 조건자는 두 개의 인수를 사용 하 고 만족 **`true`** 하지 않을 경우를 반환 **`false`** 합니다.

### <a name="return-value"></a>반환 값

**`true`** 요소가 요소 별로 비교 될 때 이진 조건자에서 범위가 동일 하거나 동급 인 경우에만 그렇지 않으면 **`false`** 입니다.

### <a name="remarks"></a>설명

검색할 범위는 유효해야 하고 모든 반복기는 역참조 가능해야 하며 처음 위치에서 증분하여 마지막 위치까지 도달할 수 있어야 합니다.

두 범위의 길이가 같은 경우 알고리즘의 시간 복잡도는 범위에 포함된 요소 수에 비례합니다. 그렇지 않으면 함수는를 즉시 반환 **`false`** 합니다.
...
*pred*\
한 요소가 다른 요소보다 작다는 의미를 정의하는 사용자 정의 조건자 함수 개체입니다. 비교 조건자는 두 개의 인수를 사용 하 고 만족 **`true`** 하지 않을 때를 반환 **`false`** 합니다.
...
*pred*\
검색 중인 요소가 충족하는 조건을 정의하는 [람다 식](../cpp/lambda-expressions-in-cpp.md) 또는 사용자 정의 조건자 함수 개체입니다. 단항 조건자는 단일 인수를 사용 하 고 **`true`** 만족 되는 경우를 반환 하 고, 충족 되지 않으면를 반환 **`false`** 합니다. *Pred* 의 시그니처는로 반드시 같아야 합니다 `bool pred(const T& arg);` `T` . 여기서은 `InputIterator` 역참조 될 때 암시적으로 변환 될 수 있는 형식입니다. **`const`** 키워드는 함수 개체 또는 람다가 인수를 수정 하지 않아야 함을 보여 주기 위해서만 표시 됩니다.

### <a name="return-value"></a>반환 값

조건자로 지정 된 조건을 충족 하는 범위 내 첫 번째 요소를 참조 하는 입력 반복기입니다 (조건자 결과 **`true`** ). 조건자를 충족 하는 요소가 없는 경우는 *last*를 반환 합니다.
...
*pred*\
검색 중인 요소가 충족하지 않는 조건을 정의하는 [람다 식](../cpp/lambda-expressions-in-cpp.md) 또는 사용자 정의 조건자 함수 개체입니다. 단항 조건자는 단일 인수를 사용 하 고 **`true`** 만족 되는 경우를 반환 하 고, 충족 되지 않으면를 반환 **`false`** 합니다. *Pred* 의 시그니처는로 반드시 같아야 합니다 `bool pred(const T& arg);` `T` . 여기서은 `InputIterator` 역참조 될 때 암시적으로 변환 될 수 있는 형식입니다. **`const`** 키워드는 함수 개체 또는 람다가 인수를 수정 하지 않아야 함을 보여 주기 위해서만 표시 됩니다.

### <a name="return-value"></a>반환 값

조건자로 지정 된 조건을 충족 하지 않는 범위의 첫 번째 요소를 참조 하는 입력 반복기입니다 (조건자 결과 **`false`** ). 모든 요소가 조건자를 충족 하는 경우 (조건자가 **`true`** 모든 요소에 대해를 반환 하는 경우)는 *마지막*을 반환 합니다.
...
**`true`** 첫 번째 정렬 된 범위에 두 번째 정렬 된 범위의 모든 요소가 포함 되어 있으면이 고, 그렇지 않으면입니다. 그렇지 않으면 **`false`** 입니다.
...
*pred*\
한 요소가 다른 요소보다 작다는 의미를 정의하는 사용자 정의 조건자 함수 개체입니다. 비교 조건자는 두 개의 인수를 사용 하 고 **`true`** 첫 번째 요소가 두 번째 요소 보다 작은 경우에는를 반환 하 고 그렇지 않으면를 반환 합니다 **`false`** .
...
**`true`** 지정 된 범위의 요소가 힙을 형성 하는 경우를 반환 합니다.
...
*pred*\
요소 순서를 테스트 하는 조건입니다. 비교 조건자는 두 개의 인수를 사용 하 고 또는를 반환 합니다 **`true`** **`false`** .

### <a name="return-value"></a>반환 값

**`true`** 지정 된 범위의 요소가 힙을 구성 하면이 고, 그렇지 않으면를 반환 **`false`** 합니다.
...
**`true`** 지정 된 범위에서 조건에 대해 테스트 하는 모든 요소가를 **`true`** 테스트 하는 요소 앞에 있는 경우를 반환 **`false`** 합니다.
...
*pred*\
테스트할 조건입니다. 검색 중인 요소가 충족할 조건을 정의하는 사용자 정의 조건자 함수 개체에 의해 제공됩니다. 단항 조건자는 단일 인수를 사용 하 고 **`true`** 또는를 반환 **`false`** 합니다.

### <a name="return-value"></a>반환 값

**`true`** 지정 된 범위에서 조건에 대해 테스트 하는 모든 요소가를 **`true`** 테스트 하는 요소 앞에 있으면를 반환 하 **`false`** 고, 그렇지 않으면를 반환 **`false`** 합니다.

### <a name="remarks"></a>설명

의 모든 요소가 **`true`** `[first, last)` *pred*로 분할 된 경우에만 템플릿 함수가를 `X` `[first, last)` `pred (X)` 반환 `Y` 합니다 `pred (Y)` . 즉 **`false`** ,에 대 한 모든 요소가 인 경우에는의 모든 요소가 인 경우에만를 반환 합니다.
...
*pred*\
동등성을 테스트 하 고을 반환 하는 조건자입니다 **`bool`** .

### <a name="return-value"></a>반환 값

**`true`** 비교 연산자 조건자에 따라 동일 하 게 범위를 다시 정렬할 수 있으면이 고, 그렇지 않으면입니다. 그렇지 않으면 **`false`** 입니다.

### <a name="remarks"></a>설명

최악의 경우 `is_permutation`은 정방형 복잡성을 갖습니다.

첫 번째 템플릿 함수는 *first2* 에서 시작 하는 범위에로 지정 된 범위 만큼 많은 요소가 있다고 가정 합니다 `[first1, last1)` . 두 번째 범위에 더 많은 요소가 있는 경우 무시되고, 더 적은 요소가 있는 경우 정의되지 않은 동작이 발생합니다. 세 째 템플릿 함수(C++14 이상)에서는 이러한 가정을 하지 않습니다. 둘 다에서 **`true`** 지정 된 범위의 각 요소 x에 대해 `[first1, last1)` *first2* 또는에서 시작 하는 범위에 있는 것과 같이 x = = Y와 동일한 범위에 있는 요소 수가 y 인 경우에만를 반환 `[first2, last2)` 합니다. 여기서는 `operator==` 피연산자 간의 쌍 비교를 수행 해야 합니다.
...
**`true`** 지정 된 범위의 요소가 정렬 된 순서 대로 되어 있으면를 반환 합니다.
...
*pred*\
두 요소 사이의 순서를 결정하기 위해 테스트할 조건입니다. 비교 조건자는 두 개의 인수를 사용 하 고 또는를 반환 합니다 **`true`** **`false`** . `operator<`와 동일한 작업을 수행합니다.
...
두 번째 버전에서는 지정 된 두 요소가 정렬 된 순서에 있는 경우를 반환 하는 비교 함수 개체를 제공할 수 **`true`** 있으며, **`false`** 그렇지 않으면를 반환 합니다.
...
*pred*\
두 요소 사이의 순서를 결정하기 위해 테스트할 조건입니다. 비교 조건자는 두 개의 인수를 사용 하 고 또는를 반환 합니다 **`true`** **`false`** .
...
**`true`** 첫 번째 범위가 두 번째 범위 보다 사전순으로 면이 고, 그렇지 않으면입니다. 그렇지 않으면 **`false`** 입니다.
...
- 같지 없고 시퀀스의 요소 수가 동일 하므로 시퀀스는 동일 하 고 비교 결과는 **`false`** 입니다.
...
`max` 알고리즘은 대체로 매개 변수로 전달된 개체를 사용하지 않습니다. 대부분의 C++ 표준 라이브러리 알고리즘은 매개 변수로 전달된 반복기가 위치를 지정하는 요소 범위에서 작동합니다. 요소 범위에서 작동하는 함수가 필요한 경우 [max_element](algorithm-functions.md#max_element)를 대신 사용합니다. Visual Studio 2017에서는 **`constexpr`** initializer_list를 사용 하는 오버 로드를 사용할 수 있습니다.
...
*pred*\
한 요소가 다른 요소 보다 작은 의미를 정의 하는 사용자 정의 조건자 함수 개체입니다. 비교 조건자는 두 개의 인수를 사용 하 여 첫 번째 인수가 **`true`** 두 번째 보다 작은 경우에는를 반환 하 고 그렇지 않으면를 반환 합니다 **`false`** .
...
첫 번째 템플릿 함수는 `pair<const Type&, const Type&>( right, left )` *right* 가 *left*보다 작은 경우를 반환 합니다. 그렇지 않으면 `pair<const Type&, const Type&>( left, right )`을 반환합니다.
...
*pred*\
각 범위에 있는 현재 요소를 비교하고 동일한지 여부를 확인하는 사용자 정의 조건자 함수 개체입니다. 충족 **`true`** 되는 경우와 **`false`** 충족 되지 않는 경우를 반환 합니다.
...
*pred*\
순서에 따라 연속적인 요소에 대해 충족될 비교 조건을 정의하는 사용자 정의 조건자 함수 개체입니다. 이진 조건자는 두 개의 인수를 사용 하 고 만족 **`true`** 하지 않을 경우를 반환 **`false`** 합니다.

### <a name="return-value"></a>반환 값

**`true`** 사전순으로 next 순열이 있고 범위의 원래 순서를 바꾼 경우 그렇지 않은 경우 **`false`** 에는 순서가 사전순으로 가장 작은 순열으로 변환 됩니다.
...
*pred*\
순서에 따라 연속적인 요소에 대해 충족될 비교 조건을 정의하는 사용자 정의 조건자 함수 개체입니다. 비교 조건자는 두 개의 인수를 사용 하 고 만족 **`true`** 하지 않을 때를 반환 **`false`** 합니다.
...
**`true`** 지정 된 범위의 요소 사이에 조건이 없는 경우를 반환 합니다.
...
*pred*\
테스트할 조건입니다. 이 조건은 조건을 정의하는 사용자 정의 조건자 함수 개체에 의해 제공됩니다. 단항 조건자는 단일 인수를 사용 하 고 **`true`** 또는를 반환 **`false`** 합니다.

### <a name="return-value"></a>반환 값

**`true`** 지정 된 범위에서 조건이 한 번 이상 검색 되지 않으면를 반환 하 고, **`false`** 조건이 검색 되 면을 반환 합니다.

### <a name="remarks"></a>설명

**`true`** `N` 범위에 있는 일부에 대해 `[0, last - first)` 조건자 `pred(*(first + N))` 가 항상 인 경우에만 템플릿 함수가를 반환 합니다 **`false`** .
...
*pred*\
요소를 분류해야 할 경우 충족해야 할 조건을 정의하는 사용자 정의 조건자 함수 개체입니다. 단항 조건자는 단일 인수를 사용 하 고 **`true`** 또는를 반환 **`false`** 합니다.
...
조건이 다른 대상에 해당 하는 요소를 대상으로 하는 요소를 복사 **`true`** **`false`** 합니다. 지정된 범위의 요소여야 합니다.
...
*pred*\
테스트할 조건입니다. 이 조건은 테스트할 조건을 정의하는 사용자 정의 조건자 함수 개체에 의해 제공됩니다. 단항 조건자는 단일 인수를 사용 하 고 **`true`** 또는를 반환 **`false`** 합니다.
...
템플릿 함수는 `it` 에서 `[first, last)` `pred(*it)` 가 인 첫 번째 반복기를 찾습니다 **`false`** . 순서는 *pred*를 기준으로 정렬 되어야 합니다.
...
**`true`** 사전순으로 이전 순열이 있고 범위의 원래 순서를 바꾼 경우 그렇지 않은 경우 **`false`** 에는 순서가 사전순으로 가장 큰 순열으로 변환 됩니다.
...
*pred*\
한 요소가 다른 요소보다 작다는 의미를 정의하는 사용자 정의 조건자 함수 개체입니다. 이진 조건자는 두 개의 인수를 사용 하 고 **`true`** 첫 번째 요소가 두 번째 요소 보다 작은 경우에는를 반환 하 고 그렇지 않으면를 반환 합니다 **`false`** .
...
*pred*\
순서에 따라 연속적인 요소에 대해 충족될 비교 조건을 정의하는 사용자 정의 조건자 함수 개체입니다. 이 이진 조건자는 두 개의 인수를 사용 하며, **`true`** 두 인수가 순서 대로 되어 있으면를 반환 하 고 **`false`** 그렇지 않으면를 반환 합니다. 이 비교 함수는 시퀀스의 요소 쌍에 대해 엄밀히 약한 순서를 적용해야 합니다. 자세한 내용은 [알고리즘](algorithms.md)을 참조하세요.
...
*pred*\
요소를 분류해야 할 경우 충족해야 할 조건을 정의하는 사용자 정의 조건자 함수 개체입니다. 단항 조건자는 단일 인수를 사용 하 고 **`true`** 만족 되는 경우를 반환 하 고, 충족 되지 않으면를 반환 **`false`** 합니다.
...
*pred*\
한 요소가 다른 요소 보다 작은 의미를 정의 하는 사용자 정의 비교 조건자 함수 개체입니다. 비교 조건자는 두 개의 인수를 사용 하 고 만족 **`true`** 하지 않을 때를 반환 **`false`** 합니다.
