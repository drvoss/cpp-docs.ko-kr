...
| 행사 | 속성 | Description |
|--|--|--|
| <a name="back-end-pass"></a>BACK_END_PASS | Type | 작업 |
...
|  | Description | 컴파일러 백 엔드 패스의 시작 및 중지 시 발생 합니다. 이 패스는 구문 분석 된 C/C++ 소스 코드를 최적화 하 고 기계어 코드로 변환 하는 일을 담당 합니다. |
| <a name="bottom-up"></a>BOTTOM_UP | Type | 작업 |
...
|  | Description | 전체 프로그램 분석의 시작 및 중지에 발생 합니다. |
| <a name="c1-dll"></a>C1_DLL | Type | 작업 |
...
|  | Description | *C1* 또는 *c1xx* 호출을 시작 및 중지할 때 발생 합니다. 이러한 Dll은 컴파일러의 C C++ 및 프런트 엔드입니다. 컴파일러 드라이버 (*cl.exe*)를 통해서만 호출 됩니다. |
| <a name="c2-dll"></a>C2_DLL | Type | 작업 |
...
|  | Description | *C2 .dll* 호출을 시작 및 중지할 때 발생 합니다. 이 DLL은 컴파일러의 백 엔드입니다. 컴파일러 드라이버 (*cl.exe*)에 의해 호출 됩니다. 링크 타임 코드 생성이 사용 되는 경우 링커 (*link .exe*) 에서도 호출 됩니다. |
| <a name="code-generation"></a>CODE_GENERATION | Type | 작업 |
|  | 부모 항목 | [C2_DLL](#c2-dll) |
...
|  | Description | 백 엔드의 코드 생성 단계를 시작 하 고 중지할 때 발생 합니다. |
...
|  | Description | 컴파일러와 링커가 명령줄 평가를 수행 하면 발생 합니다. 평가 된 명령줄에는 지시 파일을 통해 전달 된 모든 *cl.exe* 및 *링크 .exe* 매개 변수가 포함 됩니다. Cl에 대 한 매개 변수와 cl, \_CL\_, 링크 및 \_링크\_와 같은 환경 *변수를 통해* 전달 되는 *링크* 도 포함 됩니다. |
| <a name="compiler"></a>컴파일러나 | Type | 작업 |
|  | 부모 항목 | None |
...
|  | Description | *Cl.exe* 호출을 시작 및 중지할 때 발생 합니다. |
...
|  | Description | *Cl.exe* 또는 *debug.exe* 가 호출 될 때 모든 기존 환경 변수에 대해 한 번 발생 합니다. |
...
|  | Description | 링커 입력 중 하나가 DLL 또는 실행 가능한 이미지 파일인 경우 발생 합니다. |
...
|  | Description | 링커 출력 중 하나가 .exp 파일인 경우 발생 합니다 *.* |
...
|  | Description | *Cl.exe* 또는 *link .exe* 입력을 알리기 위해 발생 합니다. |
...
|  | Description | `__forceinline` 키워드를 사용 하 여 함수가 다른 함수에 강제로 인라인 될 때 발생 합니다. |
| <a name="front-end-file"></a>FRONT_END_FILE | Type | 작업 |
...
|  | Description | 컴파일러 프런트 엔드가 시작 되 고 파일 처리가 중지 될 때 발생 합니다. 이 이벤트는 재귀적입니다. 프런트 엔드가 포함 된 파일을 구문 분석할 때 재귀가 발생 합니다. |
| <a name="front-end-pass"></a>FRONT_END_PASS | Type | 작업 |
|  | 부모 항목 | [컴파일러나](#compiler) |
...
|  | Description | 컴파일러 프런트 엔드 패스의 시작 및 중지 시 발생 합니다. 이 패스는 C/C++ 소스 코드를 구문 분석 하 고 중간 언어로 변환 하는 일을 담당 합니다. |
| <a name="function"></a>칩셋용으로 | Type | 작업 |
...
|  | Description | 함수에 대 한 코드 생성을 시작 하 고 종료 하는 경우에 발생 합니다. |
...
|  | Description | 링커 출력 중 하나가 가져오기 라이브러리인 경우 발생 합니다. |
...
|  | Description | 링커 출력 중 하나가 정적 라이브러리인 경우 발생 합니다. |
| <a name="linker"></a>링커 | Type | 작업 |
|  | 부모 항목 | None |
...
|  | Description | *링크 .exe* 호출을 시작 및 중지할 때 발생 합니다. |
| <a name="ltcg"></a>LTCG | Type | 작업 |
|  | 부모 항목 | [PASS1.LOG](#pass1) |
|  | Children | [C2_DLL](#c2-dll) |
|  | 속성 | None |
|  | 클래스 캡처 | [활동](cpp-event-data-types/activity.md)<br/>[LTCG](cpp-event-data-types/ltcg.md) |
|  | Description | 링크 타임 코드 생성이 시작 및 중지 될 때 발생 합니다. |
...
|  | Description | *Cl.exe*에 의해 생성 된 모든 *.obj* 출력에 대해 한 번 발생 합니다. |
| <a name="opt-icf"></a>OPT_ICF | Type | 작업 |
|  | 부모 항목 | [PASS1.LOG](#pass1) |
|  | Children | None |
|  | 속성 | None |
|  | 클래스 캡처 | [활동](cpp-event-data-types/activity.md)<br/>[OptICF](cpp-event-data-types/opt-icf.md) |
|  | Description | 동일한 COMDAT 정리 (/OPT: ICF) 링커 최적화를 시작 하 고 중지할 때 발생 합니다. |
| <a name="opt-lbr"></a>OPT_LBR | Type | 작업 |
|  | 부모 항목 | [PASS1.LOG](#pass1) |
|  | Children | None |
|  | 속성 | None |
|  | 클래스 캡처 | [활동](cpp-event-data-types/activity.md)<br/>[OptLBR](cpp-event-data-types/opt-lbr.md) |
|  | Description | 긴 분기 (/OPT: LBR) 링커 최적화의 시작 및 중지에 발생 합니다. |
| <a name="opt-ref"></a>OPT_REF | Type | 작업 |
|  | 부모 항목 | [PASS1.LOG](#pass1) |
|  | Children | None |
|  | 속성 | None |
|  | 클래스 캡처 | [활동](cpp-event-data-types/activity.md)<br/>[OptRef](cpp-event-data-types/opt-ref.md) |
|  | Description | 참조 되지 않은 함수 및 데이터 제거 (/OPT: REF) 링커 최적화를 시작 하 고 중지할 때 발생 합니다. |
| <a name="pass1"></a>PASS1.LOG | Type | 작업 |
|  | 부모 항목 | [링커](#linker) |
...
|  | Description | 링커의 pass 1을 시작 하 고 중지할 때 발생 합니다. |
| <a name="pass2"></a>PASS2 | Type | 작업 |
|  | 부모 항목 | [링커](#linker) |
|  | Children | None |
|  | 속성 | None |
|  | 클래스 캡처 | [활동](cpp-event-data-types/activity.md)<br/>[Pass2](cpp-event-data-types/pass2.md) |
|  | Description | 링커의 pass 2를 시작 하 고 중지할 때 발생 합니다. |
| <a name="pre-ltcg-opt-ref"></a>PRE_LTCG_OPT_REF | Type | 작업 |
|  | 부모 항목 | [PASS1.LOG](#pass1) |
|  | Children | None |
|  | 속성 | None |
|  | 클래스 캡처 | [활동](cpp-event-data-types/activity.md)<br/>[PreLTCGOptRef](cpp-event-data-types/pre-ltcg-opt-ref.md) |
|  | Description | 참조 되지 않은 함수 및 데이터를 제거 하는 링커 최적화 패스의 시작 및 중지 시 발생 합니다 (/OPT: REF). 링크 타임 코드를 생성 하기 전에 수행 됩니다. |
...
|  | Description | 프런트 엔드 pass의 끝에서 발생: 템플릿 인스턴스화에 관련 된 모든 형식에 대해 한 번씩 발생 합니다. 키는 형식에 대 한 숫자 식별자 이지만 이름은 텍스트 표현입니다. 유형 키는 분석 중인 추적 내에서 고유 합니다. 그러나 다른 컴파일러 프런트 엔드 패스에서 들어오는 여러 키는 동일한 형식을 가리킬 수 있습니다. 서로 다른 컴파일러 프런트 엔드 패스 간에 형식을 비교 하려면 이름을 사용 해야 합니다. 모든 템플릿 인스턴스화가 발생 한 후에는 컴파일러 프런트 엔드 패스의 끝에 SYMBOL_NAME 이벤트를 내보냅니다. |
| <a name="template-instantiation"></a>TEMPLATE_INSTANTIATION | Type | 작업 |
...
|  | Description | 컴파일러 백 엔드 스레드 실행의 시작과 끝에서 발생 합니다. 일시 중단 되는 스레드는 종료 된 것으로 간주 됩니다. 해제 되는 스레드가 시작 된 것으로 간주 됩니다. |
| <a name="top-down"></a>TOP_DOWN | Type | 작업 |
|  | 부모 항목 | [WHOLE_PROGRAM_ANALYSIS](#whole-program-analysis) |
|  | Children | [FUNCTION](#function)<br/>[스레드](#thread) |
|  | 속성 | None |
|  | 클래스 캡처 | [활동](cpp-event-data-types/activity.md)<br/>[위쪽 아래쪽](cpp-event-data-types/top-down.md) |
|  | Description | 전체 프로그램 분석 ' 하향식 패스의 시작 및 중지에 발생 합니다. |
| <a name="whole-program-analysis"></a>WHOLE_PROGRAM_ANALYSIS | Type | 작업 |
|  | 부모 항목 | [C2_DLL](#c2-dll) |
...
|  | Description | 링크 타임 코드 생성의 전체 프로그램 분석 단계를 시작 하 고 중지할 때 발생 합니다. |
