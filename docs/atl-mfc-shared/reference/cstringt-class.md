...
- **StrTraitATL < wchar_t** &#124; **char** &#124; **tchar.h, ChTraitsCRT < wchar_t** &#124; **문자** &#124; **tchar.h > >**

   클래스는 CRT 지원이 필요 하 고 `m_hInstResource` (응용 프로그램의 모듈 클래스의 멤버)에 지정 된 모듈에서 리소스 문자열을 검색 합니다.

- **StrTraitATL < wchar_t** &#124; **char** &#124; **tchar.h, ChTraitsOS < wchar_t** &#124; **문자** &#124; **tchar.h > >**

   클래스는 CRT 지원이 필요 하지 않으며 `m_hInstResource` (응용 프로그램의 모듈 클래스의 멤버)에 지정 된 모듈에서 리소스 문자열을 검색 합니다.

- **StrTraitMFC < wchar_t** &#124; **char** &#124; **tchar.h, ChTraitsCRT < wchar_t** &#124; **문자** &#124; **tchar.h > >**

   클래스는 CRT 지원이 필요 하며 표준 MFC 검색 알고리즘을 사용 하 여 리소스 문자열을 검색 합니다.

- **StrTraitMFC < wchar_t** &#124; **char** &#124; **tchar.h, ChTraitsOS < wchar_t** &#124; **문자** &#124; **tchar.h > >**
...
|[CStringT::Format](#format)|`sprintf`에서 문자열의 형식을 지정 합니다.|
...
|[CStringT::LoadString](#loadstring)|Windows 리소스에서 기존 `CStringT` 개체를 로드 합니다.|
...
|[CStringT::SetSysString](#setsysstring)|`CStringT` 개체의 데이터로 기존 BSTR 개체를 설정 합니다.|
|[CStringT::SpanExcluding](#spanexcluding)|`pszCharSet`으로 식별 되는 문자 집합에 포함 되지 않은 첫 번째 문자부터 시작 하 여 문자열에서 문자를 추출 합니다.|
...
`CStringT` [CSimpleStringT 클래스](../../atl-mfc-shared/reference/csimplestringt-class.md)에서 상속 됩니다. 문자 조작, 순서 지정, 검색 등의 고급 기능은 `CStringT`에 의해 구현 됩니다.

> [!NOTE]
> `CStringT` 개체는 예외를 throw 할 수 있습니다. 이 문제는 어떤 이유로 든 `CStringT` 개체의 메모리가 부족 한 경우에 발생 합니다.

`CStringT` 개체는 가변 길이 문자 시퀀스로 구성 됩니다. `CStringT`는 Basic의 구문과 유사한 구문을 사용 하 여 함수 및 연산자를 제공 합니다. 연결 및 비교 연산자를 간소화 된 메모리 관리와 함께 사용 하면 일반 문자 배열 보다 `CStringT` 개체를 쉽게 사용할 수 있습니다.

> [!NOTE]
>  포함 된 null 문자를 포함 하는 `CStringT` 인스턴스를 만들 수 있지만이를 방지 하는 것이 좋습니다. 포함 된 null 문자를 포함 하는 `CStringT` 개체에 대 한 메서드 및 연산자를 호출 하면 의도 하지 않은 결과가 발생할 수 있습니다.

`BaseType` 및 `StringTraits` 매개 변수의 다양 한 조합을 사용 하 여 `CStringT` 개체는 ATL 라이브러리에 의해 미리 정의 된 다음 형식으로 제공 될 수 있습니다.

ATL 응용 프로그램에서를 사용 하는 경우:

`CString`, `CStringA`및 `CStringW`는 MFC DLL (MFC90)에서 내보내집니다. DLL), 사용자 Dll은 그렇지 않습니다. 이는 `CStringT`의 곱셈 정의를 방지 하기 위해 수행 됩니다.
...
다음 문자열 형식은 ATL_CSTRING_NO_CRT 정의 된 프로젝트에서 사용할 수 있습니다.
...
ATL_CSTRING_NO_CRT 정의 되지 않은 프로젝트에서 사용할 수 있는 문자열 형식은 다음과 같습니다.
...
- `CStringT` 개체는 "값 의미 체계"를 따릅니다. `CStringT` 개체는 문자열에 대 한 포인터가 아닌 실제 문자열로 생각 하면 됩니다.

- `PCXSTR` 함수 인수에 대 한 `CStringT` 개체를 자유롭게 대체할 수 있습니다.
...
|`PXSTR`|`CStringT` 개체와 같은 문자 형식의 문자열 ( **wchar_t** 또는 **char**)에 대 한 포인터입니다.|
|`PYSTR`|`CStringT` 개체와 반대편 문자 형식을 사용 하는 문자열 ( **wchar_t** 또는 **char**)에 대 한 포인터입니다.|
|`PCXSTR`|`CStringT` 개체와 같은 문자 형식을 사용 하는 **const** 문자열 ( **wchar_t** 또는 **char**)에 대 한 포인터입니다.|
|`PCYSTR`|`CStringT` 개체와 반대 문자 형식을 사용 하는 **const** 문자열 ( **wchar_t** 또는 **char**)에 대 한 포인터입니다.|

> [!NOTE]
>  이전에 `CString` (예: `AssignCopy`)의 문서화 되지 않은 메서드를 사용한 코드는 다음과 같은 `CStringT`의 문서화 된 메서드 (예: `GetBuffer` 또는 `ReleaseBuffer`)를 사용 하는 코드로 대체 되어야 합니다. 이러한 메서드는 `CSimpleStringT`에서 상속 됩니다.
...
BSTR 형식의 자동화 호환 문자열을 할당 하 고 종료 null 문자를 포함 하 여 `CStringT` 개체의 내용을 복사 합니다.
...
_UNICODE 정의 된 경우에는 함수를 사용할 수 없습니다.
...
`_tcscoll`일반 텍스트 함수를 사용 하 여 두 문자열을 비교 합니다.
...
`_tcscmp`일반 텍스트 함수는 TCHAR.H에 정의 되어 있습니다. H는 컴파일 시간에 정의 된 문자 집합에 따라 `strcmp`, `wcscmp`또는 `_mbscmp`에 매핑됩니다. 각 함수는 문자열의 대/소문자를 구분 하는 비교를 수행 하며 로캘의 영향을 받지 않습니다. 자세한 내용은 [strcmp, wcscmp, _mbscmp](../../c-runtime-library/reference/strcmp-wcscmp-mbscmp.md)를 참조 하세요.
...
`_tcsicmp`일반 텍스트 함수는 TCHAR.H에 정의 되어 있습니다. H는 컴파일 시간에 정의 된 문자 집합에 따라 `_stricmp`, `_wcsicmp` 또는 `_mbsicmp`에 매핑됩니다. 각 함수는 문자열의 대/소문자를 구분 하지 않는 비교를 수행 합니다. 비교는 로캘의 LC_CTYPE 측면에 따라 달라 지지만 LC_COLLATE는 그렇지 않습니다. 자세한 내용은 [_stricmp, _wcsicmp, _mbsicmp, _stricmp_l, _wcsicmp_l, _mbsicmp_l](../../c-runtime-library/reference/stricmp-wcsicmp-mbsicmp-stricmp-l-wcsicmp-l-mbsicmp-l.md)을 참조 하세요.
...
*pszSrc*<br/>
이 `CStringT` 개체로 복사할 null 종료 문자열입니다.

*pStringMgr*<br/>
`CStringT` 개체의 메모리 관리자에 대 한 포인터입니다. `CStringT`에 대 한 `IAtlStringMgr` 및 메모리 관리에 대 한 자세한 내용은 [CStringT를 사용 하 여 메모리 관리](../../atl-mfc-shared/memory-management-with-cstringt.md)를 참조 하세요.

*strSrc*<br/>
이 `CStringT` 개체에 복사할 기존 `CStringT` 개체입니다. `CThisString` 및 `CThisSimpleString`에 대 한 자세한 내용은 설명 섹션을 참조 하세요.
...
*SystemString*<br/>
`System::String`해야 하며 프로젝트는/clr을 사용 하 여 컴파일해야 합니다.
...
생성자는 입력 데이터를 새 할당 된 저장소에 복사 하기 때문에 메모리 예외가 발생할 수 있음을 알고 있어야 합니다. 이러한 생성자 중 일부는 변환 함수 역할을 합니다. 예를 들어 `CStringT` 개체가 필요한 LPTSTR를 대체할 수 있습니다.

- `CStringT`(`LPCSTR` `lpsz`): ANSI 문자열에서 유니코드 `CStringT`를 생성 합니다. 또한 아래 예제와 같이이 생성자를 사용 하 여 문자열 리소스를 로드할 수 있습니다.

- `CStringT(` `LPCWSTR` `lpsz`): 유니코드 문자열에서 `CStringT`를 생성 합니다.

- `CStringT`(`const unsigned char*` `psz`): 포인터에서 **부호 없는 char**에 대 한 `CStringT`를 생성할 수 있습니다.

> [!NOTE]
>  ANSI 문자열과 유니코드 문자열 간의 암시적 문자열 변환을 해제 하려면 _CSTRING_DISABLE_NARROW_WIDE_CONVERSION 매크로를 정의 합니다. 매크로는 변환을 지 원하는 컴파일 생성자에서 제외 됩니다.

*Strsrc* 매개 변수는 `CStringT` 또는 `CThisSimpleString` 개체 중 하나일 수 있습니다. `CStringT`의 경우 기본 인스턴스화 (`CString`, `CStringA`또는 `CStringW`) 중 하나를 사용 합니다. `CThisSimpleString`의 경우 **this** 포인터를 사용 합니다. `CThisSimpleString`는 [CSimpleStringT 클래스](../../atl-mfc-shared/reference/csimplestringt-class.md)의 인스턴스를 선언 합니다 .이 클래스는 `CStringT` 클래스 보다 기본 제공 기능이 적은 작은 문자열 클래스입니다.

오버 로드 연산자 `CSimpleStringT<>&()` `CSimpleStringT` 선언에서 `CStringT` 개체를 생성 합니다.
...
*iIndex*<br/>
삭제할 `CStringT` 개체에 있는 첫 번째 문자의 0부터 시작 하는 인덱스입니다.
...
함수는 두 개의 단일 문자 (런타임 함수 `strchr`)와 문자열 (`strstr`와 유사)을 모두 허용 하도록 오버 로드 됩니다.
...
`CStringT`Sprintf_s[가 데이터를 C 스타일 문자 배열로 서식 지정 하는 것과 동일한 방식으로 ](../../c-runtime-library/reference/sprintf-s-sprintf-s-l-swprintf-s-swprintf-s-l.md)에 형식이 지정된 데이터를 씁니다.
...
문자열 개체 자체가 `Format`에 대 한 매개 변수로 제공 되는 경우 호출이 실패 합니다. 예를 들어 다음 코드는 예측할 수 없는 결과를 발생 시킵니다.
...
> [!NOTE]
> `FormatMessage`는 새로 서식이 지정 된 문자열에 대해 시스템 메모리를 할당 하려고 시도 합니다. 이 시도가 실패 하면 메모리 예외가 자동으로 throw 됩니다.
...
*pszFormat*<br/>
형식 컨트롤 문자열을 가리킵니다. 그러면 삽입이 검색 되 고 그에 따라 형식이 지정 됩니다. 서식 문자열은 임의의 순서로 매개 변수를 삽입할 수 있도록 하는 것을 제외 하 고 런타임 함수 `printf`스타일 형식 문자열과 비슷합니다.
...
`vsprintf_s` 데이터를 C 스타일 문자 배열로 서식 지정 하는 것과 같은 방식으로 `CStringT` 문자열에 서식이 지정 된 문자열과 인수의 변수 목록을 씁니다.
...
이 `CStringT` 개체에서 가장 왼쪽 *n 개수* 문자를 추출 하 고 추출 된 부분 문자열의 복사본을 반환 합니다.
...
*Ifirst* (0부터 시작) 위치에서 시작 하 여이 `CStringT` 개체에서 길이가 *ncount* 인 부분 문자열을 추출 합니다.
...
_UNICODE 정의 된 경우에는이 함수를 사용할 수 없습니다.
...
`CThisSimpleString`에 대 한 자세한 내용은 [cstringt:: cstringt](#cstringt)의 설명 섹션을 참조 하세요.
...
`CStringT::operator+` 함수에는 7 개의 오버 로드 형식이 있습니다. 첫 번째 버전에서는 두 개의 기존 `CStringT` 개체를 연결 합니다. 다음 두 `CStringT` 개체 및 null로 끝나는 문자열을 연결 합니다. 다음 두 개의 `CStringT` 개체와 ANSI 문자를 연결 합니다. 마지막 두 개는 `CStringT` 개체와 유니코드 문자를 연결 합니다.
...
`Replace`의 두 가지 버전이 있습니다. 첫 번째 버전은 다른 부분 문자열을 사용 하 여 부분 문자열의 복사본을 하나 이상 대체 합니다. 두 부분 문자열이 모두 null로 종결 됩니다. 두 번째 버전은 다른 문자를 사용 하 여 하나 이상의 문자 복사본을 대체 합니다. 두 버전 모두 `CStringT`에 저장 된 문자 데이터에 대해 작동 합니다.
...
*pszNew* 와 *pszOld* 의 길이는 같을 필요가 없으며 이전 부분 문자열의 복사본을 여러 개 새 항목으로 변경할 수 있기 때문에 `Replace`는 문자열 길이를 변경할 수 있습니다. 함수는 대/소문자를 구분 하는 일치를 수행 합니다.

`CStringT` 인스턴스의 예는 `CString`, `CStringA`및 `CStringW`입니다.

`CStringA`의 경우 `Replace`은 MBCS (ANSI 또는 멀티 바이트) 문자를 사용 합니다. `CStringW`의 경우 `Replace`는 와이드 문자를 사용 합니다.

`CString`의 경우 문자 데이터 형식은 다음 표의 상수가 정의 되었는지 여부에 따라 컴파일 타임에 선택 됩니다.
...
*N 수가* 문자열 길이를 초과 하면 전체 문자열이 추출 됩니다. `Right` 기본 `Right` 함수와 유사 합니다. 단, Basic의 인덱스는 0부터 시작 합니다.
...
*Pstr* 가 가리키는 BSTR을 다시 할당 하 고 NULL 문자를 포함 하 여 `CStringT` 개체의 내용을 복사 합니다.
...
`CStringT` 개체의 내용에 따라 *pstr* 에서 참조 하는 BSTR의 값이 변경 될 수 있습니다. 메모리가 부족 한 경우 함수는 `CMemoryException`을 throw 합니다.
...
`SpanExcluding`는 *pszCharSet* 에서 첫 번째 문자 앞에 나오는 모든 문자를 추출 하 고 반환 합니다. 즉, *pszCharSet* 의 문자와 문자열에서 뒤에 오는 모든 문자는 반환 되지 않습니다. *PszCharSet* 의 문자를 문자열에서 찾을 수 없는 경우에는 `SpanExcluding` 전체 문자열을 반환 합니다.
...
문자열의 첫 문자가 문자 집합에 없으면 `SpanIncluding` 빈 문자열을 반환 합니다. 그렇지 않으면 집합에 있는 일련의 연속 문자를 반환 합니다.
...
`Tokenize` 함수는 대상 문자열에서 다음 토큰을 찾습니다. *PszTokens* 의 문자 집합은 찾을 토큰의 가능한 구분 기호를 지정 합니다. `Tokenize`에 대 한 각 호출에서 함수는 *Istart*에서 시작 하 고 선행 구분 기호를 건너뛰고 다음 구분 기호 문자 까지의 문자 문자열인 현재 토큰을 포함 하는 `CStringT` 개체를 반환 합니다. *Istart* 의 값은 끝 구분 기호 문자 다음의 위치로 업데이트 되거나 문자열의 끝에 도달한 경우-1로 업데이트 됩니다. Istart를 사용 하 여 문자열에서 다음 토큰을 읽을 위치를 추적 하기 위해 *Istart* 를 사용 하 여 `Tokenize`에 대 한 일련의 호출로 대상 문자열의 나머지 부분에서 더 많은 토큰을 분할할 수 있습니다. 토큰이 더 이상 없는 경우 함수는 빈 문자열을 반환 하 고 *Istart* 는-1로 설정 됩니다.

Strtok_s와 같은 CRT 토큰화 함수와 달리 [_strtok_s_l, wcstok_s, _wcstok_s_l, _mbstok_s, _mbstok_s_l](../../c-runtime-library/reference/strtok-s-strtok-s-l-wcstok-s-wcstok-s-l-mbstok-s-mbstok-s-l.md), `Tokenize`는 대상 문자열을 수정 하지 않습니다.
...
`CStringT& TrimRight(XCHAR chTarget)` 버전은 하나의 문자 매개 변수를 허용 하 고 `CStringT` 문자열 데이터의 끝에서 해당 문자의 모든 복사본을 제거 합니다. 문자열의 끝부터 시작 하 여 앞으로 작동 합니다. 다른 문자를 찾거나 `CSTringT` 문자 데이터가 부족할 때 중지 됩니다.

`CStringT& TrimRight(PCXSTR pszTargets)` 버전은 검색할 다른 모든 문자를 포함 하는 null로 끝나는 문자열을 허용 합니다. `CStringT` 개체의 해당 문자에 대 한 모든 복사본을 제거 합니다. 문자열의 끝에서 시작 하 여 앞으로 작동 합니다. 대상 문자열에 없는 문자를 찾거나 `CStringT` 문자 데이터가 부족할 때 중지 됩니다. 전체 대상 문자열을 `CStringT`끝 부분에 있는 부분 문자열과 일치 시 키 지 않습니다.

`CStringT& TrimRight()` 버전에는 매개 변수가 필요 하지 않습니다. `CStringT` 문자열의 끝에서 후행 공백 문자를 모두 자릅니다. 공백 문자는 줄 바꿈, 공백 또는 탭 일 수 있습니다.
