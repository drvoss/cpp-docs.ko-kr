...
ms.locfileid: "82032319"
---
# <a name="collections-ccx"></a>컬렉션(C++/CX)

C++/CX 프로그램에서는 STL(표준 템플릿 라이브러리) 컨테이너 또는 기타 사용자 정의 컬렉션 유형을 자유롭게 사용할 수 있습니다. 그러나 ABI(Windows 런타임 응용 프로그램 바이너리 인터페이스)를 통해 컬렉션을 XAML 컨트롤또는 JavaScript 클라이언트에 전달하면 Windows 런타임 컬렉션 형식을 사용해야 합니다.

Windows 런타임은 컬렉션 및 관련 형식에 대한 인터페이스를 정의하고 C++/CX는 collection.h 헤더 파일에서 구체적인 C++ 구현을 제공합니다. 다음 그림에서는 컬렉션 형식 간의 관계를 보여 줍니다.

![컬렉션 유형에 대한 C&#43;&#43;&#47;CX 상속 트리](../cppcx/media/cppcxcollectionsinheritancetree.png "컬렉션 유형에 대한 C&#43;&#43;&#47;CX 상속 트리")
...
- C++/CX 컬렉션 유형은 STL 컨테이너가 지원하는 동일한 나사 안전을 지원합니다.
...
클래스가 시퀀스 컨테이너를 다른 Windows 런타임 구성 요소로 전달해야 하는 경우 [Windows::Foundation::Collection:: IVector\<T>](/uwp/api/windows.foundation.collections.ivector-1) 매개 변수 또는 반환 유형으로 사용하고 [플랫폼::컬렉션::벡터\<T>](../cppcx/platform-collections-vector-class.md) 구체적인 구현으로 사용합니다. 공용 반환 값 또는 매개 변수에서 `Vector` 형식을 사용하려고 하면 컴파일러 오류 C3986이 발생합니다. `Vector` 를 `IVector`로 변경하여 이 오류를 해결할 수 있습니다.

> [!IMPORTANT]
> 자체 프로그램 안에서 시퀀스를 전달하는 경우에는 `Vector` 보다 효율적인 `std::vector` 또는 `IVector`를 사용합니다. ABI 너머로 컨테이너를 전달할 때만 `IVector` 를 사용합니다.
>
> Windows 런타임 형식 시스템은 들쭉날쭉한 배열의 개념을 지원하지 않으므로 IVector<\<플랫폼::배열 T>> 반환 값 또는 메서드 매개 변수로 전달할 수 없습니다. ABI 전반에서 가변 배열 또는 시퀀스의 시퀀스를 전달하려면 `IVector<IVector<T>^>`를 사용합니다.
...
Windows 런타임 구성 `std::vector` 요소에서 이를 다시 사용하려는 기존 코드가 있는 경우 반복기 한 쌍을 `Vector` `std::vector` 사용하는 생성자 중 하나를 사용하여 `Vector` ABI를 통해 컬렉션을 전달하는 지점에서 를 생성합니다. 다음 예제에서는 효율적인 초기화를 위해 `Vector` 에서 `std::vector`이동 생성자를 사용하는 방법을 보여 줍니다. 이동 작업 후 원래 `vec` 변수는 더 이상 유효하지 않습니다.
...
[플랫폼:컬렉션::벡터이터](../cppcx/platform-collections-vectoriterator-class.md) 및 [플랫폼::컬렉션::VectorViewIterator](../cppcx/platform-collections-vectorviewiterator-class.md) 는 [IVector\<](/uwp/api/windows.foundation.collections.ivector-1) T `range for`>컨테이너를 사용하여 [std::sort와](../standard-library/algorithm-functions.md#sort) 같은 루프 및 알고리즘을 사용할 수 있습니다. 그러나 `IVector` 요소는 C++ 포인터 역참조를 통해 액세스할 수 없습니다. 이 요소는 [GetAt](/uwp/api/windows.foundation.collections.ivector-1.getat) 및 [SetAt](/uwp/api/windows.foundation.collections.ivector-1.setat) 메서드를 통해서만 액세스할 수 있습니다. 따라서 이러한 이터레이터는 프록시 `Platform::Details::VectorProxy<T>` `Platform::Details::ArrowProxy<T>` 클래스를 사용하고 표준 라이브러리에서 __->__ 요구하는 대로 . __ \[및]__ 연산자를 통해 __\*__ 개별 요소에 대한 액세스를 제공합니다. 엄밀히 말해 `IVector<Person^> vec`지정 시 `*begin(vec)` 의 형식은 `VectorProxy<Person^>`입니다. 그러나 프록시 개체는 사용자 코드에 거의 표시되지 않습니다. 이러한 프록시 개체는 내부적으로 반복기에만 사용되므로 문서화되지 않지만 메커니즘의 작동 방식을 알면 유용합니다.
...
![루프에 기반한 범위&#45;VectorProxy](../cppcx/media/vectorproxy-1.png "루프에 기반한 범위&#45;VectorProxy")
...
컬렉션은 네 가지 범주로 구분됩니다(수정 가능한 버전 및 읽기 전용 버전의 시퀀스 컬렉션과 연결 컬렉션). 또한 C++/CX는 컬렉션 액세스를 단순화하는 세 가지 이터레이터 클래스를 제공하여 컬렉션을 향상시킵니다.

수정 가능한 컬렉션의 요소는 변경할 수 있지만 읽기 전용 컬렉션( *뷰*라고 함)의 요소는 읽을 수만 있습니다. [플랫폼의 요소:::컬렉션::벡터](../cppcx/platform-collections-vector-class.md) 또는[플랫폼::컬렉션::VectorView:VectorView](../cppcx/platform-collections-vectorview-class.md) 컬렉션은 이터레이터 또는 컬렉션의 [Vector::GetAt](../cppcx/platform-collections-vector-class.md#getat) 및 인덱스를 사용하여 액세스할 수 있습니다. 컬렉션의 [Map::Lookup](../cppcx/platform-collections-map-class.md#lookup) 및 키를 사용하여 연관 컬렉션의 요소에 액세스할 수 있습니다.
...
`Map` 및 `MapView` 는 `<K, V, C = std::less<K>>`템플릿 기반이므로 비교 연산자를 사용자 지정할 수 있습니다.  또한 `Vector` 및 `VectorView` 는 `<T, E = std::equal_to<T>>` 템플릿 기반이며 `IndexOf()`의 동작을 사용자 지정할 수 있습니다. 이는 주로 값 구조체의 `Vector` 및 `VectorView` 에 중요합니다. 예를 들어 벡터\<Windows::Foundation::DateTime> 만들려면 DateTime이 == 연산자에 과부하를 일으키지 않으므로 사용자 지정 비교기를 제공해야 합니다.
...
[플랫폼::컬렉션::입력이터 클래스](../cppcx/platform-collections-inputiterator-class.md)<br/>
STL 입력 반복기의 요구 사항을 충족하는 STL 반복기입니다.

[플랫폼::컬렉션::벡터이터 클래스](../cppcx/platform-collections-vectoriterator-class.md)<br/>
STL 변경 가능 임의 액세스 반복기의 요구 사항을 충족하는 STL 반복기입니다.

[플랫폼::컬렉션::벡터뷰이터 클래스](../cppcx/platform-collections-vectorviewiterator-class.md)<br/>
...
|반복기|Functions|
|---------------|---------------|
|[Platform::Collections::VectorIterator\<T>](../cppcx/platform-collections-vectoriterator-class.md)<br /><br /> (내부적으로 [윈도우를 저장:::파운데이션::컬렉션:: IVector\<T>](/uwp/api/windows.foundation.collections.ivector-1) int.)|[시작](../cppcx/begin-function.md)/ [끝](../cppcx/end-function.md)[(창::파운데이션::컬렉션:: IVector\<T>) ](/uwp/api/windows.foundation.collections.ivector-1)|
|[Platform::Collections::VectorViewIterator\<T>](../cppcx/platform-collections-vectorviewiterator-class.md)<br /><br /> (내부적으로 [IVectorView\<T>](/uwp/api/windows.foundation.collections.ivectorview-1)^ 및 int.)|[시작](../cppcx/begin-function.md)/ [\<끝(IVectorView T>](/uwp/api/windows.foundation.collections.ivectorview-1)^)[end](../cppcx/end-function.md)|
|[Platform::Collections::InputIterator\<T>](../cppcx/platform-collections-inputiterator-class.md)<br /><br /> (내부적으로 [는\<IIterator T>](/uwp/api/windows.foundation.collections.iiterator-1)^ 및 T.)|[시작](../cppcx/begin-function.md)/ [끝](../cppcx/end-function.md) [(이터\<가능 T>) ](/uwp/api/windows.foundation.collections.iiterable-1)|
|[Platform::Collections::InputIterator<IKeyValuePair\<K, V>^>](../cppcx/platform-collections-inputiterator-class.md)<br /><br /> (내부적으로 [는\<IIterator T>](/uwp/api/windows.foundation.collections.iiterator-1)^ 및 T.)|[시작](../cppcx/begin-function.md)/ [끝](../cppcx/end-function.md) [(IMap\<K, V>](/uwp/api/windows.foundation.collections.imap-2).|
|[Platform::Collections::InputIterator<IKeyValuePair\<K, V>^>](../cppcx/platform-collections-inputiterator-class.md)<br /><br /> (내부적으로 [는\<IIterator T>](/uwp/api/windows.foundation.collections.iiterator-1)^ 및 T.)|[시작](../cppcx/begin-function.md)/ [끝](../cppcx/end-function.md) [(창::파운데이션::컬렉션::IMapView)](/uwp/api/windows.foundation.collections.imapview-2)|
