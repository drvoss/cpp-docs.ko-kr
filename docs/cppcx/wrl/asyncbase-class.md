...
`Cancel()`은 `IAsyncInfo::Cancel`의 기본 구현 이며 실제 작업을 수행 하지 않습니다. 실제로 비동기 작업을 취소 하려면 `OnCancel()` 순수 가상 메서드를 재정의 합니다.
...
`Close()`은 `IAsyncInfo::Close`의 기본 구현 이며 실제 작업을 수행 하지 않습니다. 실제로 비동기 작업을 닫으려면 `OnClose()` 순수 가상 메서드를 재정의 합니다.
...
비동기 작업의 현재 상태가 *시작*됨 인 경우 **true** 입니다. 즉, 작업을 계속 해야 합니다. 그렇지 않으면 **false**입니다 .이는 작업이 중단 되어야 함을 의미 합니다.
...
`FireCompletion()`의 첫 번째 버전은 내부 진행 대리자 변수를 다시 설정 합니다. 비동기 작업이 완료 되 면 두 번째 버전은 완료 이벤트 처리기를 호출 합니다.
...
*status*<br/>
상태를 저장할 위치입니다. 자세한 내용은 `Windows::Foundation::AsyncStatus` 열거를 참조 하세요.
...
`Start()`은 호출자에 게 반환 되기 전에 비동기 작업 "핫 시작" 때문에 외부적으로 표시 되지 않는 보호 된 메서드입니다.
...
비동기 작업이 완료 되었으면 **true** 이 고, 그렇지 않으면입니다. 그렇지 않으면 **false**입니다.
...
내부 오류 상태가 변경 되었으면 **true** 이 고, 그렇지 않으면입니다. 그렇지 않으면 **false**입니다.
