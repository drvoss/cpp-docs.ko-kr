...
Windows 런타임는 [Hstring](/windows/win32/WinRT/hstring) 핸들을 통해 문자열에 대 한 액세스를 제공 합니다. `HString` 클래스는 HSTRING 핸들 사용을 간소화 하기 위한 편리한 함수와 연산자를 제공 합니다. 이 클래스는 RAII 패턴을 통해 소유한 HSTRING의 수명을 처리할 수 있습니다.
...
속성                                     | Description
---------------------------------------- | -------------------------------------------------------------------------------------------------------------
[HString:: Attach](#attach)               | 지정 된 `HString` 개체를 현재 `HString` 개체와 연결 합니다.
[HString:: CopyTo](#copyto)               | 현재 `HString` 개체를 HSTRING 개체에 복사 합니다.
...
[HString:: MakeReference](#makereference) | 지정 된 문자열 매개 변수에서 `HStringReference` 개체를 만듭니다.
...
지정 된 `HString` 개체를 현재 `HString` 개체와 연결 합니다.
...
현재 `HString` 개체를 HSTRING 개체에 복사 합니다.
...
분리 작업이 시작 되기 전의 내부 `HString` 값입니다.
...
두 번째 생성자는 새 `HString` 개체를 기존 *다른* 매개 변수의 값으로 초기화 한 다음 *다른* 매개 변수를 소멸 시킵니다.
...
현재 `HString` 개체가 비어 있지 않으면 **true** 이 고, 그렇지 않으면입니다. 그렇지 않으면 **false**입니다.
...
지정 된 *str* 매개 변수와 동일한 값을 갖는 `HStringReference` 개체입니다.
...
기존 *다른* 개체의 값이 현재 `HString` 개체에 복사 된 다음 *다른* 개체가 제거 됩니다.
...
*lhs*<br/>
비교할 첫 번째 매개 변수입니다. *lhs* 는 `HString` 또는 `HStringReference` 개체나 hstring 핸들 일 수 있습니다.

*rhs*<br/>
비교할 두 번째 매개 변수입니다. *rhs* 은 `HString` 또는 `HStringReference` 개체나 hstring 핸들 일 수 있습니다.
...
*lhs*<br/>
비교할 첫 번째 매개 변수입니다. *lhs* 는 `HString`에 대 한 참조일 수 있습니다.

*rhs*<br/>
비교할 두 번째 매개 변수입니다. *rhs* 는 `HString`에 대 한 참조일 수 있습니다.
...
*len*<br/>
현재 `HString` 개체에 할당 되는 *str* 매개 변수의 최대 길이입니다.
