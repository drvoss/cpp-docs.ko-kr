...
*T*<br/>
`ComPtr` 나타내는 인터페이스입니다.

*U*<br/>
현재 `ComPtr` friend 인 클래스입니다. 이 매개 변수를 사용하는 템플릿은 보호됩니다.

## <a name="remarks"></a>주의

`ComPtr<>`은 기본 인터페이스 포인터를 나타내는 형식을 선언 합니다. `ComPtr<>`를 사용 하 여 변수를 선언한 다음 화살표 멤버 액세스 연산자 (`->`)를 사용 하 여 인터페이스 멤버 함수에 액세스 합니다.
...
[ComPtr:: ~ ComPtr](#tilde-comptr) | `ComPtr`의 인스턴스를 초기화 하지 않습니다.

### <a name="public-methods"></a>공용 방법

이름                                                      | 설명
--------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
[ComPtr:: As](#as)                                         | 지정 된 템플릿 매개 변수로 식별 되는 인터페이스를 나타내는 `ComPtr` 개체를 반환 합니다.
[ComPtr:: AsIID](#asiid)                                   | 지정 된 인터페이스 ID로 식별 되는 인터페이스를 나타내는 `ComPtr` 개체를 반환 합니다.
[ComPtr:: AsWeak](#asweak)                                 | 현재 개체에 대한 약한 참조를 검색합니다.
[ComPtr:: Attach](#attach)                                 | 이 `ComPtr`를 현재 템플릿 형식 매개 변수에 지정 된 인터페이스 형식과 연결 합니다.
...
[ComPtr:: GetAddressOf](#getaddressof)                     | 이 `ComPtr`나타내는 인터페이스에 대 한 포인터를 포함 하는 [ptr_](#ptr) 데이터 멤버의 주소를 검색 합니다.
[ComPtr:: ReleaseAndGetAddressOf](#releaseandgetaddressof) | 이 `ComPtr` 연결 된 인터페이스를 해제 한 다음 해제 된 인터페이스에 대 한 포인터를 포함 하는 [ptr_](#ptr) 데이터 멤버의 주소를 검색 합니다.
[ComPtr::Reset](#reset)                                   | 이 `ComPtr`와 연결 된 인터페이스에 대 한 포인터의 모든 참조를 해제 합니다.
[ComPtr:: Swap](#swap)                                     | 현재 `ComPtr`에서 관리 하는 인터페이스를 지정 된 `ComPtr`에서 관리 하는 인터페이스와 교환 합니다.
...
[ComPtr:: operator Microsoft:: WRL::D etails:: BoolType](#operator-microsoft-wrl-details-booltype) | `ComPtr` 인터페이스의 개체 수명을 관리 하 고 있는지 여부를 나타냅니다.

### <a name="protected-data-members"></a>보호된 데이터 멤버

이름                 | 설명
-------------------- | ------------------------------------------------------------------------------------------
[ComPtr::p tr_](#ptr) | 이 `ComPtr`와 연결 되 고 관리 되는 인터페이스에 대 한 포인터를 포함 합니다.
...
지정 된 템플릿 매개 변수로 식별 되는 인터페이스를 나타내는 `ComPtr` 개체를 반환 합니다.
...
*p*<br/>
매개 변수 *U*로 지정 된 인터페이스를 나타내는 `ComPtr` 개체입니다. *P* 매개 변수는 현재 `ComPtr` 개체를 참조 하지 않아야 합니다.
...
지정 된 인터페이스 ID로 식별 되는 인터페이스를 나타내는 `ComPtr` 개체를 반환 합니다.
...
*p*<br/>
개체에 ID가 *riid*인 인터페이스가 있는 경우에는 *riid* 매개 변수로 지정 된 인터페이스에 대 한 이중 간접 포인터입니다. 그렇지 않으면 `IUnknown`에 대 한 포인터입니다.
...
이 `ComPtr`를 현재 템플릿 형식 매개 변수에 지정 된 인터페이스 형식과 연결 합니다.
...
성공 하면 S_OK 합니다. 그렇지 않으면 암시적 `QueryInterface` 작업이 실패 한 이유를 나타내는 HRESULT입니다.

### <a name="remarks"></a>주의

첫 번째 함수는이 `ComPtr`연결 된 인터페이스에 대 한 포인터의 복사본을 반환 합니다. 이 함수는 항상 S_OK 반환 합니다.

두 번째 함수는 *riid* 매개 변수로 지정 된 인터페이스에 대해이 `ComPtr` 연결 된 인터페이스에서 `QueryInterface` 작업을 수행 합니다.

세 번째 함수는이 `ComPtr`와 연결 된 인터페이스에서 *U* 매개 변수의 기본 인터페이스에 대 한 `QueryInterface` 작업을 수행 합니다.
...
이 `ComPtr`나타내는 인터페이스에 대 한 포인터를 포함 하는 [ptr_](#ptr) 데이터 멤버의 주소를 검색 합니다.
...
이 메서드는 인터페이스 포인터에 대 한 참조를 해제 하는 [ComPtr:: GetAddressOf](#getaddressof) 와는 다릅니다. 인터페이스 포인터의 주소가 필요 하지만 해당 인터페이스를 해제 하지 않으려는 경우 `ComPtr::GetAddressOf`를 사용 합니다.
...
이 도우미 함수는 STDMETHOD 매크로를 사용 하 여 발생 하는 불필요 한 오버 헤드를 제거 합니다. 이 함수를 사용 하면 `virtual`대신 `IUnknown` 형식을 `private` 합니다.
...
*other*<br/>
형식 또는 다른 `ComPtr`에 대 한 포인터, 참조 또는 rvalue 참조입니다.
...
이 연산자의 첫 번째 버전은 현재 `ComPtr`에 빈 값을 할당 합니다.

두 번째 버전에서는 할당 인터페이스 포인터가 현재 `ComPtr` 인터페이스 포인터와 같지 않은 경우 두 번째 인터페이스 포인터가 현재 `ComPtr`에 할당 됩니다.

세 번째 버전에서는 인터페이스 지정 포인터가 현재 `ComPtr`에 할당 됩니다.

네 번째 버전에서 할당 값의 인터페이스 포인터가 현재 `ComPtr` 인터페이스 포인터와 같지 않은 경우 두 번째 인터페이스 포인터가 현재 `ComPtr`에 할당 됩니다.

다섯 번째 버전은 복사 연산자입니다. `ComPtr`에 대 한 참조가 현재 `ComPtr`에 할당 됩니다.

여섯 번째 버전은 이동 의미 체계를 사용 하는 복사 연산자입니다. 정적 캐스트 형식이 고 현재 `ComPtr`에 할당 된 경우 `ComPtr`에 대 한 rvalue 참조입니다.

일곱 번째 버전은 이동 의미 체계를 사용 하는 복사 연산자입니다. *U* 형식의 `ComPtr`에 대 한 rvalue 참조는 정적 캐스팅 된 후 현재 `ComPtr`에 할당 됩니다.
...
첫 번째 연산자는 개체 *a* 가 개체 *b*와 같은 경우 `true`를 생성 합니다. 그렇지 않으면 `false`합니다.

두 번째 및 세 번째 연산자는 개체 *a* 가 `nullptr`와 같으면 `true`를 생성 합니다. 그렇지 않으면 `false`합니다.
...
첫 번째 연산자는 개체 *a* 가 개체 *b*와 같지 않은 경우 `true`를 생성 합니다. 그렇지 않으면 `false`합니다.

두 번째 및 세 번째 연산자는 개체 *a* 가 `nullptr`와 같지 않은 경우 `true`를 생성 합니다. 그렇지 않으면 `false`합니다.
...
인터페이스가이 `ComPtr`와 연결 된 경우 [BoolStruct:: Member](boolstruct-structure.md#member) 데이터 멤버의 주소를 사용 합니다. 그렇지 않으면 `nullptr`합니다.

## <a name="comptrptr_"></a><a name="ptr"></a>ComPtr::p tr_

이 `ComPtr`와 연결 되 고 관리 되는 인터페이스에 대 한 포인터를 포함 합니다.
...
이 `ComPtr` 연결 된 인터페이스를 해제 한 다음 해제 된 인터페이스에 대 한 포인터를 포함 하는 [ptr_](#ptr) 데이터 멤버의 주소를 검색 합니다.
...
현재 `ComPtr`에서 관리 하는 인터페이스를 지정 된 `ComPtr`에서 관리 하는 인터페이스와 교환 합니다.
