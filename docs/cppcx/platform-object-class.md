...
Windows [:: UI:: Xaml:: Interop:: TypeName](/uwp/api/windows.ui.xaml.interop.typename) 클래스는 windows api에서 언어와 관계 없이 windows 구성 요소와 앱 간에 형식 정보를 전달 하는 방법으로 사용 됩니다. T[Platform:: Type 클래스](../cppcx/platform-type-class.md) 에 `Type`와 `TypeName`사이를 변환 하는 연산자가 있습니다.

[Typeid](../extensions/typeid-cpp-component-extensions.md) 연산자를 사용 하 여 클래스 이름에 대 한 `Platform::Type` 개체를 반환 합니다. 예를 들어 XAML 페이지 간을 탐색할 수 있습니다.
