...
[GdiplusStartupInput](/windows/win32/api/gdiplusinit/ns-gdiplusinit-gdiplusstartupinput) 구조의 `SuppressBackgroundThread` 멤버를 **TRUE**로 설정 하 여이 작업을 수행할 수 있습니다. GDI + 백그라운드 스레드를 표시 하지 않는 경우에는 응용 프로그램의 메시지 루프를 입력 하 고 종료 하기 직전에 `NotificationHook` 및 `NotificationUnhook` 호출을 수행 해야 합니다. 이러한 호출에 대 한 자세한 내용은 [GdiplusStartupOutput](/windows/win32/api/gdiplusinit/ns-gdiplusinit-gdiplusstartupoutput)를 참조 하세요. 따라서 `GdiplusStartup` 호출 하 고 후크 알림 함수는 아래와 같이 가상 함수 [CWinApp:: Run](../mfc/reference/cwinapp-class.md#run)을 재정의 하는 것이 좋습니다.
...
응용 프로그램에서 파일 끌어서 놓기를 사용 하도록 설정 하기 위해 MFC 응용 프로그램 마법사는 `InitInstance`에서 주 프레임 창의 [CWnd](../mfc/reference/cwnd-class.md) 멤버 함수 [dragacceptfiles](../mfc/reference/cwnd-class.md#dragacceptfiles) 에 대 한 호출을 작성 합니다. 끌어서 놓기 기능을 구현할 필요가 없으면 이 호출을 제거할 수 있습니다.
