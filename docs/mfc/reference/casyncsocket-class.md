...
|[CAsyncSocket::D etach](#detach)|`CAsyncSocket` 개체에서 소켓 핸들을 분리 합니다.|
|[CAsyncSocket:: FromHandle](#fromhandle)|지정 된 소켓 핸들을 지정 하 여 `CAsyncSocket` 개체에 대 한 포인터를 반환 합니다.|
...
|[CAsyncSocket:: 수신 대기](#listen)|들어오는 연결 요청을 수신 하는 소켓을 설정 합니다.|
...
|[CAsyncSocket:: ShutDown](#shutdown)|소켓에서 `Send` 및/또는 `Receive` 호출을 사용 하지 않도록 설정 합니다.|
|[CASyncSocket:: 소켓](#socket)|소켓 핸들을 할당 합니다.|

### <a name="protected-methods"></a>Protected 메서드

|속성|Description|
|----------|-----------------|
|[CAsyncSocket:: OnAccept](#onaccept)|`Accept`를 호출 하 여 보류 중인 연결 요청을 허용할 수 있음을 수신 소켓에 알립니다.|
...
|[CAsyncSocket:: OnReceive](#onreceive)|`Receive`를 호출 하 여 검색할 데이터가 있음을 수신 소켓에 알립니다.|
|[CAsyncSocket:: OnSend](#onsend)|`Send`를 호출 하 여 데이터를 보낼 수 있음을 소켓에 알립니다.|
...
`CAsyncSocket` 개체를 사용 하려면 해당 생성자를 호출한 다음 [create](#create) 함수를 호출 하 여 허용 되는 소켓을 제외 하 고 기본 소켓 핸들 (`SOCKET`형식)을 만듭니다. 서버 소켓에서 [Listen](#listen) 멤버 함수를 호출 하 고, 클라이언트 소켓이 [Connect](#connect) 멤버 함수를 호출 합니다. 서버 소켓은 연결 요청을 받을 때 [Accept](#accept) 함수를 호출 해야 합니다. 나머지 `CAsyncSocket` 함수를 사용 하 여 소켓 간 통신을 수행할 수 있습니다. 완료 되 면 힙에 생성 된 `CAsyncSocket` 개체를 삭제 합니다. 소멸자는 [Close](#close) 함수를 자동으로 호출 합니다. 소켓 데이터 형식은 [Windows 소켓: 배경](../../mfc/windows-sockets-background.md)문서에 설명 되어 있습니다.
...
자세한 내용은 [Windows 소켓: CAsyncSocket](../../mfc/windows-sockets-using-class-casyncsocket.md) 및 관련 문서 사용 및 [WINDOWS 소켓 2 API](/windows/win32/WinSock/windows-sockets-start-page-2)를 참조 하세요.
...
이 루틴은 보류 중인 연결의 큐에 있는 첫 번째 연결을 추출 하 고,이 소켓과 동일한 속성을 사용 하 여 새 소켓을 만들고, *rConnectedSocket*에 연결 합니다. 큐에 보류 중인 연결이 없으면 `Accept` 0을 반환 하 고 `GetLastError`에서 오류를 반환 합니다. 허용 되는 소켓 ( *rConnectedSocket)* 을 사용 하 여 더 많은 연결을 허용할 수 없습니다. 원래 소켓은 열려 있고 수신 대기 상태로 유지 됩니다.
...
- 읽기 준비에 대 한 알림을 수신 하 고 FD_READ 합니다.

- 데이터를 읽을 수 있는 경우 알림을 받으려는 FD_WRITE 합니다.

- FD_OOB 대역 외 데이터 도착에 대 한 알림을 받을 수 있습니다.

- 들어오는 연결에 대 한 알림을 수신 하 고 FD_ACCEPT 합니다.

- FD_CONNECT 연결 결과에 대 한 알림을 받을 수 있습니다.
...
이 함수는 소켓에 대해 호출 되는 MFC 콜백 알림 함수를 지정 하는 데 사용 됩니다. `AsyncSelect`는이 소켓을 비블로킹 모드로 자동 설정 합니다. 자세한 내용은 [Windows 소켓: 소켓 알림](../../mfc/windows-sockets-socket-notifications.md)문서를 참조 하세요.
...
네트워크 주소 이며, "128.56.22.8"와 같은 점으로 표시 되는 숫자입니다. 이 매개 변수에 대 한 NULL 문자열을 전달 하면 `CAsyncSocket` 인스턴스가 모든 네트워크 인터페이스에서 클라이언트 작업을 수신 대기 해야 함을 나타냅니다.
...
- WSAEADDRINUSE 지정 된 주소가 이미 사용 중입니다. [SetSockOpt](#setsockopt)의 SO_REUSEADDR 소켓 옵션을 참조 하십시오.
...
이 루틴은 후속 `Connect` 또는 `Listen` 호출 하기 전에 연결 되지 않은 데이터 그램 또는 스트림 소켓에서 사용 됩니다. 수신 대기 서버 소켓은 연결 요청을 수락 하기 전에 포트 번호를 선택 하 고 `Bind`를 호출 하 여 Windows 소켓에 알려 두어야 합니다. `Bind`는 명명 되지 않은 소켓에 로컬 이름을 할당 하 여 소켓의 로컬 연결 (호스트 주소/포트 번호)을 설정 합니다.
...
개체를 생성 한 후에는 해당 `Create` 멤버 함수를 호출 하 여 소켓 데이터 구조를 만들고 해당 주소를 바인딩해야 합니다. Windows 소켓 통신의 서버 쪽에서 수신 대기 소켓이 `Accept` 호출에 사용할 소켓을 만드는 경우 해당 소켓에 대해 `Create`를 호출 하지 않습니다.)
...
이 함수는 소켓 설명자를 해제 하므로 WSAENOTSOCK 오류와 함께 실패 하는 추가 참조가 실패 합니다. 기본 소켓에 대 한 마지막 참조 인 경우 연결 된 명명 정보 및 대기 중인 데이터가 삭제 됩니다. 소켓 개체의 소멸자가 `Close`를 호출 합니다.

`CSocket`에 대 한 `CAsyncSocket`아니라 `Close`의 의미 체계는 SO_LINGER 및 SO_DONTLINGER 소켓 옵션의 영향을 받습니다. 자세한 내용은 멤버 함수 `GetSockOpt`를 참조 하세요.
...
함수가 성공 하면 0이 아닌 값입니다. 그렇지 않으면 0이 고, [GetLastError](#getlasterror)를 호출 하 여 특정 오류 코드를 검색할 수 있습니다. 이로 인해 WSAEWOULDBLOCK 오류 코드가 표시 되 고 응용 프로그램에서 재정의 가능한 콜백을 사용 하는 경우 연결 작업이 완료 되 면 응용 프로그램에서 `OnConnect` 메시지를 받게 됩니다. 이 멤버 함수에는 다음 오류가 적용 됩니다.
...
소켓이 바인딩되지 않은 경우 고유 값은 시스템에 의해 로컬 연결에 할당 되 고 소켓이 bound로 표시 됩니다. 이름 구조체의 주소 필드가 모두 0 이면 `Connect`는 0을 반환 합니다. 확장 오류 정보를 가져오려면 `GetLastError` 멤버 함수를 호출 합니다.

스트림 소켓 (유형 SOCK_STREAM)의 경우 활성 연결이 외래 호스트로 시작 됩니다. 소켓 호출이 성공적으로 완료 되 면 소켓이 데이터를 보내고 받을 준비가 된 것입니다.

데이터 그램 소켓 (유형 SOCK_DGRAM)의 경우 기본 대상은 set 이며 후속 `Send` 및 `Receive` 호출에 사용 됩니다.

##  <a name="create"></a>CAsyncSocket:: Create

소켓 개체를 생성 한 후 `Create` 멤버 함수를 호출 하 여 Windows 소켓을 만들고 연결 합니다.
...
- 쓰기 준비에 대 한 알림을 수신 하 고 FD_WRITE 합니다.

- FD_OOB 대역 외 데이터 도착에 대 한 알림을 받을 수 있습니다.

- 들어오는 연결에 대 한 알림을 수신 하 고 FD_ACCEPT 합니다.

- 완료 된 연결에 대 한 알림을 수신 하려고 FD_CONNECT.
...
- SOCK_STREAM은 시퀀스 되 고 신뢰할 수 있는 전이중 연결 기반 바이트 스트림을 제공 합니다. 인터넷 주소 제품군에 대해 TCP (전송 제어 프로토콜)를 사용 합니다.

- SOCK_DGRAM은 고정 (일반적으로 작은) 최대 길이에서 연결 되지 않은 신뢰할 수 없는 패킷 인 데이터 그램을 지원 합니다. 인터넷 주소 제품군에 대해 UDP (사용자 데이터 그램 프로토콜)를 사용 합니다.
...
> [!IMPORTANT]
> `Create`은 스레드로부터 안전 **하지 않습니다** .  서로 다른 스레드에서 동시에 호출 될 수 있는 다중 스레드 환경에서 호출 하는 경우 뮤텍스 또는 다른 동기화 잠금을 사용 하 여 각 호출을 보호 해야 합니다.

스트림 및 데이터 그램 소켓에 대 한 자세한 내용은 [Windows 소켓: 배경](../../mfc/windows-sockets-background.md) 및 [Windows 소켓: 포트 및 소켓 주소](../../mfc/windows-sockets-ports-and-socket-addresses.md) 와 [windows 소켓 2 API](/windows/win32/WinSock/windows-sockets-start-page-2)문서를 참조 하세요.

##  <a name="detach"></a>CAsyncSocket::D etach

`CAsyncSocket` 개체에서 *m_hSocket* 데이터 멤버의 소켓 핸들을 분리 하 고 *m_hSocket* 를 NULL로 설정 하려면이 멤버 함수를 호출 합니다.
...
`CAsyncSocket` 개체에 대 한 포인터 이거나 *Hsocket*에 연결 된 `CAsyncSocket` 개체가 없는 경우 NULL입니다.

### <a name="remarks"></a>설명

소켓 핸들을 지정 하면 `CAsyncSocket` 개체가 핸들에 연결 되지 않은 경우 멤버 함수는 NULL을 반환 합니다.
...
특정 멤버 함수에서 오류가 발생 했음을 나타내는 경우 `GetLastError`를 호출 하 여 적절 한 오류 코드를 검색 합니다. 해당 하는 오류 코드 목록은 개별 멤버 함수 설명을 참조 하십시오.
...
*rPeerAddress*<br/>
점으로 구분 된 숫자 IP 주소를 받는 `CString` 개체에 대 한 참조입니다.
...
- WSAEINVAL를 `Bind`사용 하 여 소켓이 주소에 바인딩되지 않았습니다.

### <a name="remarks"></a>설명

이 호출은 `Bind`를 먼저 수행 하지 않고 `Connect` 호출을 수행한 경우에 특히 유용 합니다. 이 호출은 시스템에 의해 설정 된 로컬 연결을 확인할 수 있는 유일한 수단을 제공 합니다.
...
*lpOptionValue*<br/>
요청 된 옵션의 값이 반환 될 버퍼에 대 한 포인터입니다. 선택한 옵션과 연결 된 값이 buffer *lpOptionValue*에서 반환 됩니다. *LpOptionLen* 가 가리키는 정수는 원래이 버퍼의 크기 (바이트)를 포함 해야 합니다. 반환 될 때 반환 되는 값의 크기로 설정 됩니다. SO_LINGER의 경우 `LINGER` 구조의 크기가 됩니다. 다른 모든 옵션의 경우 옵션에 따라 BOOL 또는 **int**의 크기가 됩니다. 설명 섹션에서 옵션 및 해당 크기의 목록을 참조 하세요.
...
함수가 성공 하면 0이 아닌 값입니다. 그렇지 않으면 0이 고, [GetLastError](#getlasterror)를 호출 하 여 특정 오류 코드를 검색할 수 있습니다. `SetSockOpt`를 사용 하 여 옵션을 설정 하지 않은 경우 `GetSockOpt`는 옵션에 대 한 기본값을 반환 합니다. 이 멤버 함수에는 다음 오류가 적용 됩니다.
...
- WSAENOPROTOOPT 옵션을 알 수 없거나 지원 되지 않습니다. 특히 SOCK_STREAM 형식의 소켓에서는 SO_BROADCAST 지원 되지 않지만, SO_ACCEPTCONN, SO_DONTLINGER, SO_KEEPALIVE, SO_LINGER 및 SO_OOBINLINE는 SOCK_DGRAM 형식의 소켓에서 지원 되지 않습니다.

- WSAENOTSOCK 설명자가 소켓이 아닙니다.

### <a name="remarks"></a>설명

`GetSockOpt`는 모든 형식의 소켓에 연결 된 소켓 옵션에 대 한 현재 값을 검색 하 고 그 결과를 *lpOptionValue*에 저장 합니다. 옵션은 패킷 라우팅, 대역 외 데이터 전송 등의 소켓 작업에 영향을 줍니다.

`GetSockOpt`에 대해 지원 되는 옵션은 다음과 같습니다. 형식은 *lpOptionValue*로 주소를 지정 하는 데이터 형식을 식별 합니다. TCP_NODELAY 옵션은 수준 IPPROTO_TCP를 사용 합니다. 다른 모든 옵션은 수준 SOL_SOCKET를 사용 합니다.
...
`GetSockOpt`에 대해 지원 되지 않는 BSD (Berkeley Software 배포판) 옵션은 다음과 같습니다.
...
지원 되지 않는 옵션을 사용 하 여 `GetSockOpt`를 호출 하면 `GetLastError`에서 반환 되는 오류 코드 WSAENOPROTOOPT이 발생 합니다.
...
- FIONBIO은 소켓에서 비블로킹 모드를 사용 하거나 사용 하지 않도록 설정 합니다. *LpArgument* 매개 변수는 `DWORD`에서 가리킵니다. 비블로킹 모드를 사용 하도록 설정 하는 경우 0이 아니고 사용 하지 않도록 설정 된 경우 0입니다. 소켓에서 `AsyncSelect`가 실행 된 경우 `IOCtl`를 사용 하 여 소켓을 차단 모드로 다시 설정 하려고 하면 WSAEINVAL이 발생 합니다. 소켓을 차단 모드로 설정 하 고 WSAEINVAL 오류를 방지 하려면 *lEvent* 매개 변수를 0으로 설정 하 여 `AsyncSelect`를 호출한 다음 `IOCtl`를 호출 하 여 응용 프로그램에서 먼저 `AsyncSelect`를 사용 하지 않도록 설정 해야 합니다.

- FIONREAD는이 소켓에서 하나의 `Receive` 호출로 읽을 수 있는 최대 바이트 수를 결정 합니다. *LpArgument* 매개 변수는 `IOCtl` 결과를 저장 하는 `DWORD`을 가리킵니다. 이 소켓이 SOCK_STREAM 형식이 면 FIONREAD는 단일 `Receive`읽을 수 있는 총 데이터 양을 반환 합니다. 이는 일반적으로 소켓에서 큐에 대기 중인 총 데이터 양과 동일 합니다. 이 소켓이 SOCK_DGRAM 형식이 면 FIONREAD는 소켓에서 대기 중인 첫 번째 데이터 그램의 크기를 반환 합니다.

- SIOCATMARK는 대역 외 데이터를 모두 읽 었는 지 여부를 확인 합니다. 이는 대역 외 데이터 (SO_OOBINLINE)의 인라인 수신에 대해 구성 된 SOCK_STREAM 형식의 소켓에만 적용 됩니다. 읽기 대기 중인 대역 외 데이터가 없으면 작업은 0이 아닌 값을 반환 합니다. 그렇지 않으면 0을 반환 하 고, 소켓에서 수행 되는 다음 `Receive` 또는 `ReceiveFrom`는 "mark" 앞에 있는 데이터의 일부 또는 전부를 검색 합니다. 응용 프로그램은 SIOCATMARK 작업을 사용 하 여 데이터가 남아 있는지 확인 해야 합니다. "긴급" (대역 외) 데이터 이전에 일반 데이터가 있는 경우 순서 대로 수신 됩니다. `Receive` 또는 `ReceiveFrom`는 동일한 호출에서 대역 외 및 일반 데이터를 혼합 하지 않습니다. *LpArgument* 매개 변수는 `IOCtl` 결과를 저장 하는 `DWORD`을 가리킵니다.

이 함수는 Berkeley 소켓에서 사용 되는 `ioctl()`의 하위 집합입니다. 특히 FIOASYNC와 동일한 명령이 없으며 SIOCATMARK는 지원 되는 유일한 소켓 수준 명령입니다.

##  <a name="listen"></a>CAsyncSocket:: 수신 대기
...
- WSAEINVAL 소켓이 `Bind` 바인딩되어 있지 않거나 이미 연결 되어 있습니다.
...
- WSAEOPNOTSUPP 참조 된 소켓이 `Listen` 작업을 지 원하는 형식이 아닙니다.

### <a name="remarks"></a>설명

연결을 허용 하려면 소켓이 먼저 `Create`를 사용 하 여 생성 되 고, 들어오는 연결에 대 한 백로그가 `Listen`로 지정 된 다음, `Accept`와의 연결이 허용 됩니다. `Listen`는 연결을 지 원하는 소켓, 즉 SOCK_STREAM 유형의 소켓에만 적용 됩니다. 이 소켓은 들어오는 연결이 승인 되 고 프로세스에 의해 보류 중인 보류 중인 보류 중인 "수동" 모드로 전환 됩니다.
...
사용 가능한 포트 (설명자)가 없는 경우에는 `Listen` 균형점 기능을 계속 시도 합니다. 큐를 비울 때까지 연결을 허용 합니다. 포트를 사용할 수 있는 경우 나중에 `Listen` 또는 `Accept`에 대 한 호출이 큐를 현재 또는 가장 최근 "백로그"로 다시 채우기 (가능한 경우) 하 고 들어오는 연결 수신을 다시 시작 합니다.
...
자세한 내용은 [Windows 소켓: 소켓 알림](../../mfc/windows-sockets-socket-notifications.md)을 참조 하세요.
...
> [!NOTE]
>  [CSocket](../../mfc/reference/csocket-class.md)에서 `OnConnect` notification 함수는 호출 되지 않습니다. 연결의 경우 연결이 완료 될 때 (성공 또는 오류 발생)를 반환 하는 `Connect`만 호출 합니다. 연결 알림이 처리 되는 방법은 MFC 구현에 자세히 설명 되어 있습니다.
...
MFC는 대역 외 데이터를 지원 하지만 클래스 `CAsyncSocket`의 사용자는 사용 하지 않는 것이 좋습니다. 이러한 데이터를 전달 하기 위한 두 번째 소켓을 만드는 것이 더 쉬운 방법입니다. 대역외 데이터에 대 한 자세한 내용은 [Windows 소켓: 소켓 알림](../../mfc/windows-sockets-socket-notifications.md)을 참조 하세요.

##  <a name="onreceive"></a>CAsyncSocket:: OnReceive

`Receive` 멤버 함수를 호출 하 여 검색할 수 있는 버퍼의 데이터가 있음을 소켓에 알리기 위해 프레임 워크에서 호출 됩니다.
...
이제 `Send` 멤버 함수를 호출 하 여 데이터를 보낼 수 있음을 소켓에 알리기 위해 프레임 워크에서 호출 됩니다.
...
기존 `CAsyncSocket` 개체를 다른 `CAsyncSocket` 개체로 복사 하려면이 함수를 호출 합니다.
...
- 들어오는 데이터를 피킹 (Peeking)을 MSG_PEEK 합니다. 데이터가 버퍼에 복사 되지만 입력 큐에서 제거 되지 않습니다.

- MSG_OOB 대역 외 데이터를 처리 합니다.

### <a name="return-value"></a>Return Value

오류가 발생 하지 않는 경우 `Receive`는 받은 바이트 수를 반환 합니다. 연결이 닫힌 경우 0을 반환 합니다. 그렇지 않으면 SOCKET_ERROR 값이 반환 되 고, [GetLastError](#getlasterror)를 호출 하 여 특정 오류 코드를 검색할 수 있습니다. 이 멤버 함수에는 다음 오류가 적용 됩니다.
...
- WSAESHUTDOWN 소켓이 종료 되었습니다. n을 0 또는 2로 설정 *하* 여 `ShutDown`를 호출한 후 소켓에서 `Receive`를 호출할 수 없습니다.

- WSAEWOULDBLOCK는 소켓이 비블로킹 상태로 표시 되 고 `Receive` 작업이 차단 됩니다.

- WSAEMSGSIZE 데이터 그램이 너무 커서 지정 된 버퍼에 맞지 않아 잘렸습니다.

- WSAEINVAL 소켓이 `Bind`에 바인딩되지 않았습니다.
...
SOCK_STREAM 형식의 소켓의 경우 제공 된 버퍼의 크기까지 현재 사용할 수 있는 정보는 반환 됩니다. 대역 외 데이터의 인라인 수신에 대해 소켓이 구성 된 경우 (소켓 옵션 SO_OOBINLINE) 대역 외 데이터는 읽지 않은 경우 out-of-band 데이터만 반환 됩니다. 응용 프로그램은 `IOCtlSIOCATMARK` 옵션 또는 [OnOutOfBandData](#onoutofbanddata) 를 사용 하 여 더 이상 대역 외 데이터를 읽을 수 있는지 여부를 결정할 수 있습니다.

데이터 그램 소켓의 경우 첫 번째 큐에 넣은 데이터 그램에서 제공 된 버퍼 크기까지 데이터를 추출 합니다. 데이터 그램이 제공 된 버퍼 보다 큰 경우 버퍼는 데이터 그램의 첫 부분으로 채워지고, 초과 데이터는 손실 되며, `Receive`는 WSAEMSGSIZE로 설정 된 오류 코드와 함께 SOCKET_ERROR 값을 반환 합니다. 소켓에서 들어오는 데이터를 사용할 수 없는 경우 WSAEWOULDBLOCK로 설정 된 오류 코드와 함께 SOCKET_ERROR 값이 반환 됩니다. [OnReceive](#onreceive) callback 함수를 사용 하 여 더 많은 데이터가 도착할 때를 확인할 수 있습니다.

소켓이 SOCK_STREAM 형식이 고 원격 쪽에서 연결을 정상적으로 종료 하는 경우 0 바이트가 수신 되 면 즉시 `Receive` 완료 됩니다. 연결이 다시 설정 된 경우에는 WSAECONNRESET 오류가 발생 하 여 `Receive` 실패 합니다.
...
오류가 발생 하지 않는 경우 `ReceiveFrom`는 받은 바이트 수를 반환 합니다. 연결이 닫힌 경우 0을 반환 합니다. 그렇지 않으면 SOCKET_ERROR 값이 반환 되 고 `GetLastError`를 호출 하 여 특정 오류 코드를 검색할 수 있습니다. 이 멤버 함수에는 다음 오류가 적용 됩니다.
...
- WSAESHUTDOWN 소켓이 종료 되었습니다. n을 0 또는 2로 설정 *하* 여 `ShutDown`를 호출한 후 소켓에서 `ReceiveFrom`를 호출할 수 없습니다.

- WSAEWOULDBLOCK는 소켓이 비블로킹 상태로 표시 되 고 `ReceiveFrom` 작업이 차단 됩니다.
...
SOCK_STREAM 형식의 소켓의 경우 제공 된 버퍼의 크기까지 현재 사용할 수 있는 정보는 반환 됩니다. 대역 외 데이터의 인라인 수신에 대해 소켓이 구성 된 경우 (소켓 옵션 SO_OOBINLINE) 대역 외 데이터는 읽지 않은 경우 out-of-band 데이터만 반환 됩니다. 응용 프로그램은 `IOCtlSIOCATMARK` 옵션 또는 `OnOutOfBandData`를 사용 하 여 더 이상 대역 외 데이터를 읽을 수 있는지 여부를 확인할 수 있습니다. *LpSockAddr* 및 *lpSockAddrLen* 매개 변수는 SOCK_STREAM 소켓에 대해 무시 됩니다.

데이터 그램 소켓의 경우 첫 번째 큐에 넣은 데이터 그램에서 제공 된 버퍼 크기까지 데이터를 추출 합니다. 데이터 그램이 제공 된 버퍼 보다 큰 경우 버퍼는 메시지의 첫 부분으로 채워지고, 초과 데이터는 손실 되며, `ReceiveFrom`는 오류 코드가 WSAEMSGSIZE로 설정 된 SOCKET_ERROR 값을 반환 합니다.

*LpSockAddr* 가 0이 아니고 소켓이 SOCK_DGRAM 형식이 면 데이터를 보낸 소켓의 네트워크 주소가 해당 [SOCKADDR](/windows/win32/winsock/sockaddr-2) 구조체로 복사 됩니다. *LpSockAddrLen* 가 가리키는 값은이 구조체의 크기로 초기화 되며 반환 시 수정 되어 저장 된 주소의 실제 크기를 표시 합니다. 소켓에서 들어오는 데이터를 사용할 수 없는 경우 소켓이 비블로킹 되지 않는 한 `ReceiveFrom` 호출은 데이터가 도착할 때까지 대기 합니다. 이 경우 WSAEWOULDBLOCK로 설정 된 오류 코드와 함께 SOCKET_ERROR 값이 반환 됩니다. `OnReceive` 콜백은 추가 데이터가 도착할 때를 확인 하는 데 사용할 수 있습니다.

소켓이 SOCK_STREAM 형식이 고 원격 쪽에서 연결을 정상적으로 종료 하는 경우 0 바이트가 수신 되 면 즉시 `ReceiveFrom` 완료 됩니다.
...
오류가 발생 하지 않는 경우 `ReceiveFromEx`는 받은 바이트 수를 반환 합니다. 연결이 닫힌 경우 0을 반환 합니다. 그렇지 않으면 SOCKET_ERROR 값이 반환 되 고 `GetLastError`를 호출 하 여 특정 오류 코드를 검색할 수 있습니다. 이 멤버 함수에는 다음 오류가 적용 됩니다.
...
- WSAESHUTDOWN 소켓이 종료 되었습니다. n을 0 또는 2로 설정 *하* 여 `ShutDown`를 호출한 후 소켓에서 `ReceiveFromEx`를 호출할 수 없습니다.

- WSAEWOULDBLOCK는 소켓이 비블로킹 상태로 표시 되 고 `ReceiveFromEx` 작업이 차단 됩니다.
...
데이터 그램 소켓의 경우 첫 번째 큐에 넣은 데이터 그램에서 제공 된 버퍼 크기까지 데이터를 추출 합니다. 데이터 그램이 제공 된 버퍼 보다 큰 경우 버퍼는 메시지의 첫 부분으로 채워지고, 초과 데이터는 손실 되며, `ReceiveFromEx`는 오류 코드가 WSAEMSGSIZE로 설정 된 SOCKET_ERROR 값을 반환 합니다.

*LpSockAddr* 가 0이 아니고 소켓이 SOCK_DGRAM 형식이 면 데이터를 보낸 소켓의 네트워크 주소가 해당 [SOCKADDR](/windows/win32/winsock/sockaddr-2) 구조체로 복사 됩니다. *LpSockAddrLen* 가 가리키는 값은이 구조체의 크기로 초기화 되며 반환 시 수정 되어 저장 된 주소의 실제 크기를 표시 합니다. 소켓에서 들어오는 데이터를 사용할 수 없는 경우 소켓이 비블로킹 되지 않는 한 `ReceiveFromEx` 호출은 데이터가 도착할 때까지 대기 합니다. 이 경우 WSAEWOULDBLOCK로 설정 된 오류 코드와 함께 SOCKET_ERROR 값이 반환 됩니다. `OnReceive` 콜백은 추가 데이터가 도착할 때를 확인 하는 데 사용할 수 있습니다.

소켓이 SOCK_STREAM 형식이 고 원격 쪽에서 연결을 정상적으로 종료 하는 경우 0 바이트가 수신 되 면 즉시 `ReceiveFromEx` 완료 됩니다.
...
- MSG_DONTROUTE는 데이터에 라우팅이 적용 되지 않도록 지정 합니다. Windows 소켓 공급자는이 플래그를 무시 하도록 선택할 수 있습니다.
...
- WSAESHUTDOWN 소켓이 종료 되었습니다. 가 1 또는 2로 설정 된 경우를 *사용 하 여* `ShutDown`를 호출한 후 소켓에서 `Send`를 호출할 수 없습니다.

- WSAEWOULDBLOCK는 소켓이 비블로킹 상태로 표시 되 고 요청 된 작업이 차단 됩니다.

- WSAEMSGSIZE는 SOCK_DGRAM 유형이 며, 데이터 그램이 Windows 소켓 구현에서 지 원하는 최대 크기 보다 큽니다.
...
`Send`는 연결 된 스트림 또는 데이터 그램 소켓에서 나가는 데이터를 쓰는 데 사용 됩니다. 데이터 그램 소켓의 경우 기본 서브넷의 최대 IP 패킷 크기를 초과 하지 않도록 주의를 기울여야 합니다 .이 크기는 `AfxSocketInit`에서 반환 하는 [WSADATA](/windows/win32/api/winsock2/ns-winsock2-wsadata) 구조의 `iMaxUdpDg` 요소에 의해 제공 됩니다. 데이터가 너무 길어서 기본 프로토콜을 통해 원자 단위로 전달할 수 없는 경우 WSAEMSGSIZE 오류는 `GetLastError`를 통해 반환 되 고 데이터는 전송 되지 않습니다.

데이터 그램 소켓의 경우 `Send` 성공적으로 완료 되 면 데이터가 성공적으로 배달 되었음을 알 수 없습니다.

SOCK_STREAM 형식의 `CAsyncSocket` 개체에서 기록 되는 바이트 수는 로컬 및 외부 호스트의 버퍼 가용성에 따라 1에서 요청 된 길이 사이에 있을 수 있습니다.
...
- WSAESHUTDOWN 소켓이 종료 되었습니다. 가 1 또는 2로 설정 된 경우를 *사용 하 여* `ShutDown`를 호출한 후 소켓에서 `SendTo`를 호출할 수 없습니다.
...
`SendTo`는 데이터 그램 또는 스트림 소켓에서 사용 되며 소켓에서 나가는 데이터를 쓰는 데 사용 됩니다. 데이터 그램 소켓의 경우 기본 서브넷의 최대 IP 패킷 크기를 초과 하지 않도록 주의를 기울여야 합니다 .이 크기는 [AfxSocketInit](../../mfc/reference/application-information-and-management.md#afxsocketinit)에서 [WSADATA](/windows/win32/api/winsock2/ns-winsock2-wsadata) 된 구조체의 `iMaxUdpDg` 요소에 의해 제공 됩니다. 데이터가 너무 길어서 기본 프로토콜을 통해 원자 단위로 전달할 수 없는 경우 WSAEMSGSIZE 오류가 반환 되 고 데이터가 전송 되지 않습니다.

`SendTo` 성공적으로 완료 되 면 데이터가 성공적으로 배달 된 것으로 표시 되지 않습니다.

`SendTo`는 *lpSockAddr* 매개 변수로 식별 되는 특정 소켓에 데이터 그램을 보내기 위해 SOCK_DGRAM 소켓 에서만 사용 됩니다.
...
- WSAESHUTDOWN 소켓이 종료 되었습니다. 가 1 또는 2로 설정 된 경우를 *사용 하 여* `ShutDown`를 호출한 후 소켓에서 `SendToEx`를 호출할 수 없습니다.
...
`SendToEx` 성공적으로 완료 되 면 데이터가 성공적으로 배달 된 것으로 표시 되지 않습니다.

`SendToEx`는 *lpSockAddr* 매개 변수로 식별 되는 특정 소켓에 데이터 그램을 보내기 위해 SOCK_DGRAM 소켓 에서만 사용 됩니다.
...
- SO_KEEPALIVE 설정 된 경우 WSAENETRESET 연결 시간이 초과 되었습니다.

- WSAENOPROTOOPT 옵션을 알 수 없거나 지원 되지 않습니다. 특히 SOCK_STREAM 형식의 소켓에서는 SO_BROADCAST 지원 되지 않지만 SO_DONTLINGER, SO_KEEPALIVE, SO_LINGER 및 SO_OOBINLINE는 SOCK_DGRAM 형식의 소켓에서 지원 되지 않습니다.

- SO_KEEPALIVE 설정 되 면 WSAENOTCONN 연결이 다시 설정 되었습니다.
...
소켓 옵션에는 기능이 나 동작을 사용 하거나 사용 하지 않도록 설정 하는 부울 옵션과 정수 값 또는 구조를 필요로 하는 옵션이 있습니다. 부울 옵션을 사용 하도록 설정 하려면 *lpOptionValue* 가 0이 아닌 정수를 가리킵니다. *LpOptionValue* 옵션을 사용 하지 않도록 설정 하려면 0과 같은 정수를 가리킵니다. *nOptionLen* 은 부울 옵션에 대 한 `sizeof(BOOL)`와 같아야 합니다. 다른 옵션의 경우 *lpOptionValue* 는 옵션에 원하는 값이 포함 된 정수 또는 구조체를 가리키며 *nOptionLen* 은 정수 또는 구조체의 길이입니다.
...
다른 소켓에서 원하는 주소를 이미 사용 하 고 있으므로 소켓에 대 한 `Bind` 호출을 허용 하지 않아야 함을 Windows 소켓 구현에 알리려면 응용 프로그램은 `Bind` 호출을 실행 하기 전에 소켓에 대해 SO_REUSEADDR socket 옵션을 설정 해야 합니다. 옵션은 `Bind`를 호출할 때만 해석 됩니다. 따라서 기존 주소에 바인딩하지 않을 소켓에 옵션을 설정 하는 것은 불필요 하지만, `Bind` 호출 후에는 옵션을 설정 하거나 다시 설정 하는 것이 나 다른 소켓에는 영향을 주지 않습니다.

응용 프로그램은 SO_KEEPALIVE 소켓 옵션을 설정 하 여 Windows 소켓 구현에서 TCP (전송 제어 프로토콜) 연결에 "keep-alive" 패킷을 사용 하도록 요청할 수 있습니다. Windows 소켓 구현에서는 연결 유지를 지원 하지 않아도 됩니다 .이 경우 정확한 의미 체계가 구현에 따라 다르지만 RFC 1122: "인터넷 호스트에 대 한 요구 사항-통신 계층"의 섹션 4.2.3.6을 준수 해야 합니다. "연결 유지"의 결과로 연결이 끊어지는 경우 소켓에서 진행 중인 모든 호출에 대 한 오류 코드 WSAENETRESET이 반환 되 고 모든 후속 호출은 WSAENOTCONN로 실패 합니다.

TCP_NODELAY 옵션은 Nagle 알고리즘을 사용 하지 않도록 설정 합니다. Nagle 알고리즘은 전체 크기 패킷을 보낼 수 있을 때까지 승인 되지 않은 송신 데이터를 버퍼링 하 여 호스트에서 전송 하는 작은 패킷 수를 줄이는 데 사용 됩니다. 그러나 일부 응용 프로그램의 경우이 알고리즘으로 인해 성능이 저하 될 수 있으며, TCP_NODELAY을 사용 하 여 해제할 수 있습니다. TCP_NODELAY 설정으로 인해 네트워크 성능이 상당히 저하 될 수 있으므로 응용 프로그램 작성자는 TCP_NODELAY 설정 하면 안 됩니다. TCP_NODELAY는 수준 IPPROTO_TCP를 사용 하는 유일 하 게 지원 되는 소켓 옵션입니다. 다른 모든 옵션은 수준 SOL_SOCKET를 사용 합니다.

응용 프로그램에서 SO_DEBUG 옵션이 설정 된 경우 Windows 소켓의 일부 구현에서 출력 디버그 정보를 제공 합니다.

`SetSockOpt`에 대해 지원 되는 옵션은 다음과 같습니다. 형식은 *lpOptionValue*로 주소를 지정 하는 데이터 형식을 식별 합니다.

|값|Type|의미|
|-----------|----------|-------------|
|SO_BROADCAST|BOOL|소켓에서 브로드캐스트 메시지의 전송을 허용 합니다.|
|SO_DEBUG|BOOL|디버깅 정보를 기록합니다.|
|SO_DONTLINGER|BOOL|보내지 않은 데이터가 전송 될 때까지 대기 하는 `Close` 차단 하지 않습니다. 이 옵션을 설정 하는 것은 `l_onoff` 0으로 설정 된 SO_LINGER 설정 하는 것과 같습니다.|
|SO_DONTROUTE|BOOL|라우팅되지 않음: 인터페이스로 직접 보냅니다.|
|SO_KEEPALIVE|BOOL|연결 유지를 전송 합니다.|
|SO_LINGER|`struct LINGER`|보내지 않은 데이터가 있으면 `Close` 링거 합니다.|
...
`SetSockOpt`에 대해 지원 되지 않는 BSD (Berkeley Software 배포판) 옵션은 다음과 같습니다.
...
`ShutDown`은 모든 유형의 소켓에서 수신, 전송 또는 둘 다를 사용 하지 않도록 설정 하는 데 사용 됩니다. *Nhow* 가 0 인 경우 소켓에서 후속 수신은 허용 되지 않습니다. 이는 하위 프로토콜 계층에는 영향을 주지 않습니다.
...
`ShutDown`는 소켓을 닫지 않으며 소켓에 연결 된 리소스는 `Close`가 호출 될 때까지 해제 되지 않습니다. 응용 프로그램이 종료 된 후 소켓을 다시 사용할 수 있도록 하는 데 의존해 서는 안 됩니다. 특히, 이러한 소켓에서 `Connect` 사용을 지원 하기 위해 Windows 소켓 구현이 필요 하지 않습니다.

### <a name="example"></a>예제

  [Casyncsocket:: OnReceive](#onreceive)에 대 한 예제를 참조 하세요.

##  <a name="socket"></a>CASyncSocket:: 소켓
...
- `FD_CLOSE`: 소켓 클로저의 알림을 수신 하려고 합니다.
...
이 메서드는 소켓 핸들을 할당 합니다. [Casyncsocket:: Bind](#bind) 를 호출 하 여 소켓을 지정 된 주소로 바인딩하지 않으므로 나중에 `Bind`를 호출 하 여 소켓을 지정 된 주소에 바인딩해야 합니다. [Casyncsocket:: SetSockOpt](#setsockopt) 를 사용 하 여 바인딩 전에 소켓 옵션을 설정할 수 있습니다.
