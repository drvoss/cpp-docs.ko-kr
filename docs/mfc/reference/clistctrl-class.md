...
|[CListCtrl::Create](#create)|목록 컨트롤을 만들고이를 개체에 연결 `CListCtrl` 합니다.|
...
다음은 클래스에 대 한 간략 한 개요입니다 `CListCtrl` . 개념에 대 한 자세한 내용은 CListCtrl 및 [Controls](../../mfc/controls-mfc.md) [사용](../../mfc/using-clistctrl.md) 을 참조 하세요.
...
- 보고서 뷰
   ...
   각 항목은 해당 줄에 표시 되 고 추가 정보는 오른쪽의 열에 정렬 됩니다. 맨 왼쪽 열에는 작은 아이콘 및 레이블이 포함 되 고 후속 열에는 응용 프로그램에 지정 된 하위 항목이 포함 됩니다. 포함 헤더 컨트롤 (클래스 [CHeaderCtrl](../../mfc/reference/cheaderctrl-class.md))은 이러한 열을 구현 합니다. 보고서 뷰의 헤더 컨트롤 및 열에 대 한 자세한 내용은 [CListCtrl 사용: 컨트롤에 열 추가 (보고서 뷰)](../../mfc/adding-columns-to-the-control-report-view.md)를 참조 하세요.

컨트롤의 현재 목록 뷰의 스타일에 따라 현재 뷰가 결정 됩니다. 이러한 스타일 및 사용 방법에 대 한 자세한 내용은 [CListCtrl 사용: List 컨트롤 스타일 변경](../../mfc/changing-list-control-styles.md)을 참조 하세요.

## <a name="extended-styles"></a>확장 스타일

표준 목록 스타일 외에도 클래스는 `CListCtrl` 보강 기능을 제공 하는 확장 스타일의 많은 집합을 지원 합니다. 이 기능의 몇 가지 예는 다음과 같습니다.
...
- 가상 목록 뷰
   ...
   사용 하도록 설정 하면 컨트롤에서 최대 DWORD 항목을 지원할 수 있습니다. 이는 응용 프로그램에서 항목 데이터를 관리 하는 오버 헤드를 배치 하 여 수행할 수 있습니다. 항목 선택 및 포커스 정보를 제외 하 고 모든 항목 정보는 응용 프로그램에서 관리 해야 합니다. 자세한 내용은 [Using CListCtrl: Virtual List Controls](../../mfc/virtual-list-controls.md)을 참조 하세요.
...
이러한 새 확장 스타일을 사용 하는 방법에 대 한 자세한 내용은 [CListCtrl 사용: List 컨트롤 스타일 변경](../../mfc/changing-list-control-styles.md)을 참조 하세요.
...
기본적으로 목록 뷰 컨트롤은 항목의 아이콘 및 텍스트 특성을 저장 합니다. 그러나 이러한 항목 형식 외에도 클래스는 `CListCtrl` "콜백 항목"을 지원 합니다. "콜백 항목"은 컨트롤이 아닌 응용 프로그램이 텍스트, 아이콘 또는 둘 다를 저장 하는 목록 보기 항목입니다. 콜백 마스크는 응용 프로그램에서 제공 하는 항목 특성 (텍스트 및/또는 아이콘)을 지정 하는 데 사용 됩니다. 응용 프로그램에서 콜백 항목을 사용 하는 경우 요청 시 텍스트 및/또는 아이콘 특성을 제공할 수 있어야 합니다. 콜백 항목은 응용 프로그램에서이 정보 중 일부를 이미 유지 관리 하는 경우에 유용 합니다. 자세한 내용은 [CListCtrl 사용: 콜백 항목 및 콜백 마스크 사용](../callback-items-and-the-callback-mask.md)을 참조 하세요.
...
기본적으로 목록 뷰 컨트롤은 제거 될 때 할당 된 이미지 목록을 소멸 시킵니다. 그러나 개발자는 응용 프로그램에 의해 결정 된 대로 더 이상 사용 되지 않을 때 각 이미지 목록을 삭제 하 여이 동작을 사용자 지정할 수 있습니다. 자세한 내용은 [CListCtrl 사용: 목록 항목 및 이미지 목록](../list-items-and-image-lists.md)을 참조 하세요.
...
`CSize`항목을 표시 하는 데 필요한 대략적인 너비와 높이 (픽셀)를 포함 하는 개체입니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_CANCELEDITLABEL](/windows/win32/Controls/lvm-canceleditlabel) 메시지의 기능을 에뮬레이트합니다.
...
목록 컨트롤을 만들고이를 개체에 연결 `CListCtrl` 합니다.
...
*rect*<br/>
목록 컨트롤의 크기와 위치를 지정 합니다. `CRect`개체 또는 [RECT](/windows/win32/api/windef/ns-windef-rect) 구조 중 하나일 수 있습니다.

*pParentWnd*<br/>
목록 컨트롤의 부모 창 (일반적으로)을 지정 합니다 `CDialog` . NULL이 아니어야 합니다.
...
`CListCtrl`두 단계로을 생성 합니다. 먼저 생성자를 호출한 다음을 호출 하 여 `Create` 목록 뷰 컨트롤을 만들고이를 개체에 연결 `CListCtrl` 합니다.

확장 된 Windows 스타일을 list 컨트롤 개체에 적용 하려면 대신 [Createex](#createex) 를 호출 `Create` 합니다.
...
컨트롤 (자식 창)을 만들고이를 개체에 연결 `CListCtrl` 합니다.
...
`CreateEx`*Dwexstyle*로 지정 된 확장 된 Windows 스타일을 사용 하 여 컨트롤을 만듭니다. 컨트롤과 관련 된 확장 스타일을 설정 하려면 [SetExtendedStyle](#setextendedstyle)를 호출 합니다. 예를 들어를 사용 하 여 WS_EX_CONTEXTHELP와 같은 스타일을 `CreateEx` 설정 하지만을 사용 하 여 이러한 스타일을 `SetExtendedStyle` LVS_EX_FULLROWSELECT로 설정 합니다. 자세한 내용은 Windows SDK [확장 목록 뷰 스타일](/windows/win32/Controls/extended-list-view-styles) 문서에서 설명 하는 스타일을 참조 하세요.
...
*lpPoint*<br/>
뷰 좌표에서 이미지의 왼쪽 위 모퉁이에 대 한 초기 위치를 받는 [점](/windows/win32/api/windef/ns-windef-point) 구조의 주소입니다.
...
*lpDrawItemStruct*<br/>
`DRAWITEMSTRUCT`필요한 그리기 형식에 대 한 정보를 포함 하는 구조체에 대 한 긴 포인터입니다.

### <a name="remarks"></a>설명

`itemAction` [Drawitemstruct](/windows/win32/api/winuser/ns-winuser-drawitemstruct) 구조체의 멤버는 수행할 그리기 작업을 정의 합니다.

기본적으로이 멤버 함수는 아무 작업도 수행 하지 않습니다. 소유자 그리기 개체에 대 한 그리기를 구현 하려면이 멤버 함수를 재정의 `CListCtrl` 합니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_ENABLEGROUPVIEW](/windows/win32/Controls/lvm-enablegroupview) 메시지의 기능을 에뮬레이트합니다.
...
*Pfindinfo* 매개 변수는 `LVFINDINFO` 목록 보기 항목을 검색 하는 데 사용 되는 정보를 포함 하는 구조를 가리킵니다.
...
*plvbkImage*<br/>
`LVBKIMAGE`목록 뷰의 현재 배경 이미지를 포함 하는 구조체에 대 한 포인터입니다.
...
*pColumn*<br/>
열에 대 한 정보를 검색 하 고 받을 정보를 지정 하는 [Lvcolumn](/windows/win32/api/commctrl/ns-commctrl-lvcolumnw) 구조의 주소입니다. `mask`멤버는 검색할 열 특성을 지정 합니다. `mask`멤버가 LVCF_TEXT 값을 지정 하는 경우 `pszText` 멤버는 항목 텍스트를 받는 버퍼의 주소를 포함 해야 하며 `cchTextMax` 멤버는 버퍼의 크기를 지정 해야 합니다.
...
구조에는 `LVCOLUMN` 보고서 뷰의 열에 대 한 정보가 포함 되어 있습니다.
...
이 메서드는 Windows SDK에서 설명 하는 [LVM_GETEMPTYTEXT](/windows/win32/Controls/lvm-getemptytext) 메시지를 보냅니다.
...
해당 그룹이 있는 경우 상태가 LVGS_FOCUSED 된 그룹의 인덱스입니다. 그렇지 않으면-1입니다.

### <a name="remarks"></a>설명

이 메서드는 Windows SDK에서 설명 하는 [LVM_GETFOCUSEDGROUP](/windows/win32/Controls/lvm-getfocusedgroup) 메시지를 보냅니다. 자세한 내용은 `state` [Lvgroup](/windows/win32/api/commctrl/ns-commctrl-lvgroup) 구조체의 멤버 LVGS_FOCUSED 값을 참조 하세요.
...
이 메서드는 [LVM_GETGROUPCOUNT](/windows/win32/Controls/lvm-getgroupcount) 메시지를 보냅니다 .이 메시지는 Windows SDK->에 설명 되어 있습니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_GETGROUPINFO](/windows/win32/Controls/lvm-getgroupinfo) 메시지의 기능을 에뮬레이트합니다.
...
*iIndex*\
진행 그룹의 인덱스 (0부터 시작)입니다.

*pGroup*\
제한이 *Iindex* 매개 변수로 지정 된 그룹에 대 한 정보를 수신 하는 [lvgroup](/windows/win32/api/commctrl/ns-commctrl-lvgroup) 구조에 대 한 포인터입니다. 호출자는 [Lvgroup](/windows/win32/api/commctrl/ns-commctrl-lvgroup) 구조체의 멤버를 초기화 해야 합니다. 멤버를 `cbSize` 구조체의 크기로 설정 하 고 멤버의 플래그를 설정 하 여 `mask` 검색할 정보를 지정 합니다.
...
이 메서드는 [LVM_GETGROUPINFOBYINDEX](/windows/win32/controls/lvm-getgroupinfobyindex) 메시지를 보냅니다 .이 메시지는 Windows SDK->에 설명 되어 있습니다.

### <a name="example"></a>예제

다음 코드 예제에서는 `m_listCtrl` 현재 목록 뷰 컨트롤에 액세스 하는 데 사용 되는 변수를 정의 합니다. 이 변수는 다음 예제에서 사용됩니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_GETGROUPMETRICS](/windows/win32/Controls/lvm-getgroupmetrics) 메시지의 기능을 에뮬레이트합니다.
...
*lpRect*\
[in, out] [RECT](/windows/win32/api/windef/ns-windef-rect) 구조체에 대 한 포인터입니다. 이 메서드가 성공적으로 수행 되 면 구조체가 *Igroupid*로 지정 된 그룹의 사각형 좌표를 수신 합니다.

*iCoords*\
진행 검색할 사각형 좌표를 지정 합니다. 다음 값 중 하나를 사용 합니다.

- 확장 된 전체 그룹의 LVGGR_GROUP (기본값) 좌표입니다.

- LVGGR_HEADER-머리글만 (축소 된 그룹)의 좌표입니다.

- LVGGR_SUBSETLINK-하위 집합 링크만 (태그 하위 집합)의 좌표입니다.
...
호출자는 *pRect* 매개 변수가 가리키는 [RECT](/windows/win32/api/windef/ns-windef-rect) 구조를 할당 해야 합니다.

이 메서드는 Windows SDK에서 설명 하는 [LVM_GETGROUPRECT](/windows/win32/Controls/lvm-getgrouprect) 메시지를 보냅니다.
...
*dwMask*\
진행 지정 된 그룹에 대해 검색할 상태 값을 지정 하는 마스크입니다. 자세한 내용은 `mask` [Lvgroup](/windows/win32/api/commctrl/ns-commctrl-lvgroup) 구조체의 멤버를 참조 하십시오.
...
반환 값은 *Dwmask* 매개 변수 및 `state` 현재 목록 뷰 컨트롤을 나타내는 [lvgroup](/windows/win32/api/commctrl/ns-commctrl-lvgroup) 구조체의 멤버 값에 대 한 비트 and 연산의 결과입니다.

이 메서드는 Windows SDK에서 설명 하는 [LVM_GETGROUPSTATE](/windows/win32/Controls/lvm-getgroupstate) 메시지를 보냅니다. 자세한 내용은 [ListView_GetGroupState](/windows/win32/api/commctrl/nf-commctrl-listview_getgroupstate) 매크로를 참조 하세요.
...
- 아이콘이 매우 많은 LVSIL_NORMAL 이미지 목록입니다.

- 작은 아이콘을 사용 하 여 이미지 목록을 LVSIL_SMALL 합니다.

- 상태 이미지가 포함 된 이미지 목록을 LVSIL_STATE 합니다.
...
성공 하면 TRUE를 반환 하 고 그렇지 않으면 FALSE를 반환 합니다. `cbSize`구조체의 멤버 크기가 `LVINSERTMARK` 구조체의 실제 크기와 일치 하지 않으면 FALSE가 반환 됩니다.

### <a name="remarks"></a>설명

이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_GETINSERTMARK](/windows/win32/Controls/lvm-getinsertmark) 메시지의 기능을 에뮬레이트합니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_GETINSERTMARKCOLOR](/windows/win32/Controls/lvm-getinsertmarkcolor) 메시지의 기능을 에뮬레이트합니다.
...
*pRect*<br/>
`RECT`삽입 지점을 제한 하는 사각형의 좌표를 포함 하는 구조체에 대 한 포인터입니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_GETINSERTMARKRECT](/windows/win32/Controls/lvm-getinsertmarkrect) 메시지의 기능을 에뮬레이트합니다.
...
`LVITEM`구조는 목록 보기 항목의 특성을 지정 하거나 받습니다.
...
로 지정 된 항목에 연결 된 32 비트 응용 프로그램 관련 값을 검색 합니다 `nItem` .
...
*pItemIndex*\
진행 하위 항목의 부모 항목에 대 한 [Lvitemindex](/windows/win32/api/commctrl/ns-commctrl-lvitemindex) 구조에 대 한 포인터입니다. 호출자는 [Lvitemindex](/windows/win32/api/commctrl/ns-commctrl-lvitemindex) 구조체의 멤버를 할당 하 고 설정 해야 합니다. 이 매개 변수는 NULL일 수 없습니다.

*iColumn*\
진행 컨트롤에 있는 열의 인덱스 (0부터 시작)입니다.

*rectType*\
진행 경계 사각형이 검색 되는 목록 보기 하위 항목의 부분입니다. 다음 값 중 하나를 지정합니다.

- LVIR_BOUNDS-아이콘 및 레이블을 포함 하 여 전체 하위 항목의 경계 사각형을 반환 합니다.

- LVIR_ICON-하위 항목의 아이콘 또는 작은 아이콘의 경계 사각형을 반환 합니다.

- LVIR_LABEL-하위 항목 텍스트의 경계 사각형을 반환 합니다.

*pRect*\
제한이 하위 항목의 경계 사각형에 대 한 정보를 수신 하는 [RECT](/windows/win32/api/windef/ns-windef-rect) 구조체에 대 한 포인터입니다. 호출자는 [RECT](/windows/win32/api/windef/ns-windef-rect) 구조체를 할당 해야 합니다. 이 매개 변수는 NULL일 수 없습니다.
...
이 메서드는 Windows SDK에서 설명 하는 [LVM_GETITEMINDEXRECT](/windows/win32/Controls/lvm-getitemindexrect) 메시지를 보냅니다. 자세한 내용은 [ListView_GetItemIndexRect 매크로](/windows/win32/api/commctrl/nf-commctrl-listview_getitemindexrect)를 참조 하세요.
...
*lpPoint*<br/>
뷰 좌표에서 항목의 왼쪽 위 모퉁이 위치를 받는 [점](/windows/win32/api/windef/ns-windef-point) 구조의 주소입니다.
...
- LVIR_ICON 아이콘 또는 작은 아이콘의 경계 사각형을 반환 합니다.
...
*fSmall*\
진행 항목 간격을 검색할 뷰입니다. 작은 아이콘 보기에 대해 TRUE를 지정 하 고, 아이콘 보기의 경우 FALSE를 지정 합니다.

*pnHorzSpacing*\
제한이 항목 사이의 가로 간격을 포함 합니다.

*pnVertSpacing*\
제한이 항목 간의 세로 간격을 포함 합니다.
...
이 메서드는 Windows SDK에서 설명 하는 [LVM_GETITEMSPACING](/windows/win32/Controls/lvm-getitemspacing) 메시지를 보냅니다.

## <a name="clistctrlgetitemstate"></a><a name="getitemstate"></a> CListCtrl:: GetItemState

목록 보기 항목의 상태를 검색 합니다.
...
항목의 상태는 `state` Windows SDK 설명 된 [lvitem](/windows/win32/api/commctrl/ns-commctrl-lvitemw) 구조의 멤버에 의해 지정 됩니다. 항목의 상태를 지정 하거나 변경 하는 경우 `stateMask` 멤버는 변경 하려는 상태 비트를 지정 합니다.
...
을 반환 하는 버전은 **`int`** 검색 된 문자열의 길이를 반환 합니다.

을 반환 하는 버전은 `CString` 항목 텍스트를 반환 합니다.
...
- LVNI_ABOVE 지정 된 항목 위에 있는 항목을 검색 합니다.

- LVNI_ALL는 인덱스 (기본값)로 후속 항목을 검색 합니다.

- LVNI_BELOW 지정 된 항목 아래에 있는 항목을 검색 합니다.

- LVNI_TOLEFT 지정 된 항목의 왼쪽에 있는 항목을 검색 합니다.

- LVNI_TORIGHT 지정 된 항목의 오른쪽에 있는 항목을 검색 합니다.

상태는 0 이거나 다음 값 중 하나 이상이 될 수 있습니다.

- 항목에 LVIS_DROPHILITED 상태 플래그가 설정 LVNI_DROPHILITED 합니다.

- 항목에 LVIS_FOCUSED 상태 플래그가 설정 LVNI_FOCUSED 합니다.

- 항목에 LVIS_SELECTED 상태 플래그가 설정 LVNI_SELECTED 합니다.
...
*pItemIndex*\
[in, out] 검색이 시작 되는 항목을 설명 하는 [Lvitemindex](/windows/win32/api/commctrl/ns-commctrl-lvitemindex) 구조체에 대 한 포인터 이거나, *nflags* 매개 변수의 플래그와 일치 하는 첫 번째 항목을 찾으려면-1입니다. 이 메서드가 성공적으로 수행 되 면 `LVITEMINDEX` 구조는 검색을 통해 발견 된 항목을 설명 합니다.

*nFlags*\
진행 검색을 수행 하는 방법을 지정 하는 플래그의 비트 조합 (또는)입니다. 검색은 대상 항목의 인덱스, 상태 또는 모양 또는 *Pitemindex* 매개 변수로 지정 된 항목을 기준으로 하는 대상 항목의 실제 위치에 따라 달라질 수 있습니다. 자세한 내용은 [LVM_GETNEXTITEMINDEX](/windows/win32/controls/lvm-getnextitemindex) 메시지의 *flags* 매개 변수를 참조 하세요.
...
이 메서드는 Windows SDK에서 설명 하는 [LVM_GETNEXTITEMINDEX](/windows/win32/controls/lvm-getnextitemindex) 메시지를 보냅니다.
...
*pos*<br/>
또는에 대 한 이전 호출에서 반환 된 위치 값에 대 한 참조입니다 `GetNextSelectedItem` `GetFirstSelectedItemPosition` . 이 호출을 통해 값이 다음 위치로 업데이트 됩니다.
...
`GetNextSelectedItem`를 호출 하 여 초기 위치를 설정 하는 경우 전방 반복 루프에서을 사용할 수 있습니다 `GetFirstSelectedItemPosition` .
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_GETOUTLINECOLOR](/windows/win32/Controls/lvm-getoutlinecolor) 메시지의 기능을 에뮬레이트합니다.
...
*lpPoint*<br/>
뷰 원본을 받는 [점](/windows/win32/api/windef/ns-windef-point) 구조의 주소입니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_GETSELECTEDCOLUMN](/windows/win32/Controls/lvm-getselectedcolumn) 메시지의 기능을 에뮬레이트합니다.
...
- LVIR_LABEL 아이콘 및 레이블을 포함 하 여 전체 항목의 경계 사각형을 반환 합니다. LVIR_BOUNDS와 동일 합니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_GETTILEINFO](/windows/win32/Controls/lvm-gettileinfo) 메시지의 기능을 에뮬레이트합니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_GETTILEVIEWINFO](/windows/win32/Controls/lvm-gettileviewinfo) 메시지의 기능을 에뮬레이트합니다.

## <a name="clistctrlgettooltips"></a><a name="gettooltips"></a> CListCtrl:: GetToolTips

목록 뷰 컨트롤에서 도구 설명을 표시 하는 데 사용 하는 도구 설명 컨트롤을 검색 합니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 Win32 메시지 [LVM_GETTOOLTIPS](/windows/win32/Controls/lvm-gettooltips)의 동작을 구현 합니다. 의 MFC 구현은 `GetToolTips` `CToolTipCtrl` 도구 설명 컨트롤에 대 한 핸들이 아니라 목록 컨트롤에서 사용 하는 개체를 반환 합니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_GETVIEW](/windows/win32/Controls/lvm-getview) 메시지의 기능을 에뮬레이트합니다.
...
*lpRect*<br/>
[RECT](/windows/win32/api/windef/ns-windef-rect) 구조체의 주소입니다.
...
## <a name="clistctrlgetworkareas"></a><a name="getworkareas"></a> CListCtrl:: Get근무 영역
...
*nWorkAreas*<br/>
`RECT` *PRect* 배열에 포함 된 구조체의 수입니다.

*pRect*<br/>
`RECT`목록 뷰 컨트롤의 작업 영역을 받는 구조체 (또는 [crect](../../atl-mfc-shared/reference/crect-class.md) 개체)의 배열에 대 한 포인터입니다. 이러한 구조체의 값은 클라이언트 좌표에 있습니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_HASGROUP](/windows/win32/Controls/lvm-hasgroup) 메시지의 기능을 에뮬레이트합니다.
...
*pHitTestInfo*<br/>
`LVHITTESTINFO`적중 횟수 테스트의 위치를 포함 하 고 적중 횟수 테스트의 결과에 대 한 정보를 받는 구조체의 주소입니다.

*pt*<br/>
테스트할 점입니다.

*pFlags*<br/>
테스트 결과에 대 한 정보를 수신 하는 정수에 대 한 포인터입니다. `flags`Windows SDK에서 [LVHITTESTINFO](/windows/win32/api/commctrl/ns-commctrl-lvhittestinfo) 구조체의 멤버에 대 한 설명을 참조 하세요.
...
구조체 멤버의 LVHT_ABOVE, LVHT_BELOW, LVHT_TOLEFT 및 LVHT_TORIGHT 값을 사용 하 여 `flag` 목록 뷰 컨트롤의 내용을 스크롤할 지 여부를 결정할 수 있습니다. 이러한 플래그 중 두 개를 결합할 수 있습니다. 예를 들어 위치가 클라이언트 영역의 위쪽 및 왼쪽에 있습니다.

구조체 멤버의 LVHT_ONITEM 값을 테스트 하 여 지정 된 `flag` 위치가 목록 뷰 항목 위에 있는지 여부를 확인할 수 있습니다. 이 값은 구조체 멤버의 LVHT_ONITEMICON, LVHT_ONITEMLABEL 및 LVHT_ONITEMSTATEICON 값에 대 한 비트 OR 연산입니다 `flag` .
...
*pColumn*<br/>
`LVCOLUMN`새 열의 특성을 포함 하는 구조체의 주소입니다.

*lpszColumnHeading*<br/>
열의 머리글을 포함 하는 문자열의 주소입니다.

*nFormat*<br/>
열의 맞춤을 지정 하는 정수입니다. LVCFMT_LEFT, LVCFMT_RIGHT 또는 LVCFMT_CENTER 값 중 하나일 수 있습니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_INSERTGROUP](/windows/win32/Controls/lvm-insertgroup) 메시지의 기능을 에뮬레이트합니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_INSERTGROUPSORTED](/windows/win32/Controls/lvm-insertgroupsorted) 메시지의 기능을 에뮬레이트합니다.
...
*lParam*<br/>
항목과 연결 된 32 비트 응용 프로그램 관련 값입니다. 이 매개 변수를 지정 하는 경우 LVIF_PARAM *Nmask* 특성을 설정 해야 합니다.
...
이 메서드를 호출 하면 LVM_INSERTITEM 메시지가 컨트롤 창으로 전송 될 수 있습니다. 컨트롤에 대 한 연결 된 메시지 처리기는 특정 조건 (예: LVS_OWNERDRAW 등의 창 스타일 사용)에서 항목 텍스트를 설정 하지 못할 수 있습니다. 이러한 조건에 대 한 자세한 내용은 Windows SDK [LVM_INSERTITEM](/windows/win32/Controls/lvm-insertitem) 을 참조 하세요.
...
*pPoint*<br/>
목록 컨트롤의 클라이언트 영역을 기준으로 적중 테스트 좌표를 포함 하는 [POINT](/windows/win32/api/windef/ns-windef-point) 구조체에 대 한 포인터입니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_INSERTMARKHITTEST](/windows/win32/Controls/lvm-insertmarkhittest) 메시지의 기능을 에뮬레이트합니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_ISGROUPVIEWENABLED](/windows/win32/Controls/lvm-isgroupviewenabled) 메시지의 기능을 에뮬레이트합니다.
...
*인덱싱할*\
진행 현재 목록 뷰 컨트롤에 있는 항목의 인덱스 (0부터 시작)입니다.
...
이 메서드는 Windows SDK에서 설명 하는 [LVM_ISITEMVISIBLE](/windows/win32/Controls/lvm-isitemvisible) 메시지를 보냅니다.
...
이 메서드는 Windows SDK에서 설명 하는 [LVM_MAPIDTOINDEX](/windows/win32/controls/lvm-mapidtoindex) 메시지를 보냅니다.
...
이 메서드는 Windows SDK에서 설명 하는 [LVM_MAPINDEXTOID](/windows/win32/Controls/lvm-mapindextoid) 메시지를 보냅니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_MOVEGROUP](/windows/win32/Controls/lvm-movegroup) 메시지의 기능을 에뮬레이트합니다.
...
이 메서드는 Windows SDK에 설명 된 대로 [LVM_MOVEITEMTOGROUP](/windows/win32/Controls/lvm-moveitemtogroup) 메시지의 기능을 에뮬레이트합니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_REMOVEALLGROUPS](/windows/win32/Controls/lvm-removeallgroups) 메시지의 기능을 에뮬레이트합니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_REMOVEGROUP](/windows/win32/Controls/lvm-removegroup) 메시지의 기능을 에뮬레이트합니다.
...
*size*<br/>
`CSize`가로 및 세로 스크롤 크기 (픽셀)를 지정 하는 개체입니다. `y` *크기* 의 멤버는 목록 뷰 컨트롤의 줄에 대 한 높이 (픽셀)로 나뉘어 있고, 컨트롤은 결과 줄 수 만큼 스크롤됩니다.
...
*plvbkImage*<br/>
`LVBKIMAGE`새 배경 이미지 정보를 포함 하는 구조체의 주소입니다.
...
> [!NOTE]
> 에서는 `CListCtrl::SetBkImage` OLE COM 기능을 사용 하기 때문에를 사용 하기 전에 ole 라이브러리를 초기화 해야 합니다 `SetBkImage` . 응용 프로그램이 초기화 될 때 COM 라이브러리를 초기화 하 고 응용 프로그램이 종료 될 때 라이브러리의 초기화를 취소 하는 것이 가장 좋습니다. 이는 ActiveX 기술, OLE 자동화, OLE 연결/포함 또는 ODBC/DAO 작업을 활용 하는 MFC 응용 프로그램에서 자동으로 수행 됩니다.
...
*pColumn*<br/>
Windows SDK 설명 된 대로 새 열 특성을 포함 하는 [Lvcolumn](/windows/win32/api/commctrl/ns-commctrl-lvcolumnw) 구조의 주소입니다. 구조체의 `mask` 멤버는 설정할 열 특성을 지정 합니다. `mask`멤버가 LVCF_TEXT 값을 지정 하는 경우 구조체의 `pszText` 멤버는 null로 끝나는 문자열의 주소이 고 구조체의 `cchTextMax` 멤버는 무시 됩니다.
...
*cx*<br/>
열의 새 너비입니다. Windows SDK의 [LVM_SETCOLUMNWIDTH](/windows/win32/Controls/lvm-setcolumnwidth) 에 설명 된 대로 LVSCW_AUTOSIZE 또는 LVSCW_AUTOSIZE_USEHEADER 중 하나일 수 있습니다.
...
이 메서드는 Windows SDK에서 설명 하는 [LVM_SETGROUPINFO](/windows/win32/Controls/lvm-setgroupinfo) 메시지를 보냅니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_SETGROUPMETRICS](/windows/win32/Controls/lvm-setgroupmetrics) 메시지의 기능을 에뮬레이트합니다.
...
*size*<br/>
`CSize`X 축과 y 축에 있는 아이콘 사이의 거리 (픽셀)를 지정 하는 개체입니다.
...
## <a name="clistctrlsetimagelist"></a><a name="setimagelist"></a> CListCtrl:: Seon Agelist
...
## <a name="clistctrlsetinfotip"></a><a name="setinfotip"></a> CListCtrl:: SetInfoTip 팁
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_SETINFOTIP](/windows/win32/Controls/lvm-setinfotip) 메시지의 기능을 에뮬레이트합니다.
...
성공 하면 TRUE를 반환 하 고 그렇지 않으면 FALSE를 반환 합니다. `cbSize`구조체의 멤버 크기가 `LVINSERTMARK` 구조체의 실제 크기와 같지 않거나 삽입 지점이 현재 뷰에서 적용 되지 않는 경우 FALSE가 반환 됩니다.

### <a name="remarks"></a>설명

이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_SETINSERTMARK](/windows/win32/Controls/lvm-setinsertmark) 메시지의 기능을 에뮬레이트합니다.
...
`COLORREF`이전 색을 포함 하는 구조체를 반환 합니다.

### <a name="remarks"></a>설명

이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_SETINSERTMARKCOLOR](/windows/win32/Controls/lvm-setinsertmarkcolor) 메시지의 기능을 에뮬레이트합니다.
...
*pItem*<br/>
Windows SDK 설명 된 대로 새 항목 특성을 포함 하는 [Lvitem](/windows/win32/api/commctrl/ns-commctrl-lvitemw) 구조의 주소입니다. 구조체의 `iItem` 및 `iSubItem` 멤버는 항목 또는 하위 항목을 식별 하 고, 구조체의 `mask` 멤버는 설정할 특성을 지정 합니다. 멤버에 대 한 자세한 내용은 `mask` **설명 부분**을 참조 하십시오.
...
`iItem` `iSubItem` 구조체의 및 멤버 `LVITEM` 와 *Nitem* 및 *nitem* 매개 변수는 특성을 설정할 항목 및 하위 항목을 식별 합니다.

`mask`구조체의 멤버 `LVITEM` 와 *nmask* 매개 변수는 설정할 항목 특성을 지정 합니다.

- 멤버를 LVIF_TEXT `pszText` 하거나 *lpszItem* 매개 변수는 null로 끝나는 문자열의 주소입니다 `cchTextMax` . 멤버는 무시 됩니다.

- LVIF_STATE `stateMask` member 또는 *nStateMask* 매개 변수는 변경할 항목 상태를 지정 하 고 `state` member 또는 *nstate* 매개 변수는 해당 상태에 대 한 값을 포함 합니다.
...
- 영향을 받는 항목이 현재 보기 상태인 경우를 제외 하 고는 목록 뷰 컨트롤이 다시 그려지지 않습니다. LVSICF_NOINVALIDATEALL 이것은 기본값입니다.

- 목록 뷰 컨트롤을 LVSICF_NOSCROLL 항목 수가 변경 될 때 스크롤 위치가 변경 되지 않습니다.
...
이 멤버 함수는 SDKand에 설명 된 것 처럼 Win32 매크로 [ListView_SetItemCountEx](/windows/win32/api/commctrl/nf-commctrl-listview_setitemcountex)의 동작을 구현 하 여 가상 목록 뷰에만 호출 해야 합니다.
...
*pItemIndex*\
진행 항목을 설명 하는 [Lvitemindex](/windows/win32/api/commctrl/ns-commctrl-lvitemindex) 구조체에 대 한 포인터입니다. 호출자는이 구조체를 할당 하 고 해당 멤버를 설정 해야 합니다.

*dwState*\
진행 항목을 설정 하는 상태입니다. [목록 뷰 항목 상태의](/windows/win32/Controls/list-view-item-states)비트 조합입니다. 다시 설정 하려면 0을 지정 하 고, 상태를 설정 하려면 1을 지정 합니다.

*dwMask*\
진행 *Dwstate* 매개 변수에 지정 된 상태의 유효한 비트 마스크입니다. [목록 뷰 항목 상태의](/windows/win32/Controls/list-view-item-states)비트 조합 (또는)을 지정 합니다.
...
이 메서드는 Windows SDK에서 설명 하는 [LVM_SETITEMINDEXSTATE](/windows/win32/Controls/lvm-setitemindexstate) 메시지를 보냅니다.
...
*pt*<br/>
항목의 왼쪽 위 모퉁이에 대 한 새 위치를 뷰 좌표로 지정 하는 [점](/windows/win32/api/windef/ns-windef-point) 구조입니다.
...
이 메서드는 LVS_OWNERDATA 창 스타일을 포함 하는 컨트롤에 사용 하기 위한 것이 아닙니다. 실제로는 디버그 빌드에서 어설션이 발생 합니다. 이 목록 컨트롤 스타일에 대 한 자세한 내용은 [목록 뷰 컨트롤 개요](/windows/win32/Controls/list-view-controls-overview)를 참조 하세요.
...
`COLORREF`윤곽선 색을 포함 하는 이전 구조입니다.

### <a name="remarks"></a>설명

이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_SETOUTLINECOLOR](/windows/win32/Controls/lvm-setoutlinecolor) 메시지의 기능을 에뮬레이트합니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_SETSELECTEDCOLUMN](/windows/win32/Controls/lvm-setselectedcolumn) 메시지의 기능을 에뮬레이트합니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_SETTILEINFO](/windows/win32/Controls/lvm-settileinfo) 메시지의 기능을 에뮬레이트합니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_SETTILEVIEWINFO](/windows/win32/Controls/lvm-settileviewinfo) 메시지의 기능을 에뮬레이트합니다.
...
*pWndTip*<br/>
목록 컨트롤에서 사용 하는 개체에 대 한 포인터입니다 `CToolTipCtrl` .
...
도구 설명을 사용 하지 않으려면 개체를 만들 때 LVS_NOTOOLTIPS 스타일을 지정 합니다 `CListCtrl` .
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_SETVIEW](/windows/win32/Controls/lvm-setview) 메시지의 기능을 에뮬레이트합니다.

## <a name="clistctrlsetworkareas"></a><a name="setworkareas"></a> CListCtrl:: Set작업 영역
...
*nWorkAreas*<br/>
`RECT` *LpRect*가 가리키는 배열의 구조체 (또는 [crect](../../atl-mfc-shared/reference/crect-class.md) 개체) 수입니다.

*lpRect*<br/>
`RECT` `CRect` 목록 뷰 컨트롤의 새 작업 영역을 지정 하는 구조체의 배열 (또는 개체)의 주소입니다. 이러한 영역은 클라이언트 좌표로 지정 해야 합니다. 이 매개 변수가 NULL 이면 작업 영역이 컨트롤의 클라이언트 영역으로 설정 됩니다.
...
이 멤버 함수는 Windows SDK에 설명 된 대로 [LVM_SORTGROUPS](/windows/win32/Controls/lvm-sortgroups) 메시지의 기능을 에뮬레이트합니다.
...
이 메서드는 Windows SDK에서 설명 하는 [LVM_SORTITEMS](/windows/win32/Controls/lvm-sortitems) 메시지를 보냅니다.
...
응용 프로그램 정의 비교 함수를 사용 하 여 현재 목록 뷰 컨트롤의 항목을 정렬 합니다.
...
*pfnCompare*\
진행 응용 프로그램 정의 비교 함수의 주소입니다. 정렬 작업은 두 목록 항목의 상대 순서를 결정 해야 할 때마다 비교 함수를 호출 합니다. 비교 함수는 클래스의 정적 멤버 이거나 클래스의 멤버가 아닌 독립 실행형 함수 여야 합니다.
...
이 메서드는 Windows SDK에서 설명 하는 [LVM_SORTITEMSEX](/windows/win32/Controls/lvm-sortitemsex) 메시지를 보냅니다.
...
또한이 함수는 LVS_AUTOARRANGE 스타일이 있는 경우 목록 뷰 컨트롤을 정렬 합니다.
