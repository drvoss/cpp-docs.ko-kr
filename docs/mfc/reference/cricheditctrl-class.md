...
|[CrichEditCtrl::GetText길이](#gettextlength)|이 `CRichEditCtrl` 개체에서 문자의 텍스트 길이를 검색합니다. 종료 null 문자는 포함되지 않습니다.|
...
|[리치에이트Ctrl::StreamIn](#streamin)|입력 스트림의 텍스트를 이 `CRichEditCtrl` 개체에 삽입합니다.|
...
이러한 메시지는 기본적으로 [OnNcCreate](../../mfc/reference/cwnd-class.md#onnccreate), [OnNcCalcSize](../../mfc/reference/cwnd-class.md#onnccalcsize), [OnCreate](../../mfc/reference/cwnd-class.md#oncreate)및 [OnGetMinMaxInfo](../../mfc/reference/cwnd-class.md#ongetminmaxinfo) 멤버 `CWnd` 함수에 의해 기본 클래스에서 처리됩니다. 기본 메시지 처리를 확장하려면 에서 `CRichEditCtrl`클래스를 파생합니다 . 예를 `OnCreate`들어 새 클래스에 필요한 초기화를 수행하려면 재정의합니다.
...
특정 장치에 대한 풍부한 편집 컨트롤에서 텍스트 범위의 서식을 지정합니다.
...
이 `CRichEditCtrl` 개체의 현재 텍스트 제한(바이트)입니다.
...
### <a name="example"></a>예제
  ...
  [GetLineCount에](#getlinecount)대한 예제를 참조하십시오.
...
실행 취소하거나 다시 사용할 수 있는 작업 유형에는 입력, 삭제, 끌어 놓기, 잘라내기 및 붙여넣기 작업이 포함됩니다. 이 정보는 실행 취소 및 다시 실행 가능한 작업의 드롭다운 목록 상자와 같은 실행 취소 및 다시 실행 작업에 대한 확장된 사용자 인터페이스를 제공하는 응용 프로그램에 유용할 수 있습니다.
...
지정된 줄 번호가 클 경우 *nLine* 또는 -1로 지정된 줄의 문자 인덱스는 편집 컨트롤의 줄 수입니다.
...
## <a name="cricheditctrlsetautourldetect"></a><a name="setautourldetect"></a>CrichEditCtrl::SetAutoURL검출
...
자세한 내용은 Windows SDK의 [EM_SETBKGNDCOLOR](/windows/win32/Controls/em-setbkgndcolor) 메시지 및 [COLORREF](/windows/win32/gdi/colorref) 구조를 참조하십시오.
...
- ECOOP_OR 현재 옵션을 *dwFlags*에 의해 지정된 옵션과 결합합니다.

- ECOOP_AND *dwFlags에*의해 지정된 현재 옵션만 유지합니다.

- ECOOP_XOR 논리적으로 배타적이거나 *dwFlags에*의해 지정된 옵션과 현재 옵션 .

*dwFlags*<br/>
풍부한 편집 옵션. 플래그 값은 비고 섹션에 나열됩니다.

### <a name="remarks"></a>설명

옵션은 다음 값의 조합일 수 있습니다.

- ECO_AUTOWORDSELECTION 두 번 클릭에 자동 단어 선택.

- ECO_AUTOVSCROLL 사용자가 줄 끝에 문자를 입력할 때 텍스트를 자동으로 10자로 오른쪽으로 스크롤합니다. 사용자가 ENTER 키를 누르면 컨트롤이 모든 텍스트를 다시 스크롤하여 0을 지정합니다.

- ECO_AUTOHSCROLL 사용자가 마지막 줄에서 ENTER 키를 누르면 자동으로 한 페이지 위로 텍스트를 스크롤합니다.

- ECO_NOHIDESEL 편집 컨트롤의 기본 동작을 무효화합니다. 기본 동작은 컨트롤이 입력 포커스를 잃을 때 선택을 숨기고 컨트롤이 입력 포커스를 수신할 때 선택을 표시합니다. ECO_NOHIDESEL 지정하면 컨트롤에 포커스가 없는 경우에도 선택한 텍스트가 반전됩니다.

- ECO_READONLY 사용자가 편집 컨트롤에서 텍스트를 입력하거나 편집하지 못하도록 합니다.

- ECO_WANTRETURN 사용자가 대화 상자에서 여러 줄의 풍부한 편집 컨트롤에 텍스트를 입력하는 동안 ENTER 키를 누를 때 캐리지 리턴이 삽입되도록 지정합니다. 이 스타일을 지정하지 않으면 ENTER 키를 누르면 부모 창의 기본 단추(예: 대화 상자의 확인 단추)를 클릭하는 것을 모방한 풍부한 편집 컨트롤의 부모 창에 명령을 보냅니다. 이 스타일은 한 줄 편집 컨트롤에는 영향을 주지 않습니다.

- ECO_SAVESEL 컨트롤이 포커스를 잃을 때 선택 영역을 유지합니다. 기본적으로 컨트롤의 전체 내용은 포커스를 되찾을 때 선택됩니다.

- ECO_VERTICAL 텍스트와 객체를 세로 방향으로 그립니다. 아시아 언어에서만 사용할 수 있습니다.

자세한 내용은 Windows SDK의 [EM_SETOPTIONS](/windows/win32/Controls/em-setoptions) 참조하세요.
...
이 `CRichEditCtrl` 개체의 현재 선택 영역에 대한 단락 서식 지정 특성을 설정합니다.
...
*Pf*<br/>
첫 번째 버전에서는 새 기본 단락 서식 지정 특성을 포함하는 [PARAFORMAT](/windows/win32/api/richedit/ns-richedit-paraformat) 구조에 대한 포인터입니다.
...
자세한 내용은 Windows SDK의 `PARAFORMAT` `PARAFORMAT2` [EM_SETPARAFORMAT](/windows/win32/Controls/em-setparaformat) 메시지 및 구조(및 구조)를 참조하십시오.

### <a name="example"></a>예제

[!code-cpp[NVC_MFC_CRichEditCtrl#28](../../mfc/reference/codesnippet/cpp/cricheditctrl-class_28.cpp)]

## <a name="cricheditctrlsetpunctuation"></a><a name="setpunctuation"></a>CRichEditCtrl::설정대기

풍부한 편집 컨트롤에서 문장 부호를 설정합니다.
...
*Ftype*<br/>
문장 부호 플래그입니다. 가능한 값 목록은 Windows SDK에서 [EM_SETPUNCTUATION](/windows/win32/Controls/em-setpunctuation) 대한 *fType* 매개 변수를 참조하십시오.
...
## <a name="cricheditctrlsetreadonly"></a><a name="setreadonly"></a>CRichEditCtrl:::SetRead전용

이 `CRichEditCtrl` 개체의 ECO_READONLY 옵션을 변경합니다.
...
*bReadOnly*<br/>
이 `CRichEditCtrl` 개체만 읽어야 하는지 나타냅니다.
...
이 옵션에 대한 간략한 설명은 [SetOptions](#setoptions)을 참조하십시오. 이 함수를 사용하여 이 `CRichEditCtrl` 개체의 모든 옵션을 설정할 수 있습니다.
...
## <a name="cricheditctrlsetrect"></a><a name="setrect"></a>리치에이트Ctrl:::세트렉트
...
*Lprect*<br/>
서식 지정 사각형에 대 한 새 경계를 나타내는 [RECT에](/windows/win32/api/windef/ns-windef-rect) 대 한 [CRect](../../atl-mfc-shared/reference/crect-class.md) 또는 포인터입니다.

### <a name="remarks"></a>설명

서식 지정 사각형은 텍스트의 제한 사각형입니다. 제한 사각형은 리치 편집 컨트롤 창의 크기와 독립적입니다. 이 `CRichEditCtrl` 개체를 처음 만들 때 서식 지정 사각형은 창의 클라이언트 영역과 크기가 같습니다. 서식 지정 사각형을 리치 편집 창보다 크거나 작게 만드는 데 사용합니다. `SetRect`
...
## <a name="cricheditctrlsetsel"></a><a name="setsel"></a>리치에이트Ctrl::SetSel
...
*nStartChar*<br/>
선택 영역에 대한 첫 번째 문자의 0기반 인덱스입니다.

*nEndChar*<br/>
선택 영역의 마지막 문자의 0기반 인덱스입니다.

*Cr*<br/>
현재 선택 영역의 경계를 보유하는 [CHARRANGE](/windows/win32/api/richedit/ns-richedit-charrange) 구조입니다.

### <a name="remarks"></a>설명

이 함수의 두 가지 형식은 선택 영역의 경계를 설정하는 대체 방법을 제공합니다. 이러한 양식에 대한 간략한 설명은 다음과 같습니다.

- **세트셀 ()** `cr` **)** 이 양식은 `CHARRANGE` 해당 `cpMin` 및 `cpMax` 멤버가 있는 구조를 사용하여 경계를 설정합니다.

- **세트 셀** `nStartChar` **()** `nEndChar` **)** 이 양식은 *nStartChar* 및 *nEndChar* 매개 변수를 사용하여 경계를 설정합니다.

캐터는 시작(또는`cpMin` *nStartChar)* 및 끝(또는`cpMax` *nEndChar)* 인덱스로 표시된 선택 영역의 끝에 배치됩니다. 이 함수는 카를트에 `CRichEditCtrl` 표시되도록 의 내용을 스크롤합니다.

이 `CRichEditCtrl` 개체의 모든 텍스트를 선택하려면 시작 인덱스 0과 끝 인덱스 -1을 사용하여 호출합니다. `SetSel`

자세한 내용은 Windows SDK의 [EM_EXSETSEL](/windows/win32/Controls/em-exsetsel) 메시지 및 [CHARRANGE](/windows/win32/api/richedit/ns-richedit-charrange) 구조를 참조하십시오.

### <a name="example"></a>예제

  [GetSel에](#getsel)대한 예제를 참조하십시오.

## <a name="cricheditctrlsetselectioncharformat"></a><a name="setselectioncharformat"></a>CrichEditCtrl::세트 선택차형식

이 `CRichEditCtrl` 개체의 현재 선택 항목에서 텍스트에 대한 문자 서식 특성을 설정합니다.
...
*Cf*<br/>
첫 번째 버전에서는 현재 선택 영역에 대한 새 문자 서식 지정 특성을 포함하는 [CHARFORMAT](/windows/win32/api/richedit/ns-richedit-charformata) 구조에 대한 포인터입니다.

두 번째 버전에서는 현재 선택 영역에 대한 새 문자 서식 특성을 포함하는 `CHARFORMAT` 리치 편집 2.0 구조의 [CHARFORMAT2](/windows/win32/api/richedit/ns-richedit-charformat2w) 구조에 대한 포인터입니다.
...
자세한 내용은 Windows SDK의 `CHARFORMAT2` [EM_SETCHARFORMAT](/windows/win32/Controls/em-setcharformat) `CHARFORMAT` 및 구조를 참조하십시오.
...
이 `CRichEditCtrl` 개체의 서식 지정에 WYSIWYG에 사용되는 대상 장치 및 줄 너비를 설정합니다.
...
*Hdc*<br/>
새 대상 장치에 대한 장치 컨텍스트를 처리합니다.

*l라인폭*<br/>
서식 지정에 사용할 줄 너비입니다.

*Dc*<br/>
새 대상 장치에 대한 [CDC입니다.](../../mfc/reference/cdc-class.md)
...
이 함수가 성공하면 리치 편집 컨트롤은 매개 변수로 전달된 장치 컨텍스트를 소유합니다. 이 경우 호출 함수는 장치 컨텍스트를 삭제해서는 안 됩니다.

자세한 내용은 Windows SDK의 [EM_SETTARGETDEVICE](/windows/win32/Controls/em-settargetdevice) 참조하세요.
...
풍부한 편집 컨트롤을 위해 텍스트 모드를 설정하거나 레벨을 취소하고 다시 재생합니다.
...
*f 모드*<br/>
컨트롤의 텍스트 모드에 대한 새 설정을 지정하고 레벨 매개 변수를 취소합니다. 가능한 값 목록은 Windows SDK에서 [EM_SETTEXTMODE](/windows/win32/Controls/em-settextmode) 위한 모드 매개 변수를 참조하십시오.

### <a name="return-value"></a>Return Value

성공하면 0, 그렇지 않으면 0이 아닙니다.

### <a name="remarks"></a>설명

텍스트 모드에 대한 설명은 Windows SDK의 EM_SETTEXTMODE 참조하세요.

컨트롤에 텍스트가 포함된 경우 이 멤버 함수가 실패합니다. 컨트롤이 비어 있는지 확인하려면 빈 문자열이 있는 [WM_SETTEXT](/windows/win32/winmsg/wm-settext) 메시지를 보냅니다.

## <a name="cricheditctrlsetundolimit"></a><a name="setundolimit"></a>리치에이트Ctrl:::세툰도 한계

취소 대기열에 저장할 수 있는 최대 작업 수를 설정합니다.
...
*nLimit*<br/>
취소 대기열에 저장할 수 있는 최대 작업 수를 지정합니다. 실행을 사용하지 않도록 설정하려면 0으로 설정합니다.

### <a name="return-value"></a>Return Value

리치 편집 컨트롤에 대한 새로운 최대 취소 작업 수입니다.

### <a name="remarks"></a>설명

기본적으로 취소 큐의 최대 작업 수는 100개입니다. 이 수를 늘리면 새 숫자를 수용할 수 있는 충분한 사용 가능한 메모리가 있어야 합니다. 성능을 향상하려면 제한을 가능한 가장 작은 값으로 설정합니다.

## <a name="cricheditctrlsetwordcharformat"></a><a name="setwordcharformat"></a>CrichEditCtrl::SetWordCharFormat

이 `CRichEditCtrl` 개체에서 현재 선택된 단어에 대한 문자 서식 지정 특성을 설정합니다.
...
*Cf*<br/>
첫 번째 버전에서는 현재 선택된 단어에 대한 새 문자 서식 지정 특성을 포함하는 [CHARFORMAT](/windows/win32/api/richedit/ns-richedit-charformata) 구조에 대한 포인터입니다.

두 번째 버전에서는 현재 선택한 단어에 대한 새 문자 서식 특성을 `CHARFORMAT` 포함하는 리치 편집 2.0 구조의 [CHARFORMAT2](/windows/win32/api/richedit/ns-richedit-charformat2w) 구조에 대한 포인터입니다.
...
## <a name="cricheditctrlsetwordwrapmode"></a><a name="setwordwrapmode"></a>리치에이트Ctrl::세트워드랩모드

풍부한 편집 컨트롤에 대한 단어 줄 바꿈 및 단어 분리 옵션을 설정합니다.
...
*uFlags*<br/>
단어 줄 바꿈 및 단어 나누기에 대해 설정하는 옵션입니다. 가능한 옵션 목록은 Windows SDK의 [EM_SETWORDWRAPMODE](/windows/win32/Controls/em-setwordwrapmode) 참조하세요.
...
## <a name="cricheditctrlstopgrouptyping"></a><a name="stopgrouptyping"></a>CRichEditCtrl::정지 그룹 입력
...
컨트롤은 다음 입력 작업(있는 경우)을 실행 취소 큐의 새 작업에 저장합니다.

자세한 내용은 Windows SDK의 [EM_STOPGROUPTYPING](/windows/win32/Controls/em-stopgrouptyping) 참조하세요.

## <a name="cricheditctrlstreamin"></a><a name="streamin"></a>리치에이트Ctrl::StreamIn

이 `CRichEditCtrl` 개체의 텍스트를 지정된 입력 스트림의 텍스트로 바꿉습니다.
...
*nFormat*<br/>
입력 데이터 형식을 지정하는 플래그입니다. 자세한 내용은 주의 섹션을 참조하십시오.

*es*<br/>
입력 스트림을 지정하는 [편집스트림](/windows/win32/api/richedit/ns-richedit-editstream) 구조입니다. 자세한 내용은 주의 섹션을 참조하십시오.
...
- SF_TEXT 텍스트만 읽는 것을 나타냅니다.

- SF_RTF 텍스트 읽기 및 서식을 나타냅니다.

이러한 값 중 하나를 SFF_SELECTION 함께 사용할 수 있습니다. SFF_SELECTION 지정하면 `StreamIn` 현재 선택 영역을 입력 스트림의 내용으로 바꿉습니다. 지정되지 않은 경우 `StreamIn` 이 `CRichEditCtrl` 개체의 전체 내용을 대체합니다.

`EDITSTREAM` 매개 변수 *es에서는*버퍼를 텍스트로 채우는 콜백 함수를 지정합니다. 이 콜백 함수는 입력 스트림이 소진될 때까지 반복적으로 호출됩니다.

자세한 내용은 Windows SDK의 [EM_STREAMIN](/windows/win32/Controls/em-streamin) 메시지 및 [EDITSTREAM](/windows/win32/api/richedit/ns-richedit-editstream) 구조를 참조하십시오.
...
## <a name="cricheditctrlstreamout"></a><a name="streamout"></a>CRichEditCtrl::스트림아웃

이 `CRichEditCtrl` 개체의 내용을 지정된 출력 스트림에 씁니다.
...
*nFormat*<br/>
출력 데이터 형식을 지정하는 플래그입니다. 자세한 내용은 주의 섹션을 참조하십시오.

*es*<br/>
출력 스트림을 지정하는 [편집스트림](/windows/win32/api/richedit/ns-richedit-editstream) 구조입니다. 자세한 내용은 주의 섹션을 참조하십시오.
...
- SF_TEXT 텍스트만 쓰는 것을 나타냅니다.

- SF_RTF 텍스트 작성 및 서식을 나타냅니다.

- SF_RTFNOOBJS OLE 항목을 공백으로 대체하는 텍스트 및 서식 쓰기를 나타냅니다.

- SF_TEXTIZED OLE 항목의 텍스트 표현과 함께 텍스트 작성 및 서식을 나타냅니다.

이러한 값은 SFF_SELECTION 함께 사용할 수 있습니다. SFF_SELECTION 지정하면 `StreamOut` 현재 선택 영역을 출력 스트림에 기록합니다. 지정되지 않은 경우 `StreamOut` 이 `CRichEditCtrl` 개체의 전체 내용을 기록합니다.

`EDITSTREAM` 매개 변수 *es에서는*버퍼를 텍스트로 채우는 콜백 함수를 지정합니다. 이 콜백 함수는 출력 스트림이 소진될 때까지 반복적으로 호출됩니다.

자세한 내용은 Windows SDK의 [EM_STREAMOUT](/windows/win32/Controls/em-streamout) 메시지 및 [EDITSTREAM](/windows/win32/api/richedit/ns-richedit-editstream) 구조를 참조하십시오.
...
## <a name="cricheditctrlundo"></a><a name="undo"></a>CRichEditCtrl:::취소

리치 편집 컨트롤에서 마지막 작업을 취소합니다.
...
취소 작업이 성공하면 0이 아닙니다. 그렇지 않으면 0입니다.
