...
|[CView:: System.windows.uielement.ondragenter](#ondragenter)|항목을 뷰의 끌어서 놓기 영역으로 처음 끌 때 호출 됩니다.|
|[CView:: System.windows.uielement.ondragleave](#ondragleave)|끌어 온 항목이 뷰의 끌어서 놓기 영역을 벗어나면 호출 됩니다.|
|[CView:: System.windows.uielement.ondragover](#ondragover)|뷰의 끌어서 놓기 영역 위로 항목을 끌 때 호출 됩니다.|
...
|[CView:: OnPrepareDC](#onpreparedc)|화면 표시에 대해 `OnDraw` 멤버 함수를 호출 하거나 인쇄 또는 인쇄 미리 보기에 대해 `OnPrint` 멤버 함수를 호출 하기 전에 호출 됩니다.|
...
문서의 데이터가 변경 되 면 일반적으로 변경 내용을 담당 하는 뷰가 문서에 대해 [CDocument:: UpdateAllViews](../../mfc/reference/cdocument-class.md#updateallviews) 함수를 호출 하 여 각각에 대해 `OnUpdate` 멤버 함수를 호출 하 여 다른 모든 뷰에 알립니다. `OnUpdate`의 기본 구현에서는 뷰의 전체 클라이언트 영역을 무효화 합니다. 이를 재정의 하 여 문서의 수정 된 부분에 매핑되는 클라이언트 영역의 영역만 무효화할 수 있습니다.

`CView`를 사용 하려면 클래스에서 클래스를 파생 시키고 화면 표시를 수행 하는 `OnDraw` 멤버 함수를 구현 합니다. `OnDraw`를 사용 하 여 인쇄 및 인쇄 미리 보기를 수행할 수도 있습니다. 프레임 워크는 문서 인쇄 및 미리 보기에 대 한 인쇄 루프를 처리 합니다.
...
`CScrollView`외에도 MFC 라이브러리는 `CView`에서 파생 된 9 개의 다른 클래스를 제공 합니다.
...
- [Ceditview](../../mfc/reference/ceditview-class.md)-간단한 여러 줄 텍스트 편집기를 제공 하는 뷰입니다. `CEditView` 개체를 대화 상자에서 컨트롤로 사용 하 고 문서에 대 한 보기를 사용할 수 있습니다.
...
`CView` 클래스에는 프레임 워크에서 인쇄 미리 보기를 수행 하는 데 사용 되는 `CPreviewView`파생 된 구현 클래스도 있습니다. 이 클래스는 도구 모음, 단일 또는 이중 페이지 미리 보기, 확대/축소, 미리 보기 된 이미지를 확대 하는 등 인쇄 미리 보기 창에 고유한 기능에 대 한 지원을 제공 합니다. 인쇄 미리 보기 모드에서 편집을 지원 하려는 경우와 같이 인쇄 미리 보기에 대해 고유한 인터페이스를 구현 하려는 경우가 아니면 `CPreviewView`의 멤버 함수를 호출 하거나 재정의할 필요가 없습니다. `CView`사용에 대 한 자세한 내용은 [문서/뷰 아키텍처](../../mfc/document-view-architecture.md) 및 [인쇄](../../mfc/printing.md)를 참조 하세요. 또한 인쇄 미리 보기 사용자 지정에 대 한 자세한 내용은 [기술 참고 사항 30](../../mfc/tn030-customizing-printing-and-print-preview.md) 을 참조 하십시오.
...
- 프레임 창이 비활성화 되 WA_INACTIVE입니다.

- 예를 들어 키보드 인터페이스를 사용 하 여 창을 선택 하는 등의 방법으로 마우스 클릭 이외의 방법으로 프레임 창을 활성화 하는 WA_ACTIVE.

- 마우스를 클릭 하 여 프레임 창을 활성화 하는 WA_CLICKACTIVE
...
응용 프로그램의 주 프레임 창이 활성 뷰를 변경 하지 않고 활성화 된 경우 (예: 응용 프로그램 내에서 한 뷰가 다른 응용 프로그램에서 다른 응용 프로그램으로 포커스가 전송 되는 경우 또는 MDI 자식 창 간에 전환 하는 경우)에는 *pactivateview* 및 *pdeactiveview* 매개 변수가 동일한 뷰를 가리킵니다. 이렇게 하면 필요한 경우 뷰가 해당 팔레트를 다시 인식할 수 있습니다.
...
*dwKeyState*<br/>
보조키의 상태를 포함 합니다. MK_CONTROL, MK_SHIFT, MK_ALT, MK_LBUTTON, MK_MBUTTON 및 MK_RBUTTON의 조합입니다.
...
- MK_CONTROL &#124; MK_SHIFT에 대 한 DROPEFFECT_LINK는 개체와 해당 서버 간의 링크를 만듭니다.
...
기본 구현은 아무 작업도 수행 하지 않고 DROPEFFECT_NONE을 반환 하는 것입니다.

[System.windows.uielement.ondragover](#ondragover) 멤버 함수에 대 한 이후 호출을 준비 하려면이 함수를 재정의 합니다. 나중에 `OnDragOver` 멤버 함수에서 사용할 수 있도록 데이터 개체에서 필요한 모든 데이터를 검색 해야 합니다. 또한 사용자에 게 시각적 피드백을 제공 하기 위해 뷰를 업데이트 해야 합니다. 자세한 내용은 [OLE 끌어서 놓기: 놓기 대상 구현](../../mfc/drag-and-drop-ole.md#implement-a-drop-target)문서를 참조 하세요.
...
- DROPEFFECT_SCROLL는 끌기 스크롤 작업이 대상 뷰에서 발생 하거나 발생 하 고 있음을 나타냅니다.
...
- DROPEFFECT_MOVE 데이터 개체를 현재 마우스 위치로 이동 합니다.
...
`OnDropEx` 재정의는 마우스 오른쪽 단추를 쿼리해야 합니다. [Getkeystate](/windows/win32/api/winuser/nf-winuser-getkeystate) 를 호출 하거나 [system.windows.uielement.ondragenter](#ondragenter) 처리기에서 오른쪽 마우스 단추 상태를 저장할 수 있습니다.
...
- 마우스 오른쪽 단추를 다운 하지 않으면 재정의에서이를 표준 drop 요청으로 처리 해야 합니다. *Dropdefault*에 지정 된 drop 효과를 사용 합니다. 또는 재정의에서 더미 값 (-1)을 반환 하 여 `OnDrop`이 drop 작업을 처리 함을 나타낼 수 있습니다.

*Pdataobject* 를 사용 하 여 클립보드 데이터 형식 및 지정 된 지점에서 삭제 된 데이터에 대 한 `COleDataObject`를 검사 합니다.
...
이 함수의 기본 구현은 [OnEndPrinting](#onendprinting) 멤버 함수를 호출 하 고 주 프레임 창을 인쇄 미리 보기가 시작 되기 전의 상태로 복원 합니다. 미리 보기 모드가 종료 될 때 특수 처리를 수행 하려면이 함수를 재정의 합니다. 예를 들어 미리 보기 모드에서 일반 디스플레이 모드로 전환할 때 문서에서 사용자의 위치를 유지 하려는 경우에는 *point* 매개 변수에서 설명 하는 위치로 스크롤하고 *pinfo* 매개 변수가 가리키는 `CPrintInfo` 구조체의 `m_nCurPage` 멤버로 스크롤할 수 있습니다.

일반적으로 함수 끝의 재정의에서 `OnEndPrintPreview`의 기본 클래스 버전을 호출 합니다.
...
*pInfo*<br/>
인쇄 또는 인쇄 미리 보기를 위해 `OnPrepareDC`를 호출 하는 경우 현재 인쇄 작업을 설명 하는 [Cprintinfo](../../mfc/reference/cprintinfo-structure.md) 구조체를 가리킵니다. `m_nCurPage` 멤버는 인쇄할 페이지를 지정 합니다. 화면 표시를 위해 `OnPrepareDC`를 호출 하는 경우이 매개 변수는 NULL입니다.
...
인쇄를 위해 함수가 호출 되는 경우 기본 구현은 *Pinfo* 매개 변수에 저장 된 페이지 정보를 검사 합니다. 문서의 길이가 지정 되지 않은 경우 `OnPrepareDC`는 문서를 한 페이지 길게 가정 하 고 한 페이지가 인쇄 된 후 인쇄 루프를 중지 합니다. 함수는 구조체의 `m_bContinuePrinting` 멤버를 FALSE로 설정 하 여 인쇄 루프를 중지 합니다.

다음과 같은 이유로 `OnPrepareDC`을 재정의 합니다.

- 지정 된 페이지에 대 한 필요에 따라 장치 컨텍스트의 특성을 조정 합니다. 예를 들어 장치 컨텍스트의 매핑 모드나 기타 특성을 설정 해야 하는 경우이 함수에서이 작업을 수행 합니다.

- 인쇄 시간 페이지 매김을 수행 하려면입니다. 일반적으로 인쇄를 시작할 때 [Onprepareprinting](#onprepareprinting) 멤버 함수를 사용 하 여 문서의 길이를 지정 합니다. 그러나 문서를 미리 알 수 없는 경우 (예: 데이터베이스에서 결정 되지 않은 레코드 수를 인쇄 하는 경우) 문서를 인쇄 하는 동안 문서 끝에 대 한 테스트를 `OnPrepareDC`를 재정의 합니다. 인쇄할 문서가 더 이상 없으면 `CPrintInfo` 구조의 `m_bContinuePrinting` 멤버를 FALSE로 설정 합니다.

- 페이지 별로 프린터에 이스케이프 코드를 전송 하는 것을 말합니다. `OnPrepareDC`에서 이스케이프 코드를 보내려면 *pDC* 매개 변수의 `Escape` 멤버 함수를 호출 합니다.

재정의를 시작할 때 `OnPrepareDC`의 기본 클래스 버전을 호출 합니다.
...
인쇄 및 인쇄 미리 보기를 사용 하도록 설정 하려면이 함수를 재정의 해야 합니다. [Doprepareprinting](#doprepareprinting) 멤버 함수를 호출 하 고 *pinfo* 매개 변수를 전달한 다음 반환 값을 반환 합니다. `DoPreparePrinting` 인쇄 대화 상자를 표시 하 고 프린터 장치 컨텍스트를 만듭니다. 기본값 이외의 값을 사용 하 여 인쇄 대화 상자를 초기화 하려면 *Pinfo*의 멤버에 값을 할당 합니다. 예를 들어 문서의 길이를 알고 있는 경우 `DoPreparePrinting`를 호출 하기 전에 *Pinfo* 의 [setmaxpage](../../mfc/reference/cprintinfo-structure.md#setmaxpage) 멤버 함수에 값을 전달 합니다. 이 값은 인쇄 대화 상자의 범위 부분에 있는 끝: 상자에 표시 됩니다.

`DoPreparePrinting` 미리 보기 작업에 대 한 인쇄 대화 상자를 표시 하지 않습니다. 인쇄 작업에 대 한 인쇄 대화 상자를 무시 하려면 *Pinfo* 의 `m_bPreview` 멤버가 FALSE 인지 확인 하 고 `DoPreparePrinting`에 전달 하기 전에 TRUE로 설정 합니다. 이후에 FALSE로 다시 설정 합니다.

프린터 장치 컨텍스트를 나타내는 `CDC` 개체에 액세스 해야 하는 초기화를 수행 해야 하는 경우 (예: 문서의 길이를 지정 하기 전에 페이지 크기를 확인 해야 하는 경우) `OnBeginPrinting` 멤버 함수를 재정의 합니다.

`m_nNumPreviewPages`의 값 또는 *Pinfo* 매개 변수의 `m_strPageDesc` 멤버를 설정 하려면 `DoPreparePrinting`를 호출한 후이 작업을 수행 합니다. `DoPreparePrinting` 멤버 함수는 `m_nNumPreviewPages`를 응용 프로그램의에 있는 값으로 설정 합니다. INI 파일을 `m_strPageDesc`를 기본값으로 설정 합니다.

### <a name="example"></a>예제

  프레임 워크에서 인쇄 대화 상자를 표시 하 고 프린터 DC를 만들도록 재정의 `OnPreparePrinting`를 재정의 하 고 재정의에서 `DoPreparePrinting`를 호출 합니다.

[!code-cpp[NVC_MFCDocView#184](../../mfc/codesnippet/cpp/cview-class_2.cpp)]

문서에 포함 된 페이지 수를 알고 있는 경우 `DoPreparePrinting`를 호출 하기 전에 `OnPreparePrinting`의 최대 페이지를 설정 합니다. 프레임 워크에서는 인쇄 대화 상자의 "대상" 상자에 최대 페이지 번호를 표시 합니다.
...
*pInfo*<br/>
현재 인쇄 작업을 설명 하는 `CPrintInfo` 구조체를 가리킵니다.

### <a name="remarks"></a>설명

출력 되는 각 페이지에 대해 프레임 워크는 [Onpreparedc](#onpreparedc) 멤버 함수를 호출한 직후이 함수를 호출 합니다. 인쇄 되는 페이지는 *Pinfo* 가 가리키는 [cprintinfo](../../mfc/reference/cprintinfo-structure.md) 구조의 `m_nCurPage` 멤버로 지정 됩니다. 기본 구현에서는 [OnDraw](#ondraw) 멤버 함수를 호출 하 고이를 프린터 장치 컨텍스트로 전달 합니다.

다음과 같은 이유로이 함수를 재정의 합니다.

- 다중 페이지 문서 인쇄를 허용 합니다. 현재 인쇄 중인 페이지에 해당 하는 문서 부분만 렌더링 합니다. `OnDraw`를 사용 하 여 렌더링을 수행 하는 경우 문서의 적절 한 부분만 인쇄 되도록 뷰포트 원본을 조정할 수 있습니다.

- 인쇄 된 이미지가 화면 이미지와 다르게 보이도록 (즉, 응용 프로그램이 WYSIWYG가 아닌 경우). `OnDraw`에 프린터 장치 컨텍스트를 전달 하는 대신 장치 컨텍스트를 사용 하 여 화면에 표시 되지 않은 특성을 사용 하 여 이미지를 렌더링 합니다.
...
- 머리글 또는 바닥글을 구현 합니다. 그래도 `OnDraw`를 사용 하 여 인쇄할 수 있는 영역을 제한 하 여 렌더링을 수행할 수 있습니다.

*Pinfo* 매개 변수의 `m_rectDraw` 멤버는 논리적 단위로 페이지의 인쇄 가능한 영역을 설명 합니다.

`OnPrint`재정의에서 `OnPrepareDC`를 호출 하지 마십시오. 프레임 워크는 `OnPrint`를 호출 하기 전에 `OnPrepareDC`를 자동으로 호출 합니다.

### <a name="example"></a>예제

다음은 재정의 된 `OnPrint` 함수에 대 한 기본 구조입니다.
...
이 메서드의 기본 구현에서는 뷰가 변경 되지 않지만이를 호출 하지 않는 경우에는 뷰가 `CScrollView`파생 클래스에서 스크롤되지 않습니다.

문서 너비 또는 높이가 32767 픽셀을 초과 하는 경우 `OnScrollBy`가 잘못 된 *sizeScroll* 인수를 사용 하 여 호출 되므로 32767 이전 스크롤은 실패 합니다.
...
*Lhint*를 사용 하려면 특정 힌트 값 (일반적으로 비트 마스크 또는 열거 형식)을 정의 하 고 문서에서 다음 값 중 하나를 전달 하도록 합니다. *Phint*를 사용 하려면 [CObject](../../mfc/reference/cobject-class.md) 에서 힌트 클래스를 파생 시키고 문서에서 힌트 개체에 대 한 포인터를 전달 하도록 합니다. `OnUpdate`를 재정의할 때 [CObject:: IsKindOf](../../mfc/reference/cobject-class.md#iskindof) 멤버 함수를 사용 하 여 힌트 개체의 런타임 형식을 확인 합니다.

일반적으로 `OnUpdate`에서 직접 그리기를 수행 하면 안 됩니다. 대신, 장치 좌표에서 업데이트를 필요로 하는 영역을 설명 하는 사각형을 결정 합니다. 이 사각형을 [CWnd:: InvalidateRect](../../mfc/reference/cwnd-class.md#invalidaterect)에 전달 합니다. 이렇게 하면 다음에 [WM_PAINT](/windows/win32/gdi/wm-paint) 메시지가 수신 될 때 그리기가 발생 합니다.
