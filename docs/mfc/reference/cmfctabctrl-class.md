...
ms.locfileid: "78883676"
---
# <a name="cmfctabctrl-class"></a>CMFCTabCtrl Class

`CMFCTabCtrl` 클래스는 탭 컨트롤에 대 한 기능을 제공 합니다. 탭 컨트롤은 맨 위 또는 아래에 평면 또는 3차원 탭이 포함된 도킹 가능한 창을 표시합니다. 탭은 텍스트와 이미지를 표시하고 활성화된 경우 색을 변경할 수 있습니다.
...
|[CMFCTabCtrl:: CalcRectEdit](#calcrectedit)|지정 된 탭 영역의 크기를 수축 합니다. (`CMFCBaseTabCtrl::CalcRectEdit`을 재정의함)|
|[CMFCTabCtrl:: Create](#create)|탭 컨트롤을 만들어 `CMFCTabCtrl` 개체에 연결 합니다.|
...
|[CMFCTabCtrl:: Enabletab문서 메뉴](#enabletabdocumentsmenu)|탭 창의 메뉴를 여는 단추를 사용 하 여 창 탭을 스크롤 하는 두 개의 단추를 바꿉니다.|
...
|[CMFCTabCtrl:: HideInactiveWindow](#hideinactivewindow)|프레임 워크가 비활성 탭 제어 창을 표시할지 여부를 지정 합니다.|
...
|[CMFCTabCtrl:: Istab문서 메뉴](#istabdocumentsmenu)|탭 컨트롤에서 스크롤 단추를 표시할지 아니면 탭 창의 메뉴를 표시 하는 단추를 표시할지를 나타냅니다.|
...
|[CMFCTabCtrl:: System.windows.uielement.ondragenter](#ondragenter)|커서를 탭 컨트롤 창으로 처음 끌 때 프레임 워크에서 호출 됩니다.|
|[CMFCTabCtrl:: System.windows.uielement.ondragover](#ondragover)|놓기 대상 창 위로 마우스를 이동할 때 끌기 작업을 수행 하는 동안 프레임 워크에서 호출 됩니다. [CMFCBaseTabCtrl:: system.windows.uielement.ondragover](../../mfc/reference/cmfcbasetabctrl-class.md#ondragover)를 재정의 합니다.|
|[CMFCTabCtrl:: Onshowtab문서 메뉴](#onshowtabdocumentsmenu)|탭 창의 팝업 메뉴를 표시 하 고 사용자가 탭을 선택할 때까지 대기한 다음 선택 된 탭을 활성 탭으로 만듭니다.|
...
|[CMFCTabCtrl:: SetActiveTab](#setactivetab)|탭을 활성화 합니다. ( [CMFCBaseTabCtrl:: SetActiveTab](../../mfc/reference/cmfcbasetabctrl-class.md#setactivetab)를 재정의 합니다.)|
...
|[CMFCTabCtrl:: Seon Agelist](#setimagelist)|이미지 목록을 지정 합니다. [CMFCBaseTabCtrl:: Se Agelist](../../mfc/reference/cmfcbasetabctrl-class.md#setimagelist)를 재정의 합니다.|
...
`CMFCTabCtrl` 클래스는 대화 상자와 함께 사용할 수 있지만, Microsoft Excel 및 Visual Studio와 같은 도킹 컨트롤 막대를 사용 하는 응용 프로그램을 위한 것입니다. 자세한 내용은 [CDockablePane 클래스](../../mfc/reference/cdockablepane-class.md)를 참조 하세요.
...
도킹 컨트롤 막대로 탭 창을 만드는 방법에 대 한 예제는 [CTabbedPane 클래스](../../mfc/reference/ctabbedpane-class.md)를 참조 하세요. 도킹 되지 않은 컨트롤로 `CMFCTabCtrl`을 사용 하려면 `CMFCTabCtrl` 개체를 만든 다음 [Cmfctabctrl:: create](#create)를 호출 합니다.
...
다음 예제에서는 `CMFCTabCtrl` 클래스에서 다양 한 메서드를 사용 하 여 `CMFCTabCtrl` 개체를 구성 하는 방법을 보여 줍니다. 이 예에서는 탭을 추가 하 고, 활성 탭에서 닫기 단추를 표시 하 고, 편집 가능한 탭 레이블을 사용 하도록 설정 하 고, 탭 창 레이블의 팝업 메뉴를 표시 하는 방법을 설명 합니다. 이 예제는 [상태 컬렉션 샘플](../../overview/visual-cpp-samples.md)의 일부입니다.
...
탭 컨트롤을 만들어 `CMFCTabCtrl` 개체에 연결 합니다.
...
다음 예제에서는 `CMFCTabCtrl` 클래스에서 `Create` 메서드를 사용 하는 방법을 보여 줍니다. 이 예제는 [상태 컬렉션 샘플](../../overview/visual-cpp-samples.md)의 일부입니다.
...
##  <a name="enabletabdocumentsmenu"></a>CMFCTabCtrl:: Enabletab문서 메뉴
...
탭 컨트롤의 크기를 조정할 수 있는 방법을 지정 하는 `CMFCTabCtrl::ResizeMode` 열거형 값 중 하나입니다. 사용할 수 있는 값 목록은 [Cmfctabctrl:: SetResizeMode](#setresizemode) 메서드의 설명 섹션을 참조 하세요.
...
스크롤 막대 개체에 대 한 포인터 이거나, STYLE_FLAT_SHARED_HORZ_SCROLL 스타일을 사용 하 여 탭 컨트롤을 만들지 않은 경우 NULL입니다.
...
[Cmfctabctrl:: Create](#create) 메서드의 *style* 매개 변수가 STYLE_FLAT_SHARED_HORZ_SCROLL 경우이 메서드는 TRUE를 반환 합니다.

##  <a name="istabdocumentsmenu"></a>CMFCTabCtrl:: Istab문서 메뉴
...
이 탭은 일반적으로 탭을 삽입 하 고 활성 상태로 만들 때 새 탭 창에서 가져옵니다. 원래 포커스를 유지 하려면 `CMFCTabCtrl::m_bEnableActivate` 멤버 변수를 FALSE로 설정 합니다. 기본값은 TRUE입니다.
...
*Style* 매개 변수 값은 다음 `CMFCTabCtrl::Style` 열거형 중 하나일 수 있습니다.
...
*dwKeyState*<br/>
진행 보조키의 상태를 포함 합니다. 이 매개 변수는 MK_CONTROL, MK_SHIFT, MK_ALT, MK_LBUTTON, MK_MBUTTON 및 MK_RBUTTON 값의 비트 조합 (OR)입니다. 자세한 내용은 [마우스 입력 정보](/windows/win32/inputdev/about-mouse-input)의 **메시지 매개 변수** 섹션을 참조 하세요.
...
기본적으로이 메서드는 항상 DROPEFFECT_NONE를 반환 하는 `CMFCTabCtrl::OnDragOver`만 호출 합니다.
...
*dwKeyState*<br/>
진행 MK_CONTROL, MK_SHIFT, MK_ALT, MK_LBUTTON, MK_MBUTTON 및 MK_RBUTTON의 비트 조합 (OR) 인 보조키의 상태입니다. 자세한 내용은 [마우스 입력 정보](/windows/win32/inputdev/about-mouse-input)의 "메시지 매개 변수"를 참조 하세요.
...
##  <a name="onshowtabdocumentsmenu"></a>CMFCTabCtrl:: Onshowtab문서 메뉴
...
그러면 `AFX_WM_CHANGE_ACTIVE_TAB` 알림이 부모 창과 탭 컨트롤의 부모 프레임으로 전송 됩니다.

`SetActiveTab` 메서드는 [Cmfctabctrl:: HideActiveWindowHorzScrollBar](#hideactivewindowhorzscrollbar) 메서드를 자동으로 호출 하 여 화면이 깜박이지 않도록 합니다.
...
##  <a name="setimagelist"></a>CMFCTabCtrl:: Seon Agelist
...
진행 탭 컨트롤의 크기를 조정할 수 있는 방법을 지정 하는 `CMFCTabCtrl::ResizeMode` 열거형 값 중 하나입니다. 가능한 값 목록은 설명의 표를 참조 하십시오.
...
기본 탭을 표시 하는 탭 컨트롤에 가로 스크롤 막대를 그립니다.
