...
범용 디스크 i/o에 `CFile` 및 파생 클래스를 사용 합니다. 디스크 파일에 전송 된 서식 있는 텍스트에 `ofstream` 또는 기타 Microsoft `iostream` 클래스를 사용 합니다.

일반적으로 디스크 파일은 `CFile` 생성 시 자동으로 열리고 소멸 될 때 닫힙니다. 정적 멤버 함수를 사용 하면 파일을 열지 않고 파일의 상태를 확인할 수 있습니다.

`CFile`사용에 대 한 자세한 내용은 *런타임 라이브러리 참조*에서 [MFC의 파일](../../mfc/files-in-mfc.md) 및 [파일 처리](../../c-runtime-library/file-handling.md) 문서를 참조 하세요.
...
예외를 처리할 때 `CFile::Abort`는 두 가지 중요 한 방식으로 `CFile::Close`와 다릅니다. 첫째, `Abort`에서 오류를 무시 하기 때문에 `Abort` 함수는 오류 발생 시 예외를 throw 하지 않습니다. 두 번째는 파일이 열려 있지 않거나 이전에 닫힌 경우 `Abort`를 **어설션** 하지 않습니다.

**New** 를 사용 하 여 힙에서 `CFile` 개체를 할당 한 경우 파일을 닫은 후에 삭제 해야 합니다. `Abort` 설정 `m_hFile` 에 `CFile::hFileNull`입니다.
...
기본 생성자는 멤버를 초기화 하지만 파일을 `CFile` 개체에 첨부 하지 않습니다. 이 생성자를 사용한 후에는 [CFile:: Open](#open) 메서드를 사용 하 여 파일을 열고 `CFile` 개체에 연결 합니다.
...
`CFile` 개체가 지정 된 파일을 성공적으로 열면 `CFile` 개체가 제거 될 때이 파일이 자동으로 닫힙니다. 그렇지 않으면 더 이상 `CFile` 개체에 연결 되지 않은 파일을 명시적으로 닫아야 합니다.
...
**New** 를 사용 하 여 힙에서 `CFile` 개체를 할당 한 경우 파일을 닫은 후에 삭제 해야 합니다. `Close` 설정 `m_hFile` 에 `CFile::hFileNull`입니다.
...
지정 된 파일에 대해 중복 된 `CFile` 개체를 생성 합니다.
...
`Flush`를 사용 하면 `CArchive` 버퍼 플러시가 보장 되지 않습니다. 보관 파일을 사용 하는 경우 먼저 [CArchive:: Flush](../../mfc/reference/carchive-class.md#flush) 를 호출 합니다.
...
예를 들어 `GetFileName`를 호출 하 여 파일 `c:\windows\write\myfile.wri`대 한 메시지를 사용자에 게 생성 하면 파일 이름 `myfile.wri`이 반환 됩니다.
...
예를 들어 `GetFilePath`를 호출 하 여 파일 `c:\windows\write\myfile.wri`대 한 메시지를 사용자에 게 생성 하면 파일 경로 `c:\windows\write\myfile.wri`이 반환 됩니다.
...
`Seek`에 대 한 이후 호출에서 사용할 수 있는 파일 포인터의 현재 값을 가져옵니다.
...
*rStatus*<br/>
상태 정보를 받을 사용자 제공 `CFileStatus` 구조체에 대 한 참조입니다. `CFileStatus` 구조체에는 다음과 같은 필드가 있습니다.

- 파일을 만든 날짜와 시간을 `CTime m_ctime` 합니다.

- 파일이 마지막으로 수정 된 날짜와 시간을 `CTime m_mtime` 합니다.

- 읽기 위해 파일에 마지막으로 액세스 한 날짜와 시간을 `CTime m_atime` 합니다.

- DIR 명령에 의해 보고 되는 파일의 논리적 크기 (바이트)를 `ULONGLONG m_size` 합니다.

- 파일의 특성 바이트를 `BYTE m_attribute` 합니다.

- Windows 문자 집합의 절대 파일 이름을 `char m_szFullName[_MAX_PATH]` 합니다.
...
`GetStatus`의 비정적 버전은 지정 된 `CFile` 개체와 연결 된 열린 파일의 상태 정보를 검색 합니다.  `GetStatus`의 정적 버전은 파일을 실제로 열지 않고 지정 된 파일 경로에서 파일 상태를 가져옵니다. 이 버전은 파일의 존재 여부 및 액세스 권한을 테스트 하는 데 유용 합니다.

`CFileStatus` 구조의 `m_attribute` 멤버는 파일 특성 집합을 참조 합니다. `CFile` 클래스는 **특성** 열거형 형식을 제공 하므로 파일 특성을 기호로 지정할 수 있습니다.
...
이 상수는 `CFile` 개체에 올바른 파일 핸들이 있는지 여부를 확인 하는 데 사용 됩니다.
...
`UnlockRange` 멤버 함수를 사용 하 여 영역의 잠금을 해제 하는 경우 바이트 범위는 이전에 잠긴 지역과 정확히 일치 해야 합니다. `LockRange` 함수는 인접 한 영역을 병합 하지 않습니다. 잠긴 두 지역은 인접 한 경우 각 지역의 잠금을 별도로 해제 해야 합니다.
...
`m_hFile`은 UINT 형식의 공용 변수입니다. 핸들이 할당 되지 않은 경우 운영 체제 독립적 빈 파일 지표로 `CFile::hFileNull`포함 되어 있습니다.

멤버의 의미가 파생 클래스에 따라 달라 지므로 `m_hFile`를 사용 하지 않는 것이 좋습니다. `m_hFile`은 클래스의 비 다형성 사용을 지원 하기 위해 편의를 위해 public 멤버로 설정 됩니다.
...
두 개의 `Open` 함수는 파일을 열기 위한 "안전한" 방법입니다. 여기서 오류는 정상적인 정상적인 조건입니다.

`CFile` 생성자는 오류 조건에서 예외를 throw 하지만 `Open`는 오류 조건에 대해 FALSE를 반환 합니다. 그러나 `Open`는 여전히 [Cfileexception](../../mfc/reference/cfileexception-class.md) 개체를 초기화 하 여 오류를 설명할 수 있습니다. *PError* 매개 변수를 제공 하지 않거나 *PERROR*에 대해 NULL을 전달 하는 경우 `Open` FALSE를 반환 하 고 `CFileException`을 throw 하지 않습니다. 기존 `CFileException`에 대 한 포인터를 전달 하 고 `Open` 오류가 발생 하면 함수는 해당 오류를 설명 하는 정보로이를 채웁니다. 어떤 경우에도 `Open` 예외를 throw 하지 않습니다.

다음 표에서는 `Open`의 가능한 결과에 대해 설명 합니다.
...
이 연산자를 사용 하 여 `CFile` 개체에 대 한 핸들을 [Readfileex](/windows/win32/api/fileapi/nf-fileapi-readfileex) 및 `HANDLE`를 필요로 하는 [getfiletime](/windows/win32/api/fileapi/nf-fileapi-getfiletime) 과 같은 함수에 전달 합니다.
...
`CFile` 개체와 연결 된 파일에서 버퍼로 데이터를 읽어옵니다.
...
`Remove` 멤버 함수는 연결 된 파일이 열려 있거나 파일을 제거할 수 없는 경우 예외를 throw 합니다. 이 함수는 DEL 명령과 동일 합니다.
...
이 함수를 호출 하 여 파일의 경로를 지정 합니다. 예를 들어, [CFile](../../mfc/reference/cfile-class.md) 개체가 생성 될 때 파일 경로를 사용할 수 없는 경우 `SetFilePath`를 호출 하 여 해당 경로를 제공 합니다.
...
> [!NOTE]
> `SetFilePath` 파일을 열거나 파일을 만들지 않습니다. 이 메서드는 단순히 `CFile` 개체를 경로 이름과 연결한 후 사용할 수 있습니다.
...
> [!NOTE]
>  `CMemFile`이 함수는 `CMemoryException` 개체를 throw 할 수 있습니다.
...
시간을 설정 하려면 *상태*`m_mtime` 필드를 수정 합니다.

파일의 특성만 변경 하려고 시도 하는 `SetStatus`에 대 한 호출을 수행 하 고 파일 상태 구조의 `m_mtime` 멤버가 0이 아닌 경우에는 특성도 영향을 받을 수 있습니다. 타임 스탬프를 변경 하면 특성에 부작용이 발생할 수 있습니다. 파일의 특성만 변경 하려면 먼저 파일 상태 구조의 `m_mtime` 멤버를 0으로 설정 하 고 `SetStatus`에 대 한 호출을 수행 합니다.
