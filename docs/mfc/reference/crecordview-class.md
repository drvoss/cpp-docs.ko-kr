...
|[CRecordView:: OnGetRecordset](#ongetrecordset)|에서 파생 된 클래스의 개체에 대 한 포인터를 반환 `CRecordset` 합니다. 클래스 마법사는 사용자를 위해이 함수를 재정의 하 고 필요한 경우 레코드 집합을 만듭니다.|
...
기본 구현에서는 `Move` 레코드 뷰와 연결 된 개체의 적절 한 멤버 함수를 호출 합니다 `CRecordset` .
