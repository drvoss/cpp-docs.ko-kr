...
|[CDaoQueryDef:: Close](#close)|Querydef 개체를 닫습니다. 완료 되 C++ 면 개체를 삭제 합니다.|
|[CDaoQueryDef:: Create](#create)|기본 DAO 쿼리 정의 개체를 만듭니다. 쿼리 정의를 임시 쿼리로 사용 하거나 `Append`을 호출 하 여 데이터베이스에 저장 합니다.|
...
1. 모든 경우에는 먼저 `CDaoQueryDef` 개체를 생성 하 고 쿼리가 속한 [CDaoDatabase](../../mfc/reference/cdaodatabase-class.md) 개체에 대 한 포인터를 제공 합니다.

1. 그런 다음 원하는 항목에 따라 다음을 수행 합니다.

   - 기존에 저장 된 쿼리를 사용 하려면 저장 된 쿼리의 이름을 제공 하 여 querydef 개체의 [Open](#open) 멤버 함수를 호출 합니다.

   - 새 저장 된 쿼리를 만들려면 쿼리 이름을 제공 하 여 querydef 개체의 [create](#create) member 함수를 호출 합니다. 그런 다음 [Append](#append) 를 호출 하 여 쿼리를 데이터베이스의 쿼리 정의 컬렉션에 추가 하 여 쿼리를 저장 합니다. `Create`는 쿼리 정의를 열린 상태로 전환 하므로 `Create`를 호출한 후에는 `Open`를 호출 하지 않습니다.

   - 임시 querydef를 만들려면 `Create`를 호출 합니다. 쿼리 이름에 대해 빈 문자열을 전달 합니다. `Append`를 호출하지 마세요.
...
- `CDaoRecordset` 개체를 만들려면

- 개체의 `Execute` 멤버 함수를 호출 하 여 동작 쿼리 또는 SQL 통과 쿼리를 직접 실행 하려면
...
Querydef 개체를 사용 하 여 `CDaoRecordset` 개체를 만들려면 일반적으로 위에서 설명한 대로 쿼리를 만들거나 엽니다. 그런 다음 [CDaoRecordset:: Open](../../mfc/reference/cdaorecordset-class.md#open)을 호출할 때 쿼리 정의 개체에 대 한 포인터를 전달 하 여 레코드 집합 개체를 생성 합니다. 전달 하는 querydef는 열린 상태 여야 합니다. 자세한 내용은 클래스 [CDaoRecordset](../../mfc/reference/cdaorecordset-class.md)를 참조 하세요.

열 정의를 사용 하 여 열린 상태가 아닌 경우에는 레코드 집합을 만들 수 없습니다 (쿼리 정의에 가장 일반적으로 사용 됨). `Open` 또는 `Create`를 호출 하 여 쿼리 정의를 열린 상태로 전환 합니다.
...
`Append` 데이터베이스의 쿼리 정의 컬렉션에 개체를 추가 하 여 데이터베이스에 쿼리 정의를 저장 합니다. Querydef를 추가 하지 않고 임시 개체로 사용할 수 있지만 유지 하려면 `Append`를 호출 해야 합니다.
...
- 개체가 임시 쿼리 정의 (데이터베이스에 저장 하지 않음)를 나타내는 경우 `Create`를 호출 하 고 쿼리 이름에 대해 빈 문자열을 전달 합니다. `Create`호출한 후에는 해당 특성을 직접 설정 하 여 querydef를 초기화 합니다. `Append`를 호출하지 마세요.
...
Querydef 개체가 완료 되 면 해당 [Close](#close) 멤버 함수를 호출 합니다. 쿼리 정의에 대 한 포인터가 있는 경우 **delete** 연산자를 사용 하 여 C++ 개체를 삭제 합니다.
...
Querydef를 닫으면 기본 DAO 개체가 해제 되지만 저장 된 DAO 쿼리 정의 개체 또는 C++ `CDaoQueryDef` 개체는 삭제 되지 않습니다. 이는 DAO의 데이터베이스의 쿼리 정의 컬렉션에서 쿼리를 삭제 하는 [CDaoDatabase::D eletequerydef](../../mfc/reference/cdaodatabase-class.md#deletequerydef)와 다릅니다 (임시 querydef가 아닌 경우).
...
*LpszSQL*에 SQL 문을 제공 하지 않은 경우에는 `Execute`를 사용 하 여 쿼리를 실행할 수 없지만이를 사용 하 여 레코드 집합을 만들 수 있습니다. 이 경우 MFC는 레코드 집합의 기본 SQL 문을 사용 합니다.
...
- 일관성 없는 업데이트를 `dbInconsistent` 합니다.

- 일관 된 업데이트를 `dbConsistent` 합니다.

- SQL 통과를 `dbSQLPassThrough` 합니다. SQL 문이 처리를 위해 ODBC 데이터베이스로 전달 되도록 합니다.
...
select 쿼리와 같이 레코드를 반환 하는 쿼리에는 `Execute` 작동 하지 않습니다. `Execute`은 **UPDATE**, **INSERT**또는 **SELECT INTO**와 같은 대량 작업 쿼리 또는 DDL (데이터 정의 언어) 작업에 일반적으로 사용 됩니다.
...
Querydef 개체의 [GetRecordsAffected](#getrecordsaffected) 멤버 함수를 호출 하 여 가장 최근의 `Execute` 호출의 영향을 받는 레코드 수를 확인 합니다. 예를 들어 `GetRecordsAffected`은 동작 쿼리를 실행할 때 삭제, 업데이트 또는 삽입 된 레코드 수에 대 한 정보를 반환 합니다. 연속 업데이트 또는 삭제가 적용 되는 경우 반환 되는 개수는 관련 테이블의 변경 내용을 반영 하지 않습니다.

`dbInconsistent`와 `dbConsistent`를 모두 포함 하거나 둘 다 포함 하지 않는 경우에는 결과가 기본값인 `dbInconsistent`됩니다.

`Execute`는 레코드 집합을 반환 하지 않습니다. 레코드를 선택 하는 쿼리에 `Execute`를 사용 하면 MFC에서 [CDaoException](../../mfc/reference/cdaoexception-class.md)형식의 예외를 throw 합니다.
...
이 함수는 ODBC 데이터 원본 및 특정 ISAM 드라이버 에서만 사용 됩니다. Microsoft Jet ()와 함께 사용 되지 않습니다. MDB) 데이터베이스 이 경우 `GetConnect`은 빈 문자열을 반환 합니다. 자세한 내용은 [Setconnect](#setconnect)를 참조 하세요.
...
`GetFieldCount`는 querydef의 모든 필드를 반복 하는 데 유용 합니다. 이러한 목적을 위해 [Getfieldinfo](#getfieldinfo)와 함께 `GetFieldCount`를 사용 합니다.
...
*fieldinfo*<br/>
요청 된 정보를 반환 하는 `CDaoFieldInfo` 개체에 대 한 참조입니다.
...
`GetParameterCount`는 쿼리 정의의 모든 매개 변수를 반복 하는 데 유용 합니다. 이러한 목적을 위해 [GetParameterInfo](#getparameterinfo)와 함께 `GetParameterCount`를 사용 합니다.
...
- `dbQSQLPassThrough`와 함께 사용 `dbQSPTBulk` 레코드를 반환 하지 않는 쿼리를 지정 합니다.

> [!NOTE]
>  SQL 통과 쿼리를 만들려면 `dbSQLPassThrough` 상수를 설정 하지 마십시오. 이는 querydef 개체를 만들고 연결 문자열을 설정할 때 Microsoft Jet 데이터베이스 엔진에 의해 자동으로 설정 됩니다.

SQL 문자열에 대 한 자세한 내용은 [Getsql](#getsql)을 참조 하세요. 쿼리 유형에 대 한 자세한 내용은 [Execute](#execute)를 참조 하십시오.

##  <a name="isopen"></a>CDaoQueryDef:: IsOpen

`CDaoQueryDef` 개체가 현재 열려 있는지 여부를 확인 하려면이 멤버 함수를 호출 합니다.
...
ODBC 데이터 원본에 대 한 SQL 통과 쿼리를 나타내는 querydef를 실행 하기 전에 `SetConnect` 연결 문자열을 설정 하 고 [SetReturnsRecords](#setreturnsrecords) 를 호출 하 여 쿼리에서 레코드를 반환할지 여부를 지정 합니다.
...
Querydef 이름은 고유한 사용자 정의 이름입니다. 쿼리 정의 개체를 쿼리 정의 컬렉션에 추가 하기 전에 `SetName`을 호출할 수 있습니다.
...
이 멤버 함수를 사용 하면 연결 된 데이터 원본에 대 한 후속 작업의 "제한 시간"을 기준으로 기본 시간 (초)을 재정의할 수 있습니다. 네트워크 액세스 문제, 과도 한 쿼리 처리 시간 등으로 인해 작업 시간이 초과 될 수 있습니다. 쿼리 제한 시간 값을 변경 하려는 경우이 querydef를 사용 하 여 쿼리를 실행 하기 전에 `SetODBCTimeout`를 호출 합니다. ODBC가 연결을 다시 사용할 때 동일한 연결의 모든 클라이언트에 대 한 시간 제한 값은 동일 합니다.
...
`COleVariant` 개체로 설정할 값을 지정 합니다. `COleVariant` 개체에서 원하는 값 및 형식을 설정 하는 방법에 대 한 자세한 내용은 클래스 [COleVariant](../../mfc/reference/colevariant-class.md)를 참조 하세요.
...
이 경우 쿼리 정의를 만들고 다른 `CDaoQueryDef` 멤버 함수를 사용 하 여 해당 속성을 설정 해야 합니다. 외부 데이터베이스에 대 한 설명은 [Setconnect](#setconnect)를 참조 하세요.
...
`SetSQL`의 일반적인 용도는 SQL 통과 쿼리에서 사용할 쿼리 정의 개체를 설정 하는 것입니다. 대상 DBMS에 대 한 SQL 통과 쿼리의 구문은 DBMS에 대 한 설명서를 참조 하세요.
