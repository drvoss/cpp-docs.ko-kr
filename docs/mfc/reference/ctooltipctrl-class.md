...
|[CToolTipCtrl:: CreateEx](#createex)|지정 된 Windows 확장 스타일을 사용 하 여 도구 설명 컨트롤을 만들고 `CToolTipCtrl` 개체에 연결 합니다.|
...
`CToolTipCtrl` 도구 설명의 초기 시간 및 기간, 도구 설명 텍스트를 둘러싸는 여백 너비, 도구 설명 창 자체의 너비, 도구 설명의 배경 및 텍스트 색을 제어 하는 기능을 제공 합니다. 단일 도구 설명 컨트롤은 둘 이상의 도구에 대 한 정보를 제공할 수 있습니다.

`CToolTipCtrl` 클래스는 Windows의 일반적인 도구 설명 컨트롤의 기능을 제공 합니다. 이 컨트롤과 `CToolTipCtrl` 클래스는 Windows 95/98 및 Windows NT 버전 3.51 이상에서 실행 되는 프로그램에만 사용할 수 있습니다.
...
`CToolTipCtrl`사용에 대 한 자세한 내용은 [컨트롤](../../mfc/controls-mfc.md) 및 [사용 CToolTipCtrl](../../mfc/using-ctooltipctrl.md)을 참조 하세요.
...
> [!NOTE]
>  `AddTool`를 사용 하 여 도구 설명을 정적 컨트롤로 설정할 수 없습니다.
...
*pParentWnd*<br/>
도구 설명 컨트롤의 부모 창 (일반적으로 `CDialog`)을 지정 합니다. NULL이 아니어야 합니다.
...
`CToolTipCtrl` 개체가 성공적으로 생성 되 면 0이 아닌 값입니다. 그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

`CToolTipCtrl`는 두 단계로 구성 합니다. 먼저 생성자를 호출 하 여 `CToolTipCtrl` 개체를 생성 한 다음 `Create`를 호출 하 여 도구 설명 컨트롤을 만들고 `CToolTipCtrl` 개체에 연결 합니다.
...
확장 된 windows 스타일을 사용 하 여 도구 설명 컨트롤을 만들려면 `Create`대신 [CToolTipCtrl:: CreateEx](#createex) 를 호출 합니다.
...
*lprc*<br/>
여백 정보를 수신 하는 `RECT` 구조체의 주소입니다. [RECT](/previous-versions/dd162897\(v=vs.85\)) 구조체의 멤버는 경계 사각형을 정의 하지 않습니다. 이 메시지의 목적에 따라 구조 멤버는 다음과 같이 해석 됩니다.
...
*str*<br/>
도구의 텍스트를 수신 하는 `CString` 개체에 대 한 참조입니다.
...
*PWnd* 및 *nIDTool* 매개 변수는 도구를 식별 합니다. 이전에 `CToolTipCtrl::AddTool`를 호출 하 여 도구 설명 컨트롤에 해당 도구를 등록 한 경우 *str* 매개 변수에서 참조 하는 개체에는 도구의 텍스트가 할당 됩니다.
...
*도구 정보*<br/>
도구의 텍스트를 수신 하는 `TOOLINFO` 개체에 대 한 참조입니다.
...
*Ctoolinfo* 에서 참조 하는 [toolinfo](/windows/win32/api/commctrl/ns-commctrl-tttoolinfoa) 구조의 `hwnd` 및 `uId` 멤버는 도구를 식별 합니다. `AddTool`에 대 한 이전 호출을 통해 도구 설명 컨트롤에 해당 도구를 등록 한 경우 `TOOLINFO` 구조는 도구에 대 한 정보로 채워집니다.
...
*pt*<br/>
테스트할 점의 좌표를 포함 하는 `CPoint` 개체에 대 한 포인터입니다.
...
- `ti`
   ...
   도구에 대 한 정보입니다. `TOOLINFO` 구조에 대 한 자세한 내용은 [CToolTipCtrl:: GetToolInfo](#gettoolinfo)를 참조 하세요.
...
도구 설명 컨트롤은 `RelayEvent`으로 전송 되는 다음 메시지만 처리 합니다.
...
*lprc*<br/>
설정할 여백 정보를 포함 하는 `RECT` 구조체의 주소입니다. `RECT` 구조체의 멤버는 경계 사각형을 정의 하지 않습니다. 여백 정보에 대 한 설명은 [CToolTipCtrl:: GetMargin](#getmargin) 를 참조 하세요.
