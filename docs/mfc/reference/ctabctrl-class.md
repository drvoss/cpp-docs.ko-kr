...
이 컨트롤과 `CTabCtrl` 클래스는 Windows 95/98 및 Windows NT 버전 3.51 이상에서 실행 되는 프로그램에만 사용할 수 있습니다.

`CTabCtrl`사용에 대 한 자세한 내용은 [컨트롤](../../mfc/controls-mfc.md) 및 [ctabctrl 사용](../../mfc/using-ctabctrl.md)을 참조 하세요.
...
*pParentWnd*<br/>
탭 컨트롤의 부모 창 (일반적으로 `CDialog`)을 지정 합니다. NULL이 아니어야 합니다.
...
`CTabCtrl` 개체는 두 단계로 구성 합니다. 먼저 생성자를 호출한 다음 탭 컨트롤을 만들고 `CTabCtrl` 개체에 연결 하는 `Create`를 호출 합니다.
...
확장 창 스타일을 사용 하 여 탭 컨트롤을 만들려면 `Create`대신 [Ctabctrl:: CreateEx](#createex) 를 호출 합니다.
...
컨트롤 (자식 창)을 만들어 `CTabCtrl` 개체와 연결 합니다.
...
`CreateEx` *Dwexstyle*으로 지정 된 확장 된 Windows 스타일을 사용 하 여 컨트롤을 만듭니다. [SetExtendedStyle](#setextendedstyle)를 사용 하 여 컨트롤과 관련 된 확장 스타일을 설정 합니다. 예를 들어 `CreateEx`를 사용 하 여 이러한 스타일을 WS_EX_CONTEXTHELP으로 설정 하지만 `SetExtendedStyle`를 사용 하 여 이러한 스타일을 TCS_EX_FLATSEPARATORS로 설정 합니다. 자세한 내용은 Windows SDK의 [탭 컨트롤 확장 스타일](/windows/win32/Controls/tab-control-extended-styles) 에서 설명 하는 스타일을 참조 하세요.
...
*pTabCtrlItem*<br/>
검색할 정보를 지정 하는 데 사용 되는 [Tcitem](/windows/win32/api/commctrl/ns-commctrl-tcitemw) 구조체에 대 한 포인터입니다. 탭에 대 한 정보를 수신 하는 데도 사용 됩니다. 이 구조는 `InsertItem`, `GetItem`및 `SetItem` 멤버 함수와 함께 사용 됩니다.
...
메시지가 전송 되 면 `mask` 멤버가 반환할 특성을 지정 합니다. `mask` 멤버가 TCIF_TEXT 값을 지정 하는 경우 `pszText` 멤버는 항목 텍스트를 받는 버퍼의 주소를 포함 해야 하 고 `cchTextMax` 멤버는 버퍼의 크기를 지정 해야 합니다.

- `mask`

   검색 하거나 설정할 `TCITEM` 구조 멤버를 지정 하는 값입니다. 이 멤버는 0 이거나 다음 값의 조합일 수 있습니다.

   - TCIF_TEXT `pszText` 멤버가 유효 합니다.

   - TCIF_IMAGE `iImage` 멤버가 유효 합니다.

   - TCIF_PARAM `lParam` 멤버가 유효 합니다.

   - 히브리어 또는 아랍어 시스템에서 오른쪽에서 왼쪽 읽기 순서를 사용 하 여 `pszText` 텍스트가 표시 TCIF_RTLREADING.
...
- `cchTextMax`
   ...
   `pszText`가 가리키는 버퍼의 크기입니다. 구조에서 정보를 수신 하지 않는 경우이 멤버는 무시 됩니다.

- 탭 컨트롤의 이미지 목록에 인덱스를 `iImage` 하거나 탭에 대 한 이미지가 없는 경우-1입니다.

- `lParam`

   탭과 연결 된 응용 프로그램 정의 데이터입니다. 응용 프로그램에서 사용 하는 응용 프로그램 정의 데이터의 수가 4 바이트를 초과 하는 경우 응용 프로그램은 구조를 정의 하 고 `TCITEM` 구조 대신 사용 해야 합니다. 응용 프로그램 정의 구조체의 첫 번째 멤버는 [Tcitemheader](/windows/win32/api/commctrl/ns-commctrl-tcitemheaderw)구조체 여야 합니다. `TCITEMHEADER` 구조체는 `TCITEM` 구조와 동일 하지만 `lParam` 멤버가 없습니다. 구조 크기와 `TCITEMHEADER` 구조의 크기 간 차이는 탭 당 추가 바이트 수와 같아야 합니다.
...
상태 정보를 받는 DWORD 값에 대 한 참조입니다. 다음 값 중 하나를 사용할 수 있습니다.
...
항목의 상태는 `TCITEM` 구조체의 `dwState` 멤버로 지정 됩니다.
...
*nMask*<br/>
설정할 `TCITEM` 구조체 특성을 지정 합니다. 0 또는 다음 값의 조합일 수 있습니다.
...
##  <a name="setimagelist"></a>CTabCtrl:: Seon Agelist
...
*dwState*<br/>
상태 정보를 포함 하는 DWORD 값에 대 한 참조입니다. 다음 값 중 하나를 사용할 수 있습니다.
...
`GetToolTips`를 호출 하 여 탭 컨트롤과 연결 된 도구 설명 컨트롤을 가져올 수 있습니다.
