...
|[CGopherFileFind:: GetLocator](#getlocator)|`CGopherLocator` 개체를 가져옵니다.|
...
`CGopherFileFind`는 검색을 시작 하 고, 파일을 찾고, 파일의 URL을 반환 하는 멤버 함수를 포함 합니다.

인터넷 및 로컬 파일 검색을 위해 디자인 된 다른 MFC 클래스에는 [Cftpfilefind](../../mfc/reference/cftpfilefind-class.md) 및 [cfilefind](../../mfc/reference/cfilefind-class.md)가 포함 됩니다. `CGopherFileFind`와 함께 이러한 클래스는 서버 프로토콜, 파일 형식 또는 위치 (로컬 컴퓨터 또는 원격 서버)에 관계 없이 사용자가 특정 파일을 찾기 위한 원활한 메커니즘을 제공 합니다. HTTP는 검색에 필요한 직접 파일 조작을 지원 하지 않으므로 HTTP 서버를 검색 하는 데 MFC 클래스가 없습니다.

> [!NOTE]
> `CGopherFileFind`은 기본 클래스 [Cfilefind](../../mfc/reference/cfilefind-class.md)의 다음 멤버 함수를 지원 하지 않습니다.
...
또한 `CGopherFileFind`와 함께 사용 하는 경우 `CFileFind` 멤버 함수 [Isdots](../../mfc/reference/cfilefind-class.md#isdots) 는 항상 FALSE입니다.

`CGopherFileFind` 및 기타 WinInet 클래스를 사용 하는 방법에 대 한 자세한 내용은 WinInet을 [사용한 인터넷 프로그래밍](../../mfc/win32-internet-extensions-wininet.md)문서를 참조 하세요.
...
이 멤버 함수를 호출 하 여 `CGopherFileFind` 개체를 생성 합니다.
...
`FindFile`를 호출 하 여 첫 번째 gopher 개체를 검색 한 후 [Findnextfile](#findnextfile) 을 호출 하 여 후속 gopher 파일을 검색할 수 있습니다.
...
`GetCreationTime`를 호출 하기 전에 [Findnextfile](#findnextfile) 을 한 번 이상 호출 해야 합니다.
...
`GetLastAccessTime`를 호출 하기 전에 [Findnextfile](#findnextfile) 을 한 번 이상 호출 해야 합니다.
...
`GetLastWriteTime`를 호출 하기 전에 [Findnextfile](#findnextfile) 을 한 번 이상 호출 해야 합니다.
...
`GetLength`는 Win32 구조 [WIN32_FIND_DATA](/windows/win32/api/minwinbase/ns-minwinbase-win32_find_dataw) 를 사용 하 여 파일 크기 값 (바이트)을 가져옵니다.

> [!NOTE]
>  MFC 7.0을 기반으로 `GetLength`는 64 비트 정수 형식을 지원 합니다. 이전에이 라이브러리의 최신 버전을 사용 하 여 빌드한 기존 코드를 사용 하면 잘림 경고가 발생할 수 있습니다.
...
`IsDots`를 호출 하기 전에 [Findnextfile](#findnextfile) 을 한 번 이상 호출 해야 합니다.
