...
다음 예제에서는 `CFrameWnd` 클래스에서 `CMFCDropDownFrame` 개체에 대 한 포인터를 검색 하는 방법과 자식 드롭다운 도구 모음 창을 자동으로 제거 하도록 설정 하는 방법을 보여 줍니다.
...
|-|-|
|매개 변수|Description|
|*pWndParent*|진행 드롭다운 프레임의 부모 창입니다.|
|*x*|진행 아래쪽 프레임 위치의 가로 화면 좌표입니다.|
|*y*|진행 아래쪽 프레임 위치의 세로 화면 좌표입니다.|
|*pWndOriginToolbar*|진행 이 메서드가 새 드롭다운 프레임 개체를 채우는 데 사용 하는 드롭다운 단추가 있는 도구 모음입니다.|
...
`CMFCDropDownFrame` 클래스는 제공 된 `CMFCDropDownToolBar` 매개 변수의 복사본을 만듭니다. 이 메서드는 `pWndOriginToolbar` 매개 변수에서 `m_pWndOriginToolbar` 데이터 멤버로 단추 이미지 및 단추 상태를 복사 합니다.
