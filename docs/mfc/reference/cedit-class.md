...
|[CEdit:: Create](#create)|Windows 편집 컨트롤을 만들어 `CEdit` 개체에 연결 합니다.|
|[CEdit:: Cut](#cut)|편집 컨트롤에서 현재 선택 영역 (있는 경우)을 삭제 (잘라내기) 하 고 삭제 된 텍스트를 클립보드에 CF_TEXT 형식으로 복사 합니다.|
...
|[CEdit:: Get 텍스트](#getlimittext)|이 `CEdit` 포함할 수 있는 최대 텍스트 크기를 가져옵니다.|
...
|[CEdit:: 텍스트](#limittext)|사용자가 편집 컨트롤에 입력할 수 있는 텍스트의 길이를 제한 합니다.|
...
|[CEdit::P aste](#paste)|클립보드의 데이터를 편집 컨트롤의 현재 커서 위치에 삽입 합니다. 클립보드에 CF_TEXT 형식의 데이터가 포함 되어 있는 경우에만 데이터가 삽입 됩니다.|
|[CEdit::P osFromChar](#posfromchar)|지정 된 문자 인덱스의 왼쪽 위 모퉁이의 좌표를 검색 합니다.|
...
|[CEdit:: Set 텍스트](#setlimittext)|이 `CEdit` 포함할 수 있는 최대 텍스트 크기를 설정 합니다.|
|[CEdit:: SetMargins](#setmargins)|이 `CEdit`의 왼쪽과 오른쪽 여백을 설정 합니다.|
...
대화 상자 템플릿에서 또는 코드에서 직접 편집 컨트롤을 만들 수 있습니다. 두 경우 모두 `CEdit` 생성자를 먼저 호출 하 여 `CEdit` 개체를 생성 한 다음 [create](#create) member 함수를 호출 하 여 Windows 편집 컨트롤을 만들고이를 `CEdit` 개체에 연결 합니다.

생성은 `CEdit`에서 파생 된 클래스의 단일 단계 프로세스가 될 수 있습니다. 파생 클래스에 대 한 생성자를 작성 하 고 생성자 내에서 `Create`를 호출 합니다.

`CEdit`은 `CWnd`에서 중요 한 기능을 상속 합니다. `CEdit` 개체에서 텍스트를 설정 하 고 검색 하려면 `CWnd` 멤버 함수 [Setwindowtext](cwnd-class.md#setwindowtext) 및 [getwindowtext](cwnd-class.md#getwindowtext)를 사용 합니다 .이 텍스트는 여러 줄 컨트롤 이더라도 편집 컨트롤의 전체 내용을 설정 하거나 가져옵니다. 여러 줄로 된 컨트롤의 텍스트 줄은 ' \r\n ' 문자 시퀀스로 구분 됩니다. 또한 편집 컨트롤이 여러 줄 인 경우 `CEdit` 멤버 함수 [Getline](#getline), [setsel](#setsel), [getline](#getsel)및 [ReplaceSel](#replacesel)를 호출 하 여 컨트롤의 텍스트 부분을 가져오고 설정 합니다.

편집 컨트롤에서 해당 부모 (일반적으로 `CDialog`에서 파생 된 클래스)로 보낸 Windows 알림 메시지를 처리 하려면 각 메시지의 부모 클래스에 메시지 매핑 항목과 메시지 처리기 멤버 함수를 추가 합니다.
...
- 사용자가 편집 컨트롤에서 텍스트를 변경 했을 수 있는 작업을 수행 ON_EN_CHANGE 합니다. EN_UPDATE 알림 메시지와 달리이 알림 메시지는 Windows에서 디스플레이를 업데이트 한 후에 전송 됩니다.

- ON_EN_ERRSPACE 편집 컨트롤은 특정 요청을 충족 하기에 충분 한 메모리를 할당할 수 없습니다.

- 사용자가 편집 컨트롤의 가로 스크롤 막대를 클릭 ON_EN_HSCROLL 합니다. 화면이 업데이트 되기 전에 부모 창에 알림이 표시 됩니다.

- 편집 컨트롤 ON_EN_KILLFOCUS 입력 포커스가 손실 됩니다.

- ON_EN_MAXTEXT 현재 삽입 내용이 편집 컨트롤에 대해 지정 된 문자 수를 초과 하 여 잘렸습니다. 편집 컨트롤에 ES_AUTOHSCROLL 스타일이 없고 삽입할 문자 수가 편집 컨트롤의 너비를 초과 하는 경우에도 전송 됩니다. 편집 컨트롤에 ES_AUTOVSCROLL 스타일이 없고 텍스트 삽입으로 인해 발생 하는 전체 줄 수가 편집 컨트롤의 높이를 초과 하는 경우에도 전송 됩니다.

- 편집 컨트롤에서 입력 포커스를 받을 때 ON_EN_SETFOCUS 전송 됩니다.

- 편집 컨트롤이 변경 된 텍스트를 표시 하려는 ON_EN_UPDATE입니다. 컨트롤에서 텍스트의 서식을 지정 하 고 텍스트를 화면에 표시 한 후 필요에 따라 창 크기를 변경할 수 있도록 합니다.

- 사용자가 편집 컨트롤의 세로 스크롤 막대를 클릭 ON_EN_VSCROLL 합니다. 화면이 업데이트 되기 전에 부모 창에 알림이 표시 됩니다.

대화 상자 내에서 `CEdit` 개체를 만드는 경우 사용자가 대화 상자를 닫으면 `CEdit` 개체가 자동으로 삭제 됩니다.

대화 상자 편집기를 사용 하 여 대화 상자 리소스에서 `CEdit` 개체를 만드는 경우 사용자가 대화 상자를 닫으면 `CEdit` 개체가 자동으로 삭제 됩니다.

창 내에서 `CEdit` 개체를 만드는 경우 해당 개체를 삭제 해야 할 수도 있습니다. 스택에서 `CEdit` 개체를 만들면 자동으로 삭제 됩니다. **새** 함수를 사용 하 여 힙에서 `CEdit` 개체를 만드는 경우 사용자가 Windows 편집 컨트롤을 종료할 때 개체에 대해 **delete** 를 호출 하 여 제거 해야 합니다. `CEdit` 개체에 메모리를 할당 하는 경우 `CEdit` 소멸자를 재정의 하 여 할당을 삭제 합니다.

편집 컨트롤의 특정 스타일 (예: ES_READONLY)을 수정 하려면 [ModifyStyle](cwnd-class.md#modifystyle)를 사용 하는 대신 특정 메시지를 컨트롤에 보내야 합니다. Windows SDK에서 [컨트롤 스타일 편집](/windows/win32/Controls/edit-control-styles) 을 참조 하세요.

`CEdit`에 대 한 자세한 내용은 [컨트롤](../../mfc/controls-mfc.md)을 참조 하세요.
...
`Undo` 멤버 함수를 호출 하 여 마지막 편집 작업을 실행 취소할 수 있는 경우 0이 아닙니다. 실행 취소할 수 없는 경우 0입니다.

### <a name="remarks"></a>설명

자세한 내용은 Windows SDK [EM_CANUNDO](/windows/win32/Controls/em-canundo) 을 참조 하세요.
...
자세한 내용은 Windows SDK [EM_CHARFROMPOS](/windows/win32/Controls/em-charfrompos) 을 참조 하세요.
...
[실행 취소](#undo) 멤버 함수를 호출 하 여 `Clear`에서 수행 하는 삭제 작업을 취소할 수 있습니다.

현재 선택 영역을 삭제 하 고 삭제 된 내용을 클립보드에 배치 하려면 [Cut](#cut) 멤버 함수를 호출 합니다.

자세한 내용은 Windows SDK [WM_CLEAR](/windows/win32/dataxchg/wm-clear) 을 참조 하세요.
...
자세한 내용은 Windows SDK [WM_COPY](/windows/win32/dataxchg/wm-copy) 을 참조 하세요.
...
Windows 편집 컨트롤을 만들어 `CEdit` 개체에 연결 합니다.
...
*rect*<br/>
편집 컨트롤의 크기와 위치를 지정 합니다. `CRect` 개체 또는 `RECT` 구조체 일 수 있습니다.
...
`CEdit` 개체는 두 단계로 구성 합니다. 먼저 `CEdit` 생성자를 호출한 다음 Windows 편집 컨트롤을 만들어 `CEdit` 개체에 연결 하는 `Create`를 호출 합니다.

`Create` 실행 되 면 Windows는 [WM_NCCREATE](/windows/win32/winmsg/wm-nccreate), [WM_NCCALCSIZE](/windows/win32/winmsg/wm-nccalcsize), [WM_CREATE](/windows/win32/winmsg/wm-create)및 [WM_GETMINMAXINFO](/windows/win32/winmsg/wm-getminmaxinfo) 메시지를 편집 컨트롤로 보냅니다.

이러한 메시지는 `CWnd` 기본 클래스의 [OnNcCreate](cwnd-class.md#onnccreate), [OnNcCalcSize](cwnd-class.md#onnccalcsize), [OnCreate](cwnd-class.md#oncreate)및 [OnGetMinMaxInfo](cwnd-class.md#ongetminmaxinfo) 멤버 함수에 의해 기본적으로 처리 됩니다. 기본 메시지 처리를 확장 하려면 `CEdit`에서 클래스를 파생 시키고 새 클래스에 메시지 맵을 추가 하 고 위의 메시지 처리기 멤버 함수를 재정의 합니다. 예를 들어 새 클래스에 대해 필요한 초기화를 수행 하려면 `OnCreate`를 재정의 합니다.
...
- 탭 이동 순서에 편집 컨트롤을 포함 하려면 WS_TABSTOP 합니다.
...
[실행 취소](#undo) 멤버 함수를 호출 하 여 `Cut`에서 수행 하는 삭제 작업을 취소할 수 있습니다.

삭제 된 텍스트를 클립보드에 넣지 않고 현재 선택 영역을 삭제 하려면 [Clear](#clear) 멤버 함수를 호출 합니다.

자세한 내용은 Windows SDK [WM_CUT](/windows/win32/dataxchg/wm-cut) 을 참조 하세요.
...
자세한 내용은 Windows SDK [EM_EMPTYUNDOBUFFER](/windows/win32/Controls/em-emptyundobuffer) 을 참조 하세요.
...
소프트 줄 바꿈은 두 개의 캐리지 리턴과 줄 바꿈이 줄의 끝에 삽입 된 줄 바꿈으로 구성 됩니다. 하드 줄 바꿈은 하나의 캐리지 리턴과 줄 바꿈으로 구성 됩니다. 하드 줄 바꿈으로 끝나는 줄은 `FmtLines`의 영향을 받지 않습니다.

`CEdit` 개체가 여러 줄 편집 컨트롤인 경우에만 Windows에서 응답이 발생 합니다.

`FmtLines`는 [GetHandle](#gethandle) 에서 반환 된 버퍼 및 [WM_GETTEXT](/windows/win32/winmsg/wm-gettext)에서 반환 되는 텍스트에만 영향을 줍니다. 편집 컨트롤 내의 텍스트 표시에는 영향을 주지 않습니다.

자세한 내용은 Windows SDK [EM_FMTLINES](/windows/win32/Controls/em-fmtlines) 을 참조 하세요.
...
자세한 내용은 Windows SDK [EM_GETFIRSTVISIBLELINE](/windows/win32/Controls/em-getfirstvisibleline) 을 참조 하세요.
...
DS_LOCALEDIT 스타일 플래그를 설정 하 여 대화 상자를 만든 경우에만 대화 상자에서 여러 줄 편집 컨트롤에 대 한 `GetHandle`를 호출 합니다. DS_LOCALEDIT 스타일이 설정 되지 않은 경우에도 0이 아닌 반환 값을 가져오지만 반환 된 값을 사용할 수는 없습니다.

> [!NOTE]
> `GetHandle` Windows 95/98에서 작동 하지 않습니다. Windows 95/98에서 `GetHandle`를 호출 하면 NULL이 반환 됩니다. `GetHandle`는 Windows NT 버전 3.51 이상에 설명 된 대로 작동 합니다.
...
이 메서드는 Windows SDK에서 설명 하는 [EM_GETHILITE](/windows/win32/Controls/em-gethilite) 메시지를 보냅니다. 현재 `SetHighlight`와 `GetHighlight` 모두 유니코드 빌드에서만 사용 하도록 설정 되어 있습니다.

##  <a name="getlimittext"></a>CEdit:: Get 텍스트

이 `CEdit` 개체에 대 한 텍스트 제한을 가져오려면이 멤버 함수를 호출 합니다.
...
이 `CEdit` 개체에 대 한 현재 텍스트 제한 (TCHARs)입니다.
...
자세한 내용은 Windows SDK [EM_GETLIMITTEXT](/windows/win32/Controls/em-getlimittext) 을 참조 하세요.
...
자세한 내용은 Windows SDK [EM_GETLINE](/windows/win32/Controls/em-getline) 을 참조 하세요.
...
자세한 내용은 Windows SDK [EM_GETMARGINS](/windows/win32/Controls/em-getmargins) 을 참조 하세요.
...
자세한 내용은 Windows SDK [EM_GETMODIFY](/windows/win32/Controls/em-getmodify) 을 참조 하세요.
...
자세한 내용은 Windows SDK [EM_GETRECT](/windows/win32/Controls/em-getrect) 을 참조 하세요.
...
자세한 내용은 Windows SDK [EM_GETSEL](/windows/win32/Controls/em-getsel) 을 참조 하세요.
...
텍스트 제한을 변경 하면 사용자가 입력할 수 있는 텍스트만 제한 됩니다. 편집 컨트롤에 이미 있는 텍스트에는 영향을 주지 않으며 `CWnd`의 [Setwindowtext](cwnd-class.md#setwindowtext) 멤버 함수를 통해 편집 컨트롤에 복사 되는 텍스트의 길이에도 영향을 주지 않습니다. 응용 프로그램에서 `SetWindowText` 함수를 사용 하 여 `LimitText`에 대 한 호출에 지정 된 것 보다 더 많은 텍스트를 편집 컨트롤에 추가 하는 경우 사용자는 편집 컨트롤 내에 있는 모든 텍스트를 삭제할 수 있습니다. 그러나 현재 선택 영역을 삭제 하면 텍스트가 텍스트 제한 아래에 포함 되는 경우를 제외 하 고는 텍스트 제한으로 인해 사용자가 기존 텍스트를 새 텍스트로 바꿀 수 없습니다.

> [!NOTE]
>  Win32 (Windows NT 및 Windows 95/98)에서는 [set한 집합 텍스트가](#setlimittext) 이 함수를 대체 합니다.

자세한 내용은 Windows SDK [EM_LIMITTEXT](/windows/win32/Controls/em-limittext) 을 참조 하세요.
...
자세한 내용은 Windows SDK [EM_LINEFROMCHAR](/windows/win32/Controls/em-linefromchar) 을 참조 하세요.
...
자세한 내용은 Windows SDK [EM_LINEINDEX](/windows/win32/controls/em-lineindex) 을 참조 하세요.
...
TCHAR.H 형식에 대 한 자세한 내용은 [Windows 데이터 형식](/windows/win32/WinProg/windows-data-types)테이블의 tchar.h 행을 참조 하세요.

### <a name="remarks"></a>설명

이 메서드는 Windows SDK에서 설명 하는 [EM_LINELENGTH](/windows/win32/Controls/em-linelength) 메시지에서 지원 됩니다.
...
`LineScroll`를 사용 하 여 줄의 마지막 문자를 지나서 가로로 스크롤할 수 있습니다.
...
클립보드의 데이터를 삽입 지점의 `CEdit`에 삽입 하려면이 함수를 호출 합니다.
...
자세한 내용은 Windows SDK [WM_PASTE](/windows/win32/dataxchg/wm-paste) 을 참조 하세요.
...
이 `CEdit` 개체 내에서 지정 된 문자의 위치 (왼쪽 위 모퉁이)를 가져오려면이 함수를 호출 합니다.
...
문자는 0부터 시작 하는 인덱스 값을 제공 하 여 지정 됩니다. *NChar* 가이 `CEdit` 개체의 마지막 문자 인덱스 보다 큰 경우 반환 값은이 `CEdit` 개체에서 마지막 문자 바로 다음에 오는 문자 위치의 좌표를 지정 합니다.

> [!NOTE]
>  이 구성원 함수는 Windows 95 및 Windows NT 4.0부터 사용할 수 있습니다.

자세한 내용은 Windows SDK [EM_POSFROMCHAR](/windows/win32/Controls/em-posfromchar) 을 참조 하세요.
...
자세한 내용은 Windows SDK [EM_REPLACESEL](/windows/win32/Controls/em-replacesel) 을 참조 하세요.
...
이 메서드는 Windows SDK에서 설명 하는 [EM_SETCUEBANNER](/windows/win32/Controls/em-setcuebanner) 메시지를 보냅니다. 자세한 내용은 [Edit_SetCueBannerTextFocused](/windows/win32/api/commctrl/nf-commctrl-edit_setcuebannertextfocused) 매크로를 참조 하세요.
...
*hBuffer*<br/>
로컬 메모리에 대 한 핸들을 포함 합니다. 이 핸들은 LMEM_MOVEABLE 플래그를 사용 하 여 [Localalloc](/windows/win32/api/winbase/nf-winbase-localalloc) Windows 함수를 이전에 호출 하 여 만든 것 이어야 합니다. 메모리는 null로 끝나는 문자열을 포함 하는 것으로 간주 됩니다. 그렇지 않은 경우 할당 된 메모리의 첫 번째 바이트를 0으로 설정 해야 합니다.
...
이 메서드는 Windows SDK에서 설명 하는 [EM_SETHILITE](/windows/win32/Controls/em-sethilite) 메시지를 보냅니다.  이 메서드는 Windows SDK에서 설명 하는 [EM_SETHILITE](/windows/win32/Controls/em-sethilite) 메시지를 보냅니다. `SetHighlight` 및 `GetHighlight` 모두 유니코드 빌드에서만 사용 됩니다.

##  <a name="setlimittext"></a>CEdit:: Set 텍스트

이 `CEdit` 개체에 대 한 텍스트 제한을 설정 하려면이 멤버 함수를 호출 합니다.
...
자세한 내용은 Windows SDK [EM_SETLIMITTEXT](/windows/win32/Controls/em-setlimittext) 을 참조 하세요.
...
자세한 내용은 Windows SDK [EM_SETMARGINS](/windows/win32/Controls/em-setmargins) 을 참조 하세요.
...
자세한 내용은 Windows SDK [EM_SETMODIFY](/windows/win32/Controls/em-setmodify) 을 참조 하세요.
...
`SetPasswordChar` 멤버 함수를 호출 하면 `CEdit`는 *ch*로 지정 된 문자를 사용 하 여 표시 되는 모든 문자를 다시 그립니다.

[ES_PASSWORD](styles-used-by-mfc.md#edit-styles) 스타일을 사용 하 여 편집 컨트롤을 만드는 경우 기본 암호 문자는 별표 ( <strong>\*</strong>)로 설정 됩니다. *Ch* 가 0으로 설정 된 상태에서 `SetPasswordChar`가 호출 되는 경우이 스타일은 제거 됩니다.

자세한 내용은 Windows SDK [EM_SETPASSWORDCHAR](/windows/win32/Controls/em-setpasswordchar) 을 참조 하세요.
...
자세한 내용은 Windows SDK [EM_SETREADONLY](/windows/win32/Controls/em-setreadonly) 을 참조 하세요.
...
*lpRect*<br/>
서식 지정 사각형의 새 크기를 지정 하는 `RECT` 구조체 또는 `CRect` 개체를 가리킵니다.

### <a name="remarks"></a>설명

이 멤버는 여러 줄 편집 컨트롤에 의해서만 처리 됩니다.

`SetRect`를 사용 하 여 여러 줄 편집 컨트롤의 서식 지정 사각형을 설정 합니다. 서식 지정 사각형은 텍스트의 제한 사각형으로, 편집 컨트롤 창의 크기와는 독립적입니다. 편집 컨트롤을 처음 만들 때 서식 지정 사각형은 편집 컨트롤 창의 클라이언트 영역과 동일 합니다. 응용 프로그램은 `SetRect` 멤버 함수를 사용 하 여 서식 지정 사각형을 편집 컨트롤 창 보다 크거나 작게 만들 수 있습니다.

편집 컨트롤에 스크롤 막대가 없는 경우 서식 지정 사각형이 창 보다 큰 경우 텍스트는 래핑되지 않고 잘립니다. 편집 컨트롤에 테두리가 포함 되어 있으면 서식 지정 사각형이 테두리의 크기로 줄어듭니다. `GetRect` 멤버 함수에서 반환 되는 사각형을 조정 하는 경우 `SetRect`에 사각형을 전달 하기 전에 테두리의 크기를 제거 해야 합니다.

`SetRect`가 호출 되 면 편집 컨트롤의 텍스트도 다시 포맷 되 고 다시 표시 됩니다.

자세한 내용은 Windows SDK [EM_SETRECT](/windows/win32/Controls/em-setrect) 을 참조 하세요.
...
*lpRect*<br/>
`RECT` 구조체 또는 사각형의 새 크기를 지정 하는 `CRect` 개체를 가리킵니다.

### <a name="remarks"></a>설명

서식 지정 사각형은 텍스트의 제한 사각형으로, 편집 컨트롤 창의 크기와는 독립적입니다.

`SetRectNP`은 `SetRect` 멤버 함수와 동일 합니다. 단, 편집 컨트롤 창은 다시 그려지지 않습니다.
...
자세한 내용은 Windows SDK [EM_SETRECTNP](/windows/win32/Controls/em-setrectnp) 을 참조 하세요.
...
자세한 내용은 Windows SDK [EM_SETSEL](/windows/win32/Controls/em-setsel) 을 참조 하세요.
...
|*ttiIcon*|진행 풍선 설명에 연결할 아이콘의 유형을 지정 하는 **INT** 입니다. 기본값은 TTI_NONE입니다. 자세한 내용은 [EDITBALLOONTIP](/windows/win32/api/commctrl/ns-commctrl-editballoontip) 구조체의 `ttiIcon` 멤버를 참조 하십시오.|
...
이 함수는 Windows SDK에서 설명 하는 [EM_SHOWBALLOONTIP](/windows/win32/Controls/em-showballoontip) 메시지를 보냅니다. 자세한 내용은 [Edit_ShowBalloonTip](/windows/win32/api/commctrl/nf-commctrl-edit_showballoontip) 매크로를 참조 하세요.

### <a name="example"></a>예제

다음 코드 예제에서는 현재 편집 컨트롤에 액세스 하는 데 사용 되는 변수 `m_cedit`를 정의 합니다. 이 변수는 다음 예제에서 사용됩니다.
...
실행 취소 작업을 실행 취소할 수도 있습니다. 예를 들어 `Undo`에 대 한 첫 번째 호출로 삭제 된 텍스트를 복원할 수 있습니다. 중간 편집 작업이 없는 한 `Undo`에 대 한 두 번째 호출로 텍스트를 다시 제거할 수 있습니다.

자세한 내용은 Windows SDK [EM_UNDO](/windows/win32/Controls/em-undo) 을 참조 하세요.
