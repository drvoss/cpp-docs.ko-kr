...
|[CObject:: GetRuntimeClass](#getruntimeclass)|이 개체의 클래스에 해당 하는 `CRuntimeClass` 구조체를 반환 합니다.|
...
`CFile` 및 `CObList`같은 라이브러리 클래스 뿐만 아니라 사용자가 작성 하는 클래스에 대 한 루트 역할을 합니다. `CObject`를 포함 하 여 기본 서비스를 제공 합니다.
...
`CObject`는 다중 상속을 지원 하지 않습니다. 파생 클래스에는 `CObject` 기본 클래스가 하나만 있을 수 있으며 해당 `CObject`는 계층에서 맨 왼쪽에 있어야 합니다. 그러나 오른쪽 다중 상속 분기에서 구조체 및 비 `CObject`파생 클래스를 가질 수 있습니다.

클래스 구현 및 선언에서 선택적 매크로 중 일부를 사용 하는 경우 `CObject` 파생의 주요 이점을 누릴 수 있습니다.
...
일반적으로 및 `CObject`를 사용 하 여 C++ Microsoft Foundation 클래스 및 클래스를 파생 하는 방법에 대 한 자세한 내용은 CObject 및 [Serialization](../../mfc/serialization-in-mfc.md) [사용](../../mfc/using-cobject.md) 을 참조 하세요.
...
`AssertValid`는 내부 상태를 확인 하 여이 개체에 대 한 유효성 검사를 수행 합니다. 라이브러리의 디버그 버전에서 어설션이 실패 한 줄 번호와 파일 이름을 나열 하는 메시지를 사용 하 여 프로그램을 어설션할 수 `AssertValid`.

사용자 고유의 클래스를 작성 하는 경우 클래스의 사용자 및 다른 사용자에 게 진단 서비스를 제공 하도록 `AssertValid` 함수를 재정의 해야 합니다. 재정의 된 `AssertValid`는 일반적으로 파생 클래스에 고유한 데이터 멤버를 확인 하기 전에 기본 클래스의 `AssertValid` 함수를 호출 합니다.

`AssertValid`는 **const** 함수 이므로 테스트 중에 개체 상태를 변경할 수 없습니다. 사용자 고유의 파생 클래스 `AssertValid` 함수는 예외를 throw 해서는 안 되며, 대신 잘못 된 개체 데이터를 검색 하는지 여부를 어설션해야 합니다.
...
모든 `CObject` 예제에서 사용 되는 `CAge` 클래스 목록은 [CObList:: CObList](../../mfc/reference/coblist-class.md#coblist) 를 참조 하세요.
...
표준 C++ 기본 클래스 복사 생성자는 멤버 단위로 복사를 수행 합니다. Private `CObject` 복사 생성자가 있으면 클래스의 복사 생성자가 필요 하지만 사용할 수 없는 경우 컴파일러 오류 메시지가 보장 됩니다. 따라서 클래스에이 기능이 필요한 경우 복사 생성자를 제공 해야 합니다.

### <a name="example"></a>예제

`CObject` 예제에 사용 된 `CAge` 클래스 목록은 [CObList:: CObList](../../mfc/reference/coblist-class.md#coblist) 를 참조 하세요.
...
*dc*<br/>
덤프 하기 위한 진단 덤프 컨텍스트입니다. 일반적으로 `afxDump`입니다.

### <a name="remarks"></a>설명

사용자 고유의 클래스를 작성 하는 경우 클래스의 사용자 및 다른 사용자에 게 진단 서비스를 제공 하도록 `Dump` 함수를 재정의 해야 합니다. 재정의 된 `Dump`는 일반적으로 파생 클래스에 고유한 데이터 멤버를 인쇄 하기 전에 기본 클래스의 `Dump` 함수를 호출 합니다. 클래스에서 `IMPLEMENT_DYNAMIC` 또는 IMPLEMENT_SERIAL 매크로를 사용 하는 경우 `CObject::Dump` 클래스 이름을 인쇄 합니다.

> [!NOTE]
>  `Dump` 함수는 출력의 끝 부분에서 줄 바꿈 문자를 인쇄 하면 안 됩니다.

`Dump` 호출은 MFC 라이브러리의 디버그 버전 에서만 의미가 있습니다. 조건부 컴파일에 대 한 **#ifdef _DEBUG**/ `#endif` 문을 사용 하 여 호출, 함수 선언 및 함수 구현을 묶습니다.

`Dump`는 **const** 함수 이므로 덤프 중에 개체 상태를 변경할 수 없습니다.

[CDumpContext 삽입 (< <) 연산자](../../mfc/reference/cdumpcontext-class.md#operator_lt_lt) 는 `CObject` 포인터를 삽입할 때 `Dump`를 호출 합니다.

`Dump`는 개체의 "비순환" 덤프만 허용 합니다. 예를 들어 개체 목록을 덤프할 수 있지만 개체 중 하나가 목록 자체인 경우 스택을 오버플로 하 게 됩니다.
...
이 개체의 클래스에 해당 하는 `CRuntimeClass` 구조체를 반환 합니다.
...
각 `CObject`파생 클래스에 대해 하나의 `CRuntimeClass` 구조가 있습니다. 구조체 멤버는 다음과 같습니다.
...
- **CRuntimeClass\* (파스칼\* m_pfn_GetBaseClass) ()** 응용 프로그램이 AFXDLL 버전의 MFC에 동적으로 연결 된 경우 기본 클래스의 `CRuntimeClass` 구조체를 반환 하는 함수에 대 한 포인터입니다.

- **CRuntimeClass\* m_pBaseClass** 응용 프로그램이 정적으로 MFC에 링크 된 경우 기본 클래스의 `CRuntimeClass` 구조에 대 한 포인터입니다.
...
*pClass*<br/>
`CObject`파생 클래스와 연결 된 [CRuntimeClass](../../mfc/reference/cruntimeclass-structure.md) 구조체에 대 한 포인터입니다.
...
이 함수는 다형성 기능을 C++ 무효화 하므로 광범위 하 게 사용 하지 마십시오. 대신 가상 함수를 사용 해야 합니다.
...
라이브러리의 릴리스 버전에서 operator **delete** 는 operator **new**에 의해 할당 된 메모리를 해제 합니다.
...
디버그 버전에서 operator **delete** 는 메모리 누수를 감지 하도록 설계 된 할당 모니터링 체계에 참여 합니다.

코드 줄을 사용 하는 경우

[!code-cpp[NVC_MFCCObjectSample#14](../../mfc/codesnippet/cpp/cobject-class_7.cpp)]

에서 구현 하기 전에 CPP 파일의 세 번째 버전이 **사용 되며,** 나중에 보고 하기 위해 할당 된 블록에 파일 이름 및 줄 번호를 저장 합니다. 추가 매개 변수를 제공 하는 것에 대해 걱정할 필요가 없습니다. 매크로가 사용자를 위해이 작업을 수행 합니다.

디버그 모드에서 DEBUG_NEW 사용 하지 않더라도 위에 설명 된 소스 파일 줄 번호 보고를 제외 하 고 누수 검색을 계속 받습니다.

**New** 및 **delete**연산자를 재정의 하는 경우이 진단 기능을 상실 합니다.
...
라이브러리의 릴리스 버전의 경우 operator **new** 는 `malloc`와 비슷한 방식으로 최적의 메모리 할당을 수행 합니다.
...
디버그 버전에서 operator **new** 는 메모리 누수를 감지 하도록 설계 된 할당 모니터링 체계에 참여 합니다.

코드 줄을 사용 하는 경우

[!code-cpp[NVC_MFCCObjectSample#14](../../mfc/codesnippet/cpp/cobject-class_7.cpp)]

에서 구현 하기 전에 CPP **파일의 두** 번째 버전이 사용 되며, 나중에 보고 하기 위해 할당 된 블록에 파일 이름 및 줄 번호를 저장 합니다. 추가 매개 변수를 제공 하는 것에 대해 걱정할 필요가 없습니다. 매크로가 사용자를 위해이 작업을 수행 합니다.

디버그 모드에서 DEBUG_NEW 사용 하지 않더라도 위에 설명 된 소스 파일 줄 번호 보고를 제외 하 고 누수 검색을 계속 받습니다.

> [!NOTE]
>  이 연산자를 재정의 하는 경우 **delete**도 재정의 해야 합니다. 표준 라이브러리 `_new_handler` 함수를 사용 하지 마십시오.
...
Serialize 하려는 각 클래스에 대해 `Serialize`를 재정의 해야 합니다. 재정의 된 `Serialize`는 먼저 기본 클래스의 `Serialize` 함수를 호출 해야 합니다.
...
`Serialize`는 [carchive:: ReadObject](../../mfc/reference/carchive-class.md#readobject) 및 [Carchive:: WriteObject](../../mfc/reference/carchive-class.md#writeobject)에 의해 호출 됩니다. 이러한 함수는 `CArchive` 삽입 연산자 ( **<\<** ) 및 추출 연산자 ( **>>** )와 연결 됩니다.
