...
예를 들어, `AfxParseURL` *service://server/dir/dir/object.ext:port* 폼의 url을 구문 분석 하 고 다음과 같이 저장 된 해당 구성 요소를 반환 합니다.
...
*strUsername*<br/>
사용자의 이름을 포함 하는 `CString` 개체에 대 한 참조입니다.

*strPassword*<br/>
사용자의 암호를 포함 하는 `CString` 개체에 대 한 참조입니다.
...
플래그가 없는 MFC 기본값을 사용 하는 경우 함수는 모든 안전 하지 않은 문자 및 meta 시퀀스 (예: \\., \ ... 및 \\...)를 이스케이프 시퀀스로 변환 합니다.
...
다음 목록에는 `AfxGetInternetHandleType`에서 반환 하는 가능한 인터넷 유형이 나와 있습니다.
