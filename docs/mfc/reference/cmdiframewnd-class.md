...
`LoadFrame`에는 `Create`보다 작은 인수가 필요 하며, 대신 프레임의 캡션, 아이콘, 액셀러레이터 키 테이블 및 메뉴를 비롯 한 리소스에서 대부분의 기본값을 검색 합니다. `LoadFrame`에서 액세스 하려면 이러한 모든 리소스에 동일한 리소스 ID (예: IDR_MAINFRAME)가 있어야 합니다.
