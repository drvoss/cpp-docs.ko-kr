...
`CControlBar`에 대 한 자세한 내용은 다음을 참조 하세요.
...
`CControlBar`에서 파생 하는 클래스에 고유한 동적 레이아웃을 제공 하려면이 멤버 함수를 재정의 합니다. [CToolbar](../../mfc/reference/ctoolbar-class.md)와 같은 `CControlBar`에서 파생 된 MFC 클래스는이 멤버 함수를 재정의 하 고 자체 구현을 제공 합니다.
...
또 다른 사용자 지정 방법은 `DrawBorders`를 재정의 하 고 함수를 `DrawGripper` 하 고 테두리와 그리퍼에 대 한 사용자 지정 그리기 코드를 추가 하는 것입니다. 이러한 메서드는 기본 `DoPaint` 메서드에서 호출 되기 때문에 `DoPaint` 재정의는 필요 하지 않습니다.
...
*rect*<br/>
컨트롤 막대 위치 조정 막대의 크기를 포함 하는 `CRect` 개체입니다.
...
`CControlBar` 개체의 비 HWND 항목 수를 반환 합니다.
...
0이 아니면 Windows 컨트롤 표시줄이 제거 될 때 `CControlBar` 개체가 삭제 됩니다.
...
힙에 `CControlBar` 개체를 할당 하 고 **delete**를 호출할 계획이 없는 경우이 변수를 0이 아닌 값으로 설정 합니다.
...
`OnUpdateCmdUI`는 응용 프로그램이 유휴 상태일 때 프레임 워크에서 호출 됩니다. 업데이트할 프레임 창은 표시 되는 프레임 창의 자식 창 (최소한 간접적) 이어야 합니다. `OnUpdateCmdUI`은 고급 재정의 가능입니다.
...
- CBRS_GRIPPER를 사용 하면 `CReBar` 개체의 밴드에서 사용 되는 것과 비슷한 위치 조정 기능이 `CControlBar`파생 클래스에 대해 그려집니다.
