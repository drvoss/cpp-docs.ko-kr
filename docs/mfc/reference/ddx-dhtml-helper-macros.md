...
DDX_DHtml_Img_Src 매크로를 사용 하 여 IMAGE 요소의 Src 속성을 검색 하는 경우 Internet Explorer 이미지 개체는 이미지 원본에 대해 완전히 이스케이프 된 URL을 반환 합니다. 예를 들어, DDX_DHtml_Img_Src 매크로를 사용 하 여 IMAGE 요소의 Src 속성을 "몇 가지 관심 있는 그림" 문자열로 설정 하는 경우 Internet Explorer는 "res://d: \ myapplication \ myapp.exe/some %2 0에 관심이 있는 %2 0 그림" 문자열을 반환 합니다.
