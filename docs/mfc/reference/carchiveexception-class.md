...
`CArchiveException` 클래스에는 예외의 원인을 나타내는 공용 데이터 멤버가 포함 되어 있습니다.
...
개체의 *원인* 값을 저장 하 여 `CArchiveException` 개체를 생성 합니다.
...
*lpszArchiveName*<br/>
예외를 발생 시킨 `CArchive` 개체의 이름을 포함 하는 문자열을 가리킵니다.

### <a name="remarks"></a>설명

힙에서 `CArchiveException` 개체를 만들어 직접 throw 하거나 전역 함수 [AfxThrowArchiveException](../../mfc/reference/exception-processing.md#afxthrowarchiveexception) 에서이를 처리 하도록 할 수 있습니다.

이 생성자를 직접 사용 하지 마십시오. 대신 `AfxThrowArchiveException`전역 함수를 호출 합니다.
...
이 데이터 멤버는 **int**형식의 공용 변수입니다. 해당 값은 `CArchiveException` 열거 형식에 의해 정의 됩니다. 아래에 열거자와 해당 의미가 나와 있습니다.

- `CArchiveException::none` 오류가 발생 하지 않았습니다.

- `CArchiveException::genericException` 지정 되지 않은 오류입니다.

- 로드를 위해 열린 보관 파일에 쓰려고 했습니다. `CArchiveException::readOnly`

- 개체를 읽는 동안 파일의 끝에 도달 했습니다 `CArchiveException::endOfFile`.

- 저장을 위해 열린 보관 파일에서 읽으려고 했습니다. `CArchiveException::writeOnly`

- 잘못 된 파일 형식 `CArchiveException::badIndex`.

- `CArchiveException::badClass`에서 개체를 잘못 된 형식의 개체로 읽으려고 했습니다.
