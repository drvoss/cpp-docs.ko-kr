...
|[CToolBar:: CreateEx](#createex)|포함 된 `CToolBarCtrl` 개체에 대 한 추가 스타일을 사용 하 여 `CToolBar` 개체를 만듭니다.|
...
MFC 4.0에 새로 추가 된 멤버 함수인 [CToolBar:: GetToolBarCtrl](#gettoolbarctrl)를 사용 하 여 도구 모음 사용자 지정 및 추가 기능에 대 한 Windows 공용 컨트롤의 지원을 활용할 수 있습니다. `CToolBar` 멤버 함수는 Windows 공용 컨트롤의 기능을 대부분 제공 합니다. 그러나 `GetToolBarCtrl`를 호출 하는 경우 도구 모음에 Windows 95/98 도구 모음의 특성을 더 많이 제공할 수 있습니다. `GetToolBarCtrl`호출 하는 경우 `CToolBarCtrl` 개체에 대 한 참조를 반환 합니다. Windows 공용 컨트롤을 사용 하 여 도구 모음 디자인에 대 한 자세한 내용은 [CToolBarCtrl](../../mfc/reference/ctoolbarctrl-class.md) 를 참조 하세요. 공용 컨트롤에 대 한 일반적인 정보는 Windows SDK의 [공용 컨트롤](/windows/win32/Controls/common-controls-intro) 을 참조 하세요.
...
`SetButtons` 함수는 컨트롤 Id 배열에 대 한 포인터와 배열의 요소 수를 지정 하는 정수를 사용 합니다. 함수는 각 단추의 ID를 배열의 해당 요소에 대 한 값으로 설정 하 고 각 단추에 이미지 인덱스를 할당 합니다 .이 인덱스는 비트맵에서 단추의 이미지 위치를 지정 합니다. 배열 요소의 값이 ID_SEPARATOR 이면 이미지 인덱스가 할당 되지 않습니다.
...
> [!WARNING]
> `CToolBar`은 최대 16 색의 비트맵을 지원 합니다. 이미지를 도구 모음 편집기에 로드 하면 Visual Studio에서 이미지를 자동으로 16 색 비트맵으로 변환 하 고 (필요한 경우) 이미지가 변환 되 면 경고 메시지를 표시 합니다. 16 개 이상의 색이 있는 이미지를 사용 하는 경우 (외부 편집기를 사용 하 여 이미지 편집) 응용 프로그램이 예기치 않게 동작할 수 있습니다.
...
배열을 가리키지 않고 개별 단추나 구분 기호 스타일을 설정 하려면 [Getbuttonstyle](#getbuttonstyle) 을 호출 하 여 스타일을 검색 한 다음 `SetButtons`대신 [setbuttonstyle](#setbuttonstyle) 을 호출 합니다. `SetButtonStyle`은 런타임에 단추의 스타일을 변경 하려는 경우에 가장 유용 합니다.
...
확인란 단추를 만들려면 스타일 TBBS_CHECKBOX 할당 하거나 ON_UPDATE_COMMAND_UI 처리기에서 `CCmdUI` 개체의 `SetCheck` 멤버 함수를 사용 합니다. `SetCheck`를 호출 하면 누름 단추가 확인란 단추로 전환 됩니다. 선택 하지 않은 경우 0의 인수를 전달 하 고, 선택 된 경우 1을 전달 하거나, 결정 되지 않은 경우 2를 전달 `SetCheck`

라디오 단추를 만들려면 ON_UPDATE_COMMAND_UI 처리기에서 [CCmdUI](../../mfc/reference/ccmdui-class.md) 개체의 [SetRadio](../../mfc/reference/ccmdui-class.md#setradio) 멤버 함수를 호출 합니다. 확인 되지 않았거나 0이 아닌 값으로 0의 인수를 전달 `SetRadio` 합니다. 라디오 그룹의 상호 배타적인 동작을 제공 하려면 그룹의 모든 단추에 대 한 ON_UPDATE_COMMAND_UI 처리기가 있어야 합니다.

`CToolBar`를 사용 하는 방법에 대 한 자세한 내용은 [MFC 도구 모음 구현](../../mfc/mfc-toolbar-implementation.md) 및 [Technical Note 31: 컨트롤 막대](../../mfc/tn031-control-bars.md)문서를 참조 하세요.
...
이 멤버 함수는 위치 0에서 시작 하 여 `nIDFind`와 일치 하는 명령 ID를 가진 첫 번째 도구 모음 단추의 인덱스를 반환 합니다.
...
이 멤버 함수는 Windows 도구 모음 (자식 창)을 만들어 `CToolBar` 개체에 연결 합니다.
...
이 함수를 호출 하 여 Windows 도구 모음 (자식 창)을 만들고 `CToolBar` 개체와 연결 합니다.
...
포함 된 도구 모음 컨트롤을 만드는 동안 특정 스타일을 제공 해야 하는 경우 [Create](#create)대신 `CreateEx`를 사용 합니다. 예를 들어 *dwCtrlStyle* 를 TBSTYLE_FLAT &#124; TBSTYLE_TRANSPARENT 설정 하 여 Internet Explorer 4 도구 모음과 유사한 도구 모음을 만들 수 있습니다.
...
이 멤버 함수는 `CToolBar` 개체를 생성 하 고 기본 크기를 설정 합니다.
...
구분 기호는 ID_SEPARATOR 반환 합니다.

##  <a name="getitemrect"></a>CToolBar:: GetItemRect

이 멤버 함수는 *lpRect* 에 포함 된 주소가 *n index*로 지정 된 단추나 구분 기호를 사용 하 여 해당 주소를 포함 하는 `RECT` 구조를 채웁니다.
...
`GetItemRect`를 사용 하 여 콤보 상자나 기타 컨트롤로 바꿀 구분 기호의 좌표를 가져옵니다.
...
`GetToolBarCtrl`를 사용 하 여 Windows 도구 모음 공용 컨트롤의 기능을 활용 하 고, 도구 모음 사용자 지정을 위한 지원 [CToolBarCtrl](../../mfc/reference/ctoolbarctrl-class.md) 을 활용할 수 있습니다.
...
`lpszResourceName` 또는 `nIDResource`에 지정 된 비트맵을 로드 하려면이 멤버 함수를 호출 합니다.
...
예를 들어 사용자가 단추 작업을 변경 하는 문서에 대 한 작업을 수행한 후에 `SetBitmap`를 호출 하 여 비트맵 이미지를 변경 합니다.
...
- TBBS_GROUP 그룹의 시작을 표시 합니다.

- TBBS_CHECKGROUP 확인란 단추 그룹의 시작을 표시 합니다.

- TBBS_DROPDOWN 드롭다운 목록 단추를 만듭니다.

- 단추 너비는 이미지 크기가 아니라 단추의 텍스트를 기준으로 계산 됩니다 TBBS_AUTOSIZE.
...
스타일 TBBS_SEPARATOR 있는 구분 기호의 경우이 함수는 구분 기호의 너비 (픽셀)를 *Iimage*에 저장 된 값으로 설정 합니다.

> [!NOTE]
>  *Nstyle* 매개 변수를 사용 하 여 단추 상태를 설정할 수도 있습니다. 그러나 단추 상태는 [ON_UPDATE_COMMAND_UI](message-map-macros-mfc.md#on_update_command_ui) 처리기에 의해 제어 되므로 `SetButtonInfo`를 사용 하 여 설정한 모든 상태는 다음 유휴 처리 중에 손실 됩니다. 자세한 내용은 [사용자 인터페이스 개체](../../mfc/how-to-update-user-interface-objects.md) 및 [TN031: 컨트롤 막대](../../mfc/tn031-control-bars.md) 를 업데이트 하는 방법을 참조 하세요.
...
배열의 요소에 ID_SEPARATOR 값이 있는 경우 도구 모음의 해당 위치에 구분 기호가 만들어집니다. 또한이 함수는 각 단추의 스타일을 TBBS_BUTTON 및 각 구분 기호의 스타일을 TBBS_SEPARATOR 설정 하 고 각 단추에 이미지 인덱스를 할당 합니다. 이미지 인덱스는 비트맵 내에서 단추의 이미지 위치를 지정 합니다.
...
`SetButtonStyle`를 호출 하기 전에 [Getbuttonstyle](#getbuttonstyle) 멤버 함수를 호출 하 여 단추나 구분 기호 스타일을 검색 합니다.

> [!NOTE]
>  *Nstyle* 매개 변수를 사용 하 여 단추 상태를 설정할 수도 있습니다. 그러나 단추 상태는 [ON_UPDATE_COMMAND_UI](message-map-macros-mfc.md#on_update_command_ui) 처리기에 의해 제어 되므로 `SetButtonStyle`를 사용 하 여 설정한 모든 상태는 다음 유휴 처리 중에 손실 됩니다. 자세한 내용은 [사용자 인터페이스 개체](../../mfc/how-to-update-user-interface-objects.md) 및 [TN031: 컨트롤 막대](../../mfc/tn031-control-bars.md) 를 업데이트 하는 방법을 참조 하세요.
