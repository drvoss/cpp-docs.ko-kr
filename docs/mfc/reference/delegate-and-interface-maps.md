...
이 대리자는 사용자 인터페이스 업데이트 명령 메시지를 사용 하 여 콜백 메서드를 등록 합니다. 이 대리자가 사용자 인터페이스 개체 업데이트 명령과 함께 사용 된다는 점을 제외 하 고 `CommandUIHandler`는 [Commandhandler](#commandhandler) 와 비슷합니다. 사용자 인터페이스 업데이트 명령은 메시지 처리기 메서드를 사용 하 여 일대일로 매핑되어야 합니다.
...
*ARG0*<br/>
`Object^`와 같은 관리 되는 이벤트 처리기 메서드의 첫 번째 인수입니다.

*ARG1*<br/>
`EventArgs^`와 같은 관리 되는 이벤트 처리기 메서드의 두 번째 인수입니다.
...
다음 코드 예제에서는 EVENT_DELEGATE_ENTRY를 사용 하 여 `OnClick` 이벤트 처리기에 대 한 대리자 맵에 항목을 만드는 방법을 보여 줍니다. 또한 MAKE_DELEGATE의 코드 예제를 참조 하세요. 자세한 내용은 [방법: 네이티브 C++ 클래스에서 Windows Forms 이벤트 싱크](../../dotnet/how-to-sink-windows-forms-events-from-native-cpp-classes.md)를 참조 하세요.
...
*대리자나*<br/>
[EventHandler](assetId:///T:System.EventHandler?qualifyHint=False&autoUpgrade=True)와 같은 관리 되는 이벤트 처리기 대리자의 형식입니다.
...
다음 코드 예제에서는 `MAKE_DELEGATE`를 호출 하 여 `OnClick` 이벤트 처리기를 `MyControl`MFC 컨트롤에 연결 하는 방법을 보여 줍니다. MFC 응용 프로그램에서이 매크로가 작동 하는 방법에 대 한 자세한 설명은 [방법: 네이티브 C++ 클래스에서 Windows Forms 이벤트 싱크](../../dotnet/how-to-sink-windows-forms-events-from-native-cpp-classes.md)를 참조 하세요.
