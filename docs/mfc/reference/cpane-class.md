...
ms.locfileid: "78883933"
---
# <a name="cpane-class"></a>CPane Class

`CPane` 클래스는 [Ccontrolbar 클래스](../../mfc/reference/ccontrolbar-class.md)의 향상 된 기능입니다. 기존 MFC 프로젝트를 업그레이드 하는 경우 `CControlBar`의 모든 항목을 `CPane`으로 바꿉니다.
...
|[CPane:: Create](#create)|컨트롤 막대를 만들어 `CPane` 개체에 연결 합니다.|
|[CPane:: CreateDefaultMiniframe](#createdefaultminiframe)|부동 창의 미니 프레임 창을 만듭니다.|
|[CPane:: CreateEx](#createex)|컨트롤 막대를 만들어 `CPane` 개체에 연결 합니다.|
...
|[CPane::D Ockto프레임 창](#docktoframewindow)|도킹 가능한 창을 프레임에 도킹 합니다. (`CBasePane::DockToFrameWindow`을 재정의함)|
...
|[CPane:: GetHotSpot](#gethotspot)|기본 `CMFCDragFrameImpl` 개체에 저장 된 핫 스폿을 반환 합니다.|
...
|[CPane:: IsInFloatingMultiPaneFrameWnd](#isinfloatingmultipaneframewnd)|창이 다중 창 프레임 창에 있는지 여부를 지정 합니다. (`CBasePane::IsInFloatingMultiPaneFrameWnd`을 재정의함)|
...
`CPane`에서 클래스를 파생 하는 경우 [CDockSite](../../mfc/reference/cdocksite-class.md) 에 도킹 될 수 있으며 [CPaneFrameWnd](../../mfc/reference/cpaneframewnd-class.md)에서 부동 상태로 있을 수 있습니다.
...
*bActiveTabOnly*<br/>
진행 `CPane::ConvertToTabbedDocument`에서 사용 되지 않습니다.
...
이 메서드는 Windows 창을 만들어 `CPane` 개체에 연결 합니다.

`Create`를 호출 하기 전에 [Cpane:: m_recentDockInfo](#m_recentdockinfo) 를 명시적으로 초기화 하지 않은 경우에는 창을 부동 하거나 도킹할 때 매개 변수 *rect* 가 사각형으로 사용 됩니다.
...
미니 프레임 창에 대 한 런타임 클래스 정보는 `CPane::m_pMiniFrameRTC` 멤버에 저장 됩니다. 사용자 지정 된 미니 프레임 창을 만들도록 결정 한 경우 파생 클래스를 사용 하 여이 멤버를 설정할 수 있습니다.
...
`CreateEx`를 호출 하기 전에 [Cpane:: m_recentDockInfo](#m_recentdockinfo) 를 명시적으로 초기화 하지 않은 경우에는 창을 부동 하거나 도킹할 때 매개 변수 *rect* 가 사각형으로 사용 됩니다.
...
##  <a name="dockpanestandard"></a>CPane::D Ocksestandand
...
##  <a name="docktoframewindow"></a>CPane::D Ockto프레임 창
...
창 양쪽의 현재 너비 (픽셀)를 포함 하는 [Crect](../../atl-mfc-shared/reference/crect-class.md) 개체입니다. 예를 들어 `CRect` 개체의 `left` 멤버 값은 왼쪽 테두리의 너비입니다.
...
창이 도킹 된 dock 행을 가리키는 `CDockingPanesRow`* 이거나 창이 도킹 되지 않은 경우 NULL입니다.
...
기본 `CMFCDragFrameImpl` 개체에 저장 된 핫 스폿을 반환 합니다.
...
`CPane` 클래스에는 사용자가 표준 도킹 모드로 창을 이동할 때 나타나는 사각형의 그리기를 담당 하는 `CMFCDragFrameImpl` 개체 `m_dragFrameImpl`포함 되어 있습니다. 핫 스폿은 사용자가 창을 이동할 때 현재 마우스 위치를 기준으로 사각형을 그리는 데 사용 됩니다.
...
*strName*<br/>
제한이 캡션 이름으로 채워지는 `CString` 개체입니다.

### <a name="remarks"></a>설명

창이 도킹 되거나 부동 창이 표시 되 면 캡션 영역에 창 제목이 표시 됩니다. 창이 탭 그룹의 일부인 경우 탭 영역에 제목이 표시 됩니다. 창이 자동 숨기기 모드에 있으면 제목은 `CMFCAutoHideButton`에 표시 됩니다.
...
도킹 가능한 창만 다중 창 프레임 창에서 부동 창으로 표시할 수 있습니다. 따라서 `CPane::IsInFloatingMultiPaneFrameWnd` 항상 FALSE를 반환 합니다.
...
##  <a name="isresizable"></a>CPane:: IsResizable 조정 가능
...
응용 프로그램에서 하나 이상의 도킹 창이 `GetMinSize`를 재정의 하거나 응용 프로그램이 `SetMinSize`를 호출 하는 경우 프레임 워크에서 창의 크기를 일관 되 게 처리할 수 있도록이 정적 멤버를 TRUE로 설정 하는 것이 좋습니다.
...
##  <a name="movepane"></a>Cpane:: movepmov
...
새 사각형과 이전 사각형 (이전 사각형- *rectNew*) 사이의 너비와 높이의 차이를 포함 하는 `CSize` 개체입니다.
...
기본적으로 `CDockSite::RemovePane`를 호출 하 여 도킹 창에서 창을 등록 취소 합니다.
...
##  <a name="onpressclosebutton"></a>CPane:: On보도 Sclosebutton
...
창과 연결 된 자동 숨기기 단추의 모양은 두 가지 요소를 기준으로 합니다. 창이 활성 상태이 고 `static BOOL CMFCAutoHideButton::m_bOverlappingTabs` TRUE 이면 프레임 워크에서 자동 숨기기 단추를 아이콘과 레이블로 표시 합니다. 비활성 창의 경우 프레임 워크는 자동 숨기기 아이콘만 표시 합니다.

`CMFCAutoHideButton::m_bOverlappingTabs` FALSE 인 경우 또는 창이 그룹에 없는 경우 프레임 워크는 연결 된 자동 숨기기 단추를 아이콘과 레이블로 표시 합니다.
...
*size*<br/>
진행 창의 허용 되는 최소 크기를 포함 하는 `CSize` 개체입니다.
...
*rect*<br/>
진행 설정할 가상 사각형을 지정 하는 `CRect` 개체입니다.
...
창이 부동 되 면 [CPaneFrameWnd](../../mfc/reference/cpaneframewnd-class.md) (미니 프레임) 창에 배치 됩니다. [Cpane:: CreateDefaultMiniframe](#createdefaultminiframe) 가 호출 될 때 사용 되는 사용자 지정 `CPaneFrameWnd`파생 클래스를 제공할 수 있습니다.
...
*ptOffset*<br/>
진행 창을 이동할 오프셋을 지정 하는 `CPoint` 개체입니다.

*sizeNew*<br/>
진행 창의 새 크기를 지정 하는 `CSize` 개체입니다.
