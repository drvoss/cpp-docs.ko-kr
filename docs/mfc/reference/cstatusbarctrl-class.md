...
|속성|Description|
|----------|-----------------|
|[CStatusBarCtrl:: Create](#create)|상태 표시줄 컨트롤을 만들어 `CStatusBarCtrl` 개체에 연결 합니다.|
|[CStatusBarCtrl:: CreateEx](#createex)|지정 된 Windows 확장 스타일을 사용 하 여 상태 표시줄 컨트롤을 만들고 `CStatusBarCtrl` 개체에 연결 합니다.|
|[CStatusBarCtrl::D rawItem](#drawitem)|소유자 그리기 상태 표시줄 컨트롤의 시각적 측면이 변경 될 때 호출 됩니다.|
...
|[CStatusBarCtrl:: SetSimple](#setsimple)|상태 표시줄 컨트롤에서 간단한 텍스트를 표시할지 또는 `SetParts`에 대 한 이전 호출로 설정 된 모든 컨트롤 파트를 표시할지 여부를 지정 합니다.|
...
이 컨트롤과 `CStatusBarCtrl` 클래스는 Windows 95/98 및 Windows NT 버전 3.51 이상에서 실행 되는 프로그램에만 사용할 수 있습니다.

`CStatusBarCtrl`사용에 대 한 자세한 내용은 [컨트롤](../../mfc/controls-mfc.md) 및 [사용 CStatusBarCtrl](../../mfc/using-cstatusbarctrl.md)을 참조 하세요.
...
상태 표시줄 컨트롤을 만들어 `CStatusBarCtrl` 개체에 연결 합니다.
...
*pParentWnd*<br/>
상태 표시줄 컨트롤의 부모 창 (일반적으로 `CDialog`)을 지정 합니다. NULL이 아니어야 합니다.
...
`CStatusBarCtrl`는 두 단계로 구성 합니다. 먼저 생성자를 호출한 다음 상태 표시줄 컨트롤을 만들어 `CStatusBarCtrl` 개체에 연결 하는 `Create`를 호출 합니다.

상태 창의 기본 위치는 부모 창의 아래쪽에 있지만 부모 창의 클라이언트 영역 맨 위에 표시 되도록 CCS_TOP 스타일을 지정할 수 있습니다. 상태 창의 오른쪽 끝에 크기 조정 그립을 포함 하도록 SBARS_SIZEGRIP 스타일을 지정할 수 있습니다. 시스템이 상태 창에 그려지는 경우에도 결과 크기 조정 그립은 작동 하지 않으므로 CCS_TOP 및 SBARS_SIZEGRIP 스타일을 결합 하지 않는 것이 좋습니다.

확장 창 스타일을 사용 하 여 상태 표시줄을 만들려면 `Create`대신 [CStatusBarCtrl:: CreateEx](#createex) 를 호출 합니다.
...
컨트롤 (자식 창)을 만들어 `CStatusBarCtrl` 개체와 연결 합니다.
...
이 메서드는 Windows SDK에서 설명 하는 [SB_GETICON](/windows/win32/Controls/sb-geticon) 메시지를 보냅니다.
...
다음 코드 예제에서는 현재 상태 표시줄 컨트롤에 액세스 하는 데 사용 되는 변수 `m_statusBar`를 정의 합니다. 이 변수는 다음 예제에서 사용됩니다.
...
- 텍스트를 테두리 없이 그릴 SBT_NOBORDERS 합니다.
...
- 부모 창에서 텍스트를 그릴 SBT_OWNERDRAW입니다.
...
*lpszText*<br/>
설정할 텍스트를 지정하는 null 종료 문자열의 주소입니다. *NType* 가 SBT_OWNERDRAW 이면 *lpszText* 는 32 비트 데이터를 나타냅니다.
...
이 메시지는 컨트롤이 다음에 WM_PAINT 메시지를 받을 때 새 텍스트를 표시 하도록 변경 된 컨트롤 부분을 무효화 합니다.
