...
|속성|Description|
|----------|-----------------|
|[COleServerDoc:: Docobject](#activatedocobject)|연결 된 DocObject 문서를 활성화 합니다.|
|[COleServerDoc:: ActivateInPlace](#activateinplace)|내부 편집을 위해 문서를 활성화 합니다.|
|[COleServerDoc::D eactivateAndUndo](#deactivateandundo)|서버의 사용자 인터페이스를 비활성화 합니다.|
|[COleServerDoc::D iscardUndoState](#discardundostate)|실행 취소 상태 정보를 삭제 합니다.|
...
|[COleServerDoc:: Get확대/비율](#getzoomfactor)|확대/축소 비율 (픽셀)을 반환 합니다.|
...
|[COleServerDoc::D estroyInPlaceFrame](#destroyinplaceframe)|내부 편집을 위해 프레임 창을 제거 하기 위해 프레임 워크에서 호출 됩니다.|
...
|[COleServerDoc:: On프레임 창 활성화](#onframewindowactivate)|컨테이너의 프레임 창이 활성화 또는 비활성화 될 때 프레임 워크에서 호출 됩니다.|
|[COleServerDoc:: OnGetEmbeddedItem](#ongetembeddeditem)|전체 문서를 나타내는 `COleServerItem`을 가져오기 위해 호출 됩니다. 포함 된 항목을 가져오는 데 사용 됩니다. 구현이 필요 합니다.|
...
서버 문서는 포함 되거나 연결 된 항목에 대 한 서버 인터페이스를 나타내는 [COleServerItem](../../mfc/reference/coleserveritem-class.md) 개체를 포함할 수 있습니다. 컨테이너에서 서버 응용 프로그램을 시작 하 여 포함 된 항목을 편집 하는 경우 항목이 자체 서버 문서로 로드 됩니다. `COleServerDoc` 개체는 전체 문서로 구성 된 하나의 `COleServerItem` 개체만 포함 합니다. 연결 된 항목을 편집 하기 위해 컨테이너에서 서버 응용 프로그램을 시작 하면 기존 문서가 디스크에서 로드 됩니다. 문서 내용의 일부가 강조 표시 되어 연결 된 항목을 표시 합니다.

`COleServerDoc` 개체에는 [COleClientItem](../../mfc/reference/coleclientitem-class.md) 클래스의 항목이 포함 될 수도 있습니다. 이렇게 하면 컨테이너 서버 응용 프로그램을 만들 수 있습니다. 프레임 워크는 `COleServerItem` 개체를 서비스 하는 동안 `COleClientItem` 항목을 올바르게 저장 하는 함수를 제공 합니다.
...
`COleServerDoc`를 사용 하려면 클래스에서 클래스를 파생 시키고 서버에서 포함 된 항목을 지원할 수 있도록 하는 [OnGetEmbeddedItem](#ongetembeddeditem) 멤버 함수를 구현 합니다. `COleServerItem`에서 클래스를 파생 시켜 문서에 있는 항목을 구현 하 고 `OnGetEmbeddedItem`에서 해당 클래스의 개체를 반환 합니다.

연결 된 항목을 지원 하기 위해 `COleServerDoc` [OnGetLinkedItem](../../mfc/reference/colelinkingdoc-class.md#ongetlinkeditem) 멤버 함수를 제공 합니다. 기본 구현을 사용 하거나 문서 항목을 관리 하는 고유한 방법이 있는 경우이를 재정의할 수 있습니다.

응용 프로그램에서 지 원하는 각 서버 문서 형식에 대해 하나의 `COleServerDoc`파생 클래스가 필요 합니다. 예를 들어 서버 응용 프로그램에서 워크시트와 차트를 지 원하는 경우 두 개의 `COleServerDoc`파생 클래스가 필요 합니다.

서버에 대 한 자세한 내용은 서버 [: 서버 구현](../../mfc/servers-implementing-a-server.md)문서를 참조 하세요.
...
##  <a name="activatedocobject"></a>COleServerDoc:: Docobject
...
기본적으로 `COleServerDoc`는 액티브 문서 (DocObjects 라고도 함)를 지원 하지 않습니다. 이 지원을 사용 하도록 설정 하려면 [GetDocObjectServer](#getdocobjectserver) And 클래스 [CDocObjectServer](../../mfc/reference/cdocobjectserver-class.md)를 참조 하세요.
...
OLE 시스템 Dll과 연결 하지 않고 `COleServerDoc` 개체를 생성 합니다.
...
OLE와의 통신을 열려면 [COleLinkingDoc:: Register](../../mfc/reference/colelinkingdoc-class.md#register) 를 호출 해야 합니다. 응용 프로그램에서 [COleTemplateServer](../../mfc/reference/coletemplateserver-class.md) 를 사용 하는 경우 `COleLinkingDoc`의 `OnNewDocument`, `OnOpenDocument`및 `OnSaveDocument`구현에 따라 `COleLinkingDoc::Register`가 호출 됩니다.
...
*pDocSite*<br/>
이 문서를 서버에 연결 하는 `IOleDocumentSite` 인터페이스에 대 한 포인터입니다.

### <a name="return-value"></a>Return Value

`CDocObjectServer`에 대 한 포인터입니다. 작업이 실패 한 경우 NULL입니다.

### <a name="remarks"></a>설명

DocObject 서버가 활성화 되 면 NULL이 아닌 포인터를 반환 하면 클라이언트가 DocObjects를 지원할 수 있음을 보여 줍니다. 기본 구현에서는 NULL을 반환 합니다.

DocObjects를 지 원하는 문서에 대 한 일반적인 구현은 단순히 새 `CDocObjectServer` 개체를 할당 하 고 호출자에 게 반환 합니다. 다음은 그 예입니다.
...
*lpClipRect*<br/>
항목의 클리핑 사각형 좌표를 수신 하기 위한 `RECT` 구조체 또는 `CRect` 개체에 대 한 포인터입니다.
...
*lpPosRect*<br/>
항목의 좌표를 받을 `RECT` 구조체 또는 `CRect` 개체에 대 한 포인터입니다.
...
##  <a name="getzoomfactor"></a>COleServerDoc:: Get확대/비율

`GetZoomFactor` 멤버 함수는 내부 편집을 위해 활성화 된 항목의 "확대/축소 비율"을 결정 합니다.
...
*lpSizeNum*<br/>
확대/축소 비율의 분자를 포함 하는 `CSize` 클래스의 개체에 대 한 포인터입니다. NULL일 수 있습니다.

*lpSizeDenom*<br/>
확대/축소 비율의 분모를 포함 하는 `CSize` 클래스의 개체에 대 한 포인터입니다. NULL일 수 있습니다.

*lpPosRect*<br/>
항목의 새 위치를 설명 하는 `CRect` 클래스의 개체에 대 한 포인터입니다. 이 인수가 NULL 이면 함수는 항목의 현재 위치를 사용 합니다.
...
`COleServerDoc` 개체가 컨테이너에 포함 된 개체를 나타내는 문서인 경우 0이 아닙니다. 그렇지 않으면 0입니다.
...
`COleServerDoc` 개체가 현재 활성화 되어 있으면 0이 아닌 값이 고, 그렇지 않으면 0입니다.
...
일반적으로 사용자가 서버 문서의 크기와 같은 일부 전역 특성을 변경한 후이 함수를 호출 합니다. OLE 항목이 자동 링크를 사용 하 여 문서에 연결 된 경우 해당 항목은 변경 내용을 반영 하도록 업데이트 됩니다. MFC 라이브러리를 사용 하 여 작성 된 컨테이너 응용 프로그램에서 `COleClientItem`의 [OnChange](../../mfc/reference/coleclientitem-class.md#onchange) 멤버 함수가 호출 됩니다.
...
사용자가 파일 메뉴에서 닫기 명령을 선택 하면 [Onclosedocument](../../mfc/reference/cdocument-class.md#onclosedocument) 멤버 함수 `COleServerDoc`의 구현에서 `NotifyClosed`가 호출 됩니다. MFC 라이브러리를 사용 하 여 작성 된 컨테이너 응용 프로그램에서 `COleClientItem`의 [OnChange](../../mfc/reference/coleclientitem-class.md#onchange) 멤버 함수가 호출 됩니다.
...
사용자가 파일 메뉴에서 다른 이름으로 저장 명령을 선택 하면 [Onsavedocument](../../mfc/reference/cdocument-class.md#onsavedocument) 멤버 함수 `COleServerDoc`의 구현에서 `NotifyRename`가 호출 됩니다. 이 함수는 OLE 시스템 Dll에이를 알리고 컨테이너에이를 알립니다. MFC 라이브러리를 사용 하 여 작성 된 컨테이너 응용 프로그램에서 `COleClientItem`의 [OnChange](../../mfc/reference/coleclientitem-class.md#onchange) 멤버 함수가 호출 됩니다.
...
사용자가 파일 메뉴에서 저장 명령을 선택 하면 [Onsavedocument](../../mfc/reference/cdocument-class.md#onsavedocument)를 `COleServerDoc`구현 하 여 `NotifySaved`가 호출 됩니다. 이 함수는 OLE 시스템 Dll에이를 알리고 컨테이너에이를 알립니다. MFC 라이브러리를 사용 하 여 작성 된 컨테이너 응용 프로그램에서 `COleClientItem`의 [OnChange](../../mfc/reference/coleclientitem-class.md#onchange) 멤버 함수가 호출 됩니다.
...
- 파일이 수정 된 경우 저장 되 OLECLOSE_SAVEIFDIRTY 합니다.

- 파일이 저장 되지 않고 닫혀 OLECLOSE_NOSAVE 합니다.

- OLECLOSE_PROMPTSAVE 파일을 수정한 경우 사용자에 게 파일을 저장 하 라는 메시지가 표시 됩니다.

### <a name="remarks"></a>설명

기본 구현에서는 `CDocument::OnCloseDocument`를 호출 합니다.
...
*pvarargIn*<br/>
명령에 대 한 입력 인수가 포함 된 VARIANTARG에 대 한 포인터입니다. NULL일 수 있습니다.

*pvarargOut*<br/>
명령에서 출력 반환 값을 받는 VARIANTARG에 대 한 포인터입니다. NULL일 수 있습니다.

### <a name="return-value"></a>Return Value

성공 하면 S_OK 반환 합니다. 그렇지 않으면 다음 오류 코드 중 하나입니다.
...
`COleCmdUI`를 사용 하 여 DocObject 사용자 인터페이스 명령의 다른 속성을 설정, 업데이트 및 설정할 수 있습니다. 명령이 초기화 된 후 `OnExecOleCmd`를 사용 하 여 실행할 수 있습니다.
...
대부분의 명령은 인수 또는 반환 값을 사용 하지 않습니다. 대부분의 명령은 호출자가 *Pvarargin* 및 *Pvarargin*에 대 한 null을 전달할 수 있습니다. 입력 값을 필요로 하는 명령의 경우 호출자는 VARIANTARG 변수를 선언 하 고 초기화 하 고 *의 Pvarargin*변수에 포인터를 전달할 수 있습니다. 단일 값이 필요한 명령의 경우 인수를 VARIANTARG에 직접 저장 하 고 함수에 전달할 수 있습니다. `IDispatch` 및 SAFEARRAY와 같이 지원 되는 형식 중 하나를 사용 하 여 VARIANTARG 내에서 여러 인수를 패키지 해야 합니다.

마찬가지로, 명령에서 인수를 반환 하는 경우 호출자는 VARIANTARG을 선언 하 고, VT_EMPTY로 초기화 하 고, *Pvarargout*에서 해당 주소를 전달 합니다. 명령에서 단일 값을 반환 하는 경우 개체는 해당 값을 *Pvarargout*에 직접 저장할 수 있습니다. 여러 출력 값은 VARIANTARG에 적절 한 방식으로 패키지 되어야 합니다.

이 함수의 기본 클래스 구현에서는 명령 대상과 연결 된 OLE_COMMAND_MAP 구조를 탐색 하 고 적절 한 처리기에 명령을 디스패치합니다. 기본 클래스 구현은 인수나 반환 값을 허용 하지 않는 명령 에서만 작동 합니다. 인수 또는 반환 값을 허용 하는 명령을 처리 해야 하는 경우이 함수를 재정의 하 고 *Pvarargin* 및 *Pvarargin* 매개 변수를 직접 사용 해야 합니다.

##  <a name="onframewindowactivate"></a>COleServerDoc:: On프레임 창 활성화
...
기본 구현은 없습니다. 전체 문서를 나타내는 항목을 반환 하려면이 함수를 재정의 해야 합니다. 이 반환 값은 `COleServerItem`파생 클래스의 개체 여야 합니다.
...
응용 프로그램에서 실행 취소를 지 원하는 경우이 함수를 재정의 합니다. 일반적으로 실행 취소 작업을 수행 하 고 `ActivateInPlace`를 호출 하 여 항목을 활성화 합니다. MFC 라이브러리를 사용 하 여 컨테이너 응용 프로그램을 작성 하는 경우 `COleClientItem::ReactivateAndUndo`를 호출 하면이 함수가 호출 됩니다.
...
*lpRectBorder*<br/>
테두리의 좌표를 지정 하는 `RECT` 구조체 또는 `CRect` 개체에 대 한 포인터입니다.

*lpUIWindow*<br/>
현재 내부 편집 세션을 소유 하는 `IOleInPlaceUIWindow` 클래스의 개체에 대 한 포인터입니다.
...
*lpPosRect*<br/>
컨테이너 응용 프로그램의 클라이언트 영역을 기준으로 내부 프레임 창의 위치를 지정 하는 `CRect` 개체 또는 `RECT` 구조체에 대 한 포인터입니다.

*lpClipRect*<br/>
컨테이너 응용 프로그램의 클라이언트 영역을 기준으로 내부 프레임 창의 클리핑 사각형을 지정 하는 `RECT` 구조체 또는 `CRect` 개체에 대 한 포인터입니다.

### <a name="remarks"></a>설명

필요한 경우이 함수를 재정의 하 여 뷰의 확대/축소 비율을 업데이트 합니다.

이 함수는 일반적으로 컨테이너에서 내부 항목의 위치 변경을 요청 하기 위해 언제 든 지 호출할 수 있지만 일반적으로 `RequestPositionChange` 호출에 대 한 응답으로 호출 됩니다.
...
서버 문서를 숨기 거 나 표시 해야 하는 경우 프레임 워크는 `OnShowDocument` 함수를 호출 합니다.
...
*Bshow* 가 TRUE 이면 필요에 따라 기본 구현에서 서버 응용 프로그램을 활성화 하 고 컨테이너 응용 프로그램이 해당 창을 스크롤하여 항목이 표시 되도록 합니다. *Bshow* 가 FALSE 이면 기본 구현에서 `OnDeactivate`호출을 통해 항목을 비활성화 한 다음 첫 번째 문서를 제외 하 고 문서에 대해 만들어진 모든 프레임 창을 제거 하거나 숨깁니다. 표시 되는 문서가 남아 있지 않으면 기본 구현에서 서버 응용 프로그램을 숨깁니다.
...
*lpPosRect*<br/>
항목의 새 위치를 포함 하는 `CRect` 개체 또는 `RECT` 구조체에 대 한 포인터입니다.

### <a name="remarks"></a>설명

이 함수는 내부 활성 항목의 데이터가 변경 된 경우 일반적으로 `UpdateAllItems`와 함께 호출 됩니다. 이 호출을 수행 하면 컨테이너는 `OnSetItemRects`를 호출 하 여 변경을 수행할 수도 있고 그렇지 않을 수도 있습니다. 결과 위치가 요청한 것과 다를 수 있습니다.
...
이 함수는 `OnUpdateDocument`에서 자동으로 호출 됩니다. 이 함수를 사용 하면 항목이 디스크에서 업데이트 되므로 일반적으로 특정 사용자 작업의 결과로만 호출 됩니다.

##  <a name="scrollcontainerby"></a>COleServerDoc:: ScrollContainerBy

`ScrollContainerBy` 멤버 함수를 호출 하 여 `sizeScroll`으로 표시 되는 크기 (픽셀) 만큼 컨테이너 문서를 스크롤합니다.
...
일반적으로 사용자가 서버 문서를 변경한 후이 함수를 호출 합니다. OLE 항목이 자동 링크를 사용 하 여 문서에 연결 된 경우 해당 항목은 변경 내용을 반영 하도록 업데이트 됩니다. MFC 라이브러리를 사용 하 여 작성 된 컨테이너 응용 프로그램에서 `COleClientItem`의 [OnChange](../../mfc/reference/coleclientitem-class.md#onchange) 멤버 함수가 호출 됩니다.

이 함수는 송신 항목을 제외한 각 문서 항목에 대해 `OnUpdate` 멤버 함수를 호출 하 여 *Phint*, *Lhint*및 *ndrawaspect*를 전달 합니다. 이러한 매개 변수를 사용 하 여 문서에 대 한 수정 내용에 대 한 정보를 항목에 전달 합니다. *Lhint* 를 사용 하 여 정보를 인코딩하거나 `CObject`파생 클래스를 정의 하 여 수정 정보를 저장 하 고 *phint*를 사용 하 여 해당 클래스의 개체를 전달할 수 있습니다. `COleServerItem`파생 클래스에서 `OnUpdate` 멤버 함수를 재정의 하 여 프레젠테이션이 변경 되었는지 여부에 따라 각 항목의 업데이트를 최적화 합니다.
