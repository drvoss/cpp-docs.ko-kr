...
|[CRichEditView:: QueryAcceptData](#queryacceptdata)|`IDataObject`데이터에 대 한 정보를 확인 하는 쿼리입니다.|
|[CRichEditView:: WrapChanged](#wrapchanged)|`m_nWordWrap`의 값을 기반으로 하 여이 rich edit 뷰의 대상 출력 장치를 조정 합니다.|
...
`CRichEditView` 텍스트의 텍스트와 서식 특성을 유지 관리 합니다. `CRichEditDoc` 뷰에 있는 OLE 클라이언트 항목의 목록을 유지 관리 합니다. `CRichEditCntrItem`는 OLE 클라이언트 항목에 대 한 컨테이너 쪽 액세스를 제공 합니다.
...
이 함수를 호출 하 여 `CRichEditView` 개체를 만듭니다.
...
반환 값이 success를 나타내는 경우 `IRichEditOleCallback::GetClipboardData`는 *lplpdataobj*에서 액세스 하는 `IDataObject` 반환 합니다. 그렇지 않으면 *lpRichDataObj*에서 액세스 한 항목을 반환 합니다. 사용자 고유의 클립보드 데이터를 제공 하려면이 함수를 재정의 합니다. 이 함수의 기본 구현은 E_NOTIMPL을 반환 합니다.
...
*lpoleobj*<br/>
선택에 OLE 항목이 하나 이상 포함 되어 있는 경우 첫 번째 선택 된 OLE 개체를 지정 하는 `OLEOBJECT` 구조체에 대 한 포인터입니다. 선택 항목에 항목이 없으면 *lpoleobj* 가 NULL입니다. `OLEOBJECT` 구조체에는 OLE 개체 v-table에 대 한 포인터가 포함 되어 있습니다.
...
이 뷰와 연결 된 `CRichEditDoc`에 대 한 포인터를 가져오려면이 함수를 호출 합니다.
...
이 `CRichEditView` 개체에서 현재 활성화 된 OLE 항목을 가져오려면이 함수를 호출 합니다.
...
이 `CRichEditView` 개체에서 현재 선택 된 OLE 항목 (`CRichEditCntrItem` 개체)을 검색 하려면이 함수를 호출 합니다.
...
이 `CRichEditView` 개체에서 텍스트의 길이를 검색 하려면이 함수를 호출 합니다.
...
이 `CRichEditView` 개체에서 텍스트의 길이를 계산 하려면이 멤버 함수를 호출 합니다.
...
`GetTextLengthEx`는 텍스트의 길이를 결정 하는 추가 방법을 제공 합니다. Rich Edit 2.0 기능을 지원 합니다. 자세한 내용은 Windows SDK의 [Rich Edit Controls 정보](/windows/win32/Controls/about-rich-edit-controls) 를 참조 하세요.

##  <a name="insertfileasobject"></a>CRichEditView:: Insertfil 개체
...
- `WrapToWindow`는 창의 너비를 기준으로 단어 잘림을 나타냅니다.

- `WrapToTargetDevice`는 대상 장치의 특성에 따라 자동 줄 바꿈을 나타냅니다.
...
`CRichEditView`내에서 텍스트를 찾으려면이 함수를 호출 합니다. 파생 된 뷰 클래스에 대 한 검색 특성을 변경 하려면이 함수를 재정의 합니다.
...
일반적으로 `IStorage`주위에서 [Colestreamfile](../../mfc/reference/colestreamfile-class.md) 을 만들어이 작업을 수행 합니다. 데이터를 로드 하기 위해 호출 된 archive 및 [CObject:: Serialize](../../mfc/reference/cobject-class.md#serialize) 에 `COleStreamFile`를 연결할 수 있습니다.
...
##  <a name="onprinterchanged"></a>CRichEditView:: On프린터 변경 됨
...
HRESULT 및 `IDataObject`에 대 한 자세한 내용은 Windows SDK의 [COM 오류 코드](/windows/win32/com/structure-of-com-error-codes) 및 [IDataObject](/windows/win32/api/objidl/nn-objidl-idataobject)의 구조를 참조 하세요.
...
*Cf* 의 `dwMask` 멤버로 지정 된 특성만이 함수에 의해 변경 됩니다.
...
[M_nWordWrap](#m_nwordwrap) `WrapToTargetDevice`되는 경우이 함수를 사용 하 여 인쇄 특성을 조정한 후 [WrapChanged](#wrapchanged) 를 호출 해야 합니다.

[System.drawing.printing.printdocument.printpage>](#printpage) 에서 사용 하는 여백은 논리적 페이지가 아니라 물리적 페이지를 기준으로 합니다. 따라서 여백이 0 인 경우에는 페이지에 인쇄할 수 없는 영역이 있으므로 많은 프린터에서 텍스트를 클리핑 하는 경우가 많습니다. 텍스트를 클리핑 하지 않으려면 인쇄 하기 전에 `SetMargins` 사용을 호출 하 여 적절 한 프린터 여백을 설정 해야 합니다.
...
이 `CRichEditView` 개체에서 현재 선택 영역에 대 한 단락 서식 특성을 설정 하려면이 함수를 호출 합니다.
...
이 함수는 *pf* 의 `dwMask` 멤버로 지정 된 특성만 변경 합니다.
