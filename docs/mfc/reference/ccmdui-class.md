...
는 `CCmdTarget`파생 클래스의 `ON_UPDATE_COMMAND_UI` 처리기 내 에서만 사용 됩니다.
...
|[CCmdUI:: SetRadio](#setradio)|`SetCheck` 멤버 함수와 같지만 라디오 그룹에서 작동 합니다.|
...
|[CCmdUI:: m_pSubMenu](#m_psubmenu)|`CCmdUI` 개체가 나타내는 포함 된 하위 메뉴를 가리킵니다.|
...
메뉴를 끌어올 때 프레임 워크는 각 ON_UPDATE_COMMAND_UI 처리기를 검색 하 고 호출 하며, 각 처리기는 `Enable` 및 `Check`와 같은 멤버 함수 `CCmdUI`를 호출 하 고 프레임 워크는 각 메뉴 항목을 적절 하 게 표시 합니다.

메뉴 항목은 `ON_UPDATE_COMMAND_UI` 처리기 내에서 코드를 변경 하지 않고 컨트롤 막대 단추 또는 다른 명령 사용자 인터페이스 개체로 바꿀 수 있습니다.

다음 표에서는 `CCmdUI`의 멤버 함수가 다양 한 명령 사용자 인터페이스 항목에 미치는 영향을 요약 하 여 보여 줍니다.
...
`CCmdUI` 개체가 나타내는 메뉴 항목, 도구 모음 단추 또는 기타 사용자 인터페이스 개체의 ID입니다.
...
`CCmdUI` 개체가 나타내는 메뉴 항목, 도구 모음 단추 또는 기타 사용자 인터페이스 개체의 인덱스입니다.
...
`CCmdUI` 개체가 나타내는 메뉴에 대 한 포인터 (`CMenu` 형식)입니다.
...
`CCmdUI` 개체가 나타내는 포함 된 하위 메뉴에 대 한 포인터 (`CMenu` 형식)입니다.
...
항목이 메뉴가 아니면 NULL입니다. 하위 메뉴가 팝업 인 경우 팝업 메뉴의 첫 번째 항목에 대 한 ID를 포함 *m_nID* 합니다. 자세한 내용은 [Technical Note 21](../../mfc/tn021-command-and-message-routing.md)을 참조 하세요.

##  <a name="m_pother"></a>CCmdUI:: m_pOther

알림을 보낸 도구 또는 상태 표시줄과 같은 창 개체에 대 한 `CWnd`형식의 포인터입니다.
...
이 멤버 함수는 라디오 그룹의 일부로 작동 하는 사용자 인터페이스 항목에 대해 작동 한다는 점을 제외 하 고 `SetCheck`처럼 작동 합니다. 항목 자체가 라디오 그룹 동작을 유지 해야 하는 경우가 아니면 그룹의 다른 항목을 선택을 취소 하는 것은 자동으로 진행 되지 않습니다.
