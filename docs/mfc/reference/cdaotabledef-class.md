...
|[CDaoTableDef:: GetAttributes](#getattributes)|`CDaoTableDef` 개체의 특성을 하나 이상 나타내는 값을 반환 합니다.|
|[CDaoTableDef:: GetConnect](#getconnect)|테이블의 원본에 대 한 정보를 제공 하는 값을 반환 합니다.|
|[CDaoTableDef::GetDateCreated](#getdatecreated)|`CDaoTableDef` 개체의 기본 테이블이 만들어진 날짜와 시간을 반환 합니다.|
...
|[CDaoTableDef:: SetAttributes](#setattributes)|`CDaoTableDef` 개체의 특성을 하나 이상 나타내는 값을 설정 합니다.|
...
`CDaoTableDef` 개체를 사용 하 여 테이블 정의를 조작 합니다. 예를 들어, 다음을 수행할 수 있습니다.

- 데이터베이스에 있는 로컬, 연결 또는 외부 테이블의 필드 및 인덱스 구조를 검사 합니다.

- 연결 된 테이블에 대 한 `SetConnect` 및 `SetSourceTableName` 멤버 함수를 호출 하 고 `RefreshLink` 멤버 함수를 사용 하 여 연결 된 테이블에 대 한 연결을 업데이트 합니다.

- `CanUpdate` 멤버 함수를 호출 하 여 테이블에서 필드 정의를 편집할 수 있는지 확인 합니다.

- `GetValidationRule` 및 `SetValidationRule`및 `GetValidationText` 및 `SetValidationText` 멤버 함수를 사용 하 여 유효성 검사 조건을 가져오거나 설정 합니다.

- `Open` 멤버 함수를 사용 하 여 테이블, 다이너셋 또는 스냅숏 형식 `CDaoRecordset` 개체를 만들 수 있습니다.
...
1. 모든 경우에는 먼저 `CDaoTableDef` 개체를 생성 하 고 테이블이 속한 [CDaoDatabase](../../mfc/reference/cdaodatabase-class.md) 개체에 대 한 포인터를 제공 합니다.
...
1. 그런 다음 원하는 항목에 따라 다음을 수행 합니다.
   ...
   - [Append](#append) 를 호출 하 여 테이블을 데이터베이스의 테이블 컬렉션 컬렉션에 추가 하 여 테이블을 저장 합니다. `Create`는 테이블 정의를 열린 상태로 전환 하므로 `Create`를 호출한 후 `Open`를 호출 하지 않습니다.

        > [!TIP]
        >  저장 된 테이블을 만드는 가장 쉬운 방법은이를 만들어 Microsoft Access를 사용 하 여 데이터베이스에 저장 하는 것입니다. 그런 다음 MFC 코드에서 열고 사용할 수 있습니다.

열거나 만든 테이블 형식 개체를 사용 하려면 *nOpenType* 매개 변수에 `dbOpenTable` 값을 사용 하 여 테이블 형식 이름을 지정 하 `CDaoRecordset` 개체를 만들고 엽니다.

테이블 정의 개체를 사용 하 여 `CDaoRecordset` 개체를 만들려면 일반적으로 위에서 설명한 대로 테이블 정의를 만들거나 연 다음 [CDaoRecordset:: open](../../mfc/reference/cdaorecordset-class.md#open)을 호출할 때 테이블 정의 개체에 대 한 포인터를 전달 하 여 레코드 집합 개체를 생성 합니다. 전달 하는 tabledef는 열린 상태 여야 합니다. 자세한 내용은 클래스 [CDaoRecordset](../../mfc/reference/cdaorecordset-class.md)를 참조 하세요.
...
함수는 데이터베이스의 테이블 컬렉션에 개체를 추가 합니다. 테이블 정의를 추가 하지 않고 정의 하는 동안 임시 개체로 정의를 사용할 수 있지만 저장 하 고 사용 하려는 경우 `Append`를 호출 해야 합니다.
...
`CDaoTableDef` 개체의 기반이 되는 테이블의 정의를 변경할 수 있는지 여부를 확인 하려면이 멤버 함수를 호출 합니다.
...
기본적으로 `CDaoTableDef` 개체를 기반으로 하는 새로 만든 테이블을 업데이트할 수 있으며, `CDaoTableDef` 개체의 기본 연결 된 테이블은 업데이트할 수 없습니다. 결과 레코드 집합을 업데이트할 수 없는 경우에도 `CDaoTableDef` 개체를 업데이트할 수 있습니다.
...
개체를 생성 한 후에는 [Create](#create) 또는 [Open](#open) 멤버 함수를 호출 해야 합니다. 개체를 완료 한 후에는 해당 [Close](#close) 멤버 함수를 호출 하 고 `CDaoTableDef` 개체를 삭제 해야 합니다.
...
일반적으로 `Close`를 호출한 후에는 **새**로 할당 된 경우에는 tabledef 개체를 삭제 합니다.

`Close`를 호출한 후 [Open](#open) 을 다시 호출할 수 있습니다. 이렇게 하면 테이블 정의 개체를 재사용할 수 있습니다.
...
테이블 정의의 이름을 지정한 후 [Append](#append) 를 호출 하 여 데이터베이스의 테이블 정의 컬렉션에 테이블 정의를 저장할 수 있습니다. `Append`를 호출한 후에는 테이블 정의가 열린 상태 이며이를 사용 하 여 [CDaoRecordset](../../mfc/reference/cdaorecordset-class.md) 개체를 만들 수 있습니다.
...
Ole (`DAOField`) 개체가 만들어지고 `DAOTableDef` (OLE) 개체의 Fields 컬렉션에 추가 됩니다. 개체 속성을 검사 하는 데 사용 하는 것 외에도 `CDaoFieldInfo`를 사용 하 여 테이블 정의에 새 필드를 만들기 위한 입력 매개 변수를 생성할 수 있습니다. `CreateField`의 첫 번째 버전을 사용 하는 것이 더 간단 하지만 보다 세부적인 제어를 원하는 경우에는 `CDaoFieldInfo` 매개 변수를 사용 하는 두 번째 버전의 `CreateField`를 사용할 수 있습니다.

`CDaoFieldInfo` 매개 변수를 사용 하는 `CreateField` 버전을 사용 하는 경우 `CDaoFieldInfo` 구조의 다음 멤버를 신중 하 게 설정 해야 합니다.
...
`CDaoFieldInfo`의 나머지 멤버는 해당 멤버에 적절 하 게 **0**, FALSE 또는 빈 문자열로 설정 해야 합니다. 그렇지 않으면 `CDaoException` 발생할 수 있습니다.
...
`CDaoIndexInfo` 구조체의 다음 멤버를 설정 해야 합니다.

- `m_strName` 이름을 제공 해야 합니다.

- `m_pFieldInfos` `CDaoIndexFieldInfo` 구조체의 배열을 가리켜야 합니다.

- `m_nFields` `CDaoFieldInfo` 구조체 배열의 필드 수를 지정 해야 합니다.

FALSE로 설정 된 경우 나머지 멤버는 무시 됩니다. 또한 인덱스를 만드는 동안에는 `m_lDistinctCount` 멤버가 무시 됩니다.
...
`CDaoTableDef` 개체의 경우 반환 값은 `CDaoTableDef` 개체로 표시 되는 테이블의 특성을 지정 하며 이러한 상수의 합이 될 수 있습니다.
...
`CDaoTableDef` 개체의 특성을 하나 이상 나타내는 값을 반환 합니다.
...
테이블의 경로와 데이터베이스 유형을 포함 하는 `CString` 개체입니다.

### <a name="remarks"></a>설명

연결 된 테이블을 나타내는 `CDaoTableDef` 개체의 경우 `CString` 개체는 한 부분 또는 두 부분으로 구성 됩니다 (데이터베이스 유형 지정자와 데이터베이스의 경로).
...
암호가 필요 하지만 제공 되지 않은 경우 ODBC 드라이버는 테이블에 처음 액세스할 때 로그인 대화 상자를 표시 하 고 연결을 닫은 후 다시 열 때 다시 열 수 있습니다. 연결 된 테이블에 `dbAttachSavePWD` 특성이 있으면 테이블을 다시 열 때 로그인 프롬프트가 표시 되지 않습니다.

관련 내용은 DAO 도움말의 "연결 속성" 항목을 참조 하십시오.

##  <a name="getdatecreated"></a>CDaoTableDef::GetDateCreated

`CDaoTableDef` 개체의 기반이 되는 테이블의 날짜와 시간을 확인 하려면이 함수를 호출 합니다.
...
`CDaoTableDef` 개체의 기반이 되는 테이블이 생성 된 날짜 및 시간을 포함 하는 값입니다.
...
`CDaoTableDef` 개체의 기반이 되는 테이블을 마지막으로 업데이트 한 날짜와 시간을 확인 하려면이 함수를 호출 합니다.
...
`CDaoTableDef` 개체의 기반이 되는 테이블을 마지막으로 업데이트 한 날짜와 시간을 포함 하는 값입니다.
...
- `AFX_DAO_PRIMARY_INFO` (기본값) 이름, 형식, 크기, 특성입니다. 가장 빠른 성능을 위해이 옵션을 사용 합니다.

- 기본 정보를 더하기: 서 수 위치, 필수, 0 길이 허용, 데이터 정렬 순서, 외래 이름, 원본 필드, 원본 테이블 `AFX_DAO_SECONDARY_INFO`

- 기본 및 보조 정보는 물론, 유효성 검사 규칙, 유효성 검사 텍스트, 기본값 `AFX_DAO_ALL_INFO` 합니다.
...
- 주 정보를 포함 하는 기본 정보를 추가 합니다. 기본, 고유, 클러스터형, Null 무시, 필수, 외래 `AFX_DAO_SECONDARY_INFO`
...
이 멤버 함수를 호출 하 여 `CDaoTableDef` 개체에 있는 레코드 수를 확인 합니다.
...
테이블 형식 `CDaoTableDef` 개체에 대 한 `GetRecordCount`를 호출 하는 것은 테이블의 대략적인 레코드 수를 반영 하며 테이블 레코드를 추가 및 삭제 하는 즉시 영향을 받습니다. [CDaoWorkSpace:: CompactDatabase](../../mfc/reference/cdaoworkspace-class.md#compactdatabase)를 호출할 때까지 롤백된 트랜잭션은 레코드 수의 일부로 표시 됩니다. 레코드가 없는 `CDaoTableDef` 개체에는 레코드 수 속성 설정 0이 있습니다. 연결 된 테이블 또는 ODBC 데이터베이스로 작업할 때 `GetRecordCount` 항상-1을 반환 합니다.
...
연결 된 테이블의 원본 이름을 지정 하는 `CString` 개체 또는 네이티브 데이터 테이블의 경우 빈 문자열입니다.
...
필드에 있는 데이터의 유효성을 검사 하거나 테이블에 추가 하는 `CString` 개체입니다.

### <a name="remarks"></a>설명

유효성 검사 규칙은 업데이트 작업에 연결 하는 데 사용 됩니다. 테이블 정의에 유효성 검사 규칙이 포함 된 경우 해당 테이블 정의에 대 한 업데이트는 데이터를 변경 하기 전에 미리 결정 된 조건과 일치 해야 합니다. 변경 내용이 조건과 일치 하지 않는 경우 [GetValidationText](#getvalidationtext) 값을 포함 하는 예외가 throw 됩니다. `CDaoTableDef` 개체의 경우이 `CString`는 연결 된 테이블에 대 한 읽기 전용 이며 기본 테이블에 대 한 읽기/쓰기입니다.
...
사용자가 유효성 검사 규칙과 일치 하지 않는 데이터를 입력 하는 경우 표시 되는 텍스트를 지정 하는 `CString` 개체입니다.

### <a name="remarks"></a>설명

`CDaoTableDef` 개체의 경우이 `CString`는 연결 된 테이블에 대 한 읽기 전용 이며 기본 테이블에 대 한 읽기/쓰기입니다.

관련 내용은 DAO 도움말의 "ValidationText 속성" 항목을 참조 하십시오.

##  <a name="isopen"></a>CDaoTableDef:: IsOpen

`CDaoTableDef` 개체가 현재 열려 있는지 여부를 확인 하려면이 멤버 함수를 호출 합니다.
...
`CDaoTableDef` 개체가 열려 있으면 0이 아닌 값이 고, 그렇지 않으면 0입니다.
...
`CDaoTableDef` 개체의 내부에 있는 DAO tabledef 개체의 OLE 인터페이스에 대 한 포인터를 포함 합니다.
...
해당 `CDaoTableDef` 개체에서 [Setconnect](#setconnect) 를 호출한 다음 `RefreshLink` 멤버 함수를 사용 하 여 정보를 업데이트 하면 연결 된 테이블에 대 한 연결 정보를 변경할 수 있습니다. `RefreshLink`를 호출 하면 연결 된 테이블의 속성은 변경 되지 않습니다.
...
`CDaoTableDef` 개체의 특성을 하나 이상 나타내는 값을 설정 합니다.
...
*lAttributes*<br/>
`CDaoTableDef` 개체로 표시 되는 테이블의 특성으로, 이러한 상수의 합계를 사용할 수 있습니다.
...
여러 특성을 설정 하는 경우 비트 or 연산자를 사용 하 여 적절 한 상수의 합계를 구하고 조합할 수 있습니다. 연결 되지 않은 테이블에 `dbAttachExclusive`을 설정 하면 예외가 발생 합니다. 다음 값을 결합 하면 예외도 생성 됩니다.
...
연결 된 테이블을 나타내는 `CDaoTableDef` 개체의 경우 string 개체는 한 부분 또는 두 부분으로 구성 됩니다 (데이터베이스 유형 지정자와 데이터베이스의 경로).
...
> [!NOTE]
>  "DATABASE = drive:\\\path" 형식의 등호 (=) 경로 문 주위에 공백을 포함 하지 마십시오. 이로 인해 예외가 throw 되 고 연결에 실패 합니다.
...
|HTML 내보내기|"HTML Export;"|"`drive`:\\\ *경로*"|
|텍스트|"텍스트;"|"drive:\\\path"|
|ODBC|DB 데이터베이스 = `database`; UID = *user*; PWD = *password*; DSN = *datasourcename;* LOGINTIMEOUT = *초;* " 이는 모든 서버에 대 한 완전 한 연결 문자열이 아닐 수 있습니다. 단지 예입니다. 매개 변수 사이에 공백을 포함 하지 않는 것이 중요 합니다.|None|
|Exchange|"Exchange;<br /><br /> MAPILEVEL = *folderpath*;<br /><br /> [TABLETYPE={ 0 &#124; 1 };]<br /><br /> [프로필 = *프로필*;]<br /><br /> [PWD = *password*;]<br /><br /> [DATABASE = `database`;] "|*"drive*:\\\ *path*\\\ *파일 이름*입니다. 않았더라도|

> [!NOTE]
>  Btrieve는 DAO 3.5에서 더 이상 지원 되지 않습니다.

연결 문자열에서 이중 백슬래시 (\\\\)를 사용 해야 합니다. `SetConnect`를 사용 하 여 기존 연결의 속성을 수정한 경우 [Refreshlink](#refreshlink)를 호출 해야 합니다. `SetConnect`를 사용 하 여 연결 속성을 초기화 하는 경우 `RefreshLink`를 호출할 필요가 없지만 먼저 테이블 정의를 추가 해야 합니다.

암호가 필요 하지만 제공 되지 않은 경우 ODBC 드라이버는 테이블에 처음 액세스할 때 로그인 대화 상자를 표시 하 고 연결을 닫은 후 다시 열 때 다시 열 수 있습니다.

`Create` 멤버 함수에 소스 인수를 제공 하 여 `CDaoTableDef` 개체에 대 한 연결 문자열을 설정할 수 있습니다. 데이터베이스의 유형, 경로, 사용자 ID, 암호 또는 ODBC 데이터 원본을 결정 하는 설정을 확인할 수 있습니다. 자세한 내용은 특정 드라이버에 대 한 설명서를 참조 하세요.
...
이 멤버 함수를 호출 하 여 연결 된 테이블의 이름을 지정 하거나, `CDaoTableDef` 개체가 기반으로 하는 기본 테이블의 이름을 데이터 원본에 있는 것과 동일 하 게 지정 합니다.
...
유효성 검사는 Microsoft Jet 데이터베이스 엔진을 사용 하는 데이터베이스에 대해서만 지원 됩니다. 식은 사용자 정의 함수, 도메인 집계 함수, SQL 집계 함수 또는 쿼리를 참조할 수 없습니다. `CDaoTableDef` 개체에 대 한 유효성 검사 규칙은 해당 개체에서 여러 필드를 참조할 수 있습니다.
...
이 멤버 함수를 호출 하 여 Microsoft Jet 데이터베이스 엔진에서 지 원하는 기본 테이블을 사용 하 여 `CDaoTableDef` 개체에 대 한 유효성 검사 규칙의 예외 텍스트를 설정 합니다.
