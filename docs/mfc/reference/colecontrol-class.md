...
|[COleControl::DoClick](#doclick)|Stock `DoClick` 메서드의 구현입니다.|
...
|[COleControl::FireClick](#fireclick)|Stock `Click` 이벤트를 발생 시킵니다.|
|[COleControl::FireDblClick](#firedblclick)|Stock `DblClick` 이벤트를 발생 시킵니다.|
|[COleControl::FireError](#fireerror)|Stock `Error` 이벤트를 발생 시킵니다.|
|[COleControl::FireEvent](#fireevent)|사용자 지정 이벤트를 발생 시킵니다.|
|[COleControl::FireKeyDown](#firekeydown)|Stock `KeyDown` 이벤트를 발생 시킵니다.|
|[COleControl::FireKeyPress](#firekeypress)|Stock `KeyPress` 이벤트를 발생 시킵니다.|
|[COleControl::FireKeyUp](#firekeyup)|Stock `KeyUp` 이벤트를 발생 시킵니다.|
|[COleControl::FireMouseDown](#firemousedown)|Stock `MouseDown` 이벤트를 발생 시킵니다.|
|[COleControl::FireMouseMove](#firemousemove)|Stock `MouseMove` 이벤트를 발생 시킵니다.|
|[COleControl::FireMouseUp](#firemouseup)|Stock `MouseUp` 이벤트를 발생 시킵니다.|
|[COleControl::FireReadyStateChange](#firereadystatechange)|컨트롤의 준비 상태가 변경 될 때 이벤트를 발생 시킵니다.|
|[COleControl::GetActivationPolicy](#getactivationpolicy)|`IPointerInactive` 인터페이스를 지 원하는 컨트롤의 기본 활성화 동작을 변경 합니다.|
...
|[COleControl::InternalGetFont](#internalgetfont)|스톡 글꼴 속성의 `CFontHolder` 개체를 반환 합니다.|
...
|[COleControl::OnClose](#onclose)|`IOleControl::Close` 호출 되었음을 컨트롤에 알립니다.|
...
|[COleControl::OnGetColorSet](#ongetcolorset)|`IOleObject::GetColorSet` 호출 되었음을 컨트롤에 알립니다.|
...
|[COleControl::OnInactiveMouseMove](#oninactivemousemove)|컨트롤에 메시지 WM_MOUSEMOVE 하는 마우스 포인터에서 비활성 컨트롤의 컨테이너를 포함 하도록 재정의 합니다.|
|[COleControl::OnInactiveSetCursor](#oninactivesetcursor)|컨트롤에 메시지 WM_SETCURSOR 하는 마우스 포인터에서 비활성 컨트롤의 컨테이너를 포함 하도록 재정의 합니다.|
...
|[COleControl::OnSetClientSite](#onsetclientsite)|`IOleControl::SetClientSite` 호출 되었음을 컨트롤에 알립니다.|
...
|[COleControl::ResetStockProps](#resetstockprops)|`COleControl` 스톡 속성을 기본값으로 초기화 합니다.|
...
|[COleControl::SerializeStockProps](#serializestockprops)|`COleControl` 스톡 속성을 serialize 하거나 초기화 합니다.|
...
`CWnd`에서 파생 되는이 클래스는 Windows 창 개체의 모든 기능과 OLE 관련 추가 기능 (예: 이벤트 발생, 메서드 및 속성을 지 원하는 기능)을 상속 합니다.
...
창 없는 활성화를 사용 하는 경우 창이 있는 컨테이너는 컨트롤의 자체 창에서 제공 하는 서비스를 제공 해야 합니다. 예를 들어 컨트롤에서 키보드 포커스를 쿼리하거나 마우스 캡처를 쿼리하거나 장치 컨텍스트를 가져와야 하는 경우 이러한 작업은 컨테이너에 의해 관리 됩니다. `COleControl` 창 없는 작업 멤버 함수는 컨테이너에서 이러한 작업을 호출 합니다.

창 없는 활성화를 사용 하도록 설정 하면 컨테이너는 입력 메시지를 컨트롤의 `IOleInPlaceObjectWindowless` 인터페이스 (창 없는 지원에 대 한 [IOleInPlaceObject](/windows/win32/api/oleidl/nn-oleidl-ioleinplaceobject) 확장)에 위임 합니다. 이 인터페이스의 `COleControl`구현에서는 마우스 좌표를 적절 하 게 조정한 후 컨트롤의 메시지 맵을 통해 이러한 메시지를 디스패치합니다. 메시지 맵에 해당 항목을 추가 하 여 일반 창 메시지와 같은 이러한 메시지를 처리할 수 있습니다.

창 없는 컨트롤에서는 해당 `CWnd` 멤버 함수 또는 관련 Windows API 함수 대신 항상 `COleControl` 멤버 함수를 사용 해야 합니다.
...
OLE 컨트롤 프레임 워크를 개발 하는 방법에 대 한 자세한 내용은 [Mfc Activex 컨트롤](../../mfc/mfc-activex-controls.md) 및 [개요: Mfc Activex 컨트롤 프로그램 만들기](../../mfc/reference/mfc-activex-control-wizard.md)문서를 참조 하세요. 창 없는 컨트롤 및 깜빡임 없는 컨트롤을 포함 하 여 OLE 컨트롤을 최적화 하는 방법에 대 한 자세한 내용은 [MFC ActiveX 컨트롤: 최적화](../../mfc/mfc-activex-controls-optimization.md)를 참조 하세요.
...
속성 집합 메서드를 통해 변경이 수행 되지 않은 경우에도 속성 값이 변경 될 때마다이 메서드를 호출 해야 합니다. 멤버 변수에 매핑되는 바인딩된 속성에 대해 특히 주의 해야 합니다. 멤버 변수가 변경 될 때마다 `BoundPropertyChanged`를 호출 해야 합니다.

##  <a name="boundpropertyrequestedit"></a>  COleControl::BoundPropertyRequestEdit

컨트롤에서 제공 하는 바인딩된 속성 값을 변경 하기 위해 `IPropertyNotifySink` 인터페이스에서 권한을 요청 합니다.
...
창 없는 개체는 겹치는 개체에 의해 캐럿이 부분적으로 숨겨지는지 아니면 완전히 숨겨지는지를 먼저 확인 하지 않고 캐럿을 안전 하 게 표시할 수 없습니다. 가능 하 게 하기 위해 개체는 `ClipCaretRect`를 사용 하 여 캐럿이 조정 영역에 맞게 조정 (축소) 되도록 할 수 있습니다.

캐럿을 만드는 개체는 `ClipCaretRect`에 대 한 캐럿 사각형을 전송 하 고 캐럿에 대해 조정 된 사각형을 사용 해야 합니다. 캐럿이 완전히 숨겨지면이 메서드는 FALSE를 반환 하 고이 경우에는 캐럿을 표시 하지 않아야 합니다.
...
*scode*<br/>
보고할 상태 코드 값입니다. 가능한 코드의 전체 목록은 [ActiveX 컨트롤: 고급 항목](../../mfc/mfc-activex-controls-advanced-topics.md)을 참조 하세요.
...
재정의할 수 있는 `COleControl::OnClick` 멤버 함수가 호출 되 고 컨트롤에서 지 원하는 경우에는 스톡 Click 이벤트가 발생 합니다.

이 함수는 `COleControl` 기본 클래스에서 DoClick 이라는 스톡 메서드로 지원 됩니다. 자세한 내용은 [ActiveX 컨트롤: 메서드](../../mfc/mfc-activex-controls-methods.md)문서를 참조 하세요.
...
컨트롤 마법사를 사용 하 여 OLE 컨트롤 프로젝트를 만든 경우에는이 함수의 재정의 된 버전이 기본 클래스 함수 `COleControl::DoPropExchange`를 호출 하 여 `COleControl`에서 지 원하는 스톡 속성을 직렬화 합니다. OLE 컨트롤에 사용자 정의 속성을 추가 하는 경우이 함수를 수정 하 여 새 속성을 serialize 해야 합니다. Serialization에 대 한 자세한 내용은 [ActiveX 컨트롤: serialize](../../mfc/mfc-activex-controls-serializing.md)문서를 참조 하세요.
...
이 함수 및 Windows 컨트롤 서브클래싱에 대 한 자세한 내용은 [ActiveX 컨트롤: Windows 컨트롤 서브클래싱](../../mfc/mfc-activex-controls-subclassing-a-windows-control.md)문서를 참조 하세요.
...
이 함수는 재정의 가능한 `OnDraw` 함수를 직접 호출 합니다.
...
이 함수는 일반적으로 `COleControl::DoPropExchange`의 기본 구현에 의해 호출 됩니다.
...
일반적으로이 함수는 컨트롤의 `COleControl::DoPropExchange`재정의에 의해 호출 되는 첫 번째 함수입니다. 로드할 때이 함수는 영구 데이터의 버전 번호를 읽고 그에 따라 [Cpropexchange](../../mfc/reference/cpropexchange-class.md) 개체의 버전 특성을 설정 합니다. 저장 하는 경우이 함수는 영구 데이터의 버전 번호를 씁니다.

지 속성 및 버전 관리에 대 한 자세한 내용은 [ActiveX 컨트롤: serialize](../../mfc/mfc-activex-controls-serializing.md)문서를 참조 하세요.
...
> [!NOTE]
>  Variant 데이터 상수에 대 한 포인터를 제공 하는 VTS_FONT 및 VTS_PICTURE를 제외 하 고 모든 variant 형식에 대해 추가 변형 상수가 정의 되었습니다. 이러한 상수는 **VTS_P**`constantname` 규칙을 사용 하 여 명명 됩니다. 예를 들어 VTS_PCOLOR은 VTS_COLOR 상수에 대 한 포인터입니다.
...
- 작업을 수행 하는 동안 SHIFT 키를 눌렀음을 SHIFT_MASK 합니다.

- 작업 중에 CTRL 키를 눌렀음을 CTRL_MASK 합니다.

- 작업 중에 ALT 키를 눌렀음을 ALT_MASK 합니다.
...
이벤트의 수신자는 *Pnchar*를 수정할 수 있습니다. 예를 들어 모든 소문자를 대문자로 변환 합니다. 수정 된 문자를 검사 하려면 `OnKeyPressEvent`를 재정의 합니다.
...
- 마우스 왼쪽 단추를 눌렀습니다 LEFT_BUTTON 합니다.

- 마우스 가운데 단추를 눌렀음을 MIDDLE_BUTTON 합니다.

- 마우스 오른쪽 단추를 눌렀습니다 RIGHT_BUTTON 합니다.
...
- 작업을 수행 하는 동안 마우스 왼쪽 단추를 눌렀습니다 LEFT_BUTTON 합니다.

- 작업을 수행 하는 동안 마우스 가운데 단추를 눌렀음을 MIDDLE_BUTTON 합니다.

- 작업을 수행 하는 동안 마우스 오른쪽 단추를 눌렀습니다 RIGHT_BUTTON 합니다.
...
- 마우스 왼쪽 단추를 놓았음을 LEFT_BUTTON 합니다.

- 마우스 가운데 단추를 놓았음을 MIDDLE_BUTTON 합니다.

- 마우스 오른쪽 단추를 놓았음을 RIGHT_BUTTON 합니다.
...
`IPointerInactive` 인터페이스를 지 원하는 컨트롤의 기본 활성화 동작을 변경 합니다.
...
`IPointerInactive` 인터페이스를 사용 하도록 설정 하면 컨테이너는 WM_SETCURSOR을 위임 하 고 메시지에 메시지를 WM_MOUSEMOVE 합니다. 이 인터페이스의 `COleControl`구현에서는 마우스 좌표를 적절 하 게 조정한 후 컨트롤의 메시지 맵을 통해 이러한 메시지를 디스패치합니다.

컨테이너가 `IPointerInactive`를 지 원하는 비활성 개체 위에 마우스 포인터를 놓았을 때 WM_SETCURSOR 또는 WM_MOUSEMOVE 메시지를 받을 때마다 인터페이스에서 `GetActivationPolicy`를 호출 하 고 POINTERINACTIVE 열거에서 플래그를 반환 해야 합니다.

메시지 맵에 해당 항목을 추가 하 여 일반 창 메시지와 마찬가지로 이러한 메시지를 처리할 수 있습니다. 처리기에서 먼저 해당 값이 NULL이 아닌지 확인 하지 않고 `m_hWnd` 멤버 변수 (또는이를 사용 하는 멤버 함수)를 사용 하지 않습니다.

마우스 커서를 설정 하거나 마우스 이동 이벤트를 실행 하는 것 보다 많은 개체는 특별 한 시각적 피드백을 제공 하는 것과 같이 POINTERINACTIVE_ACTIVATEONENTRY 플래그를 반환 하 고 활성 상태인 경우에만 피드백을 그려야 합니다. 개체가이 플래그를 반환 하는 경우 컨테이너는 즉시이 플래그를 활성화 한 다음 `GetActivationPolicy`에 대 한 호출을 트리거한 동일한 메시지로 전달 해야 합니다.

POINTERINACTIVE_ACTIVATEONENTRY 및 POINTERINACTIVE_DEACTIVATEONLEAVE 플래그가 모두 반환 되는 경우 개체를 마우스로 가리킬 때만 개체가 활성화 됩니다. POINTERINACTIVE_ACTIVATEONENTRY 플래그만 반환 되는 경우에는 마우스가 처음 개체에 들어가면 개체가 한 번만 활성화 됩니다.
...
`GetActivationPolicy`에서 통신 하는 정보는 컨테이너에서 캐시 되어서는 안 됩니다. 대신, 마우스가 비활성 개체에 들어갈 때마다이 메서드를 호출 해야 합니다.
...
일반적으로 `IPointerInactive` 인터페이스를 사용 하도록 설정 하면 컨트롤에서 마우스 메시지를 항상 처리할 수 있습니다. `IPointerInactive` 인터페이스를 지원 하지 않는 컨테이너에서이 동작을 가져오려면 컨트롤이 표시 될 때 항상 활성화 되어 있어야 합니다. 즉, 컨트롤의 기타 플래그 사이에 OLEMISC_ACTIVATEWHENVISIBLE 플래그가 있어야 합니다. 그러나 `IPointerInactive`를 지 원하는 컨테이너에서이 플래그를 적용 하지 않으려면 OLEMISC_IGNOREACTIVATEWHENVISIBLE 플래그를 지정 하면 됩니다.
...
`GetAmbientProperty`를 사용 하 여 앰비언트 DisplayName 및 ScaleUnits 속성을 검색 하는 경우 *vtProp* 를 VT_BSTR로 설정 하 고 *pvProp* 를 `CString*`로 설정 합니다. 앰비언트 글꼴 속성을 검색 하는 경우 *vtProp* 을 VT_FONT로 설정 하 고 *PVPROP* 를 lpdisp disp *로 설정 합니다.
...
성공 하면 반환 값은 현재 배경색을 OLE_COLOR 값으로 지정 합니다. `TranslateColor`에 대 한 호출을 사용 하 여이 값을 COLORREF 값으로 변환할 수 있습니다.
...
`COleControl` 개체에 마우스 캡처가 있는지 여부를 확인 합니다.
...
그렇지 않으면 마우스 캡처가 있는 `CWnd` 개체를 반환 합니다 (`CWnd::GetCapture`와 동일).
...
OLE 컨트롤의 컨테이너 안에는 사각형 영역이 있습니다. 컨트롤의 클라이언트 영역은 테두리 및 스크롤 막대를 제외한 컨트롤 영역입니다. `GetClientOffset`로 검색 되는 오프셋은 컨트롤의 사각형 영역 왼쪽 위 모퉁이와 해당 클라이언트 영역의 왼쪽 위 모퉁이 간 차이입니다. 컨트롤에 표준 테두리 및 스크롤 막대가 아닌 비클라이언트 요소가 있으면이 멤버 함수를 재정의 하 여 오프셋을 지정 합니다.
...
*lpRect*<br/>
창 없는 컨트롤의 클라이언트 영역 크기를 포함 하는 `RECT` 구조체에 대 한 포인터입니다. 즉, 컨트롤의 크기가 창 테두리, 프레임, 스크롤 막대 등을 뺀 값입니다. *LpRect* 매개 변수는 컨트롤이 위치가 아닌 클라이언트 사각형의 크기를 나타냅니다.
...
반환 된 포인터는 `IOleClientSite`의 인스턴스를 가리킵니다. 컨테이너에 의해 구현 되는 `IOleClientSite` 인터페이스는 해당 컨텍스트의 개체 뷰입니다. 문서에 고정 되어 있습니다. 여기에서 저장소, 사용자 인터페이스 및 기타 리소스를 가져옵니다.
...
|`clipPaintDC`|설정 하지 않으면 `COleControl`에서 `IntersectClipRect`에 대 한 호출을 사용 하지 않도록 설정 하 고 속도가 약간 향상 됩니다. 창 없는 활성화를 사용 하는 경우 플래그는 영향을 주지 않습니다.|
|`pointerInactive`|설정 하는 경우 기본적으로 사용 하지 않도록 설정 된 `IPointerInactive` 인터페이스의 `COleControl`구현을 사용 하 여 컨트롤이 비활성화 되어 있는 동안 마우스 상호 작용을 제공 합니다.|
...
OLE 컨트롤의 `GetControlFlags` 및 기타 최적화에 대 한 자세한 내용은 [ActiveX 컨트롤: 최적화](../../mfc/mfc-activex-controls-optimization.md)를 참조 하세요.
...
- OLEDC_NODRAW은 개체가 장치 컨텍스트를 사용 하 여 그리기를 수행 하는 것이 아니라 단순히 디스플레이 장치에 대 한 정보를 가져오도록 지정 합니다. 컨테이너는 추가 처리 없이 창의 DC를 전달 하기만 하면 됩니다.

- OLEDC_PAINTBKGND DC를 반환 하기 전에 컨테이너가 배경을 칠하는 것을 요청 합니다. 개체는 투명 한 배경으로 영역을 다시 그리기 위해 DC를 요청 하는 경우이 플래그를 사용 해야 합니다.
...
성공 하면 컨테이너 `CWnd` 클라이언트 영역에 대 한 디스플레이 장치 컨텍스트에 대 한 포인터입니다. 그렇지 않으면 반환 값이 NULL입니다. 표시 장치 컨텍스트는 컨테이너 창의 클라이언트 영역에서 그리는 다음 GDI 함수에서 사용할 수 있습니다.

### <a name="remarks"></a>설명

그리기 후에 컨텍스트를 해제 하려면 [Releasedc](#releasedc) 구성원 함수를 호출 해야 합니다. `GetDC`를 호출 하는 경우 개체는 그리려는 사각형을 자체 클라이언트 좌표로 전달 합니다. `GetDC`는 컨테이너 클라이언트 영역의 좌표로 변환 합니다. 개체는 자체 클라이언트 영역 사각형 보다 큰 원하는 그리기 사각형을 요청 하지 않아야 합니다 .이 크기는 [Getclientrect](#getclientrect)를 사용 하 여 검색할 수 있습니다. 이렇게 하면 개체가 의도 하지 않은 위치에서 실수로 그려지지 않습니다.
...
이 개체는 `IDispatch` 인터페이스를 통해 조작할 수 있습니다. `QueryInterface`를 사용 하 여 개체에서 제공 하는 다른 사용 가능한 인터페이스를 가져올 수도 있습니다. 그러나 개체는 특정 인터페이스 집합을 지 원하는 데 필요 하지 않습니다. 컨테이너의 확장 컨트롤 개체의 특정 기능에 의존 하 여 컨트롤의 이식성은 다른 임의의 컨테이너로 제한 됩니다.
...
그렇지 않으면 포커스가 있는 `CWnd` 개체를 반환 합니다 (`CWnd::GetFocus`와 동일).
...
완료 되 면 호출자가 개체를 해제 해야 합니다. 컨트롤의 구현 내에서 `InternalGetFont`를 사용 하 여 컨트롤의 스톡 글꼴 개체에 액세스 합니다. 컨트롤에서 글꼴을 사용 하는 방법에 대 한 자세한 내용은 activex 컨트롤 [: Activex 컨트롤의 글꼴 사용](../../mfc/mfc-activex-controls-using-fonts.md)문서를 참조 하세요.
...
이러한 글꼴은 [COleControl:: select글꼴](#selectfontobject) 함수를 사용 하 여 선택할 수 있습니다. 성공 하면 `GetFontTextMetrics`는 `fontHolder`의 글꼴에 대 한 유효한 메트릭 정보를 사용 하 여 *lptm* 가 가리키는 `TEXTMETRIC` 구조를 초기화 하거나, 성공 하지 않을 경우 0으로 구조를 채웁니다. 포함 된 OLE 개체와 마찬가지로 컨트롤을 메타 파일로 렌더링 해야 할 수 있으므로 컨트롤을 [그릴 때이](/windows/win32/api/wingdi/nf-wingdi-gettextmetrics) 함수를 사용 해야 합니다.

[Select 개체](#selectfontobject) 함수를 호출 하면 기본 글꼴의 `TEXTMETRIC` 구조가 새로 고쳐집니다. 스톡 글꼴 속성을 선택한 후에만 `GetFontTextMetrics`를 호출 하 여 제공 된 정보가 유효한 지를 보장 해야 합니다.
...
성공 하면 반환 값은 현재 전경색을 OLE_COLOR 값으로 지정 합니다. `TranslateColor`에 대 한 호출을 사용 하 여이 값을 [Colorref](/windows/win32/gdi/colorref) 값으로 변환할 수 있습니다.
...
`GetStockTextMetrics` 함수는 성공 하는 경우 유효한 메트릭 정보를 사용 하 여 *lptm* 가 가리키는 `TEXTMETRIC` 구조를 초기화 하거나, 성공 하지 않으면 0으로 구조를 채웁니다. 포함 된 OLE 개체와 같은 컨트롤이 메타 파일로 렌더링 되어야 하는 경우 컨트롤을 그릴 때 [Gettextmetrics](/windows/win32/api/wingdi/nf-wingdi-gettextmetrics) 대신이 함수를 사용 합니다.

`SelectStockFont` 함수를 호출 하면 기본 글꼴의 `TEXTMETRIC` 구조가 새로 고쳐집니다. 제공 된 정보가 유효한 지 확인할 수 있도록 스톡 글꼴을 선택한 후에만이 함수를 호출 해야 합니다.
...
이 함수의 호출자는 리소스를 해제 하기 위해 반환 되는 문자열에 대 한 `SysFreeString`를 호출 해야 합니다. 컨트롤의 구현 내에서 `InternalGetText`를 사용 하 여 컨트롤의 스톡 텍스트 또는 캡션 속성에 액세스 합니다.

##  <a name="getwindowlessdroptarget"></a>  COleControl::GetWindowlessDropTarget

창 없는 컨트롤을 OLE 끌어서 놓기 작업의 대상으로 사용할 경우 `GetWindowlessDropTarget`를 재정의 합니다.
...
개체의 `IDropTarget` 인터페이스에 대 한 포인터입니다. 창에는 창이 없으므로 창 없는 개체는 `IDropTarget` 인터페이스를 등록할 수 없습니다. 그러나 끌어서 놓기를 참여 하기 위해 창 없는 개체는 인터페이스를 계속 구현 하 고 `GetWindowlessDropTarget`에 반환할 수 있습니다.

### <a name="remarks"></a>설명

일반적으로 이를 위해서는 컨트롤의 창이 놓기 대상으로 등록되어야 합니다. 그러나 컨트롤에 자체 창이 없기 때문에 컨테이너는 자체 창을 놓기 대상으로 사용 합니다. 이 컨트롤은 컨테이너가 적절 한 시간에 호출을 위임할 수 있는 `IDropTarget` 인터페이스의 구현을 제공 하기만 하면 됩니다. 다음은 그 예입니다.
...
이를 사용 하 여 컨테이너 내에서 창 없는 컨트롤을 다시 그릴 수 있습니다. 업데이트 영역의 다른 모든 영역과 함께 무효화 된 영역은 다음 [WM_PAINT](/windows/win32/gdi/wm-paint) 메시지가 전송 될 때 그리기 위해 표시 됩니다.
...
VBX 컨트롤을 사용 하는 폼을 OLE 컨트롤을 사용 하는 폼으로 변환 하는 경우 OLE 컨트롤에 대 한 특수 로드 코드가 필요할 수 있습니다. 예를 들어 OLE 컨트롤의 인스턴스를 로드 하는 경우 `DoPropExchange`에서 [PX_Font](persistence-of-ole-controls.md#px_font) 에 대 한 호출이 있을 수 있습니다.

[!code-cpp[NVC_MFCAxCtl#3](../../mfc/reference/codesnippet/cpp/colecontrol-class_4.cpp)]

그러나 VBX 컨트롤에는 Font 개체가 없습니다. 각 글꼴 속성은 개별적으로 저장 되었습니다. 이 경우 `IsConvertingVBX`를 사용 하 여 다음 두 가지 경우를 구분 합니다.

[!code-cpp[NVC_MFCAxCtl#4](../../mfc/reference/codesnippet/cpp/colecontrol-class_5.cpp)]

또 다른 경우는 VBX 컨트롤이 소유 이진 데이터 (VBM_SAVEPROPERTY 메시지 처리기)를 저장 하 고 OLE 컨트롤에서 이진 데이터를 다른 형식으로 저장 하는 경우입니다. OLE 컨트롤이 VBX 컨트롤과 이전 버전과 호환 되도록 하려면 VBX 컨트롤 또는 OLE 컨트롤의 로드 여부를 구분 하 여 `IsConvertingVBX` 함수를 사용 하 여 이전 형식과 새 형식을 모두 읽을 수 있습니다.
...
프레임 워크의 `IDispatch::Invoke` 구현에서는 `IsInvokeAllowed`를 호출 하 여 지정 된 함수 (`dispid`으로 식별 됨)를 호출할 수 있는지 여부를 확인 합니다. OLE 컨트롤의 기본 동작은 컨트롤이 초기화 된 경우에만 자동화 메서드를 호출할 수 있도록 하는 것입니다. 그러나 `IsInvokeAllowed`는 가상 함수 이며 필요한 경우 (예: 컨트롤이 자동화 서버로 사용 되는 경우) 재정의 될 수 있습니다.
...
*dispID*<br/>
변경 된 앰비언트 속성의 디스패치 ID 이거나, 여러 속성이 변경 된 경우 DISPID_UNKNOWN입니다.
...
이 속성이 변경 된 후 알림을 표시 하려면이 함수를 재정의 합니다. 기본 구현에서는 `InvalidateControl`를 호출 합니다.
...
기본 구현에서는 `InvalidateControl`를 호출 합니다.
...
- 마우스 왼쪽 단추를 클릭 LEFT_BUTTON.

- 마우스 가운데 단추를 클릭 MIDDLE_BUTTON입니다.

- 마우스 오른쪽 단추를 클릭 RIGHT_BUTTON.

### <a name="remarks"></a>설명

기본 구현에서는 `COleControl::FireClick`를 호출 합니다.
...
*dwSaveOption*<br/>
로드 하기 전에 개체를 저장 해야 하는지 여부를 나타내는 플래그입니다. 유효한 값은 다음과 같습니다.
...
기본적으로 `OnClose`는 컨트롤이 수정 되었고 *Dwsaveoption* 이 OLECLOSE_SAVEIFDIRTY 또는 OLECLOSE_PROMPTSAVE 인 경우 컨트롤 개체를 저장 합니다.

##  <a name="ondoverb"></a>  COleControl::OnDoVerb

컨테이너가 `IOleObject::DoVerb` 멤버 함수를 호출할 때 프레임 워크에서 호출 됩니다.
...
`OnDraw`은 일반적으로 화면 표시를 위해 호출 되며, 화면 장치 컨텍스트를 *pDC*로 전달 합니다. *Rcbounds* 매개 변수는 현재 매핑 모드를 기준으로 대상 장치 컨텍스트의 사각형을 식별 합니다. *Rcinvalid* 매개 변수가 잘못 된 실제 사각형입니다. 어떤 경우에는 *Rcbounds*보다 작은 영역입니다.
...
이 함수는 일반적으로 ON_OLEVERB 메시지 맵 항목에 대 한 처리기 함수로 사용 됩니다. 이렇게 하면 컨트롤의 "개체" 메뉴에서 "편집" 동사를 사용할 수 있습니다. 다음은 그 예입니다.
...
컨테이너가 `IOleObject::EnumVerbs` 멤버 함수를 호출할 때 프레임 워크에서 호출 됩니다.
...
*ppenumOleVerb*<br/>
컨트롤의 동사를 열거 하는 `IEnumOLEVERB` 개체에 대 한 포인터입니다.
...
기본 구현에서는 `COleControl::InvalidateControl`를 호출 합니다. 컨트롤이 Windows 컨트롤을 서브클래싱하는 경우 기본 구현은 컨트롤의 창에 WM_SETFONT 메시지도 보냅니다.
...
컨테이너에서 `IOleControl::FreezeEvents`를 호출한 후 프레임 워크에서 호출 됩니다.
...
컨테이너가 `IViewObject::GetColorSet` 멤버 함수를 호출할 때 프레임 워크에서 호출 됩니다.
...
컨테이너의 `IViewObjectEx::GetNaturalExtent` 요청에 대 한 응답으로 프레임 워크에서 호출 됩니다.
...
*pExtentInfo*<br/>
크기 조정 데이터를 지정 하는 `DVEXTENTINFO` 구조체를 가리킵니다. `DVEXTENTINFO` 구조는 다음과 같습니다.
...
- DVEXTENT_CONTENT 콘텐츠를 정확 하 게 맞추기 위한 컨트롤의 크기 (맞춤-크기)

- 크기를 조정 하는 경우 DVEXTENT_INTEGRAL 컨트롤에 제안 된 크기를 전달 합니다.
...
`COleControl::OnGetPredefinedStrings`재정의에서 이전에 반환 된 미리 정의 된 문자열 중 하나에 해당 하는 값을 가져오기 위해 프레임 워크에서 호출 됩니다.
...
*dwCookie*<br/>
`COleControl::OnGetPredefinedStrings`재정의에서 이전에 반환 된 쿠키 값입니다.

*lpvarOut*<br/>
속성 값이 반환 되는 `VARIANT` 구조체에 대 한 포인터입니다.
...
컨테이너의 `IViewObjectEx::GetRect` 요청에 대 한 응답으로 프레임 워크에서 호출 됩니다.
...
- 전체 개체의 경계 사각형을 DVASPECT_CONTENT 합니다. 개체의 원점에서 왼쪽 위 모퉁이 이며 `GetViewExtent`에서 반환 하는 범위와 같습니다 *.*
...
개체의 크기는 지정 된 위치에서 시작 하는 사각형으로 `OnGetViewRect` 변환 됩니다. 기본값은 표시의 왼쪽 위 모퉁이입니다. 컨트롤에서 2 패스 그리기를 사용 하 고 불투명 부분과 투명 부분에 서로 다른 차원이 있는 경우이 함수를 재정의 합니다.

##  <a name="ongetviewstatus"></a>  COleControl::OnGetViewStatus

컨테이너의 `IViewObjectEx::GetViewStatus` 요청에 대 한 응답으로 프레임 워크에서 호출 됩니다.
...
|||
|-|-|
|VIEWSTATUS_OPAQUE|개체가 완전히 불투명 합니다. 이 비트가 설정 되지 않은 경우 개체는 투명 한 부분을 포함 합니다. 이 비트는 콘텐츠 관련 측면에만 적용 되 고 DVASPECT_ICON 또는 DVASPECT_DOCPRINT는 적용 되지 않습니다.|
|VIEWSTATUS_SOLIDBKGND|개체에 브러시 패턴이 아닌 단색으로 구성 된 단색 배경이 있습니다. 이 비트는 VIEWSTATUS_OPAQUE 설정 되 고 콘텐츠 관련 측면에만 적용 되 고 DVASPECT_ICON 하거나 DVASPECT_DOCPRINT 하지 않는 경우에만 의미가 있습니다.|
|VIEWSTATUS_DVASPECTOPAQUE|개체가 DVASPECT_OPAQUE을 지원 합니다. 이 측면에서 그리기 측면을 매개 변수로 사용 하는 모든 IViewObjectEx 메서드를 호출할 수 있습니다.|
|VIEWSTATUS_DVASPECTTRANSPARENT|개체가 DVASPECT_TRANSPARENT을 지원 합니다. 이 측면에서 드로잉 측면을 매개 변수로 사용 하는 모든 `IViewObjectEx` 메서드를 호출할 수 있습니다.|

### <a name="remarks"></a>설명

컨트롤에서 2 패스 그리기를 사용 하는 경우이 함수를 재정의 합니다. 기본 구현에서는 VIEWSTATUS_OPAQUE을 반환 합니다.
...
구현에서는 `OnShowToolbars`표시 되는 모든 도구 모음을 숨깁니다.

##  <a name="oninactivemousemove"></a>  COleControl::OnInactiveMouseMove

WM_MOUSEMOVE 메시지가 수신 될 때 마우스 포인터 아래의 비활성 개체에 대 한 컨테이너에 의해 호출 됩니다.
...
*dwKeyState*<br/>
키보드에서 키보드 보조키의 현재 상태를 식별 합니다. 유효한 값 MK_CONTROL, MK_SHIFT, MK_ALT, MK_BUTTON, MK_LBUTTON, MK_MBUTTON 및 MK_RBUTTON 플래그를 조합 하 여 사용할 수 있습니다.
...
WM_SETCURSOR 메시지가 수신 될 때 마우스 포인터 아래의 비활성 개체에 대 한 컨테이너에 의해 호출 됩니다.
...
*lpclsid*<br/>
클래스 ID가 반환 되는 `CLSID` 구조체에 대 한 포인터입니다.
...
이 함수를 사용 하 여 컨트롤의 속성 페이지가 표시 되도록 할 수도 있습니다. *HWndParent* 매개 변수에 컨트롤 부모의 핸들을 전달 하 여 `OnProperties` 함수에 대 한 호출을 수행 합니다. 이 경우 *Lpmsg* 및 *lpRect* 매개 변수의 값은 무시 됩니다.

##  <a name="onqueryhitpoint"></a>  COleControl::OnQueryHitPoint

컨테이너의 `IViewObjectEx::QueryHitPoint` 요청에 대 한 응답으로 프레임 워크에서 호출 됩니다.
...
*pRectBounds*<br/>
OLE 컨트롤 클라이언트 영역의 경계 사각형을 지정 하는 `RECT` 구조체에 대 한 포인터입니다.

*ptlLoc*<br/>
적중에 대해 확인할 점을 지정 하는 `POINT` 구조체에 대 한 포인터입니다. 지점은 OLE 클라이언트 영역 좌표로 지정 됩니다.

*lCloseHint*<br/>
적중에 대해 확인 된 지점에 "close"를 정의 하는 거리입니다.

*pHitResult*<br/>
적중 쿼리의 결과에 대 한 포인터입니다. 해당 값은

- HITRESULT_OUTSIDE *Ptlloc* 가 OLE 개체 외부에 있으므로 닫히지 않습니다.

- HITRESULT_TRANSPARENT *Ptlloc* 가 OLE 개체의 범위 내에 있지만 이미지와는 근접 하지 않습니다. 예를 들어 투명 원 가운데의 점은 HITRESULT_TRANSPARENT 수 있습니다.
...
개체의 표시 사각형이 지정 된 지점 (지점 적중)과 겹치는 지 여부를 쿼리 합니다. 사각형이 아닌 개체에 대 한 적중 테스트를 위해 `QueryHitPoint`를 재정의할 수 있습니다.

##  <a name="onqueryhitrect"></a>  COleControl::OnQueryHitRect

컨테이너의 `IViewObjectEx::QueryHitRect` 요청에 대 한 응답으로 프레임 워크에서 호출 됩니다.
...
*prcLoc*<br/>
개체의 왼쪽 위 모퉁이를 기준으로 적중 (개체 사각형과 겹치기)을 확인할 사각형을 지정 하는 `RECT` 구조체에 대 한 포인터입니다.
...
- OLE 개체에서 사각형의 점을 적중 하지 HITRESULT_OUTSIDE 합니다.
...
개체의 표시 사각형이 지정 된 사각형의 모든 지점과 겹치는 지 여부를 쿼리 합니다 (사각형 적중). 사각형이 아닌 개체에 대 한 적중 테스트를 위해 `QueryHitRect`를 재정의할 수 있습니다.
...
지정 된 형식은 이전에 지연 된 렌더링을 위해 [delayrenderdata](../../mfc/reference/coledatasource-class.md#delayrenderdata) 또는 [delayrenderfiledata](../../mfc/reference/coledatasource-class.md#delayrenderfiledata) 멤버 함수를 사용 하 여 컨트롤 개체에 배치 되었습니다. 제공 된 저장소 미디어가 파일 또는 메모리 일 경우이 함수의 기본 구현에서는 각각 `OnRenderFileData` 또는 `OnRenderGlobalData`를 호출 합니다. 요청 된 형식이 CF_METAFILEPICT 또는 영구 속성 집합 형식인 경우 기본 구현은 적절 한 데이터를 렌더링 하 고 0이 아닌 값을 반환 합니다. 그렇지 않으면 0을 반환 하 고 아무 작업도 수행 하지 않습니다.
...
요청 된 형식 및 중간에 데이터를 제공 하려면이 함수를 재정의 합니다. 데이터에 따라이 함수의 다른 버전 중 하나를 대신 재정의할 수 있습니다. 데이터가 작고 크기가 고정 된 경우 `OnRenderGlobalData`를 재정의 합니다. 데이터가 파일에 있거나 가변 크기인 경우 `OnRenderFileData`를 재정의 합니다.

자세한 내용은 Windows SDK에서 `FORMATETC` 및 `STGMEDIUM` 구조를 참조 하세요.
...
요청 된 형식 및 중간에 데이터를 제공 하려면이 함수를 재정의 합니다. 데이터에 따라이 함수의 다른 버전 중 하나를 대신 재정의 하는 것이 좋습니다. 여러 저장소 미디어를 처리 하려면 `OnRenderData`를 재정의 합니다. 데이터가 파일에 있거나 가변 크기인 경우 `OnRenderFileData`를 재정의 합니다.

자세한 내용은 Windows SDK의 `FORMATETC` 구조를 참조 하세요.
...
요청 된 형식 및 중간에 데이터를 제공 하려면이 함수를 재정의 합니다. 데이터에 따라이 함수의 다른 버전 중 하나를 대신 재정의할 수 있습니다. 여러 저장소 미디어를 처리 하려면 `OnRenderData`를 재정의 합니다. 데이터가 파일에 있거나 가변 크기인 경우 `OnRenderFileData`를 재정의 합니다.
...
컨트롤 작성자는이 재정의 가능에서 OLE 컨트롤에 대 한 초기화 코드를 삽입할 수 있습니다. 이 함수는 [ 또는 ](/windows/win32/api/objidl/nf-objidl-ipersiststream-load)를 먼저 호출하지 않고 [IPersistStream::load](/windows/win32/api/objidl/nf-objidl-ipersiststorage-load) 또는 [IPersistStorage::Load](/windows/win32/api/ocidl/nf-ocidl-ipersiststreaminit-initnew)가 실패하거나[IPersistStreamInit::InitNew](/windows/win32/api/objidl/nf-objidl-ipersiststorage-initnew) 또는 `IPersistStream::Load`IPersistStorage::InitNew`IPersistStorage::Load`가 호출될 때 호출됩니다.
...
기본적으로 `OnSetClientSite`는 데이터 경로 속성이 로드 되는지 여부를 확인 하 고, 있는 경우 `DoDataPathPropExchange`를 호출 합니다.
...
데이터가 영구 속성 집합 형식인 경우 기본 구현은 컨트롤의 상태를 적절 하 게 수정 합니다. 그렇지 않으면 기본 구현은 아무 작업도 수행 하지 않습니다. *BRelease* 가 TRUE 이면 `ReleaseStgMedium`에 대 한 호출이 수행 됩니다. 그렇지 않으면입니다.
...
*lpSizeL*<br/>
정수 (long)를 사용 하 여 HIMETRIC 단위로 표현 되는 컨트롤의 너비와 높이를 나타내는 `SIZEL` 구조체에 대 한 포인터입니다.
...
기본 구현은 컨트롤 범위의 크기 조정을 처리 합니다. 컨트롤이 내부 활성 상태 이면 컨테이너의 `OnPosRectChanged`에 대 한 호출이 생성 됩니다.
...
*lpRectPos*<br/>
컨테이너에 상대적인 컨트롤의 새 위치와 크기를 나타내는 `RECT` 구조체에 대 한 포인터입니다.

*lpRectClip*<br/>
컨트롤을 클리핑할 사각형 영역을 나타내는 `RECT` 구조체에 대 한 포인터입니다.
...
컨테이너의 `IOleInPlaceObjectWindowless::OnWindowMessage` 요청에 대 한 응답으로 프레임 워크에서 호출 됩니다.
...
창 없는 컨트롤에 대 한 창 메시지를 처리 합니다. 마우스 메시지와 키보드 메시지 이외의 창 메시지에는 `COleControl``OnWindowlessMessage` 사용 해야 합니다. `COleControl`은 창 없는 OLE 개체에 대 한 마우스 캡처 및 키보드 포커스를 얻기 위해 특별히 [Setcapture](#setcapture) 및 [SetFocus](#setfocus) 를 제공 합니다.

창 없는 개체에는 창이 없으므로 컨테이너에서 메시지를 디스패치할 수 있도록 하는 메커니즘이 필요 합니다. 창 없는 OLE 개체는 `IOleInPlaceObjectWindowless` 인터페이스의 `OnWindowMessage` 메서드를 통해 컨테이너에서 메시지를 가져옵니다 (창 없는 지원에 대 한 [IOleInPlaceObject](/windows/win32/api/oleidl/nn-oleidl-ioleinplaceobject) 확장). `OnWindowMessage`는 `HWND` 매개 변수를 사용 하지 않습니다.
...
- 화면 배경이 나 windows 간의 분할 선 (`DefWndProc` Windows 함수가 오류를 나타내는 시스템 경고음을 생성 한다는 점을 제외 하 고는 HTNOWHERE과 동일)을 HTERROR 합니다.
...
모달 대화 상자를 표시 한 후이 함수를 호출 합니다. 컨테이너가 `PreModalDialog`에서 사용 하지 않도록 설정 된 최상위 창을 사용할 수 있도록 하려면이 함수를 호출 해야 합니다. 이 함수는 `PreModalDialog`에 대 한 호출과 쌍을 이루어야 합니다.
...
모달 대화 상자를 표시 하기 전에이 함수를 호출 합니다. 컨테이너가 모든 최상위 창을 사용 하지 않도록 설정할 수 있도록이 함수를 호출 해야 합니다. 모달 대화 상자가 표시 된 후 `PostModalDialog`를 호출 해야 합니다.
...
이 함수는 `COleControl` 기본 클래스에서 새로 고침 이라는 스톡 메서드로 지원 됩니다. 이렇게 하면 OLE 컨트롤의 사용자가 특정 시간에 컨트롤을 다시 그릴 수 있습니다. 이 메서드에 대 한 자세한 내용은 [ActiveX 컨트롤: 메서드](../../mfc/mfc-activex-controls-methods.md)문서를 참조 하세요.
...
응용 프로그램은 [GetDC](#getdc)를 호출할 때마다 `ReleaseDC`를 호출 해야 합니다.
...
속성은 모양, 배경색, 테두리, 캡션, 사용, 글꼴, ForeColor, hWnd 및 텍스트입니다. 스톡 속성에 대 한 설명은 [ActiveX 컨트롤: 스톡 속성 추가](../../mfc/mfc-activex-controls-adding-stock-properties.md)를 참조 하세요.

`ResetStockProps` 및 `ResetVersion`를 사용 하 여 `COleControl::OnResetState`를 재정의 하 여 컨트롤의 이진 초기화 성능을 향상 시킬 수 있습니다. 아래 예제를 참조하세요. 초기화 최적화에 대 한 자세한 내용은 [ActiveX 컨트롤: 최적화](../../mfc/mfc-activex-controls-optimization.md)를 참조 하세요.
...
`ResetVersion` 및 `ResetStockProps`를 사용 하 여 `COleControl::OnResetState`를 재정의 하 여 컨트롤의 이진 초기화 성능을 향상 시킬 수 있습니다. [ResetStockProps](#resetstockprops)의 예제를 참조 하세요. 초기화 최적화에 대 한 자세한 내용은 [ActiveX 컨트롤: 최적화](../../mfc/mfc-activex-controls-optimization.md)를 참조 하세요.
...
*lpClipRect*<br/>
클리핑할 사각형을 지정 하는 `CRect` 개체 또는 `RECT` 구조체를 가리킵니다. 사각형 안의 픽셀만 스크롤됩니다. 사각형 외부의 비트는 *lpRect* 사각형에 있더라도 영향을 받지 않습니다. *LpClipRect* 가 NULL 이면 스크롤 사각형에서 클리핑이 수행 되지 않습니다.
...
*ar*<br/>
Serialize 하거나에서 serialize 할 `CArchive` 개체입니다.

### <a name="remarks"></a>설명

`SerializeExtent`, `SerializeStockProps`및 `SerializeVersion`를 사용 하 여 `COleControl::Serialize`를 재정의 하 여 컨트롤의 이진 지 속성 성능을 향상 시킬 수 있습니다. 아래 예제를 참조하세요. 초기화 최적화에 대 한 자세한 내용은 [ActiveX 컨트롤: 최적화](../../mfc/mfc-activex-controls-optimization.md)를 참조 하세요.
...
모양, 배경색, 테두리, 캡션, 사용, 글꼴, 전경색 및 텍스트 `COleControl` 스톡 속성의 상태를 serialize 하거나 초기화 합니다.
...
스톡 속성에 대 한 설명은 [ActiveX 컨트롤: 스톡 속성 추가](../../mfc/mfc-activex-controls-adding-stock-properties.md)를 참조 하세요.

`SerializeStockProps`, `SerializeExtent`및 `SerializeVersion`를 사용 하 여 `COleControl::Serialize`를 재정의 하 여 컨트롤의 이진 지 속성 성능을 향상 시킬 수 있습니다. 예제는 [SerializeExtent](#serializeextent)에서 코드를 참조 하세요. 초기화 최적화에 대 한 자세한 내용은 [ActiveX 컨트롤: 최적화](../../mfc/mfc-activex-controls-optimization.md)를 참조 하세요.
...
컨트롤의 버전 번호입니다. 지정 된 보관 파일이 로드 되는 경우 `SerializeVersion`는 해당 보관 파일에서 로드 된 버전을 반환 합니다. 그렇지 않으면 현재 로드 된 버전을 반환 합니다.

### <a name="remarks"></a>설명

`SerializeVersion`, `SerializeExtent`및 `SerializeStockProps`를 사용 하 여 `COleControl::Serialize`를 재정의 하 여 컨트롤의 이진 지 속성 성능을 향상 시킬 수 있습니다. 예제는 [SerializeExtent](#serializeextent)에서 코드를 참조 하세요. 초기화 최적화에 대 한 자세한 내용은 [ActiveX 컨트롤: 최적화](../../mfc/mfc-activex-controls-optimization.md)를 참조 하세요.
...
스톡 속성에 대 한 자세한 내용은 [ActiveX 컨트롤: 속성](../../mfc/mfc-activex-controls-properties.md)을 참조 하세요.
...
*dwBackColor*<br/>
컨트롤의 배경 그리기에 사용 되는 OLE_COLOR 값입니다.

### <a name="remarks"></a>설명

이 속성 및 기타 관련 속성을 사용 하는 방법에 대 한 자세한 내용은 [ActiveX 컨트롤: 속성](../../mfc/mfc-activex-controls-properties.md)문서를 참조 하세요.
...
이전에 마우스 입력을 받은 `CWnd` 창 개체에 대 한 포인터입니다.

### <a name="remarks"></a>설명

컨트롤이 활성화 되 고 창이 없는 경우이 함수를 사용 하면 컨트롤의 컨테이너 창에서 컨트롤을 대신 하 여 마우스 캡처를 소유 하 게 됩니다. 그렇지 않으면이 함수는 컨트롤 자체에서 마우스 캡처를 소유 하 게 합니다 (`CWnd::SetCapture`와 동일).
...
이 속성을 설정한 후 `OnEnabledChange`가 호출 됩니다.
...
이전에 입력 포커스가 있었던 `CWnd` 창 개체에 대 한 포인터 이거나, 이러한 창이 없으면 NULL입니다.

### <a name="remarks"></a>설명

컨트롤이 활성화 되 고 창이 없는 경우이 함수를 사용 하면 컨트롤의 컨테이너 창이 컨트롤을 대신 하 여 입력 포커스를 소유 하 게 됩니다. 입력 포커스는 키보드 입력을 컨테이너의 창으로 전달 하 고, 컨테이너는 `SetFocus`를 호출 하는 OLE 개체에 모든 후속 키보드 메시지를 디스패치합니다. 이전에 입력 포커스가 있던 모든 창이 손실 됩니다.

컨트롤이 창이 없는 경우이 함수는 컨트롤 자체에서 입력 포커스를 소유 하 게 합니다 (`CWnd::SetFocus`와 동일).
...
*dwForeColor*<br/>
컨트롤의 전경 그리기에 사용 되는 OLE_COLOR 값입니다.
...
기본 구현에서는 CF_METAFILEPICT 및 영구 속성 집합의 두 가지 형식을 지정 합니다.
...
`BoundPropertyRequestEdit` 실패할 때이 함수를 호출 합니다. 이 함수는 set 작업이 허용 되지 않았음을 나타내는 `COleDispScodeException` 형식의 예외를 throw 합니다.
...
*lpptlHimetric*<br/>
HIMETRIC unit의 좌표가 포함 된 `POINTL` 구조체에 대 한 포인터입니다.

*lpptfContainer*<br/>
컨테이너 단위 크기의 좌표가 포함 된 `POINTF` 구조체에 대 한 포인터입니다.
...
- HIMETRIC 단위를 컨테이너 단위로 변환 XFORMCOORDS_HIMETRICTOCONTAINER 합니다.

- XFORMCOORDS_CONTAINERTOHIMETRIC 컨테이너의 단위를 HIMETRIC unit으로 변환 합니다.

### <a name="remarks"></a>설명

처음 두 플래그 XFORMCOORDS_POSITION 및 XFORMCOORDS_SIZE는 좌표를 위치 또는 크기로 처리할지 여부를 나타냅니다. 나머지 두 플래그는 변환 방향을 표시 합니다.
