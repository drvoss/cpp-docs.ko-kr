...
일반적으로 이를 위해서는 컨트롤의 창이 놓기 대상으로 등록되어야 합니다. 그러나 컨트롤에 자체 창이 없기 때문에 컨테이너는 자체 창을 놓기 대상으로 사용 합니다. 컨트롤은 `IDropTarget` 컨테이너가 적절 한 시간에 호출을 위임할 수 있는 인터페이스의 구현을 제공 하기만 하면 됩니다. 예를 들어:
