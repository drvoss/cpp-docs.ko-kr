...
`CMapStringToString`의 멤버 함수는 [CMapStringToOb](../../mfc/reference/cmapstringtoob-class.md)클래스의 멤버 함수와 비슷합니다. 이처럼 두 함수가 비슷하므로 `CMapStringToOb` 참조 설명서에서 멤버 함수 관련 사항을 확인할 수 있습니다. `CObject` 포인터가 반환 값 또는 "output" 함수 매개 변수로 표시 되는 경우 포인터를 **char**로 대체 합니다. `CObject` 포인터가 "입력" 함수 매개 변수로 표시 되는 경우 포인터를 **char**로 대체 합니다.
...
|[CMapStringToString::P GetFirstAssoc](#pgetfirstassoc)|맵의 첫 번째 `CString`에 대 한 포인터를 가져옵니다.|
|[CMapStringToString::P GetNextAssoc](#pgetnextassoc)|반복을 위해 다음 `CString`에 대 한 포인터를 가져옵니다.|
|[CMapStringToString::P 조회](#plookup)|지정 된 값과 일치 하는 값을 가진 `CString`에 대 한 포인터를 반환 합니다.|
...
|속성|Description|
|----------|-----------------|
|[CMapStringToString:: operator \[ \]](../../mfc/reference/cmapstringtoob-class.md#operator_at)|`SetAt`에 대 한 연산자 대체 (map)에 요소를 삽입 합니다.|

## <a name="remarks"></a>설명

`CMapStringToString`는 serialization 및 요소 덤프를 지원하기 위해 `IMPLEMENT_SERIAL` 매크로를 통합합니다. 각 요소는 오버 로드 된 삽입 ( **<<** ) 연산자 또는 `Serialize` 멤버 함수를 사용 하 여 맵이 보관 파일에 저장 되는 경우 차례로 serialize 됩니다.

개별 `CString`- `CString` 요소의 덤프가 필요한 경우 덤프 컨텍스트의 깊이를 1 이상으로 설정 해야 합니다.

`CMapStringToString` 개체가 삭제 되거나 해당 요소가 제거 되 면 `CString` 개체가 적절 하 게 제거 됩니다.

`CMapStringToString`에 대 한 자세한 내용은 [컬렉션](../../mfc/collections.md)문서를 참조 하세요.
...
Map의 모든 요소를 반복 하려면이 메서드를 호출 합니다. `PGetFirstAssoc`에 대 한 호출을 사용 하 여 첫 번째 요소를 검색 한 다음 `PGetNextAssoc`에 대 한 연속 호출을 사용 하 여 맵을 반복 합니다.

### <a name="example"></a>예제

  [Cmapstringtostring::P GetFirstAssoc](#pgetfirstassoc)의 예제를 참조 하세요.

##  <a name="plookup"></a>CMapStringToString::P 조회
